...
(function (root, factory) {
  ...
  p.pathname = function (v, build) {
    if (v === undefined || v === true) {
      const res = this._parts.path || (this._parts.urn ? '' : '/')
  ...
  p.href = function (href, build) {
    ...
    const _object = typeof href === 'object' && (href.hostname || href.path)
  ...
  const _protocol = p.protocol
  const _port = p.port
  const _hostname = p.hostname
  ...
  p.addQuery = function (name, value, build) {
    const data = URI.parseQuery(this._parts.query)
  ...
  p.absoluteTo = function (base) {
    ...
    let properties = ['protocol', 'username', 'password', 'hostname', 'port']
    let basedir, i, p
    if (this._parts.urn) {
  ...
  p.relativeTo = function (base) {
    ...
    let common, _base, _this, _base_diff, _this_diff
    if (this._parts.urn) {
      throw new Error('URNs do not have any generally defined hierachical components')
    }
    if (!(base instanceof URI)) {
    ...
    for (let i = 0, p; p = properties[i]; i++) {
    ...
    // this is a descendant of base
    let parents = '../'
    const _common = new RegExp('^' + escapeRegEx(common))
    let _parents = _base.replace(_common, '/').match(/\//g).length - 1
    while (_parents--) {
      parents += '../'
    }
    ...
    for (key in one_map) {
      if (hasOwn.call(one_map, key)) {
        if (!isArray(one_map[key])) {
          ...
          one_map[key].sort()
          two_map[key].sort()
          for (let i = 0, l = one_map[key].length; i < l; i++) {
