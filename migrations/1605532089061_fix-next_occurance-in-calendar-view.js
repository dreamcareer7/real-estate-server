  ...
  `CREATE OR REPLACE VIEW analytics.calendar AS (
    ...
    (
      SELECT
        ...
        date_trunc('day', cdc."date" AT TIME ZONE 'UTC') AS "date",
    ...
    (
      SELECT
        ...
        "date" + ((extract(year from age("date")) + (CASE WHEN "date" > now() THEN 0 ELSE 1 END)) * interval '1' year) as next_occurence,
        NULL::timestamptz AS end_date,
        True AS recurring,
        (CASE
    ...
    (
      SELECT
        ...
        date_trunc('day', a.created_at AT TIME ZONE 'UTC') AS "date",
