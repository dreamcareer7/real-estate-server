module.exports = (data) => {
  ...
  address.matrix_unique_id = parseInt(data.Matrix_Unique_ID) || undefined; // -1;
  ...
  property.bedroom_count = parseInt(data.BedsTotal) || undefined; // -1;
  property.bathroom_count = parseFloat(data.BathsTotal) || undefined; // -1.0;
  property.half_bathroom_count = parseFloat(data.BathsHalf) || undefined; // -1.0;
  ...
  property.year_built = parseInt(data.YearBuilt) || undefined; // -1;
  property.parking_spaces_covered_total = parseFloat(data.ParkingSpacesCoveredTotal) || undefined; // -1.0;
  ...
  property.number_of_parking_spaces = parseFloat(data.NumberOfParkingSpaces) || undefined; // -1.0;
  property.parking_spaces_carport = parseFloat(data.ParkingSpacesCarport) || undefined; // -1.0;
  property.parking_spaces_garage = parseFloat(data.ParkingSpacesGarage) || undefined; // -1.0;
  property.garage_length = parseFloat(data.GarageLength) || undefined; // -1.0;
  property.garage_width = parseFloat(data.GarageWidth) || undefined; // -1.0;
  property.number_of_dining_areas = parseInt(data.NumberOfDiningAreas) || undefined; // -1;
  property.number_of_living_areas = parseInt(data.NumberOfLivingAreas) || undefined; // -1;
  property.fireplaces_total = parseInt(data.FireplacesTotal) || undefined; //-1;
  ...
  property.ceiling_height = parseFloat(data.CeilingHeight.trim()) || undefined; // -1.0;
  ...
  property.lot_size = parseFloat(data.LotSize.trim()) || undefined; //-1.0;
  property.lot_size_area = parseFloat(data.LotSizeArea.trim()) || undefined; // -1.0;
  property.lot_size_dimensions = data.LotSizeDimensions.trim();
  property.map_coordinates = data.MapCoordinates.trim();
  property.number_of_pets_allowed = parseInt(data.NumberOfPetsAllowed.trim()) || undefined; // -1;
  property.number_of_units = parseInt(data.NumberOfUnits.trim()) || undefined; // -1;
  property.pets_yn = Boolean(parseInt(data.PetsYN.trim()));
  property.photo_count = parseInt(data.PhotoCount.trim()) || -1;
  property.room_count = parseInt(data.RoomCount.trim()) || undefined; // -1;
  property.subdivided_yn = Boolean(parseInt(data.SubdividedYN.trim()));
  property.surface_rights = data.SurfaceRights.trim();
  property.unit_count = parseInt(data.UnitCount.trim()) || undefined; // -1;
  ...
  listing.low_price = parseFloat(data.ListPriceLow) || undefined; // 0.0;
  listing.original_price = parseFloat(data.OriginalListPrice) || undefined; // 0.0;
  listing.association_fee = parseFloat(data.AssociationFee) || undefined; // 0.0;
  ...
  listing.close_price = parseFloat(data.ClosePrice.trim()) || null;
