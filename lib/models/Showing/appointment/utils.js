...
/** @typedef {{ id: UUID, time: Date }} PartialShowingAppointment */
...
async function extractListingBindings (appt) {
  ...
  return {
    ...
    address: /** @type {*} */(showing.address)?.full_address,
    beds: showing.listing?.property.bedroom_count,
    bathrooms: showing.listing?.property.bathroom_count,
    area: toSquareFoot(showing.listing?.property.square_meters),
  }
}

/** @param {ShowingAppointmentPopulated} appt */
async function extractContactBindings (appt) {
...
/**
 * @param {object} opts
 * @param {string | [number, number]} [opts.size=[512, 179]]
...
async function staticMap (opts) {
  ...
  opts = {
    ...
    zoom: opts.zoom ?? 14,
    key: opts.key ?? config.google.api_key,
  }
...
async function shortApprovalLinkTo (action, appt, user) {
  ...
  // TODO: Specify approval link address:
  const url = `${config.showings.domain}/showings/appointments/${token}/approval/${action}`
...
async function extractApprovalBindings (appt, approved = true) {
  const found = findRoleApproval(appt, false)
  if (!found) { return null }
 
  return {
    fullName: `${found.role.user.first_name} ${found.role.user.last_name}`,
    role: found.role.role,
    comment: found.approval.comment,
  }
}
 ...
 * @param {ShowingAppointmentPopulated} appt
 * @param {boolean=} [approved=true]
 * @returns {{ role: ShowingRolePopulated, approval: ShowingApproval }?}
 */
function findRoleApproval (appt, approved = true) {
  assert(Array.isArray(appt?.approvals), 'appt.approvals must be an array')
  assert(Array.isArray(appt?.showing?.roles), 'appt.showing.roles must be an array')

  /** @type {any} */
  const approval = appt.approvals.find(a => a.approved === approved)
  if (!approval) { return null}
  
  const roleId = typeof approval === 'object' ? approval.role.id : approval.role
  assert.equal(typeof roleId, 'string', 'Invalid roleId')

  const role = appt.showing.roles.find(r => r.id === roleId)
  assert(role, 'role not found for approval!')
...
function formatTime (appt, timezone = undefined) {
  ...
  const time1 = appt.time instanceof Date ? appt.time.getTime() : appt.time
  const time2 = time1 + appt.showing.duration * 1000
  ...
  Context.log('ShowingDuration: ' + appt.showing.duration)
