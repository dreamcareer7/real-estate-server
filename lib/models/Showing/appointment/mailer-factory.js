...
async function forBuyerOriginatedNotification (notifObj) {
  ...
  switch(notif.action) {
    case 'Created':
      links = await utils
        .shortApprovalLinks(['confirm', 'reject'], appt, role.user)
...
async function forSellerOriginatedNotification (notifObj) {
  ...
  switch (notif.action) {
    case 'Confirmed':
      ...
      assert(
        ['Confirmed', 'Requested', 'Rescheduled'].includes(appt.status),
        `appointment status cannot be ${appt.status}`
      )
      const links = await utils.shortApprovalLinks(
        appt.status === 'Confirmed' ? ['cancel'] : ['confirm', 'reject'],
        appt,
        role.user,
      )
