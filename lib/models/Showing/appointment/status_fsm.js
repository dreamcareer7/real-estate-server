...
async function updateStatus(appointment_id, newStatus) {
  ...
  if (byRole) {
    if (newStatus === 'Confirmed') {
      ...
      const confirmerRole = showing.roles.find(r => r.id === confirmedApproval.role)
    ...
    if (['Requested', 'Rescheduled'].includes(appointment.status) && newStatus === 'Canceled') {
      const approvals = await getApprovals()
      ...
      if (rejectedApproval) {
        ...
        const rejecterRole = showing.roles.find(r => r.id === rejectedApproval.role)
    ...
    if (appointment.status === 'Confirmed' && newStatus === 'Canceled') {
      ...
      const cancelerRole = showing.roles.find(r => r.id === canceledApproval.role)
