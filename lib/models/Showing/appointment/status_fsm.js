...
/**
 * @typedef {'*' | AppointmentStatus[] | AppointmentStatus} MatchArg
 *
 * @param {AppointmentStatus} oldStatus
 * @param {AppointmentStatus} newStatus
 * @param {MatchArg} oldStatuses
 * @param {MatchArg} newStatuses
 * @returns {boolean}
 */
function transitionMatcher (oldStatus, newStatus, oldStatuses, newStatuses) {
  const m1 = oldStatuses === '*' || [oldStatuses].flat().includes(oldStatus)
  const m2 = newStatuses === '*' || [newStatuses].flat().includes(newStatus)
  return m1 && m2
}
...
async function updateStatus(appointment_id, newStatus) {
  ...
  const matches = transitionMatcher.bind(null, oldStatus, newStatus)
  const is = matches.bind(null, '*')
  ...
  if (byRole && matches(['Requested', 'Rescheduled'], 'Canceled')) {
    await requestCanceledByRole(showing, appointment, await getApprovals())
  }

  if (byRole && matches('Confirmed', 'Canceled')) {
