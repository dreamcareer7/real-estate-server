...
/**
 * @callback StatusMatcher
 * @param {...AppointmentStatus} stts
 * @returns {boolean}
 */
...
const utils = (function () {
   ...
   * @param {ShowingRole['id'][]} roleIds
   * @returns {Promise<boolean>}
   */
  async function everyoneApproved (approvals, roleIds) {
    // if there is no approval, it's not approved yet.
    if (!roleIds.length || !approvals.length) { return false }

    // ignore approvals of deleted roles
    approvals = approvals.filter(a => roleIds.includes(a.role))

    // if there is at least one rejection, it's not approved (and won't be):
    const anyRejected = approvals.some(a => !a.approved)
    if (anyRejected) { return false }

    // if all roles have approved, it's approved:
    const approvedRoleIds = approvals.map(a => a.role)
    const remainingRoleIds = difference(roleIds, approvedRoleIds)
    if (!remainingRoleIds.length) { return true }

    // if none of remaining roles can approve, it's approved:
    const remainingRoles = await Role.getAll(remainingRoleIds)
    return remainingRoles.every(r => !r.can_approve)
  }
   ...
   */
  async function guessNewStatus ({ action, showing, appointment, approvals }) {
    ...
    switch (action) {
      ...
      case 'ApprovalPerformed':
        ...
        if (await utils.everyoneApproved(approvals, showing.roles)) {
          return 'Confirmed'
        }
        break
...
const handlers = (function () {
  ...
  async function rescheduled ({ appointment: appt }) {
    ...
    await sendAppointmentRescheduledNotificationToBuyer(appt.id)
