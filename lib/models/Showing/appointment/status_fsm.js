...
/** @typedef {'ApprovalPerformed' | 'BuyerCanceled' | 'BuyerRescheduled' | 'Finished' | 'BuyerRequested' | 'BuyerGaveFeedback'} FsmAction */
/** 
...
const handlers = (function () {
  ...
  /** @type {FsmHandler} */
  async function buyerGaveFeedback ({ appointment: appt }) {
    await sendFeedbackReceivedNotificationToRoles(appt.id)
    await utils.send(mailerFactory.forReceivedFeedback(appt))
  }

  /** @type {FsmHandler} */
  async function buyerRescheduled ({ appointment: appt }) {
    /* FIXME: send a new acknowledgement notification to buyer with
     * cancel/reschedule links */

    /* TODO: Whenever someone reschedule's a auto-confirmed one, the appointment
     * shoule be auto-confirmed again */
  ...
  async function handleAction (payload) {
    ...
    // XXX: its not a good idea:
    if (action !== 'BuyerGaveFeedback') {
    ...
    const remains = status => was(status) && gonna(status)

    if (action === 'BuyerRescheduled') {
      await handlers.buyerRescheduled(payload)
    }
    
    if (action === 'BuyerGaveFeedback') {
      await handlers.buyerGaveFeedback(payload)
    }
    
    if (action === 'BuyerRequested') {
