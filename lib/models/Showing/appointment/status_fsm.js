...
class FsmHandler {
   ...
   */
  constructor ({ appointment, showing }) {
    /**
     * @protected
     * @readonly
  ...
  async getApprovals () {
    if (!this.appointment.approvals?.length) { return [] }
    return this._approvals
      ?? (this._approvals = await Approval.getAll(this.appointment.approvals))
  }
  ...
  async anywayCompleted () {
    ...
    await mailerFactory.forGetFeedbackEmail(this.appointment).then(m => {
      if (m) {
  ...
  async anywayConfirmedByRole () {
    (await mailerFactory.forConfirmedAppointment(this.appointment))?.send()
    await sendAppointmentConfirmedNotificationToBuyer(this.appointment.id)
  ...
  async requestCanceledByRole () {
    ...
    (await mailerFactory.forRejectedAppointment(this.appointment))?.send()
  ...
  async canceledAfterConfirmByRole () {
    (await mailerFactory.forCanceledAppointmentAfterConfirm(this.appointment))?.send()
  ...
  async guessNewStatus () {
    ...
    if (approvals.every(a => this.showing.roles.includes(a.role))) {
      return 'Confirmed'
    }
...
async function updateStatus(appointment_id, newStatus) {
  ...
  const handler = new FsmHandler({ appointment, showing })
