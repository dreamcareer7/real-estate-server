...
async function updateStatus(appointment_id, newStatus) {
  ...
  if (newStatus === 'Confirmed') {
    await mailerFactory.forConfirmedAppointment(appointment).send()
  ...
  if (newStatus === 'Canceled') {
    ...
    if (rejectedApproval) {
      await mailerFactory.forRejectedAppointment(appointment).send()
      ...
      /* This is not a rejection. Is there anything to do here? */
    }
  }

  if (newStatus === 'Requested') {
    await mailerFactory.forRequestedAppointment(appointment).send()
  }

  if (appointment.status === 'Confirmed' && newStatus === 'Canceled') {
    await mailerFactory.forCanceledAppointmentAfterConfirm(appointment).send()
  }

  if (newStatus === 'Rescheduled') {
    await mailerFactory.forRescheduledAppointment(appointment).send()
  }
