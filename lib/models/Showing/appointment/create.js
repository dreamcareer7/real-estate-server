...
const { sendAppointmentRequestNotification, sendAppointmentRequestReceiptToBuyer } = require('./notification')
const Orm = {
  ...require('../../Orm/index'),
  ...require('../../Orm/context'),
}
...
async function request(showing_id, payload) {
  ...
  await sendAppointmentRequestReceiptToBuyer(id)

  const appointment = await Appointment.get(id)
  await mailerFactory.forRequestedAppointment(appointment).then(m => m?.send())

  await ShowingHub.emit.appointmentRequested(id)
