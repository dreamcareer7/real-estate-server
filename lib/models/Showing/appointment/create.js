...
const { Range, RANGE_LB_INC } = require('postgres-range')
...
function validateAppointmentRequest(showing, time) {
  ...
  if (
    !showing.availabilities.some((a) => {
      const m_low = m_time.clone().startOf('day').minutes(a.availability[0])
      const m_high = m_time.clone().startOf('day').minutes(a.availability[1])
      return m_time.format('dddd') === a.weekday && m_time.isBetween(m_low, m_high, 'minutes')
    })
  ) {
