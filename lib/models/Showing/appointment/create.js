...
const { sendAppointmentRequestNotification } = require('./notification')
const Orm = {
  ...require('../../Orm/index'),
  ...require('../../Orm/context'),
}
...
function validateAppointmentRequest(showing, time) {
  ...
  const sellerAgent = showing.roles.find((r) => r.role === 'SellerAgent')
  if (!sellerAgent) {
    throw Error.Forbidden('The showing does not specify a seller agent.')
  }
  const m_time = moment(time, moment.ISO_8601).tz(sellerAgent.user.timezone)
...
async function request(showing_id, payload) {
  Orm.setEnabledAssociations(['showing.roles', 'showing.availabilities', 'showing_role.user'])
