 ...
 * @param {'cancel' | 'confirm' | null} [action] null means all roles
 */
async function notifyRoles (notif, roles, action = null) {
  for (const r of roles) {
    const transports = action ? r[`${action}_notification_type`] : ['email', 'push', 'sms']

    if (transports.length) {
      const notifWithTransports = { ...notif, transports }

      if (r.phone_number) {
        notifWithTransports.phone_number = r.phone_number
      }
