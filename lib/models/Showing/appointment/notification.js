...
const issueNotificationForUser = promisify(Notification.issueForUser)

/**
 * @param {ShowingRole} r
 * @param {'cancel' | 'confirm' | null} [action]
 */
function chooseTransports (r, action = null) {
  if (action) {
    return r[`${action}_notification_type`] || []
  }

  /* If `action` is not provided, return union of all notification types. This
   * is a little bit confusing, but it's the simplest workaround to choose
   * appropriate transports for `gave feedback` notifications. */
  const union = [...r.confirm_notification_type, ...r.cancel_notification_type]
  return [...new Set(union)]
}

/**
 * @param {Partial<INotificationInput>} notif
 * @param {ShowingRole[]} roles
 * @param {'cancel' | 'confirm' | null} [action] null for other actions
 */
async function notifyRoles (notif, roles, action = null) {
  for (const r of roles) {
    const transports = chooseTransports(r, action)
    if (!transports.length) { continue }

    const notifWithTransports = {
      ...notif,
      ...(r.phone_number ? { phone_number: r.phone_number } : null),
      transports,
    }

    await issueNotificationForUser(notifWithTransports, r.user_id)
  }
}
...
async function sendAppointmentRequestNotification(appointment_id) {
  ...
  return notifyRoles({
    ...
    message: '',
    app: 'showingapp',
  }, appointment.showing.roles, 'confirm')
}
...
async function sendCancelNotificationToRoles(appointment_id, message) {
  ...
  return notifyRoles({
    ...
    message: '',
    app: 'showingapp',
  }, appointment.showing.roles, 'cancel')
}
 ...
 * @param {UUID} apptId
 */
async function sendAppointmentRescheduledNotificationToBuyer (apptId) {
  return sendTextMessageToBuyer(
    apptId,
    'showing_appointment_rescheduled.tmpl',
    appt => ({
...
async function sendAppointmentConfirmedNotificationToOtherRoles (
  ...
  const otherRoles = appointment.showing.roles.filter(r => r.id !== confirmerRoleId)
...
async function sendAppointmentRejectedNotificationToOtherRoles (
  ...
  const otherRoles = appointment.showing.roles.filter(r => r.id !== rejecterRoleId)
...
module.exports = {
  ...
  sendAppointmentRescheduledNotificationToBuyer,
