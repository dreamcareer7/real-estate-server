...
async function sendGetFeedbackTextMessageToBuyer (appointmentId) {
  async function getShowingSellerAgent (showing) {
    /* TODO: Do not use Orm.populate */
    const [{ roles }] = await Orm.populate({
      models: [showing],
      associations: 'showing.roles',
    })
    ...
    const sellerAgentUser = await User.get(sellerAgentRole.user)
    if (!sellerAgentUser) { return null }

    /* TODO: Do not use Orm.populate */
    const [{ agent }] = await Orm.populate({
      models: [sellerAgentUser],
  ...
  const url = `${config.showings.domain}/showings/appointments/${token}/cancel`
  ...
  const body = await renderSMS('get_buyer_feedback.tmpl', {
    ...
    /* TODO: extract true brokerage name */
