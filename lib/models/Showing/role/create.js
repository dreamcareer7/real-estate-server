...
async function guessUserId (agentId) {
  ...
  if (!userIds.length) {
    throw Error.Validation(`Invalid agent: ${agentId}`)
  }
  
  throw Error.Validation('user is optional only if user has got exactly one agent')
}
...
async function guessAgentId (userId) {
  ...
  throw Error.Validation('agent is required when user has got several agents')
}
...
async function ensureAssociation (userId, agentId) {
  ...
  throw Error.Validation(`agent ${agentId} is not belongs to user ${userId}`)
}
