...
const utils = {
  ...
  guessRequiredParticipants (roles) {
  ...
  guessConfirmationType ({ approval_type: type, instructions }) {
...
const emit = {
  ...
  async showingCreated (showingId) {
    ...
    await client.api.appListingConfigureshowablelistingCreate({
      ...
      // comments: '',
      requiredParticipants: utils.guessRequiredParticipants(roles),
      showingMethod: ShowingMethod.InPersonOnly,
      confirmationType: utils.guessConfirmationType(showing),
