...
/** @typedef {import('./types').DateTimeReoccurringRestrictions} ReoccurringRestriction */
/** @typedef {import('./types').DateTimeRestrictions} Restriction */
...
timespan.toReocurringRestrictions = function (timespans, { beginDate }) {
  ...
  return Object.entries(splitted).flatMap(([dayStr, spans]) => {
    ...
    return spans.map(([from, to]) => ({
      ...
      dayOfWeek: /** @type {?} */ (weekday),

      // XXX: is it [really] required?
      showListingId: /** @type {?} */ (undefined),
...
timespan.fromReoccurringRestriction = function (reoRest) {
  if (isNil(reoRest.startDatetime) ||
      isNil(reoRest.endDatetime) ||
      isNil(reoRest.dayOfWeek)) {
    return null
  }
