...
function millisToWeekday (millis) {
  assert(0 <= millis && millis < 7 * DAY, 'millis must be in 0-7 days')
  return Math.floor(startOfDay(millis) / DAY)
}
...
timespan.inverse = function (spans) {
  ...
  for (let i = 0, len = spans.length; i < len; i++) {
    ...
    if (i === 0) {   
      timespan.pushTo(inversed, start, curr[FROM])
    }

    if (next) {
      timespan.pushTo(inversed, curr[TO], next[FROM])
    }
...
timespan.fromShowingAvailability = function (avail) {
  ...
  assert(lower !== null, 'lower bound cannot be null')
  assert(upper !== null, 'upper bound cannot be null')
...
timespan.fromShowingAvailabilities = function (avails) {
  ...
  for (const avail of avails) {
    const [from, to] = timespan.fromShowingAvailability(avail)
    timespan.pushTo(timespans, from, to)  
  }
...
timespan.fromReoccurringRestrictions = function (reoRests) {
  ...
  for (const reoRest of reoRests) {
    const [from, to] = timespan.fromReoccurringRestriction(reoRest)
    timespan.pushTo(timespans, from, to)
  }
 ...
 */
function hubRestrictions ({ availablities: avails }) {
  const availTimespans = timespan.fromShowingAvailabilities(avails)
  const restTimespans = timespan.inverse(availTimespans)

  return {
    reoccurringRestrictions: timespan.toReocurringRestrictions(restTimespans),
    restrictions: [],
  }
}
