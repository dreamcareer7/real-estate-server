...
/**
 * @param {UUID[]} ids
 * @returns {Promise<ShowListingResponse[]>}
 */
function getAll(ids) {
 ...
 * @returns {Promise<ShowListingResponse>}
 */
async function findByShowingId(showing) {
  const id = await db.selectId('showing/hub/showable_listing/find_by_showing', [showing])
  return get(id)
}
