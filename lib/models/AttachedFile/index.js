...
AttachedFile.preSave = async ({
  ...
  if (!isAllowed(filename, extensions))
 ...
 * @template {string} F
 * @property {B} fields
 * @property {Record<F, RequestFile>} files
 */
/**
 * @template {Record<string, any>} B
 * @template {string} F
 ...
 * @param {Record<string, any>} [args.busboyOptions]
 * @returns {Promise<MultipartData<B, F>>}
 */
function getRequestData ({
  ...
  /** @type {MultipartData<B, F>['fields']} */
  const fields = {}

  /** @type {MultipartData<B, F>['files']} */
  const files = {}

  /** @type {boolean} */
  let isDone = false

  /** @type {Deferred<MultipartData<B, F>>} */
  ...
  bb.on('file', readFile)
    .on('field', readField)
    .on('finish', done)
    .on('error', (/** @type {Error} */ err) => done(err))
    .on('limit', () => {
