...
/**
 * @param {object} args
 * @param {IRequest} args.req
 * @param {FileRelation[] | () => FileRelation[]} args.relations
 * @param {string | () => string} args.path
...
AttachedFile.saveFromRequest = ({
  ...
  /** @type {boolean} used to ensure that cb is called when resolveWithoutFile is true */
  ...
  const uploadFile = (fieldname, file, filename, encoding, mime) => {
    relations = typeof relations === 'function' ? relations() : relations
  ...
  const readFile = (fieldname, file, filename, encoding, mime) => {
    ...
    const writable = concat({
      ...
      if (beforeUpload) {
        Promise.resolve(beforeUpload(fields)).then(doUpload, cb)
      } else {
        doUpload()
      }
    })
