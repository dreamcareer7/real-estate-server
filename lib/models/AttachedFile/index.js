...
AttachedFile.saveFromRequest = ({req, relations, path, public = undefined, sizeLimit = 200}, cb) => {
  /*
  ...
  const attachFile = (fieldname, file, filename, encoding, mime) => {
    ...
    file.on('limit', function(){
      throw Error.Validation(`File size exceeds! Attachments could not be greater than ${sizeLimit} MB.`)
    })
...
AttachedFile.getFileSize = async file => {
  ...
  // Calculate base-64 length
  const length = Number(result['content-length'])
  const chars  = ((length + 3 - 1) / 3) * 4
