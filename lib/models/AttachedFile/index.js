...
const upload = ({key, mime, stream, name, public}, cb) => {
  if (!AttachedFile.isAllowed(name)) {
    const types = config.allowed_upload_types.join(', ')
    const message = `You can only upload a file with following extensions: ${types}`
    cb(Error.Validation(types))
    return
  }
...
AttachedFile.saveFromRequest = ({
  ...
  const uploadFile = (fieldname, file, filename, encoding, mime) => {
    if (file.truncated)
      return cb(Error.Validation(`Maximum allowed file size is ${numeral(fileSize).format('0b')}.`))
  ...
  const attachFile = (fieldname, file, filename, encoding, mime) => {
    ...
    file.on('end', () => {
      uploadFile(fieldname, file, filename, encoding, mime)
    })
  }
