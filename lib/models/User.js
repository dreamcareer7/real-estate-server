...
User.initiatePasswordReset = function (email, cb) {
  User.getByEmail(email, function (err, user) {
    ...
    db.query(sql_record_pw_recovery, [email, user.id, token], function (err, res) {
      ...
      const url = config.webapp.base_url + config.webapp.password_reset_suffix + encodeURIComponent(pw_token)
...
User.sendActivation = function (user_id, context, cb) {
  ...
  const renderHTML = (cb, results) => {
    ...
    if (!results.contacting_agent) {
      results.base_url = config.webapp.base_url
  ...
  async.auto({
    ...
    html: ['url', 'brand', renderHTML],
    send_email: ['html', sendEmail],
    text: ['url', 'brand', renderText],
    send_sms: ['url', 'text', 'contacting_agent', sendSMS]
  }, cb)
}
