...
User.get = function (user_id, cb) {
  db.query('user/get', [user_id], function (err, res) {
    if (err)
      return cb(err)
...
UserActivity.get = function(id, cb) {
  db.query('user/get_activity', [id], (err, res) => {
    if(err)
      return cb(err)
...
UserActivity.add = function(user_id, activity, cb) {
  async.auto({
    user: cb => {
      User.get(user_id, cb)
    },
    parse: cb => {
      return ObjectUtil.parseActivity(activity, 'object', cb)
    },
    record: [
      'user',
      'parse',
      (cb, results) => {
        ...
        console.log(object, object_class, action)
        db.query('user/record_activity', [
          user_id,
    ...
    get: [
      'record',
      (cb, results) => {
        UserActivity.get(results.record, cb)
      }
    ]
  }, (err, results) => {
    if(err)
...
UserActivity.publicize = function(model) {
  if(model.object_sa) {
