 ...
 * @returns {boolean}
 */
User.updatePassword = function ({id, password, keep_shadow}, cb) {
  User.hashPassword(password, (err, hashed) => {
    if (err)
      return cb(err)

    db.query('user/change_password', [id, hashed, keep_shadow ? keep_shadow : false], cb)
  })
}
...
User.resetPasswordByShadowToken = function ({email, phone_number, token, password, keep_shadow}, cb) {
  async.auto({
    user: cb => {
      if (email) {
        User.getByEmail(email, (err, user) => {
