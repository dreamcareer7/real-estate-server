    ...
    emailAvailable(user, function(err) {
      ...
      User.hashPassword(user.password, function(err, hash) {
        ...
        insert(user, function(err, user_id) {
          User.claimInvitations(user_id, user.email, function(err) {
            if(err)
              return cb(err);
...
User.stringSearch = function(user_id, term, cb) {
  db.query(sql_string_search, [user_id, term], function(err, res) {
    if(err)
      return cb(err);
