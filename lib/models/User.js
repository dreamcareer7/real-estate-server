...
function phoneAvailable(user, cb) {
...
User.create = function(user, cb) {
  async.auto({
    ...
    email_available: ['validate',
                      function(cb) {
                        return emailAvailable(user, cb);
                      }],
    phone_available: ['validate',
                      function(cb) {
                        return phoneAvailable(user, cb);
                      }],
    hash_password: ['email_available',
                    'phone_available',
                    function(cb) {
...
User.patch = function(user_id, user, cb) {
  async.auto({
    ...
    get_by_email: function(cb) {
      if(!user.email)
        return cb();

      User.getByEmail(user.email, function(err, user) {
        if(user && user.id != user_id)
          return cb(Error.Validation('This email is already associated with another user'));

        return cb();
      });
    },
    get_by_phone_number: function(cb) {
      if(!user.phone_number)
        return cb();

      User.getByPhoneNumber(user.phone_number, function(err, user) {
        if(user && user.id != user_id)
          return cb(Error.Validation('This phone number is already associated with another user'));
