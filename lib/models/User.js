...
User.connectToRoom = function (user_id, room_id, cb) {
  ...
  Room.get(room_id, err => {
    ...
    return Room.addUser(false, user_id, room_id, false, cb)
  })
}
...
User.getFormattedPhoneNumber = function (phone) {
  if (!_u.isString(phone) || _u.isEmpty(phone))
    return null
...
User.isIdInArray = function (id, array) {
  return (_u.isString(id) && _u.isArray(array) && _u.contains(array, id))
}
