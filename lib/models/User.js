...
var schema_user_create = {
  type: 'object',
  properties: {
    action: {
      type: 'string',
      enum: [ 'create_alert', 'favorite_listing', 'listing_inquiry' ],
      required: true
    },
...
User.create = function(user, cb) {
  ...
  var handle_create_alert = (r, opts, cb) => {
    var alert = r.alert;
    alert.created_by = opts.insert;

    return Alert.create(opts.personal_room.id, alert, (err, alert) => {
      if(err)
        return cb(err);
  ...
  async.auto({
    ...
    user_connect: [
      ...
      'brand',
      (cb, results) => {
        return handle_user_connect(results.insert, user.user_connect, 'BrokerageWidget', results.brand.id, cb);
      }
    ],
    room_connect: [
      'insert',
    ...
    handle_actions: [
      ...
      (cb, results) => {
        ...
        async.map(user.actions, (r, cb) => {
          async.auto({
            validate: cb => {
              return validator(schema_user_create, r, cb);
            },
            listing: cb => {
              if(!r.listing)
                return cb();
            ...
            brand: cb => {
              ...
              return Brand.get(r.brand, (err, brand) => {
                if(err)
                  return cb(err);

                console.log('id:', r.brand, 'foo:', brand);
      ...
      branch: [
        ...
        (cb, results) => {
          ...
          if(!team)
            ...
            if(team.alert)
              redirect = '&alert_id=' + encodeURIComponent(team.alert);
            else if (team.listing)
              redirect = '&listing_id=' + encodeURIComponent(team.listing);
      ...
      send_email: [
        ...
        (cb, results) => {
          ...
          if(team) {
            ...
            // template_params.agent_profile_url = team.user_connect.profile_image_url;
            // template_params.agent_profile_url = 'https://s3-us-west-2.amazonaws.com/shortlisted-test/15533339.png';
            template_params.agent_name = team.user_connect.first_name + ' ' + team.user_connect.last_name;
          }
