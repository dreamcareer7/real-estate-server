...
const sql_insert = require('../sql/user/insert.sql')
const sql_update = require('../sql/user/update.sql')
const sql_get = require('../sql/user/get.sql')
const sql_get_by_email = require('../sql/user/get_by_email.sql')
const sql_get_by_phone = require('../sql/user/get_by_phone.sql')
const sql_get_by_code = require('../sql/user/get_by_code.sql')
const sql_delete = require('../sql/user/delete.sql')
const sql_set_address = require('../sql/user/set_address.sql')
const sql_unset_address = require('../sql/user/unset_address.sql')
const sql_change_password = require('../sql/user/change_password.sql')
const sql_get_hashed = require('../sql/user/get_hashed.sql')
const sql_claim_invitations = require('../sql/user/claim_invitations.sql')
const sql_claim_contacts = require('../sql/user/claim_contacts.sql')
const sql_record_pw_recovery = require('../sql/user/record_pw_recovery.sql')
const sql_check_pw_reset_token = require('../sql/user/check_pw_reset_token.sql')
const sql_remove_pw_reset_token = require('../sql/user/remove_pw_reset_token.sql')
const sql_get_all = require('../sql/user/get_all.sql')
const sql_patch_timezone = require('../sql/user/patch_timezone.sql')
const sql_patch_facebook_access_token = require('../sql/user/patch_facebook_access_token.sql')
const sql_patch_avatars = require('../sql/user/patch_avatars.sql')
const sql_ok_push = require('../sql/user/ok_push.sql')
const sql_string_search = require('../sql/user/string_search.sql')
const sql_string_search_fuzzy = require('../sql/user/string_search_fuzzy.sql')
const sql_get_office = require('../sql/user/get_office.sql')
const sql_confirm_email = require('../sql/user/confirm_email.sql')
const sql_confirm_phone = require('../sql/user/confirm_phone.sql')
const sql_check_shadow_token_email = require('../sql/user/check_shadow_token_email.sql')
const sql_check_shadow_token_phone = require('../sql/user/check_shadow_token_phone.sql')
const sql_upgrade_to_agent = require('../sql/user/upgrade_to_agent.sql')
const sql_update_personal_room = require('../sql/user/update_personal_room.sql')
const sql_find_direct = require('../sql/user/find_direct.sql')
...
const bulk_search = {
  email: require('../sql/user/bulk_search_email.sql'),
  phone: require('../sql/user/bulk_search_phone.sql')
}
...
User.get = function (user_id, cb) {
  db.query(sql_get, [user_id], function (err, res) {
    ...
    Url.create({
     uri: '/get/' + user.user_code,
     brand: user.brand ? user.brand : false
    }, (err, url) => {
      if(err)
        return cb(err)

      user.invitation_url = url
...
User.getDisplayName = function (user) {
  if (!_u.isEmpty(user.first_name) && !_u.isEmpty(user.last_name) && !user.is_shadow)
    return user.first_name + ' ' + user.last_name

  if (!_u.isEmpty(user.email) && !user.fake_email && user.is_shadow)
    return user.email

  if (!_u.isEmpty(user.phone_number) && user.fake_email && user.is_shadow)
...
User.getAbbreviatedDisplayName = function (user) {
  if (!_u.isEmpty(user.first_name) && !user.is_shadow)
...
User.create = function (user, cb) {
  ...
  const handle_listing_inquiry = (user_id, r, brand_id, external_info, cb) => {
    ...
    validator(clone, r, (err) => {
      ...
      return Listing.inquiry(user_id,
                             r.listing,
                             r.agent,
                             brand_id,
                             r.source_type,
...
User.connectToRoom = function (user_id, room_id, cb) {
  ...
  Room.get(room_id, err => {
    ...
    return Room.addUser(user_id, room_id, false, cb)
  })
}
...
User.resetPasswordByShadowToken = function (email, phone_number, token, password, cb) {
  async.auto({
    ...
    check: [
      ...
      cb => {
        if (email) {
          db.query(sql_check_shadow_token_email, [email, token], (err, res) => {
          ...
          db.query(sql_check_shadow_token_phone, [phone_number, token], (err, res) => {
            if(err)
              return cb(err)
...
User.confirmPhone = function (id, cb) {
  db.query(sql_confirm_phone, [id], (err, res) => {
    if(err)
      return cb(err)
...
User.patchFacebookAccessToken = function (user_id, token, cb) {
  User.get(user_id, function (err, user) {
    ...
    Facebook.getAccessToken(token, (err, long_lived) => {
      ...
      db.query(sql_patch_facebook_access_token, [user_id, long_lived], function (err, res) {
        if (err)
          return cb(err)
...
User.bulkSearchByFamily = function (family, queries, cb) {
  const _queries = '{' + queries.join(',') + '}'
  const _sql = bulk_search[family]
  if (!_sql)
...
User.stringSearch = function (terms, limit, cb) {
  ...
  db.query(sql_string_search, [terms, limit], (err, res) => {
    if (err)
      return cb(err)
...
User.stringSearchFuzzy = function (terms, limit, similarity, cb) {
  ...
  db.query(sql_string_search_fuzzy, [terms, limit, similarity], (err, res) => {
    if (err)
      return cb(err)
...
User.getOrCreateByPhoneNumberBulk = function (phones, cb) {
  async.map(phones, (r, cb) => {
...
User.combineAndUniqueUserReferences = function (user_id, users, data) {
  const e = users || []
  const se = data.email_shadows || []
  const sp = data.phone_shadows || []
...
User.sendActivation = function (user_id, context, cb) {
  const contacting_agent = cb => {
    ...
    const brand = Brand.getCurrentId()

    if (!brand)
      return cb()

    Brand.proposeContactingUser(brand, cb)
  }
  ...
  const renderHTML = (cb, results) => {
    ...
    if (!results.contacting_agent) {
      results.base_url = results.base_url;
  ...
  async.auto({
    ...
    brand: Brand.getCurrent,
    contacting_agent: contacting_agent,
    url: ['user', 'contacting_agent', 'agent', createUrl],
    base_url:         cb => Url.create({}, cb),
    html:             ['base_url', 'url', 'brand', renderHTML],
    send_email:       ['html', send_email],
    text:             ['url', 'brand', renderText],
    send_sms:         ['url','contacting_agent', send_sms]
  }, cb);
};
...
User.upgradeToAgent = function (user_id, agent_id, cb) {
  db.query(sql_upgrade_to_agent, [user_id, agent_id], cb)
}

User.createPersonalRoom = function (user_id, cb) {
  const copy = {}
  copy.room_type = 'Personal'
  copy.owner = user_id
  copy.client_type = 'Unknown'

  Room.create(copy, (err, room) => {
    if (err)
      return cb(err)

    db.query(sql_update_personal_room, [user_id, room.id], (err, res) => {
      if (err)
        return cb(err)
...
User.getOrCreateDirectRoom = function (user_id, peer_id, cb) {
  async.auto({
    ...
    search: [
      ...
      'peer',
      cb => {
        db.query(sql_find_direct, [user_id, peer_id], (err, res) => {
          if (res.rows.length < 1)
            return cb()
...
User.getStatus = function (user_id, cb) {
  ...
  job.on('complete', cb)
  job.on('failed', err => cb(null, User.OFFLINE))
...
User.isIdentical = function (user, email, phone_number, user_id) {
  ...
  if (user.phone_number && phone_number && (user.phone_number.toLowerCase() == phone_number))
    return true

  return false
}

User.isIdInArray = function (id, array) {
  return (!!id && !!array && _u.contains(array, id))
}

User.isEmailInArray = function (email, array) {
  if (!email || !array)
    return false
  ...
  return (_u.contains(a, email))
}

User.isPhoneInArray = function (phone, array) {
  if (!phone || !array)
    return false

  phone = ObjectUtil.formatPhoneNumberForDialing(phone)
  const a = array.map(r => {
    return ObjectUtil.formatPhoneNumberForDialing(r)
  })

  return (_u.contains(a, phone))
}
