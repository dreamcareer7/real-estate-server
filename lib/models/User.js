...
User.get = function (user_id, cb) {
  async.auto({
    get: cb => {
      db.query('user/get', [user_id], (err, res) => {
        if (err)
          return cb(err)
    ...
    contacts: [
      'get',
      (cb, results) => {
        if(!process.domain || !process.domain.user || (process.domain.__CGUARD_CONTACT_USER_REL && process.domain.__CGUARD_CONTACT_USER_REL[user_id]))
          return cb()

        if (!process.domain.__CGUARD_CONTACT_USER_REL)
          process.domain.__CGUARD_CONTACT_USER_REL = {}

        process.domain.__CGUARD_CONTACT_USER_REL[user_id] = true

        db.query('user/contacts', [
          process.domain.user.id,
          results.get.email,
          results.get.phone_number
        ], (err, res) => {
          if(err)
            return cb(err)

          const contact_ids = res.rows.map(r => r.contact)
          return cb(null, contact_ids)
        })
      }
    ]
  }, (err, results) => {
    ...
    if(results.contacts)
      user.contacts = results.contacts
