  ...
  async.auto({
    ...
    deref_connect: [
      ...
      cb => {
        ...
        return User.getByCode(user.connect, cb);
      }
    ],
    connect: [
      'insert',
      'deref_connect',
      (cb, results) => {
        if(!user.connect || !results.deref_connect)
          return cb();
        ...
        return Room.bulkCreateWithUsers(results.insert, [results.deref_connect.id], override, cb);
      }
    ]
  }, (err, results) => {
    if(err)
