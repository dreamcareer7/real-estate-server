...
User.create = function(user, cb) {
  ...
  var handle_listing_inquiry = (r, opts, results, external_info, cb) => {
    var in_user = results.agent.user_id || results.brand.default_user;
    var clone = _u.clone(schema_handle_action);

    if(!in_user)
      return cb(Error.Validation('Cannot find a person to connect user to'));
    ...
    async.auto({
      ...
      room: [
        'validate',
        cb => {
          return handle_user_connect(opts.insert, in_user, r.source_type, r.brand, cb);
        }
      ],
      recommendation: [
        'validate',
        'room',
        (cb, res) => {
          return Room.recommendListing(res.room, r.listing, external_info, cb);
        }
      ],
      message: [
        'validate',
        ...
        (cb, res) => {
          ...
          return Message.post(res.room, message, true, cb);
        }
      ]
    }, (err, res) => {
      if(err)
...
User.combineCurrentShadowAndPhone = function(user_id, users, data) {
  var e = users || [];
  var s = data.shadows || [];
  var p = data.phones.existing.ids || [];

  var combine = e.concat(s).concat(p).filter(Boolean);
      ...
      send_email: [
        ...
        (cb, results) => {
          ...
          if(team) {
            ...
            template_params.agent_profile_url = team.agent.profile_image_url;
