...
User.create = function(user, cb) {
  ...
  var handle_favorite_listing = (r, opts, results, external_info, cb) => {
    var clone = _u.clone(schema_handle_action);

    async.auto({
      validate: cb => {
        return validator(clone, r, cb);
      },
      recommendation: [
        'validate',
        cb => {
          Room.recommendListing(opts.personal_room.id, r.listing, external_info, cb);
        }
      ],
      favorite: [
        'validate',
        'recommendation',
        (cb, results) => {
          Recommendation.patch('favorite', true, opts.insert, results.recommendation.id, true, cb);
        }
      ]
    }, (err, res) => {
      if(err)
  ...
  var handle_create_alert = (r, opts, results, cb) => {
    var alert = r.alert;
    var clone = _u.clone(schema_handle_action);
    clone.properties.alert.required = true;
    clone.properties.agent.required = false;
    clone.properties.listing.required = false;

    alert.created_by = opts.insert;
    ...
    async.auto({
      ...
      alert: [
        'validate',
        (cb, res) => {
          return Alert.create(opts.personal_room.id, alert, cb);
        }
      ],
      default_user: cb => {
      ...
      return cb(null, {
        ...
        brand: results.brand,
        alert: res.alert.id,
        agent: res.default_user
      });
    });
  };

  var handle_listing_inquiry = (r, opts, results, external_info, cb) => {
    var clone = _u.clone(schema_handle_action);
    ...
    validator(clone, r, (err) => {
      ...
      return Listing.inquiry(opts.insert,
                             r.listing,
                             results.agent,
                             results.brand,
                             r.source_type,
  ...
  async.auto({
            ...
            handle: [
              ...
              (cb, results) => {
                if (r.action == 'favorite_listing') {
                  return handle_favorite_listing(r, opts, results, external_info, cb);
                } else if (r.action == 'create_alert') {
                  return handle_create_alert(r, opts, results, cb);
                } else if (r.action == 'listing_inquiry') {
                  return handle_listing_inquiry(r, opts, results, external_info, cb);
                } else if (r.action == 'user_connect') {
                  return handle_user_connect(opts.insert, r.user, r.source_type, r.brand, cb);
    ...
    send_activation: [
      ...
      (cb, results) => {
        ...
        if(results.handle_actions && results.handle_actions.brand) {
          team = {};
...
User.sendActivation = function(user_id, team, cb) {
    ...
    async.auto({
      ...
      branch: [
        ...
        (cb, results) => {
          ...
          if(!team)
            ...
            var customized_desktop_url = desktop_url.replace('://', '://' + team.brand.subdomain + '.');
            var redirect = '';

            if(team.action == 'create_alert')
              redirect = '&action=create_alert' + '&alert_id=' + encodeURIComponent(team.alert);
            else if(team.action == 'listing_inquiry')
              redirect = '&action=listing_inquiry' + '&room_id=' + encodeURIComponent(team.room);
            else if (team.action == 'favorite_listing')
              redirect = '&action=favorite_listing' + '&listing_id=' + encodeURIComponent(team.listing);
      ...
      send_email: [
        ...
        (cb, results) => {
          ...
          if(team) {
            template_params.team_logo = team.brand.logo_url;
            template_params.team_name = team.brand.title;
            template_params.team_color = team.brand.palette.primary_color;
            template_params.agent_profile_url = team.agent.profile_image_url || 'http://assets.rechat.com/mail/avatar.png';
            template_params.agent_name = team.agent.first_name + ' ' + team.agent.last_name;
          }
          ...
          return Email.send({
            ...
            to: [ user.email ],
            source: config.email.source,
            html_body: team ? html_activation_team : html_body,
            suppress_outer_template: team ? true : false,
            message: {
              body: {
                html: {
                  data: team ? html_activation_team : html_activation
                },
                text: {
