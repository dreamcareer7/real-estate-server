...
var sql_ok_push_shortlist = require('../sql/user/ok_push_shortlist.sql');
    ...
    async.parallel({
      address: function(cb) {
        ...
        Address.get(user.address_id, cb);
         ...
         user.invitation_url = config.webapp.base_download_url + user.user_code;
...
User.isPushOKOnShortlist = function(id, shortlist_id, cb) {
  db.query(sql_ok_push_shortlist, [id, shortlist_id], function(err, res) {
    if(err)
      return cb(err);
