  ...
  async.auto({
    ...
    room_connect: [
      ...
      (cb, results) => {
        ...
        Room.get(user.room_connect, err => {
          ...
          return Room.addUser(results.insert, user.room_connect, false, cb);
        });
      }
    ],
    get: [
    ...
    send_activation: [
      ...
      (cb, results) => {
        ...
        return User.sendActivation(results.insert, cb);
      }
    ]
  }, (err, results) => {
    if(err)
...
User.publicize = function(model) {
  ...
  if(model.agent) Agent.publicize(model.agent);
...
User.createPersonalRoom = function(user_id, cb) {
  ...
  Room.create(copy, (err, room) => {
    ...
    db.query(sql_update_personal_room, [user_id, room.id], cb);
  });
};
