    ...
    user_connect: [
      ...
      (cb, results) => {
        ...
        var override = {
          ...
          from: user.user_connect
        };

        User.get(user.user_connect, (err, user_connect) => {
          if(err)
            return cb(err);

          var contact = {
            contact: user.user_connect,
            source_type: 'BrokerageWidget',
            email: user_connect.email,
            brand: user.brand
          }

          Contact.add(results.insert, contact, err => {
            if(err)
              return cb(err);

            Room.bulkCreateWithUsers(results.insert, [user.user_connect], override, cb);
          });
        });
      }
    ],
    ...
    handle_actions: [
      ...
      (cb, results) => {
        ...
        async.map(user.actions, (r, cb) => {
          validator(schema_user_create, r, err => {
            if(err)
              return cb(err);
            ...
            if (r.action == 'favorite_listing') {
              async.auto({
                recommendation: cb => {
                  Room.recommendListing(results.personal_room.id, r.listing, external_info, cb);
                },
                favorite: [
                  'recommendation',
                  (cb, results) => {
                ...
                return cb(null, {listing: r.listing});
              });
            } else if (r.action == 'create_alert') {
              var alert = r.alert;
              alert.created_by = results.insert;

              return Alert.create(results.personal_room.id, alert, (err, alert) => {
                if(err)
                  return cb(err);

                return cb(null, {alert: alert.id});
              });
            } else if (r.action == 'listing_inquiry') {
              if(!results.user_connect)
                return cb(Error.Validation('Cannot find a person to connect user to'));

              var in_room = results.user_connect[0];
              async.auto({
                recommendation: cb => {
                  Room.recommendListing(in_room, r.listing, external_info, cb);
                },
                user_connect: cb => User.get(user.user_connect, cb),
                message: [
                  'recommendation',
                  cb => {
                    var message = {
                      author: user.user_connect,
                      message_type: 'TopLevel',
                      comment: 'I see that you\'re interested in this house. Let me know how I can help you.'
                    };

                    return Message.post(in_room, message, true, cb);
                  }
                ]
              }, (err, results) => {
                if(err)
                ...
                return cb(null, {listing: r.listing, user_connect:results.user_connect});
              });
            } else {
              return cb();
            }
      ...
      branch: [
        ...
        (cb, results) => {
          ...
          if(!team)
            ...
            if(team.alert)
              redirect = '&alert_id=' + team.alert;
            else if (team.listing)
              redirect = '&listing_id=' + team.listing;

            customized_desktop_url += encodeURIComponent(redirect);
      ...
      send_email: [
        ...
        (cb, results) => {
          ...
          return Email.send({
            ...
            to: [ user.email ],
            source: config.email.source,
            html_body: team ? html_activation_team : html_activation,
