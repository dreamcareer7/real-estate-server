...
User.setAvatar = function(user_id, avatar, ext, cb) {
  User.get(user_id, function(err, user) {
    if(err)
      return cb(err);

    S3.upload(config.buckets.avatars, avatar, ext, function(err, url) {
      if(err)
        return cb(err);
...
User.setCover = function(user_id, avatar, ext, cb) {
  User.get(user_id, function(err, user) {
    if(err)
      return cb(err);

    S3.upload(config.buckets.user_covers, avatar, ext, function(err, url) {
      if(err)
        return cb(err);
