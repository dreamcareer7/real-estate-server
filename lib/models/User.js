...
User.patch = function(user_id, user, new_password, cb) {
  if (new_password) {
    User.hashPassword(new_password, function(err, hash) {
      if(err)
        return cb(err);
...
User.getHashedPassword = function(id, cb) {
  db.query(sql_get_hashed, [id], function(err, res) {
    if(err)
      return cb(err);
