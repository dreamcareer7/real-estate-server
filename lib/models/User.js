...
User.getByEmail = function (email, cb) {
  db.query('user/get_by_email', [email], (err, res) => {
    if (err)
      return cb(err)
...
User.patch = function (user_id, user, cb) {
  const email_owner = cb => {
    ...
    User.getByEmail(user.email, (err, email_owner) => {
      if (err)
        return cb(err)
...
User.initiatePasswordReset = function (email, cb) {
  User.getByEmail(email, (err, user) => {
    ...
    db.query('user/record_pw_recovery', [email, user.id, token], (err, res) => {
      ...
      /* Rahul's initial fix attempt
      const mailgun_options = {}
      const subject = text_subject_password_recovery
      Email.sendSane({
        from: 'Rechat <' + config.email.from + '>',
        to: [user.email],
        subject,
        mailgun_options,
        html: html_password_recovery
      }, cb) */
      ...
      const sendEmail = (cb, results) => {
        ...
        Email.sane({
          from: config.email.from,  //TODO check from email
          to: [ results.user.email ],
          html: results.html,
          subject: 'Reset your Rechat Password'
        }, cb)
      }

      async.auto({
        user: cb => User.get(user.id, cb),
        token: ['user', getToken],
        url: ['token', getUrl],
        html: ['url', renderHTML],
...
User.resetPassword = function (email, token, password, cb) {
  async.auto({
    ...
    update_password: [
      ...
      'check',
      (cb, results) => {
        User.updatePassword(results.get.id, password, cb)
      }
    ],
    email: [
      'get',
      ...
      (cb, results) => {
        ...
        const mailgun_options = {}
        const subject = text_subject_password_recovery_done
        ...
        Email.sendSane({
          ...
          subject,
          mailgun_options,
          html: html_password_recovery_done
        }, cb) */
