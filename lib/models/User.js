  ...
  async.auto({
    ...
    send_activation: [
      ...
      (cb, results) => {
        ...
        if(results.agent) {
          debug('>>> (User::create::send_activation) Matched agent id:', results.agent, 'for this user');
          verification.agent = results.agent;
          verification.token = results.get.secondary_password;
        }

        return EmailVerification.create(verification, true, cb);
      }
    ]
  }, (err, results) => {
    if(err)
    ...
    async.auto({
      ...
      branch: [
        ...
        (cb, results) => {
          ...
          var token_plain = JSON.stringify({
            email: user.email,
            token: user.secondary_password
          });

          if(results.agent) {
            token_plain.agent = results.agent;
