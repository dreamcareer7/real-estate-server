...
User.setAddress = function(user_id, address, cb) {
  Address.create(address, function(err, address_id) {
    ...
    db.query(sql_set_address, [address_id, user_id], cb);
  });
}
...
User.unsetAddress = function(user_id, cb) {
  User.get(user_id, function(err, user) {
    ...
    Address.delete(user.address_id, function(err) {
      if(err)
        return cb(err);
...
/**
 * Returns all message rooms a user is current a part of
 * @name getRooms
 * @function
 * @memberof User
 * @instance
 ...
 * @param {uuid} user_id - ID of the referenced user
 * @param {callback} cb - callback function
 * @returns {room[]} collection of rooms
 */
User.getRooms = function(user_id, cb) {
  db.query(sql_rooms, [user_id], function(err, res) {
    if(err)
      return cb(err);
    ...
    async.map(message_room_ids, Room.get, function(err, message_rooms) {
      if(err)
        return cb(err);
