...
var sql_patch_facebook_access_token     = require('../sql/user/patch_facebook_access_token.sql');
...
User.patch = function(user_id, user, cb) {
  ...
  var email_verification = (cb, results) => {
    if(!user.email)
      return cb();
  ...
  var phone_verification = (cb, results) => {
    if(!user.phone_number)
      return cb();
...
User.stringSearch = function(terms, limit, similarity, cb) {
  terms = terms.join('|');

  db.query(sql_string_search, [terms, limit, similarity], (err, res) => {
    if(err)
      return cb(err);
  ...
  User.get(user_id, (err, user) => {
    ...
    async.auto({
      ...
      contacting_agent: [
        'brand',
        (cb, results) => {
          if(!context.agent && !results.brand)
            return cb();
