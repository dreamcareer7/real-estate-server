...
var sql_string_search_fuzzy             = require('../sql/user/string_search_fuzzy.sql');
...
User.sendActivation = function(user_id, context, cb) {
  var contacting_agent = cb => {
    if(context.agent)
      return Agent.get(context.agent, cb);


    var brand = Brand.getCurrentId();
  ...
  var renderHTML = (cb, results) => {
    var template = __dirname + '/../html/user/';
  ...
  var renderText = (cb, results) => {
    var template = __dirname + '/../asc/user/activation.asc';
    Template.render(template, results, cb)
  }

  var send_sms = (cb, results) => {
