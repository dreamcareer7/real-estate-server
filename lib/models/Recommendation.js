...
sql_actives_user              = require('../sql/recommendation/actives_user.sql');
...
Recommendation.getSetForUserOnRoom = function(set, user_id, room_id, paging, cb) {
  ...
  async.auto({
    ...
    collection: [
      ...
      'set',
      (cb, results) => {
        var recommendation_ids = results.set.rows.map(function(r) {
          return r.id;
        });
...
Recommendation.get = function(recommendation_id, cb) {
  db.query(sql_get, [recommendation_id], function(err, res) {
    ...
    async.parallel({
      ...
      counts: function(cb) {
        return Recommendation.getCounts(recommendation_id, cb);
      }
    }, function(err, results) {
      if(err)
        return cb(err);
      ...
      recommendation.comment_count = results.counts.comment_count;
      recommendation.document_count = results.counts.document_count;
      recommendation.video_count = results.counts.video_count;
      recommendation.image_count = results.counts.image_count;
