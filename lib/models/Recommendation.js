...
function getFeedForUserOnShortlist(user_id, shortlist_id, paging, cb) {
  db.query(sql_feed, [user_id, shortlist_id, paging.type, paging.timestamp, paging.limit, paging.filter], function(err, res) {
    if(err) {
      return cb(err);
    }
    ...
    db.query(sql_pin, [recommendation_id], function(err, res) {
      ...
      Notification.issueForShortlistExcept(recommendation.referred_shortlist.id,
                                           recommendation.referring_user.id,
                                           {
                                             ...
                                             message: recommendation.referring_user.first_name + ' liked ' + recommendation.listing.property.address.title + ' in ' + recommendation.listing.property.address.city,
