...
function getSeenForUserOnShortlist(user_id, shortlist_id, cb) {
  db.query(sql_seen, [user_id, shortlist_id], function(err, res) {
    if(err) {
      return cb(err);
    }
...
Recommendation.getSeenForUserOnShortlist = function(user_id, shortlist_id, cb) {
  User.get(user_id, function(err, user) {
    if(err)
      return cb(err);
    ...
    Shortlist.get(shortlist_id, function(err, shortlist) {
      ...
      getSeenForUserOnShortlist(user_id, shortlist_id, cb);
    });
  });
}
