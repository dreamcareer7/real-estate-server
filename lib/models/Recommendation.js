...
function getFeedForUserOnShortlist(user_id, shortlist_id, paging, cb) {
  db.query(sql_feed, [user_id, shortlist_id, paging.type, paging.id, paging.ascending, paging.limit], function(err, res) {
    if(err) {
      return cb(err);
    }
...
function getFeedForUser(user_id, paging, cb) {
  db.query(sql_feed_solo, [user_id, paging.type, paging.id, paging.ascending, paging.limit], function(err, res) {
    if(err) {
      return cb(err);
    }
...
function getFavoritesForUserOnShortlist(user_id, shortlist_id, paging, cb) {
  db.query(sql_favorites, [user_id, shortlist_id, paging.type, paging.id, paging.ascending, paging.limit], function(err, res) {
    if(err) {
      return cb(err);
    }
...
function getFavoritesForUser(user_id, paging, cb) {
  db.query(sql_favorites_solo, [user_id, paging.type, paging.id, paging.ascending, paging.limit], function(err, res) {
    if(err) {
      return cb(err);
    }
...
Recommendation.publicize = function(model) {
  ...
  if (model.message_room) MessageRoom.publicze(model.message_room);
