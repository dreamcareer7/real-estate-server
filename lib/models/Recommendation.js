...
Recommendation.patch = function(property, action, user_id, recommendation_id, notify, cb) {
  async.auto({
    ...
    room_users: [
      ...
      'patch',
      (cb, results) => {
        return Room.getUsers(results.recommendation.room, cb);
      }
    ],
    notification: [
      'room',
      ...
      (cb, results) => {
        if(notify) {
          ...
          if(property == 'favorite' && action) {
            ...
            notification.message = '#' + Room.getTitle({title: results.room.title, users: results.room_users})
              + ': @' + results.user.first_name + ' favorited ' + address_line;
