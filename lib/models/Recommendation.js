...
const sql_insert = require('../sql/recommendation/insert.sql')
const sql_get = require('../sql/recommendation/get.sql')
const sql_add_reference_to_recs = require('../sql/recommendation/add_reference_to_recs.sql')
const sql_unhide_recs = require('../sql/recommendation/unhide_recs.sql')
const sql_counts = require('../sql/recommendation/counts.sql')
const sql_user_favorites = require('../sql/recommendation/user_favorites.sql')
const sql_map_to_alerts = require('../sql/recommendation/map_to_alerts.sql')

const sqls = {
  feed:            require('../sql/recommendation/feed.sql'),
  actives:         require('../sql/recommendation/actives.sql'),
  seen:            require('../sql/recommendation/seen.sql'),
  favorites:       require('../sql/recommendation/favorites.sql'),
  tours:           require('../sql/recommendation/tours.sql'),
  add_read:        require('../sql/recommendation/add_read.sql'),
  remove_read:     require('../sql/recommendation/remove_read.sql'),
  add_favorite:    require('../sql/recommendation/add_favorite.sql'),
  remove_favorite: require('../sql/recommendation/remove_favorite.sql'),
  add_tour:        require('../sql/recommendation/add_tour.sql'),
  remove_tour:     require('../sql/recommendation/remove_tour.sql')
}
  ...
  async.auto({
    ...
    set: [
      ...
      cb => {
        ...
        return db.query(set_sql, [user_id, room_id, filter, paging.type, paging.timestamp, paging.limit], cb)
      }
    ],
    collection: [
      'user',
      ...
      'set',
      (cb, results) => {
        const recommendation_ids = results.set.rows.map(function (r) {
          return r.id
        })
        ...
        async.map(recommendation_ids, Recommendation.get, function (err, recommendations) {
          ...
          if (results.set.rows.length > 0)
            recommendations[0].total = results.set.rows[0].total
...
Recommendation.patch = function (property, action, user_id, recommendation_id, notify, cb) {
  async.auto({
    ...
    sql: cb => {
      ...
      const _cmd = sqls[_action + property]
      ...
      (cb, results) => {
        if (notify) {
          ...
          if (property == 'favorite' && action) {
            ...
            notification.message = '#' + results.room.proposed_title + ': @' + results.user.first_name + ' favorited ' + address_line
            ...
            notification.message = '[TOUR ALERT] #' + results.room.proposed_title + ': @' + results.user.first_name + ' wants to go see ' + address_line
            notification.message = notification.message.trim()
            return Notification.issueForRoomExcept(notification, user_id, cb)
          } else {
            return cb()
...
Recommendation.getUserFavorites = function (user_id, cb) {
  db.query(sql_user_favorites, [user_id], (err, res) => {
    if (err)
      return cb(err)
...
Recommendation.mapToAlerts = function (recommendations, cb) {
  db.query(sql_map_to_alerts, [recommendations], (err, res) => {
    if (err)
      return cb(err)
