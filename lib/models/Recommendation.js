...
Recommendation.createAll = async function (recommendations, referringId) {
  ...
  const query = squel.insert()
    ...
    .onConflict(['room', 'listing'], {
      hidden: false,
      updated_at: squel.rstr('transaction_timestamp()'),
      referring_objects: squel.case()
        .when(`${TABLE_NAME}.referring_objects @> ARRAY[?]::uuid[]`, referringId)
        .then(squel.rstr(`${TABLE_NAME}.referring_objects`))
        .else(squel.rstr(`ARRAY_APPEND(${TABLE_NAME}.referring_objects, ?)`, referringId))
    })
