    ...
    Shortlist.get(shortlist_id, function(err, shortlist) {
      ...
      db.query(sql_get_shortlist, [id, shortlist_id], function(err, res_base) {
        ...
        async.parallel({
          ...
          added_tour_by: function(cb) {
            if(!recommendation.added_tour_by)
              return cb();

            recommendation.added_tour_by = recommendation.added_tour_by.filter(Boolean);
            async.map(recommendation.added_tour_by, User.get, cb);
          },
          message_room: function(cb) {
            if(!recommendation.message_room)
              return cb();
            ...
            MessageRoom.getForUser(recommendation.message_room, recommendation.referring_user, cb);
             ...
             res_final.added_tour_by = results.added_tour_by || [];
             res_final.message_room = results.message_room || null;
...
Recommendation.pin = function(recommendation_id, cb) {
  Recommendation.get(recommendation_id, function(err, recommendation) {
    ...
    db.query(sql_pin, [recommendation_id], function(err, res) {
      ...
      //                                        notifying_users: recommendation.referring_user.id
      //                                      }, function(err, ok) {
      //                                           if(err)
      //                                             return cb(err);
...
Recommendation.patch = function(recommendation_id, favorite, cb) {
  Recommendation.get(recommendation_id, function(err, recommendation) {
    if(err)
      return cb(err);
    ...
    User.get(user_id, function(err, user) {
      ...
      db.query(sql_patch_tour, [recommendation_id, tour], function(err, res) {
        ...
        if (tour) {
          ...
          notification.message = '[TOUR ALERT] #' + recommendation.listing.property.address.title + ': @' + user.first_name + ' wants to tour '
                               + ((recommendation.listing.property.address.street_dir_prefix) ? (recommendation.listing.property.address.street_dir_prefix + ' ') : '')
                               + recommendation.listing.property.address.street_number + ' '
                               + recommendation.listing.property.address.street_name + ' '
                               + ((recommendation.listing.property.address.street_suffix) ? (recommendation.listing.property.address.street_suffix + ' ') : '')
                               + ((recommendation.listing.property.address.street_dir_suffix) ? (recommendation.listing.property.address.street_dir_suffix + ' '): '');
          notification.message = notification.message.trim();

          notification.object = recommendation.referred_shortlist.id;
          notification.notifying_users = user_id;
          notification.listing = recommendation.listing.id;
          notification.subject = recommendation.listing.id;
...
Recommendation.getToursForUserOnShortlist = function(user_id, shortlist_id, paging, cb) {
  User.get(user_id, function(err, user) {
    ...
    Shortlist.get(shortlist_id, function(err, shortlist) {
      ...
      getToursForUserOnShortlist(user_id, shortlist_id, paging, cb);
    });
  });
}
...
Recommendation.getSeenForUserOnShortlist = function(user_id, shortlist_id, paging, cb) {
  User.get(user_id, function(err, user) {
    ...
    Shortlist.get(shortlist_id, function(err, shortlist) {
      ...
      getSeenForUserOnShortlist(user_id, shortlist_id, paging, cb);
    });
  });
}
    ...
    Listing.matchingShortlistsbyAlerts(id, function(err, sat_list) {
      ...
      async.mapSeries(sat_list, function(sat, cb) {
        console.log('Recommending Listing with MUI:'.cyan,
                    ...
                    sat.id.yellow);
        Shortlist.recommendListing(sat.shortlist, listing.id, sat.id, function(err, results) {
          if(err)
            return cb(null, null);
...
Recommendation.publicize = function(model) {
  ...
  if (model.added_tour_by) model.added_tour_by.map(User.publicize);
