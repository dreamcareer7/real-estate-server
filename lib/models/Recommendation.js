    ...
    User.get(user_id, function(err, user) {
      ...
      db.query(sql_patch_tour, [recommendation_id, tour], function(err, res) {
        ...
        if (tour) {
          ...
          notification.message = user.first_name + ' toured a listing';
          ...
          Notification.issueForShortlistExcept(recommendation.referred_shortlist.id, alert.created_by.id, notification, function(err, ok) {
            if(err)
              return cb(err);
