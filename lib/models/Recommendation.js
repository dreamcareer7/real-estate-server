...
Recommendation.validate = function (recomObj) {
  return promisify(validate)(recomObj)
}

Recommendation.insertList = async function (data) {
  const query = squel.insert()
    .into(TABLE_NAME)
    .setFieldsRows(data)
  
  const {text, values} = query.toParam()
  const extendedQueryText = text + ' on conflict do nothing returning id'
  return promisify(db.executeSql)(extendedQueryText, values)
}
...
Recommendation.get = function (recommendation_id, cb) {
  Recommendation.getAll([recommendation_id], (err, recommendations) => {
    if(err)
      return cb(err)
...
Recommendation.getAll = function(recommendation_ids, cb) {
...
Recommendation.generateForListing = function (id, cb) {
  Listing.get(id, (err, listing) => {
    ...
    // This gets id, and room from alerts
    Listing.matchingRoomsByAlerts(id, (err, sat_list) => {
      if (err)
        return cb(err)

      // Room.recommendListingToRooms(listing, sat_list, cb)
