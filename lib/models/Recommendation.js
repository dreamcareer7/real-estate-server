  ...
  User.get(user_id, function(err, user) {
    ...
    Room.get(room_id, function(err, room) {
      ...
      db.query(set_sql, [user_id, room_id, filter, paging.type, paging.timestamp, paging.limit], function(err, res) {
        ...
        async.map(recommendation_ids, function(r, cb) {
          async.auto({
            recommendation: function(cb) {
              return Recommendation.get(r, cb);
            },
            counts: function(cb) {
              return Recommendation.getCounts(r, cb);
            }
          }, function(err, results) {
            if(err)
              return cb(err);

            results.recommendation.comment_count = results.counts.comment_count;
            results.recommendation.document_count = results.counts.document_count;
            results.recommendation.video_count = results.counts.video_count;
            results.recommendation.image_count = results.counts.image_count;
