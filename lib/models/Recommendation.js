...
var sql_favorite = require('../sql/recommendation/favorite.sql');
var sql_feed_solo = require('../sql/recommendation/feed_solo.sql');
var sql_unfavorite = require('../sql/recommendation/unfavorite.sql');
...
Recommendation.favorite = function(recommendation_id, cb) {
  Recommendation.get(recommendation_id, function(err, recommendation) {
    ...
    db.query(sql_favorite, [recommendation_id], function(err, res) {
      if (err)
        return cb(err);
...
Recommendation.unfavorite = function(recommendation_id, cb) {
  db.query(sql_unfavorite, [recommendation_id], function(err, res) {
    if (err)
      return cb(err);
...
Recommendation.patchRecommendationForUserOnShortlist = function(recommendation_id, hidden, cb) {
  db.query(sql_patch, [recommendation_id, hidden], function(err, res) {
    if (err)
      return cb(err);
...
Recommendation.publicize = function(model) {
  ...
  if (model.favorited_by) model.favorited_by = model.favorited_by.filter(Boolean);
