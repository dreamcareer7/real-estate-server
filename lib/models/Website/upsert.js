...
const generateHostnames = async ({ listing, brand_id }) => {
  ...
  if (listing) {
    const address = listing.property.address.street_address.replace(/\s{1,}/g, ' ')
    suggestions.push(address.replace(/\s/g, ''))
    suggestions.push(address.replace(/\s/g, '-'))

    let dynamic = address.replace(/\s/g, '-')
    while(dynamic.indexOf('-') > -1) {
  ...
  const brand = brand_id ? await Brand.get(brand_id) : null
  const host = brand?.websites_root_domain || config.webserver.host
  const suffix = `.${host}`
