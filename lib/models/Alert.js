...
Alert.get = function (alert_id, cb) {
  const user_id = (process.domain && process.domain.user) ? process.domain.user.id : undefined

  db.query('alert/get', [alert_id, user_id], (err, res_base) => {
    if (err)
      return cb(err)
  ...
  async.auto({
    ...
    activity: [
      ...
      (cb, results) => {
        ...
        Activity.add(alert.created_by, 'User', activity, cb)
      }
    ]
  }, (err, results) => {
    if (err)
...
Alert.getListingIds = function(alert_id, cb) {
  db.query('alert/listings', [alert_id], (err, res) => {
    if(err)
      return cb(err)

    cb(null, res.rows.map( r => r.listing ))
  })
}
...
Alert.delete = function (alert_id, cb) {
  async.auto({
    ...
    delete: [
      'hide',
      cb => {
        return db.query('alert/delete', [alert_id], cb)
      }
    ]
  }, (err, results) => {
    if (err)
