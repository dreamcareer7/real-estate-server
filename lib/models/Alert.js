...
Alert.get = function (alert_id, cb) {
  db.query(sql_get, [alert_id], (err, res_base) => {
    ...
    async.auto({
      ...
      users: cb => {
          return Room.getUsers(alert.room, cb);
      }
    }, (err, results) => {
      if (err)
        return cb(err);
      ...
      alert.users = results.users || null;
...
Alert.sendAsSMSToNonUser = function(user_id, data, phone, cb) {
  async.auto({
    invite: cb => {
      return Invitation.invitePhoneNumberToNewRoom(user_id, phone, data.alert_title, cb);
    },
    archive_room: [
      'invite',
      (cb, results) => {
...
Alert.send = function(transport, data, cb) {
  ...
  var job_data = {
    ...
    alert_title: data.alert.title || 'New Alert',
    ...
    alert_uri: data.room.id ?
      config.webapp.base_url + '/dashboard/recents/' + data.room.id + '?alert=' + data.alert.id + '&token=' + encodeURIComponent(data.token) :
      data.url,
