                   ...
                   Shortlist.incAlertIndex(shortlist_id, function(err, ok) {
                     ...
                     Alert.get(res.rows[0].id, function(err, alert) {
                       ...
                       Alert.recommendListings(alert.id, shortlist_id, function(err, reulsts) {
                         ...
                         notification.message = user.first_name + ' set an alert for your group';
                         notification.object = shortlist_id;
                         notification.notifying_user = alert.created_by.id;
                         notification.subject = alert_id;
                         notification.subject_class = 'Alert';
                   ...
                   Alert.removeFromRecommendationsReferences(alert_id, alert.shortlist.id, function(err, ok) {
                     ...
                     if(err)
                       ...
                       Alert.recommendListings(alert_id, shortlist_id, function(err, reulsts) {
                         ...
                         notification.object_class = 'Shortlist';
                         notification.message = user.first_name + ' changed an alert setting for your group';
                         notification.object =  shortlist_id;
                         notification.notifying_user = user_id;
                         notification.subject = alert_id;
                         notification.subject_class = 'Alert';

                         Notification.issueForShortlistExcept(shortlist_id, alert.created_by, notification, function(err, ok) {
                           if(err)
                             return cb(err);
