    ...
    User.get(alert.created_by, function(err, owner) {
      ...
      validate(alert, function(err) {
        ...
        db.query(sql_insert, [alert.currency,
                              ...
                              alert.title || 'Alert #' + shortlist.alert_index,
...
Alert.check = function(shortlist_id, alert, cb) {
  Shortlist.get(shortlist_id, function(err, shortlist) {
    if(err)
      return cb(err);
    ...
    User.get(alert.created_by, function(err, user) {
      ...
      validate(alert, function(err) {
        ...
        Alert.matchingListingsForAlertData(alert, function(err, listings) {
          if(err)
            return cb(err);

          return cb(null, {type: 'count', count: listings.length});
        });
      });
    });
  });
