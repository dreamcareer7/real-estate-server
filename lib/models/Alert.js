...
function parseArea(alert) {
  ...
  var parsePart = function(part) {
    ...
    if(part.indexOf(',') > -1)
      return part.split(',').map(num => parseInt(num));
    ...
    if(part.indexOf('-') > -1) {
      ...
      if(range.length !== 2)
        return new Error.Validation('Cannot parse area');
  ...
  alert.mls_area_major = parsePart(majorPart);

  if(minorPart)
    alert.mls_area_minor = parsePart(minorPart);
}
