...
Alert.get = function(id, cb) {
  db.query(sql_get, [id], function(err, res_base) {
    ...
    async.parallel({
      ...
      search_area: function(cb) {
        if(!alert.search_area)
          return cb();

        var search_area = JSON.parse(alert.search_area);
        search_area = search_area.coordinates[0].map(function(r) {
          return ({longitude: r[0], latitude: r[1], type: 'location'});
        });

        return cb(null, search_area);
      }
         ...
         res_final.search_area = results.search_area || null;
...
Alert.create = function(shortlist_id, alert, cb) {
  Shortlist.get(shortlist_id, function(err, shortlist) {
    ...
    validate(alert, function(err) {
      ...
      var search_area = Alert.getGeomTextFromLocationArray(alert.search_area);
      ...
      db.query(sql_insert, [alert.currency,
                            ...
                            'Hot sheet ' + shortlist.alert_index,
                 ...
                 Shortlist.incAlertIndex(shortlist_id, function(err, ok) {
                   ...
                   Alert.get(res.rows[0].id, function(err, alert) {
                     ...
                     Alert.recommendListings(alert.id, shortlist_id, function(err, reulsts) {
                       if(err)
                         console.log('ERR-CRIT: (CREATE FLOW) on recommending listings to shortlists with id:', shortlist_id, 'err:', err);
                     });
                   });
                 });
               });
...
Alert.recommendListings = function(id, shortlist_id, cb) {
  Alert.matchingListings(id, function(err, listings) {
    ...
    async.mapSeries(listings, function(r, cb) {
      console.log('---> Recommending', r, 'to', shortlist_id);
      return Shortlist.recommendListing(shortlist_id, r, id, cb);
    }, function(err, results) {
         if(err)
           return cb(err);
...
Alert.patch = function(shortlist_id, alert_id, alert, cb) {
  Alert.get(alert_id, function(err, data) {
    ...
    var search_area = Alert.getGeomTextFromLocationArray(data.search_area);
             ...
             function(err, res) {
               ...
               Alert.get(alert_id, function(err, alert) {
                 ...
                 Alert.removeFromRecommendationsReferences(alert_id, alert.shortlist.id, function(err, ok) {
                   ...
                   Shortlist.unpinOrphanedRecommendations(alert.shortlist.id, function(err, ok) {
                     if(err)
                       return cb(err);

                     Shortlist.removeOrphanedRecommendations(alert.shortlist.id, function(err, ok) {
                       if(err)
                         return cb(err);
                       ...
                       Alert.recommendListings(alert_id, shortlist_id, function(err, reulsts) {
                         if(err)
                           console.log('ERR-CRIT: (PATCH FLOW) on recommending listings to shortlists with id:', shortlist_id, 'err:', err);
                       });
                     });
                   });
                 });
