...
Alert.send = function(transport, data, cb) {
  ...
  async.auto({
    base_url: cb => {
      Url.create({}, (err, base_url) => {
        if(err)
          return cb(err);

        job_data.listing_uri = base_url;
        job_data.url = data.url || base_url;
        job_data.mls_terms = base_url + '/terms/mls';
      })
    },
    sender: cb => {
    ...
    url: cb => {
      ...
      var url = {
        uri: '/dashboard/mls/alerts/' + data.alert.id,
        query: {
          token: data.token
        }
      }
    ...
    branch: ['url', (cb, results) => {
      ...
      b['$desktop_url'] = results.url;
      b['$fallback_url'] = results.url;
      b.token = data.token;
