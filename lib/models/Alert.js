...
Alert.create = function(room_id, alert, cb) {
  async.auto({
    ...
    notification: ['recommend_listings',
                   function(cb, results) {
                     // var notification = {};
                     ...
                     // Notification.issueForRoomExcept(room_id, alert.created_by.id, notification, cb);
                     return cb();
                   }]
  }, function(err, results) {
       if(err)
  ...
  async.auto({
    ...
    notification: ['owner',
                   ...
                   function(cb, results) {
                     ...
                     // notification.message = '#' + results.room.title + ': @' + results.owner.first_name + ' edited an Alert, check your new listings';

                     // Notification.issueForRoomExcept(room_id, user_id, notification, cb);
                     return cb();
                   }]
  }, function(err, results) {
       if(err)
...
Alert.recommendListings = function(alert_id, room_id, cb) {
  Alert.matchingListings(alert_id, function(err, listings) {
    ...
    async.map(listings, function(r, cb) {
      console.log('---> Recommending', r, 'to', room_id);
      var external_info = {};
      external_info.ref_object_id = alert_id;

      Room.recommendListing(room_id, r, external_info, cb);
    }, function(err, results) {
         if(err)
           return cb(err);
...
Alert.delete = function(alert_id, cb) {
  Alert.get(alert_id, function(err, alert) {
    ...
    Alert.removeFromRecommendationsReferences(alert_id, alert.room, function(err, ok) {
      if(err)
        return cb(err);

      Room.hideOrphanedRecommendations(alert.room, function(err, ok) {
        if(err)
          return cb(err);

        db.query(sql_delete, [alert_id], function(err, res) {
          if(err)
            return cb(err);
...
Alert.removeFromRecommendationsReferences = function(alert_id, room_id, cb) {
  db.query(sql_remove_recs_refs, [alert_id, room_id], function(err, res) {
    if(err)
      return cb(err);
