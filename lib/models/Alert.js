...
Alert.get = function (alert_id, cb) {
  db.query(sql_get, [alert_id], (err, res_base) => {
    ...
    composeAreas(alert, cb);
  });
};

function composeAreas(alert, cb) {
  if(!alert.mls_areas)
    return cb(null, alert);

  var areas = alert.mls_areas;
  ...
  areas.forEach( a => {
    ...
    if(a.sub_areas)
      a.sub_areas.forEach( s => all.push( [s, a.area] ) )
  });

  alert.mls_areas = [];

  var fillArea = (pair, cb) => {
    Listing.getArea(pair[0], pair[1], (err, area) => {
      if(err)
        return cb(err);

      alert.mls_areas.push(area);
      cb();
    })
  }

  async.forEach(all, fillArea, err => cb(err, alert));
}
