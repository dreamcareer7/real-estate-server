...
Alert.processEmail = function(job, cb) {
  ...
  return Email.send({
    from: 'Rechat <mailer@' + config.email.seamless_address + '>',
    to: [ data.to ],
...
Alert.send = function(transport, data, cb) {
  var job_data = {
    ...
    alert_uri: config.webapp.base_url,
    listing_uri: config.webapp.base_url,
    subject: '[Rechat] ' + data.room.title,
    mls_terms: config.webapp.base_url + '/mls/terms',
  ...
  async.auto({
    sender: cb => {
      return User.get(data.sender, (err, user) => {
        ...
        job_data.agency_phone = user.phone_number || '';
