  ...
  async.auto({
    ...
    notification: [
      ...
      (cb, results) => {
        ...
        notification.auxiliary_object_class = 'Room';
        notification.message = (results.room.title ? ('#' + results.room.title + ': ') : '') +
          '@' + results.user.first_name + ' edited an Alert, check your new listings';
        notification.room = room_id;
...
var size = function(square_meters) {
...
Alert.proposeTitle = function(alert) {
  ...
  if(hasmin || hasmax) {
    if(hasmin > 0)
      area += size(alert.minimum_square_meters);
    ...
    if(hasmax)
      area += size(alert.maximum_square_meters);
  }
