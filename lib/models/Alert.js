...
Alert.get = function (alert_id, cb) {
  db.query(sql_get, [alert_id], (err, res_base) => {
    ...
    alert.proposed_title = Alert.getTitle(alert);
...
Alert.processEmail = function(job, cb) {
  ...
  return Email.send({
    ...
    mailgun_options: {
      ...
      'h:In-reply-to': data.from,
      'h:References': data.from
    },
    message: {
      body: {
        html: {
...
Alert.getTitle = function(alert) {
  ...
  var title = alert.property_type + ' ';
  title += price(alert.minimum_price) + '-' + price(alert.maximum_price);
