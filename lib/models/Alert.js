...
var html_alert_share_body = require('../html/alert/share_alert.html');
var asc_alert_share_body  = require('../asc/alert/share_alert.asc');
...
Alert.delete = function (alert_id, cb) {
  Alert.get(alert_id, (err, alert) => {
    ...
    Alert.removeFromRecommendationsReferences(alert_id, alert.room, err => {
      if (err)
        return cb(err);

      Room.hideOrphanedRecommendations(alert.room, err => {
        if (err)
          return cb(err);
...
Alert.sendAsEmail = function(data, cb) {
  async.auto({
    alert: cb => {
      return Alert.get(data.alert, cb);
    },
    sender: cb => {
      return User.get(data.sender, cb);
    },
    email: [
      'alert',
      'sender',
      (cb, results) => {
        var job = Job.queue.create('alert_share_email', {
          first_name: data.user.first_name,
          alert_title: results.alert.title,
          sender_name: results.sender.first_name,
          map_url: 'http://a.tile.openstreetmap.org/16/35741/22729.png',
          listing_count: data.count,
          bedroom_count: results.alert.minimum_bedrooms,
          bathroom_count: results.alert.minimum_bathrooms,
          square_meters: results.alert.minimum_square_meters,
          status: results.alert.listing_statuses.join(','),
          alert_uri: 'https://rechat.com',
          agent_details: 'Details',
          to: data.user.email,
          from: results.alert.room + '@' + config.email.seamless_address,
          subject: 'Rechat',
  ...
  async.auto({
    ...
    email: [
      ...
      (cb, results) => {
        ...
        var job = Job.queue.create('alert_share_sms', {
          first_name: data.user.first_name,
          alert_title: results.alert.title,
          sender_name: results.sender.first_name,
          map_url: 'http://a.tile.openstreetmap.org/16/35741/22729.png',
