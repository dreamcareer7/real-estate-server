...
Photo.markError = function (matrix_unique_id, error, cb) {
  db.query('photo/mark_as_errored', [
    error,
    matrix_unique_id
  ], cb)
}
...
Photo.getPhotosWithoutExif = function (options, cb) {
  ...
  db.query('photo/get_without_exif', [
    options.limit
  ], (err, res) => {
    if (err)
      return cb(err)
...
Photo.getUncheckedListings = function (cb) {
  db.query('photo/unchecked_listings', [], (err, res) => {
    if (err)
      return cb(err)
