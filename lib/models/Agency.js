...
Agency.get = function(id, cb) {
  db.query(sql_get, [id], function(err, res) {
    ...
    if(res.rows.length < 1)
      return cb(Error.ResourceNotFound('Agency not found'));

    var agency = res.rows[0];

    async.parallel({
      address: function(cb) {
        if(!agency.address_id)
          return cb();

        Address.get(agency.address_id, cb);
      }
    }, function(err, results) {
         if(err)
           return cb(err);

         agency.address = results.address || null;
         return cb(null, agency);
       });
  });
}
