...
Email.create = async email => {
  ...
  const { from, to, subject, html, text, headers, domain, campaign } = email
...
Email.createAll = async emails => {
  ...
  const fields = ['domain', 'from', 'to', 'subject', 'html', 'text', 'headers', 'campaign']
  ...
  for (const email of emails) {
    ...
    if (!email.campaign) email.campaign = null
