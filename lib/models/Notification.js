...
Notification.schedule = function(notification, cb) {
  ...
  var job = queue.create('create_notification', {
...
Notification.processSeamless = function(job, cb) {
  ...
  async.mapSeries(data.items, (r, cb) => {
    ...
    redisClient.del(key, (err) => {
      ...
      return Email.send({from: data.from,
                         to: [ data.to ],
...
Notification.send = function(user_id, room_id, notification, cb) {
  ...
  var stackForEmail = (cb, results) => {
    ...
    var key = ObjectUtil.getRedisJobKey({
    ...
    redisClient.get(key, (err, previous_job) => {
      console.log('&&&&&&&&&&&&&&&&&&&', err, previous_job);
      if(err)
        return cb(err);
        ...
        job: [
          ...
          (cb, results) => {
            if(!previous_job) {
              ...
              var job = queue.create('seamless_communication', {
                user_id: user_id,
                room_id: room_id,
                to: params.user.email,
                from: room_id + '@rechat.co',
                subject: 'Rechat',
              ...
              kue.Job.get(previous_job, (err, job) => {
                if(err)
                  return cb(err);
