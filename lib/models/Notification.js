...
Notification.create = function (notification, cb) {
  ...
  const push = (user_id, notification, cb) => {
    const airship = _u.clone(notification)
    ...
    insertForUser(notification.id, user_id, (err) => {
      if (err)
        return cb(err)
...
const sendAsPush = function(notification, user_id, room_id, tokens, delay, cb) {
  async.auto({
    user: (cb, results) => {
      User.get(user_id, cb)
    },
...
const sendAsText = function(notification, user_id, room_id, delay, cb) {
  const getUser = cb => {
...
Notification.send = function (user_id, notification, cb) {
  ...
  const send = (cb, results) => {
    ...
    if (results.transport === 'Push')
      return sendAsPush(notification, user_id, room_id, results.tokens, delay, cb)

    else if (results.transport === 'SMS')
      return sendAsText(notification, user_id, room_id, delay, cb)
  ...
  const getMessage = (cb, results) => {
    ...
    Orm.populate(n, {}, (err, data) => {
      if(err)
        return cb(err)

      data.target_user = results.user
      data.room = results.room

      Notification.formatForDisplay(n, data, 'generic', (err, message) => {
        if(err)
          return cb(err)
  ...
  async.auto({
    ...
    message: ['user', 'transport', 'room', getMessage],
    send: [
      'user',
      'room',
      'tokens',
...
Notification.formatForDisplay = function(notification, data, transport, cb) {
  const template = __dirname + '/../templates/notifications/' +
                   ...
                   Notification.templateFile(notification) + '_' + transport + '.tmpl'
