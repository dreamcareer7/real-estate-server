...
Job.on('saved', (job) => {
  ...
  if (Job.requiresTracking(job)) {
    ...
    console.log('>>> DOES NOT REQUIRE TRACKING');
  }
});
        ...
        job: [
          ...
          (cb, results) => {
            var item = {
              ...
              uri: 'https://rechat.com',
              type : ''
            };

            if(results.message.recommendation) {
              item.price = Listing.priceHumanReadable(results.message.recommendation.listing.price);
              item.address = Address.getLocalized(results.message.recommendation.listing.property.address);
              item.bedroom_count = results.message.recommendation.listing.property.bedroom_count;
              item.bathroom_count = results.message.recommendation.listing.property.bathroom_count;
              item.square_meters = parseInt(Listing.getSquareFeet(results.message.recommendation.listing.property.square_meters));
              item.lot_size = results.message.recommendation.listing.property.lot_size || '';
              item.year_built = results.message.recommendation.listing.property.year_built || '';
              item.uri = 'https://rechat.com';
            ...
            if(!previous_job) {
              ...
              var job = Job.queue.create('seamless_communication', {
                ...
                _title: '',
                items: [ item ]
              }).removeOnComplete(true).delay(10000);
              process.domain.jobs.push(job);
            } else {
