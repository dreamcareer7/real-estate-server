...
Notification.ackRoom = function(user_id, room_id, cb) {
  User.get(user_id, err => {
    ...
    Room.get(room_id, err => {
      ...
      Message.cancelSeamless(user_id, room_id);
...
Notification.ackType = function (user_id, room_id, type, cb) {
  // This is the most stupid piece of code I've ever seen created
  // For god's sake give me a break.

  if (type.subjects.hasIndex('User') && type.actions.hasIndex('Sent') && type.objects.hasIndex('Room'))
    Message.cancelSeamless(user_id, room_id);
...
Notification.sendPushForUnread = function(cb) {
  ...
  var queue = async.queue(sendSingle, 1);
  queue.drain = cb;
  ...
  db.query(sql_unread, [], (err, res) => {
    ...
    res.rows.map(set => {
      set.notifications.forEach(notification => {
        queue.push({
          notification,
          user: set.user
        });
...
Notification.send = function(user_id, notification, cb) {
    ...
    async.auto({
      ...
      email: [
        ...
        (cb, results) => {
          ...
          Notification.saveDelivery(notification.id, data.user.id, data.user.email, 'email', err => {
            if(err)
              return cb(err);

            Alert.sendAsEmail(data, cb);
          });
        }
      ],
      sms: [
        ...
        cb => {
          if(data.user.email)
            return cb(); //Dont send SMS. We're going to send an email.

          if(!data.user.phone_number)
            return cb(); //Dont try sending an SMS. We dont have his number.

          Notification.saveDelivery(notification.id, data.user.id, data.user.phone_number, 'sms', err => {
            if(err)
              return cb(err);

            Alert.sendAsSMS(data, cb);
          });
        }
      ]
    }, (err, results) => {
  ...
  var shareListing = (cb, results) => {
    ...
    async.auto({
      ...
      message: cb => {
        ...
        Message.retrieve(params.room.id, q, (err, messages) => {
          var m = messages.filter( m => typeof m.notification !== 'undefined' )[0];
          data.message = m;
          cb(err);
        })
      },
  ...
  var stackForEmail = (cb, results) => {
    ...
    Job.redis.get(key, (err, previous_job) => {
      ...
      async.auto({
        delivery: cb => Notification.saveDelivery(notification.id, params.user.id, params.user.email, 'email', cb),
        ...
        recommendation: [
          ...
          (cb, results) => {
            ...
            return Recommendation.get(results.message.recommendation, cb);
          }
        ],
        listing: [
          'recommendation',
          ...
          (cb, results) => {
            ...
            if(!previous_job) {
              ...
              Job.kue.Job.get(previous_job, (err, job) => {
                ...
                return process.domain.jobs.push(job);
              });
            }
