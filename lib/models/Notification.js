                ...
                function(cb, results) {
                  async.map(results.users, function(user_id, cb) {
                    Notification.totalUnreadCount(user_id, function(err, count) {
                      ...
                      return Notification.send(user_id, notification.room, airship_notification, cb);
                    });
                  }, cb);
                }],
             ...
             if (results.room_ok_for_push) {
               async.map(results.tokens, function(token, cb) {
                 ...
                 console.log('* (Airship-Transport) scheduled a notification'.green,
                             ...
                             'for user', results.user.id.blue, '(' + results.user.first_name, results.user.last_name + ')',
                             ('delay +' + delay).red);
                 cb();
               }, cb);
             } else {
