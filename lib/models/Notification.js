...
Notification.send = function(user_id, room_id, notification, cb) {
  var send = (cb, results) => {
    ...
    if ((notification.action != 'BecameAvailable') &&
      ...
      debug('>>> (Notification::send) User-generated notification, scheduling now for user:', user_id, 'on room:', room_id);
      delay = 0;
    }
    else {
      delay = (results.user_ok_for_push < 0) ? 0 : results.user_ok_for_push;
      debug('>>> (Notification::send) System-generated notification, scheduling in', delay, 'for user:', user_id, 'on room:', room_id);
    }
    if (!results.room_ok_for_push &&
        (notification.action == 'BecameAvailable' ||
         ...
         debug('>>> (Notification::send) Room is muted. Not sending System-generated push for user:', user_id, 'on room:', room_id);
         return cb();
       }
