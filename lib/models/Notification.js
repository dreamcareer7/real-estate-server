...
Notification.ack = function (user_id, notification_id, cb) {
  Notification.get(notification_id, (err, notification) => {
    ...
    db.query(sql_ack, [user_id, notification_id], (err, res) => {
      ...
      Socket.send('Notification.Ackonwledged', notification.room, [{
...
Notification.sendTextForUnread = function (cb) {
  const sendSingle = (payload, cb) => {
    const getUser = cb => {
      // eslint-disable-next-line handle-callback-err
      User.get(payload.user, (err, user) => {
    ...
    async.auto({
      user: getUser,
      brand: ['user', getBrand],
      notification: cb => Notification.get(payload.notification, cb),
