...
const sendAsPush = function(notification, user, room, tokens, delay, cb) {
  async.auto({
    ...
    message: (cb, results) => {
      Notification.formatForDisplay(notification, notification, Notification.TRANSPORT_NONE, (err, message) => {
        if(err)
          return cb(err)
      ...
      (cb, results) => {
        async.map(tokens, (token, cb) => {
          Notification.saveDelivery(notification.notification_id, user.id, token, 'airship', (err) => {
            ...
            debug('>>> (Notification::send) Job pushed to domain for user:', User.getDisplayName(user), 'on room:', room.proposed_title,
