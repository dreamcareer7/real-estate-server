...
const sql_get = require('../sql/notification/get.sql')
const sql_insert = require('../sql/notification/insert.sql')
const sql_ack = require('../sql/notification/ack.sql')
const sql_ack_room = require('../sql/notification/ack_room.sql')
const sql_ack_type = require('../sql/notification/ack_type.sql')
const sql_ack_task = require('../sql/notification/ack_task.sql')
const sql_ack_tasks = require('../sql/notification/ack_tasks.sql')
const sql_ack_transaction = require('../sql/notification/ack_transaction.sql')
const sql_ack_transactions = require('../sql/notification/ack_transactions.sql')
const sql_ack_contact = require('../sql/notification/ack_contact.sql')
const sql_ack_contacts = require('../sql/notification/ack_contacts.sql')
const sql_ack_alert = require('../sql/notification/ack_alert.sql')
const sql_resolve_message = require('../sql/notification/resolve_message.sql')
const sql_user = require('../sql/notification/user.sql')
const sql_register_push = require('../sql/notification/register_push.sql')
const sql_unregister_push = require('../sql/notification/unregister_push.sql')
const sql_device_tokens = require('../sql/notification/device_tokens.sql')
const sql_summary = require('../sql/notification/summary.sql')
const sql_insert_user = require('../sql/notification/insert_user.sql')
const sql_insert_delivery = require('../sql/notification/insert_delivery.sql')
const sql_unread = require('../sql/notification/unread.sql')
const sql_unread_room = require('../sql/notification/unread_room.sql')
...
Notification.create = function (notification, cb) {
  ...
  const push = (user_id, populated, cb) => {
...
Notification.ack = function (user_id, notification_id, cb) {
  Notification.get(notification_id, (err, notification) => {
    ...
    db.query(sql_ack, [user_id, notification_id], (err, res) => {
      if (err)
        return cb(err)
...
Notification.ackRoom = function (user_id, room_id, cb) {
  User.get(user_id, err => {
    ...
    Room.get(room_id, err => {
      ...
      db.query(sql_ack_room, [user_id, room_id], err => {
        if(err)
          return cb(err)

        Socket.send('Room.Ackonwledged', room_id, [{
          user: user_id
        }])
...
Notification.summary = function (user_id, cb) {
  User.get(user_id, (err, user) => {
    ...
    db.query(sql_summary, [user_id], (err, res) => {
      if (err)
        return cb(err)
...
Notification.saveDelivery = function(nid, u, token, type, cb) {
  Notification.get(nid, (err, n) => {
    ...
    db.query(sql_insert_delivery, [n.id, u, token,type], err => {
      if(err)
        return cb(err)
...
Notification.sendForUnread = function (cb) {
  const sendSingle = (payload, cb) => {
    Notification.get(payload.notification, (err, notification) => {
      ...
      Notification.summary(payload.user, (err, summary) => {
        if(err)
          return cb(err)

        clone.badge_count = parseInt(summary.total_notification_count) || 0
...
Notification.sendAsPush = function(notification, user_id, room_id, tokens, delay, opts, cb) {
  async.map(tokens, (token, cb) => {
    Notification.saveDelivery(notification.notification_id, user_id, token, 'airship', (err) => {
      ...
      console.log('⇪ (Airship-Transport) scheduled a notification'.green,
                  ...
                  'for user', User.getFormattedForLogs(opts.user),
                  'on room', '#' + (opts.room ? opts.room.proposed_title : 'N/A'.red),
                  ('(' + (opts.room ? opts.room.id : 'N/A') + ')').blue,
...
Notification.sendAsText = function(notification, user_id, room_id, delay, opts, cb) {
  const getUser = cb => {
    // eslint-disable-next-line handle-callback-err
    User.get(user_id, (err, user) => {
      if (!User.shouldTrySMS(user))
        return cb(Error.NotImplemented('User has texts disabled'))
  ...
  const branch = (cb, results) => {
    if (Notification.branchType(results.notification) === 'Room')
      return Room.getBranchLink(results.user, results.notification.room, cb)

    Notification.getBranchLink(results.notification, results.user, results.notification.room, cb)
  }
  ...
  const saveDelivery = (cb, results) => {
    Notification.saveDelivery(results.notification.id, results.user.id, results.user.phone_number, 'sms', cb)
  }

  const send = (cb, results) => {
    if (!results.from) {
      console.log('NO PHONE NUMBER ASSOCIATED WITH USER:', results.user.id, 'ON ROOM:', results.notification.room)
      return cb()
    }

    // We have not yet implemented this correctly
    if (!results.notification.room)
      return cb()

    let text = results.notification.message
    if(results.branch)
      text += '\n' + results.branch
  ...
  const resolvePhoneNumber = (cb, results) => {
    ...
    Room.resolvePhoneForSeamless(results.user.id, results.notification.room, cb)
  }
  ...
  async.auto({
    user: getUser,
    brand: ['user', getBrand],
    notification: cb => Notification.get(notification.notification_id, cb),
    from: ['user', 'notification', resolvePhoneNumber],
    branch: ['user', 'brand', 'notification', branch],
    delivery: ['notification', 'branch', saveDelivery],
    send: ['from', 'delivery', send]
  }, report)
}
...
Notification.send = function (user_id, notification, cb) {
  ...
  const send = (cb, results) => {
    ...
    if (results.transport === 'Push')
      return Notification.sendAsPush(notification, user_id, room_id, results.tokens, delay, results, cb)

    else if (results.transport === 'SMS')
      return Notification.sendAsText(notification, user_id, room_id, delay, results, cb)
...
Notification.getBranchLink = function(notification, user, room_id, cb) {
  ...
  if(type === 'UserSharedListing') {
    url = '/dashboard/mls/' + notification.object
    b.listing = notification.object
    b.action = 'RedirectToListing'
  } else if (type === 'UserCreatedAlert') {
    url = '/dashboard/mls/alerts/' + notification.object
    b.alert = notification.object
    b.action = 'RedirectToAlert'
  } else {
    return cb(Error.NotImplemented('Notification type not implemnted for SMS transport'))
  }
