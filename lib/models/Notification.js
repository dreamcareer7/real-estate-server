...
Notification.get = function (id, cb) {
  ...
  async.auto({
    get: cb => {
      db.query('notification/get', [id, user_id], function (err, res_base) {
        if (err)
          return cb(err)
...
Notification.getForUser = function (user_id, paging, cb) {
  User.get(user_id, (err, user) => {
    ...
    db.query('notification/user', [
      ...
      async.map(notification_ids, Notification.get, (err, notifications) => {
        if (err)
          return cb(err)
...
Notification.ackPersonal = function (user_id, cb) {
  console.log('AckPersonal()', user_id)
  async.auto({
    user: cb => {
      User.get(user_id, cb)
    },
  ...
  async.auto({
    summary: (cb, results) => {
      Notification.getAppBadgeForUser(user.id, (err, count) => {
        ...
        notification.badge_count = _u.isEmpty(count) ? 0 : count
        return cb()
      })
    },
    message: (cb, results) => {
