...
function airship_transport_send(user_id, shortlist_id, notification, cb) {
  Notification.getDeviceTokensForUser(user_id, function(err, tokens) {
    if(err)
      return cb(err);

    User.isPushOK(user_id, function(err, ok) {
      if(err)
        return cb(err);

      if(ok || ((notification.action != 'PriceDropped') &&
                (notification.action != 'StatusChanged') &&
                (notification.action != 'BecameAvailable'))) {
        User.isPushOKOnShortlist(user_id, shortlist_id, function(err, ok) {
          if(ok) {
...
Notification.create = function(user_id, shortlist_id, notification, cb) {
    ...
    if(!shortlist.archived) {
      async.auto({
        record_notification: function(cb) {
          return create(user_id, shortlist_id, notification, cb);
        },
        default_message_room: function(cb) {
          return Shortlist.getDefaultRoom(shortlist_id, cb);
        },
        badge_count: ['record_notification',
                      function(cb, results) {
                        return Notification.totalUnreadCount(user_id, cb);
                      }],
        airship_push: ['record_notification',
                       ...
                       function(cb, results) {
                         ...
                         return airship_transport_send(user_id, shortlist_id, airship_notification, cb);
                       }],
        notification_view_messages: ['record_notification',
                                     'default_message_room',
                                     function(cb, results) {
                                       if((notification.action != 'Sent') ||
                                          (notification.action == 'Sent' && notification.auxiliary_object.message_room_type == 'Comment')) {
                                         var message = {};
                                         ...
                                         message.viewable_by = user_id;
                                         return Message.record(results.default_message_room, message, false, cb);
                                       } else {
                                         return cb();
                                       }
                                     }]
...
Notification.togglePushSettings = function(user_id, shortlist_id, enable, cb) {
  db.query(sql_toggle_push_settings, [user_id, shortlist_id, enable], function(err, res) {
    if(err)
      return cb(err);
