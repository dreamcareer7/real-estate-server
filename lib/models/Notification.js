...
Notification.create = function (notification, cb) {
  console.log('Creating notification', typeof process.domain)
  const getUsers = (cb) => {
    console.log('getUsersIds', typeof process.domain)
    if (notification.room) {
      ...
      Room.getUsersIDs(notification.room, function (err, ids) {
        console.log('gotUsersIds', typeof process.domain)
        if (err)
          return cb(err)
  ...
  const push = (user_id, results, cb) => {
    console.log('3', typeof process.domain)
    ...
    const send = () => {
      console.log('sending', typeof process.domain)
      // eslint-disable-next-line
      Socket.send('Notification', user_id, [airship], (err) => {
        console.log('Socket.sent', typeof process.domain)
        ...
        console.log('User.getStatus', typeof process.domain)
        User.getStatus(user_id, (err, status) => {
          console.log('Got User.getStatus', typeof process.domain)
    ...
    console.log('inserting', typeof process.domain)
    insertForUser(populated.id, user_id, (err) => {
      console.log('insert done', typeof process.domain)
      if (err)
        return cb(err)
  ...
  const pushToAll = (cb, results) => {
    console.log('pushToAll', typeof process.domain)
  ...
  const sendViewMessage = (cb, results) => {
    console.log('sendViewMessage', typeof process.domain)
    if (!results.room) {
  ...
  const done = (err, results) => {
    console.log('Done', typeof process.domain)
    if (err)
      return cb(err)
  ...
  const getRoom = (notification, cb) => {
    console.log('getRoom', typeof process.domain)
    if (!notification.room)
      return cb()
  ...
  async.auto({
    ...
    populated: [
      'saved',
      (cb, results) => {
        console.log('Populating', typeof process.domain)
        Orm.populate(results.saved, {}, (err, res) => {
          console.log('Populated', process.domain)
          cb(err, res)
        }, [
    ...
    message: [
      ...
      'populated',
      (cb, results ) => {
        console.log('message', typeof process.domain)
