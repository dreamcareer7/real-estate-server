  ...
  db.query(sql_get, [id], function(err, res_base) {
    ...
    async.parallel({
      ...
      object: function(cb) {
        ...
        return ObjectUtils.dereference(notification.object_class, notification.object, ref, cb);
      },
      subject: function(cb) {
        if(!notification.subject)
          return cb();

        return ObjectUtils.dereference(notification.subject_class, notification.subject, ref, cb);
      },
      auxiliary_object: function(cb) {
        if(!notification.auxiliary_object)
          return cb();

        return ObjectUtils.dereference(notification.auxiliary_object_class, notification.auxiliary_object, ref, cb);
      },
      auxiliary_subject: function(cb) {
        if(!notification.auxiliary_subject)
          return cb();

        return ObjectUtils.dereference(notification.auxiliary_subject_class, notification.auxiliary_subject, ref, cb);
      },
...
Notification.create = function(user, shortlist, notification, cb) {
  async.auto({
    ...
    default_message_room: function(cb) {
      return Shortlist.getDefaultRoom(shortlist, cb);
    },
    ...
    notification_view_messages:['record_notification',
                                ...
                                function(cb, results) {
                                  if(notification.action != 'Sent') {
                                    ...
                                    message.subjects = '{' + notification.subject + '}';
                                    message.objects = '{' + notification.object + '}';
                                    message.subject_class = notification.subject_class;
                                    message.object_class = notification.object_class;
