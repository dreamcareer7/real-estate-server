...
var seamless_inner_message_item = require('../html/user/communication_item.html');
var seamless_message_body       = require('../html/user/communication.html');
...
Notification.create = function(notification, cb) {
  ...
  var push = (user_id, saved, cb) => {
    ...
    Socket.send('Notification', user_id, [airship], (err) => {
      ...
      debug('>>> (Notification::create::push) Notifying user via socket:', user_id);
      Notification.summary(user_id, function(err, summary) {
        airship.badge_count = parseInt(summary.total_notification_count) || 0;
        debug('>>> (Notification::create::push) Setting badge count for user:', user_id, 'to:', summary.total_notification_count);
...
Notification.processSeamless = function(job, cb) {
  var data = job.data;

  async.mapSeries(data.items, (r, cb) => {
    var inner = sprintf(seamless_inner_message_item, r);
...
Notification.send = function(user_id, room_id, notification, cb) {
  var send = (cb, results) => {
    ...
    if ((notification.action != 'BecameAvailable') &&
      ...
      debug('>>> (Notification::send) User-generated notification, scheduling now');
      delay = 0;
    }
    else {
      delay = (results.user_ok_for_push < 0) ? 0 : results.user_ok_for_push;
      debug('>>> (Notification::send) System-generated notification, scheduling in', delay);
    }
    if (!results.room_ok_for_push &&
        (notification.action == 'BecameAvailable' ||
         ...
         debug('>>> (Notification::send) Room is muted. Not sending System-generated push');
         return cb();
       }
    ...
    async.map(results.tokens, function(token, cb) {
      ...
      debug('>>> (Notification::send) Job pushed to domain');
      process.domain.jobs.push(job);
  ...
  var shareAlertEmail = (cb, results) => {
    var params = results;

    async.auto({
      alert: cb => {
        return Alert.get(notification.object, cb);
      },
      sender: cb => {
        return User.get(notification.subject, cb);
      },
      listing: cb => {
        if(!notification.alert_results.listing)
          return cb();

        return Listing.get(notification.alert_results.listing, cb);
      },
      email: [
        'alert',
        'sender',
        ...
        (cb, results) => {
          ...
          var job = Job.queue.create('alert_share', {
            first_name: params.user.first_name,
            alert_title: results.alert.title,
            sender_name: results.sender.first_name,
            map_uri: 'http://a.tile.openstreetmap.org/16/35741/22729.png',
            listing_count: notification.alert_results.count,
            bedroom_count: results.listing.property.bedroom_count,
            bathroom_count: results.listing.property.bathroom_count,
            square_meters: results.listing.property.square_meters,
            status: results.listing.status,
            alert_uri: 'https://rechat.com',
  ...
  var determineSeamlessEmailType = (cb, results) => {
    ...
    if (notification.action == 'Created' && notification.object_class == 'Alert' && notification.subject_class == 'User' && room_id)
      return shareAlertEmail(cb, results);
          ...
          (cb, results) => {
            if(!previous_job) {
              ...
              var job = Job.queue.create('seamless_communication', {
                ...
                items: [
                  {
                    image_url: results.sender.profile_image_url || 'http://assets.rechat.com/mail/avatar.png',
                    name: results.sender.first_name + ' ' + results.sender.last_name,
                    datetime: params.user.current_time,
                    message: results.message.comment
                  }
                ]
              }).removeOnComplete(true).delay(60000);
              process.domain.jobs.push(job);
            } else {
              ...
              Job.kue.Job.get(previous_job, (err, job) => {
                ...
                job.data.items = job.data.items.concat({
                  image_url: results.sender.profile_image_url,
                  name: results.sender.first_name + ' ' + results.sender.last_name,
                  datetime: params.user.current_time,
                  message: results.message.comment
                });
