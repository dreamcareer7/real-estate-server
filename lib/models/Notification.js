  ...
  db.query(sql_get, [id], function(err, res_base) {
    ...
    async.parallel({
      ...
      object: function(cb) {
        ...
        return dereferenceObject(notification.object_class, notification.object, ref, cb);
      },
      subject: function(cb) {
        if(!notification.subject)
          return cb();

        return dereferenceObject(notification.subject_class, notification.subject, ref, cb);
      },
      auxiliary_object: function(cb) {
        if(!notification.auxiliary_object)
          return cb();

        return dereferenceObject(notification.auxiliary_object_class, notification.auxiliary_object, ref, cb);
      },
      auxiliary_subject: function(cb) {
        if(!notification.auxiliary_subject)
          return cb();

        return dereferenceObject(notification.auxiliary_subject_class, notification.auxiliary_subject, ref, cb);
      },
      recommendation: function(cb) {
        ...
        return Recommendation.getOnShortlist(notification.recommendation, notification.shortlist, cb);
         ...
         res_final.objects = [ results.object ] || null;
         res_final.subjects = [ results.subject ] || null;
         res_final.auxiliary_subject = results.auxiliary_subject || null;
         res_final.auxiliary_object = results.auxiliary_object || null;
         res_final.recommendations = [ results.recommendation ] || null;
