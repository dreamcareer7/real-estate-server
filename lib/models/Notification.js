...
const urbanAirshipTokenHelper = require('../utils/urban_airship_reports_service')
  ...
  const push = (user_id, results, cb) => {
    ...
    const send = () => {
      ...
      Socket.send('Notification', user_id, [airship], (err) => {
        ...
        if (_u.contains(notification.exclude, user_id))
          return cb()
  ...
  User.get(user_id, (err, user) => {
    ...
    db.query('notification/user_channels', [user_id], (err, res) => {
      ...
      async.each(
        res.rows,
        (r, calbk) => {
          urbanAirshipTokenHelper.isTokenValid(r.channel, (err, result) => {
            // TODO: (JAVAD) => What if other situations happen, i.e. any network error, then shall we delete the token?
            if (err || !result) {
        ...
        (err) => {
          ...
          Notification.deleteInvalidTokensForUser(user_id, invalidTokens).nodeify(err => {
            if (err) {
              console.log('Error deleting invalid tokens form notifications_tokens for user: %s\n', user_id, err)
            }
  ...
  async.auto({
    ...
    seen: [
      ...
      cb => {
        ...
        db.query('notification/patch_seen', [user_id, notificationIDs, _u.isEmpty(notificationIDs)], cb)
      }
    ]
  }, cb)
}
...
Notification.issueForRoomExcept = function (notification, exceptions, cb) {
  debug('>>> (Notification::issueForRoomExcept)', 'Notification Object:', JSON.stringify(notification), 'Except:', exceptions)
...
Notification.sendToDevice = function (notification, channel_id, user_id, cb) {

  notification.message = emojify.replace(notification.message)
  ...
  airship.push.send(pushInfo, (err, data) => {
    if (err) {
      console.log('<- (Airship-Transport) Error sending push notification to'.red, channel_id, ':', err)
      return cb()
    }

    console.log('<- (Airship-Transport) Successfully sent a push notification to device'.green, channel_id)
...
Notification.deleteInvalidTokensForUser = async function (userID, tokenList) {
  return db.query.promise('notification/delete_invalid_tokens', [userID, tokenList])
}
