...
Notification.create = function (notification, cb) {
  ...
  const getUsers = (cb) => {
    console.log('1', typeof process.domain)
    if (notification.room) {
      ...
      Room.getUsersIDs(notification.room, function (err, ids) {
        console.log('2', typeof process.domain)
        if (err)
          return cb(err)
  ...
  const push = (user_id, results, cb) => {
    ...
    const send = () => {
      console.log('4', typeof process.domain)
      // eslint-disable-next-line
      Socket.send('Notification', user_id, [airship], (err) => {
        console.log('5', typeof process.domain)
        ...
        User.getStatus(user_id, (err, status) => {
          console.log('6', typeof process.domain)
    ...
    insertForUser(populated.id, user_id, (err) => {
      console.log('7', typeof process.domain)
      if (err)
        return cb(err)
  ...
  const pushToAll = (cb, results) => {
    console.log('8', typeof process.domain)
  ...
  const sendViewMessage = (cb, results) => {
    console.log('9', typeof process.domain)
    if (!results.room) {
  ...
  const done = (err, results) => {
    console.log('10', typeof process.domain)
    if (err)
      return cb(err)
  ...
  const getRoom = (notification, cb) => {
    console.log('11', typeof process.domain)
    if (!notification.room)
      return cb()
