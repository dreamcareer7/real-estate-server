...
Notification.sendTextForUnread = function (cb) {
  const sendSingle = (payload, cb) => {
    ...
    const getBranch = (cb, results) => {
      ...
      const b = {}
      b.room = results.notification.room
    ...
    const send = (cb, results) => {
      ...
      console.log('to:', results.user.phone_number, 'from:', results.from, 'body:', text)
      SMS.send({
    ...
    const resolvePhoneNumber = (cb, results) => {
      console.log('resolving phone for user:', results.user.id, 'room:', results.notification.room)
    ...
    async.auto({
      ...
      branch: ['user', 'brand', 'notification', getBranch],
