...
function dereferenceObject(object, id, refs, cb) {
  if (global[object]) {
...
Notification.get = function(id, cb) {
  db.query(sql_get, [id], function(err, res_base) {
    ...
    var ref = {
      user: notification.notifying_user,
    ...
    async.parallel({
      ...
      object: function(cb) {
        ...
        dereferenceObject(notification.object_class, notification.object, ref, cb);
      },
      subject: function(cb) {
        if(!notification.subject)
          return cb();

        dereferenceObject(notification.subject_class, notification.subject, ref, cb);
      },
      auxiliary_object: function(cb) {
        if(!notification.auxiliary_object)
          return cb();

        dereferenceObject(notification.auxiliary_object_class, notification.auxiliary_object, ref, cb);
      },
      auxiliary_subject: function(cb) {
        if(!notification.auxiliary_subject)
          return cb();

        dereferenceObject(notification.auxiliary_subject_class, notification.auxiliary_subject, ref, cb);
      },
      recommendation: function(cb) {
        if(!notification.recommendation)
          return cb();

        Recommendation.getOnShortlist(notification.recommendation, notification.shortlist, cb);
      }
         ...
         res_final.objects = [ results.object ] || [];
         res_final.subjects = [ results.subject ] || [];
