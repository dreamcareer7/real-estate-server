  ...
  db.query(sql_get, [id], function(err, res_base) {
    ...
    async.parallel({
      ...
      auxiliary_object: function(cb) {
        ...
        global[notification.auxiliary_object_class].get(notification.auxiliary_object, cb);
      },
      auxiliary_subject: function(cb) {
        if(!notification.auxiliary_subject)
          return cb();

        global[notification.auxiliary_subject_class].get(notification.auxiliary_subject, cb);
      },
