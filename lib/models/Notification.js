...
Notification.create = function(notification, cb) {
  ...
  var push = (user_id, saved, cb) => {
    ...
    insertForUser(saved.id, user_id, (err) => {
      if(err)
        return cb(err);
...
var idOfModel = (n, association, cb) => {
  var model_name = n[association+'_class']
  if(['Message', 'Room'].indexOf(model_name) > -1)
    return cb();

  if(n[association] == null)
    return cb();

  if(typeof n[association] === 'object')
    return cb(null, n[association].id)
  else
    cb(null, n[association]);
}

var idOfModels = (n, association, cb) => {
  var model_name = n[association+'_class']
  if(['Message', 'Room'].indexOf(model_name) > -1)
    return cb();
...
Notification.associations = {
  ...
  recommendations: {
    collection:true,
    model: (n, cb) => cb(null, n.recommendation)
  },

  object: {
    optional:true,
    model: (n, cb) => cb(null, n.object_class),
    id: (n, cb) => idOfModel(n, 'object', cb)
  },

  objects: {
    collection:true,
    model: (n, cb) => cb(null, n.object_class),
    ids: (n, cb) => idOfModels(n, 'object', cb)
  },

  subject: {
    optional:true,
    model: (n, cb) => cb(null, n.subject_class),
    id: (n, cb) => idOfModel(n, 'subject', cb)
  },

  subjects: {
    collection:true,
    model: (n, cb) => cb(null, n.subject_class),
    ids: (n, cb) => idOfModels(n, 'subject', cb)
  },

  auxiliary_object: {
    optional:true,
    model: (n, cb) => cb(null, n.auxiliary_object_class),
    id: (n, cb) => idOfModel(n, 'auxiliary_object', cb)
  },

  auxiliary_subject: {
    optional:true,
    model: (n, cb) => cb(null, n.auxiliary_subject_class),
    id: (n, cb) => idOfModel(n, 'auxiliary_subject', cb)
  }
}
