  ...
  db.query(sql_get, [id], function(err, res_base) {
    ...
    async.parallel({
      ...
      subject: function(cb) {
        ...
        global[notification.subject_class].get(notification.subject, cb);
      },
      notifying_users: function(cb) {
        if(!notification.notifying_users)
          return cb();

        User.get(notification.notifying_users, cb);
      },
      recommendation: function(cb) {
        ...
        return Recommendation.getOnShortlist(notification.recommendation, notification.shortlist, cb);
         ...
         res_final.notifying_users = [ results.notifying_users ] || null;
...
Notification.publicize = function(model) {
  ...
  if(model.notifying_users) model.notifying_users.map(User.publicize);
