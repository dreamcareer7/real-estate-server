...
Brand.getAll = function(brand_ids, cb, user) {
  const _this = ObjectUtil.getCurrentUser()

  if (!user && _this)
    user = _this

  db.query('brand/get', [brand_ids, user], (err, res) => {
    if (err)
      return cb(err)

    const brands = res.rows.map(brand => {
      organizeRoles(brand)
      ...
      brand.room_possible = isRoomPossible(brand, user)
...
Brand.getRoom = ({brand, user}, cb) => {
  Brand.get(brand, (err, brand) => {
    if (err)
      return cb(err)

    initRoom(brand, user, cb)
  })
}

const isRoomPossible = (brand, user) => {
...
function initRoom(brand, user, cb) {
  if (brand.room) //Already have a room together
    return Room.get(brand.room, cb)

  if (!isRoomPossible(brand, user))
    return cb()

  Room.create({
    users: brand.roles.Backoffice,
    title: 'Back Office',
    room_type: 'Group',
    owner: user,
