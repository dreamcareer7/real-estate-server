...
const sql_get = require('../sql/brand/get.sql')
const sql_get_by_hostname = require('../sql/brand/get_hostname.sql')

Brand = {}
Orm.register('brand', Brand)

Brand.get = function (id, cb) {
  const user = process.domain.user ? process.domain.user.id : null
  ...
  db.query(sql_get, [id, user], (err, res) => {
    ...
    brand.base_url = (brand.hostnames && brand.hostnames.length) ? brand.hostnames[0] : config.webapp.base_url
