...
const notifyAgent = async ({ listing, template, user, template_types, subject, enable_websites }) => {
...
const notify = async ({ listing, template, template_types, subject, enable_websites }) => {
...
const _openHouse = async listingId => {
  ...
  await zrem(openhouseNotifKey, listingId)
  await del(`${openhouseNotifKey}-${listingId}`)
...
const openHouse = async listingId => {
  await zadd(openhouseNotifKey, Date.now(), listingId)
  await zadd(`${openhouseNotifKey}-${listingId}`, Date.now(), listingId)
}
