...
const UserRole = require('../User/role/get')
...
const notifyAgent = async ({listing, template, user, template_types, subject}) => {
  ...
  const [ first ] = [...social_templates, ...email_templates, ...website_templates ]
  ...
  return Email.create({
    to: ['emil@rechat.com', 'shayan@rechat.com'],
    html,
...
const notify = async ({listing, template, template_types}) => {
...
const justListed = peanar.job({
  ...
  queue: 'listing_justlisted',
  exchange: 'listing_justlisted',
  max_retries: 3,
  retry_exchange: 'listing_justlisted.retry',
  error_exchange: 'listing_justlisted.error',
})

const openHouse = peanar.job({
  handler: _openHouse,
  name: 'listing_openhouse',
  queue: 'listing_openhouse',
  exchange: 'listing_openhouse',
  max_retries: 3,
  retry_exchange: 'listing_openhouse.retry',
  error_exchange: 'listing_openhouse.error',
})

const priceImprovement = peanar.job({
  handler: _priceImprovement,
  name: 'listing_priceimprovement',
  queue: 'listing_priceimprovement',
  exchange: 'listing_priceimprovement',
  max_retries: 3,
  retry_exchange: 'listing_priceimprovement.retry',
  error_exchange: 'listing_priceimprovement.error',
})
