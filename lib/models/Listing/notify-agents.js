...
const notifyAgent = async ({ listing, template, user, template_types, subject, enable_websites, subscriptions = [] }) => {
...
const notify = async ({ listing, template, template_types, subject, enable_websites }) => {
  const rows = await db.select('listing/agents', [listing.id])
