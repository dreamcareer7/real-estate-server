...
Listing.create = function ({address, property, listing}, cb) {
  ...
  function saveAddress (cb, results) {
    console.log('Saving listing address', listing.matrix_unique_id)
    Address.create(address, cb)
  }

  function geocode (cb, results) {
    console.log('Updating listing address', listing.matrix_unique_id)
    Address.updateGeo(results.address, cb)
  }

  function saveProperty (cb, results) {
    console.log('Saving listing property', listing.matrix_unique_id)
    property.address_id = results.address
    Property.create(property, cb)
  }

  function saveListing (cb, results) {
    console.log('Saving listing', listing.matrix_unique_id)
  ...
  function getOld (cb) {
    console.log('Getting old listing', listing.matrix_unique_id)
  ...
  function getNew (cb, results) {
    console.log('Getting new listing address', listing.matrix_unique_id)
    Listing.get(results.saved.rows[0].id, cb)
  }

  function recommendation (cb, results) {
    console.log('Creating recommendation for listing', listing.matrix_unique_id)
...
Listing.issueHitNotifications = function (listing_id, cb) {
  ...
  async.auto({
    ...
    recommendation: [
      cb => {
        Recommendation.generateForListing(listing_id, cb)
      }
    ],
...
Listing.issueChangeNotifications = function (listing_id, before, after, cb) {
  ...
  async.auto({
    ...
    usersInterestedInListing: cb => {
      ListingSetting.getUsersWithStatus([listing_id], ['ListingStatusChange', 'ListingPriceDrop'])
    },
    ...
    price_drop_trigger: [
      ...
      (cb, results) => {
        ...
        async.map(allUsers, (interested, cb) => {
          if (!interested.status.includes('AlertPriceDrop') || !interested.status.includes('ListingPriceDrop')) {
            return cb()
          }
          const notification = {}
    ...
    status_change_trigger: [
      ...
      (cb, results) => {
        ...
        const allUsers = results.usersWithAlertEnabled.concat(results.usersInterestedInListing)  
        async.map(allUsers, (interested, cb) => {
          if (!interested.status.includes('AlertStatusChange') || !interested.status.includes('ListingStatusChange')) {
            return cb()
          } 
          const notification = {}
