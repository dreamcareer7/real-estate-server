...
Listing.create = function ({address, property, listing, revision}, cb) {
  ...
  const saveAddress = (cb, results) => {
    Address.create(address, (err, address_id) => {
      if (err) {
        Context.log('Address.create failed:', err)
        return cb(err)
      }

      cb(undefined, address_id)
    })
  }
  ...
  const validate_listing = (cb) => {
    validate(listing, (err, res) => {
      if (err) {
        Context.log('Listing.create-validate failed:', err)
        return cb(err)
      }
  ...
  async.auto({
    ...
    validate: ['address', 'property', validate_listing],
    ...
    if (err) {
      Context.log('Listing.create-async.auto failed:', revision, err)
      return cb(err)
    }

    Context.log('Listing.create-async.auto completed:', revision)
    cb(undefined, res)
  })
}
...
Listing.issueHitNotifications = function (listing_id, cb) {
  ...
  async.auto({
    ...
    send: [
      ...
      'notification',
      (cb, results) => {
        Context.log('Creating Notifications', results.users)
