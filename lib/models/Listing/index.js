  ...
  async.auto({
    ...
    price_drop_trigger: [
      ...
      (cb, results) => {
        ...
        async.map(allUsers, (interested, cb) => {
          if (!interested.status.includes('AlertPriceDrop') || sentUsers.includes(interested.id)) {
            return cb()
          }
    ...
    status_change_trigger: [
      ...
      (cb, results) => {
        ...
        const allUsers = results.usersWithAlertEnabled.concat(results.usersInterestedInListing)
        async.map(allUsers, (interested, cb) => {
          if (!interested.status.includes('ListingStatusChange') || sentUsers.includes(interested.id)) {
            return cb()
          }
          sentUsers.push(interested.id)
          const notification = {}
