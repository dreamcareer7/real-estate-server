...
Listing.get = function (id, cb) {
  Listing.getAll([id]).nodeify((err, listings) => {
    if(err)
      return cb(err)
...
Listing.getAll = async function(listing_ids, cb) {
...
Listing.getCompacts = async function (ids) {
  const brand = Brand.getCurrent()
  const brand_id = brand ? brand.id : null
  const associations = Orm.getEnabledAssociations()
...
Listing.create = function ({address, property, listing, revision}, cb) {
  ...
  function getOld (cb) {
    ...
    Listing.getByMUI(listing.matrix_unique_id).nodeify((err, listing) => {
...
Listing.getByMUI = async function (id) {
  const ids = await db.selectIds('listing/get_mui', [id])

  if (ids.length < 1) {
    throw Error.ResourceNotFound(`Listing ${id} not found.`)
  }

  return promisify(Listing.get)(ids[0])
}

Listing.getByMLSNumber = async function (id) {
...
Listing.stringSearch = async function ({query, status, limit}) {
  ...
  const listing_ids = await db.selectIds('listing/string_search', [
    tsquery(query),
    status,
    limit
  ])
...
Listing.inquiry = function (user_id, listing_id, agent_id, brand_id, source_type, external_info, cb) {
  async.auto({
    ...
    agent: cb => {
      ...
      return Agent.get(agent_id).nodeify(cb)
    },
    listing: cb => {
      return Listing.get(listing_id, cb)
    },
