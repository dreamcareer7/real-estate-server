  ...
  async.auto(
    {
      ...
      create: [
        'room',
        (cb, results) => {
          if (deal_role) return add(deal_role, 'DealRole', activity, cb)

          add(user_id, 'User', activity, cb)
        }
      ],
      parse: [
        'room',
        ...
        'create',
        (cb, results) => {
          return parse(activity).nodeify((err, p) => {
      ...
      message: [
        ...
        'parse',
        (cb, results) => {
          formatForDisplay(results.parse, results.reference, cb)
        }
      ],
      post: [
        'create',
        ...
        (cb, results) => {
          ...
          postMessage(room_id, message, push, cb)
        }
      ]
    },
    (err, results) => {
