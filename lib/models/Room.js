...
Room.recommendListings = async function (room_id, listing_ids, external_info, cb) {
  ...
  try {
    ...
    const base = {}
    base.source = external_info.source || 'MLS'
    base.source_url = external_info.source_url || 'http://www.ntreis.net/'
    base.room = room_id
    base.referring_objects = '{' + ref_id + '}'
    ...
    const allRecoms = await Recommendation.createAll(toInsert, ref_id)
    ...
    // of Hit or Share but because our kick off point i.e. line 565 of this file, calls Room.recommendListings with
    // {ref_alert_id: alert_id} as external_info.
    let found
    for (const n of newOnes) {
      found = listings.find(l => l.id === n.id)
