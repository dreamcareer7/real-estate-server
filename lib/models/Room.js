...
const sql_search_users_29 = require('../sql/room/search_users_29.sql')
  ...
  async.auto({
    ...
    create: [
      ...
      (cb, results) => {
        ...
        const p = results.classify_phones.existing.ids || []
        const u = room.users || []
        const self = room.owner || null

        const users = _u.unique(u.concat(e).concat(p).concat(self).filter(Boolean))
        const should_create_direct = (users.length === 2) && (_u.isEmpty(results.classify_phones.non_existing))
  ...
  async.auto({
    ...
    phones: cb => {
      ...
      async.map(phones, (r, cb) => {
        const p = ObjectUtil.formatPhoneNumberForDialing(r)
