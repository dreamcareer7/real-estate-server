...
Room.combineUserReferences = function(users, emails, phones, cb) {
  async.auto({
    users: cb => {
      return async.map(users, User.get, cb);
    },
    emails: cb => {
      return async.map(emails, User.getByEmail, cb);
    },
    phones: cb => {
      return async.map(phones, User.getByPhoneNumber, cb);
    },
    check: [
      'users',
      'emails',
      ...
      (cb, results) => {
        var u = users ? users : [];
        var e = results.emails ? results.emails : [];
        var p = results.phones ? results.phones : [];

        return cb(null, _u.unique(u.concat(e).concat(p)));
      }
    ]
  }, (err, results) => {
    if(err)
...
Room.searchForUsers = function(users, cb) {
  var all = _u.unique(users);
  db.query(sql_search_parties, [all], (err, res) => {
    if(err)
      return cb(err);
...
Room.searchForParties = function(users, emails, phones, cb) {
  Room.combineUserReferences(users, emails, phones, (err, results) => {
    if(err)
      return cb(err);

    var a = ((users) ? users: []) + ((emails) ? emails : []) + ((phones) ? phones : []);
    var r = results;

    if (a.length != r.length)
      return cb();

    return Room.searchForUsers(r, cb);
  });
};

Room.compose = function(users, emails, phones, cb) {
  async.auto({
    unique: cb => {
      return Room.combineUserReferences(users, emails, phones, cb);
    },
    search: cb => {
      return Room.searchForParties(users, emails, phones, cb);
    },
    return: [
      'unique',
      'search',
      (cb, results) => {
        ...
        if(results.unique.length < 2)
          return cb(Error.Validation('Cannot compose a room with yourself or with no users'));

        if(results.unique.length == 2) {
          Room.createDirect(results.unique[0], results.unique[1], (err, res) => {
            if(err)
              return cb(err);
