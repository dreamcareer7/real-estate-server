...
Room.compose = function(room, cb) {
  async.auto({
    ...
    phone_shadows: (cb, results) => {
      return async.map(room.phones, (r, cb) => {
...
Room.stringSearch = function(user, terms, limit, similarity, cb) {
  terms = terms.join('|');

  db.query(sql_string_search, [user, terms, limit, similarity], (err, res) => {
    if(err)
      return cb(err);
...
Room.searchForParties = function(user_id, users, emails, phones, cb) {
  Room.combineUserReferences(users, emails, phones, (err, results) => {
    ...
    var a = ((users) ? users: []).concat((emails) ? emails : []).concat((phones) ? phones : []);
    var r = results;
