...
var sql_get_recommendation     = require('../sql/room/get_recommendation.sql');
...
var sql_string_search_fuzzy    = require('../sql/room/string_search_fuzzy.sql');
...
Room.recommendListing = function(room_id, listing_id, external_info, cb) {
  ...
  async.auto({
      ...
      (cb, results) => {
        ...
        async.auto({
          ...
          notifications: [
            'user',
            (cb, results) => {
              listing_notification(opts.listing, opts.room, results.user, results.insert, external_info, cb);
            }
          ]
        }, (err, results) {
          if(err)
    ...
    get: [
      ...
      'not_dup',
      (cb, results) => {
        var r = results.not_dup || results.dup;
          ...
          (cb, results) => {
            ...
            async.auto({
              ...
              notifications: [
                ...
                'insert',
                (cb, results) => {
                  listing_notification(opts.listing, room, results.user, results.insert, external_info, cb);
                }
              ]
            }, (err, results) => {
              if(err)
...
Room.getTitle = function(room) {
  ...
  var names = p.map(r => {
    if (process.domain && process.domain.user)
      if (r.id == process.domain.user.id)
        return null;

    return User.getAbbreviatedDisplayName(r);
  });
...
Room.publicize = function(model) {
  model.proposed_title = Room.getTitle(model);
