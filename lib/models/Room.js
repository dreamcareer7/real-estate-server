  ...
  db.query(sql_get, [room_id], function(err, res) {
    ...
    async.parallel({
      ...
      latest_message: function(cb) {
        ...
        return Message.get(room.latest_message, cb);
      }
         ...
         room.lead_agent = results.lead_agent || null;
         room.latest_message = results.latest_message || null;
...
Room.create = function(room, cb) {
  async.auto({
    ...
    add_users: ['validate',
                'user',
                'insert',
                'add_owner', function(cb, results) {
                  async.map(room.users, function(r, cb) {
                    User.get(r, function(err) {
                      if(err)
                        return cb(err);
  ...
  Room.get(room_id, function(err, room) {
    ...
    Listing.get(listing_id, function(err, listing) {
      ...
      Room.isDup(room_id, listing_id, function(err, dup) {
        ...
        if (dup) {
          async.auto({
            add_reference_to_recommendation: function(cb) {
            ...
            unhide_recommendation: ['add_reference_to_recommendation',
                                    function(cb) {
                                      Recommendation.unhide(dup, cb);
                                    }],
            recommendation: ['add_reference_to_recommendation',
            ...
            notification: ['user',
                           'recommendation',
                           function(cb, results) {
                              listing_notification(listing, room, results.user, results.recommendation.id, external_info, cb);
                           }]
          }, function(err, results) {
               if(err)
                 return cb(err);
...
Room.recommendListings = function(room_id, listing_ids, external_info, cb) {
  ...
  Room.get(room_id, function(err, room) {
    var processListing = function(listing_id, cb) {
      Listing.get(listing_id, function(err, listing) {
        if(err)
          return cb(err);
...
Room.stringSearch = function(user, string, cb) {
  db.query(sql_search, [user], function(err, res) {
    if(err)
      return cb(err);
    ...
    var room_ids = res.rows.map(function(r) {
      if (Room.arrayMatch(r.first_names, string) ||
          Room.arrayMatch(r.last_names, string) ||
          Room.arrayMatch(r.phone_numbers, string) ||
          Room.arrayMatch(r.emails, string) ||
          Room.arrayMatch([r.title], string))
        return r.id;
    }).filter(Boolean);
...
Room.arrayMatch = function(array, string) {
  array = array.filter(Boolean);

  for (var i in array) {
    var r = new RegExp(string, 'i');
...
Room.bulkCreateWithUsers = function(user, users, cb) {
  var room = {
    client_type: 'Unknown',
    room_type: 'Direct',
    owner: user
  ...
  async.map(users, function(r, cb) {
    ...
    r.users = [r];
    Room.create(clone, function(err, room) {
      if(err)
        return cb(err);
