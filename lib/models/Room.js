...
Room.validateRecommendation = function (recomObj) {
  if (!(recomObj.ref_alert_id || recomObj.ref_user_id))
    return false
  
  if (recomObj.ref_user_id && (recomObj.notification !== 'Share' &&
    recomObj.notification !== 'None' &&
    !_u.isUndefined(recomObj.notification)))
    return false
  
  if (recomObj.ref_alert_id && (recomObj.notification !== 'Hit' &&
    recomObj.notification !== 'None' &&
    !_u.isUndefined(recomObj.notification)))
    return false
...
Room.recommendListingToRooms = async function (listing, data, cb) {
  const valids = []
  for (const item of data) {
    const external_info = {}
    external_info.ref_alert_id = item.id
    external_info.notification = 'Hit'
    
    if (!Room.validateRecommendation(external_info)) {
      continue
    }
  ...
  const res = await Recommendation.insertList(valids)
  cb(res)
  // return cb(valids)
}
