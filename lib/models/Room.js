 ...
 */
function add_user (user_id, room_id, notification_setting, cb) {
  db.query('room/add_user', [user_id, room_id, notification_setting || 'N_ALL'], (err, res) => {
    if (err) {
      if (err.code === '23505') {
...
function listing_notification (listing, room, user, recommendation_id, external_info, cb) {
  ...
  if (external_info.notification === 'Hit') {
    ...
    Notification.issueForRoomExcept(notification, [user.id])
    return cb(undefined, {})
  }
...
Room.getAll = function(room_ids, cb) {
  ...
  db.query('room/get', [[room_ids], user_id], (err, res) => {
    ...
    const rooms = res.rows.map(room => {
      room.proposed_title = Room.getTitleForUser(room.users_info, ObjectUtil.getCurrentUser())
 ...
 * @returns {boolean}
 */
Room.addUser = function ({inviting_id, user_id, room_id, notification_setting, connect_override}, cb) {
  ...
  async.auto({
    ...
    add_user: [
      ...
      'room',
      cb => {
        add_user(user_id, room_id, notification_setting, cb)
      }
    ],
    connect: [
      'user',
    ...
    invite_notification: [
      ...
      (cb, results) => {
        ...
        Notification.issueForRoomExcept(notification, [inviting_id])
        return cb(undefined, {})
      }
    ],
    notification: [
...
Room.getTitleForUser = function (users, user_id) {
  if(!users)
    return 'Empty'
