...
Room.on = emitter.on.bind(emitter)
 ...
 * Creates a `room`
 */
Room.create = async function (room) {
...
Room.compose = function(room, cb) {
  async.auto({
    ...
    if(users.length === 2)
      return Room.getOrCreateDirectRoom(users[0], users[1]).nodeify(cb)

    room.users = _u.without(users, self)

    return Room.create(room).nodeify(cb)
  })
}
 ...
 */
Room.getOrCreateDirectRoom = async function (user_id, peer_id) {
  ...
  return Room.createDirect(user_id, peer_id)
}
...
Room.addUser = function ({inviting_id, user_id, room_id, notification_setting, connect_override, relax}, cb) {
  ...
  async.auto({
    ...
    inviting_user: cb => {
      ...
      User.get(inviting_id).nodeify(cb)
    },
    room: cb => {
      Room.get(room_id, cb)
    },
 ...
 * @returns {Promise<UUID>} ID of the created room
 */
Room.createDirect = async function (user_id, peer_id) {
  const room = {
    room_type: 'Direct',
      ...
      (cb, results) => {
        ...
        async.auto({
          ...
          user: cb => {
            ...
            return User.get(external_info.ref_user_id).nodeify(cb)
          },
...
Room.recommendListings = async function (room_id, listing_ids, external_info, cb) {
  ...
  try {
    const listings = await Listing.getAll(listing_ids)
    if (listings.length === 0) {
      cb(null, [])
      return []
    }
    
    const allRecoms = await Recommendation.createForListings(listings, room_id, external_info)
    const newOnes = allRecoms.filter(x => x.is_new)
    let user
    if (external_info.ref_user_id) {
      user = await User.get(external_info.ref_user_id)
    }
    const room = await promisify(Room.get)(room_id)
    ...
    async.auto({
      ...
      message: [
        ...
        (cb, results) => {
          ...
          return Message.post(results.room_id, message, true, cb)
        }
      ]
    }, (err, results) => {
      if (err)
