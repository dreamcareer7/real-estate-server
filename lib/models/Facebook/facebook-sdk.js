...
module.exports = {
  ...
  getAccessToken: async ({ code }) => {
    try {
      ...
      Context.error('Getting access token from FB Failed')
      Context.error(error)
    ...
    return {
      ...
      getPages: async () => {
        try {
          ...
          Context.error('Getting pages from FB Failed')
          Context.error(error)
      ...
      getInstagramId: async ({ pageId }) => {
        try {
          ...
          Context.error('Getting Instagram Id Failed')
          Context.error(error)
      ...
      getInstagramInfo: async ({ igID }) => {
        try {
          ...
          Context.error('Getting Instagram info Failed')
          Context.error(error)
      ...
      getFacebookProfile: async () => {
        try {
          ...
          Context.error('Getting facebook profile Failed')
          Context.error(error)
      ...
      createMediaContainer: async ({ igUserId, imageURL, caption }) => {
        ...
        try {
          ...
          Context.error('Creating media container Failed')
          Context.error(err)
          throw new FacebookError(err?.error?.error_user_msg, errorType.CreatingContainer)
        }
      },
      /** @returns {Promise<string>} */
      publishMedia: async ({ igUserId, mediaContainerId }) => {
        ...
        try {
          ...
          Context.error('publishing media failed')
          Context.error(err)
          throw new FacebookError(err?.error?.error_user_msg, errorType.PublishingContainer)
        }
      },
      /** @returns {Promise<string>} */
      getMediaInfo: async ({ publishedMediaContainerId }) => {
        ...
        try {
          ...
          Context.error('Getting media info Failed')
          Context.error(err)
          throw new FacebookError(err?.error?.error_user_msg, errorType.PublishingContainer)
        }
      },
