...
const { get, getRevision } = require('./get')



const saveRevisionPdf = async ({pdf, form, values, metadata, revision, user, deal, path, legacy=false}) => {
  const url = {
    url: legacy ? 'https://forms.rechat.com/generate.pdf' : 'https://rechat-forms-boer.herokuapp.com/generate.pdf',
    json: true,
    body: {
      values,
      metadata,
...
const update = async ({id, user_id, state, values, metadata, path, deal, pdf}) => {
  ...
  const legacy = submission.created_at < 1627811696 
