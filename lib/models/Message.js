...
const sendStackedEmail = (user_id, room_id, first, last, cb) => {
    ...
    Message.retrieve(room_id, {
      ...
      async.map(messages, (m, cb) => {
        ...
        messages.forEach( m => {
          ...
          const l = m.recommendation.listing
  ...
  const getToken = (cb, results) => {
    Token.getOrCreateForUserFull(results.user.id, (err, tokens) => {
      ...
      const token = Crypto.encrypt(JSON.stringify({
        id: results.user.id,
        tokens: tokens
      }))
  ...
  const getBranch = (cb, results) => {
    Url.create({
      uri: '/dashboard/recents/' + room_id,
      brand: results.user.brand,
      query: {
        token: encodeURIComponent(results.token)
      }
    }, (err, url) => {
      if(err)
        return cb(err)
  ...
  async.auto({
    ...
    base_url: ['user', getBaseUrl],
    send: ['html', 'room', 'delivery', 'room_users', 'base_url', send]
  }, report)
}
