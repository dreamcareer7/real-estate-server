...
Message.get = function(id, cb) {
  db.query(sql_get, [id], function(err, res) {
    ...
    if(res.rows.length < 1)
      return cb(Error.ResourceNotFound('Message not found'));
...
Message.retrieve = function(message_room, from, order, limit, offset, cb) {
  console.log("from:", from, "order:", order, "limit:", limit, "offset:", offset);
  db.query(sql_retrieve, [message_room,
           ...
           function(err, res) {
             ...
             async.map(message_ids, Message.get, function(err, messages) {
               ...
               if (messages[0]) messages[0].full_count = res.rows[0].full_count;
