...
Message.postSeamless = function(incoming, cb) {
  ...
  var matches = incoming.recipient.match("(.*)@" + config.mailgun.domain);
  if(!matches || !matches[1])
    return cb(Error.Validation('Cannot determine the destination of this message'));

  var to = matches[1];
  var room_id = to.split('.')[0];
  var user_id = to.split('.')[1];
...
var sendStackedEmail = (user_id, room_id, first, last, cb) => {
  ...
  var send = (cb, results) => {
    Email.send({
      ...
      mailgun_options: {
        'h:Reply-To': room_id + '.' + results.user.id + '@' + config.email.seamless_address,
