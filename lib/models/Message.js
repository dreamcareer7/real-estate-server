                 ...
                 function(err, res) {
                   ...
                   Message.updateEverything(message_room_id, message_room.shortlist.id, message_room.listing, function(err, ok) {
                     ...
                     MessageRoom.recordAcks(message_room_id, message.author, res.rows[0].id, function(err, acks) {
                       ...
                       notification.object_class = 'MessageRoom';
                       notification.message = user.first_name + aux;
                       notification.object =  message_room_id;
                       notification.auxiliary_object_class = (message_room.message_room_type === 'Comment') ? 'Recommendation' : null;
                       notification.listing = ((message_room.listing) ? message_room.listing.id : null);
                       notification.notifying_user =  user.id;
                       ...
                       Notification.issueForMessageRoomExcept(message_room_id, message.author, message_room.shortlist.id, notification, function(err, ok) {
                         if(err)
                           // This is a temporary fix until we find an atomic solution for our endpoints
                           console.log('(ERR-WARNING) Unable to send out push notifications:', err);
                           // return cb(err);
