...
Message.postSeamless = function(incoming, cb) {
  ...
  var attachments = (cb, results) => {
    console.log(typeof incoming.attachments, incoming.attachments, incoming.length);

    var attachments = JSON.parse(incoming.attachments)
      .filter( a => Attachment.isAllowed(a.name, a.size ) )
  ...
  var user = (cb) => {
    Contact.resolve(author[1], undefined, (err, user_id) => {
      ...
      if(!user_id)
        return cb(Error.Forbidden('Cannot match senders credentials with any user'));
  ...
  var message = (cb, results) => {
    console.log(results.attachments);
    ...
    Message.post(room[1], message, true, cb);
  }

  async.auto({
    user:        user,
    room:        cb => Room.get(room[1], cb),
...
var sendStackedEmail = (user_id, room_id, first, last, cb) => {
  ...
  var send = (cb, results) => {
    Email.send({
      ...
      mailgun_options: {
        'h:Reply-To': room_id + '@' + config.email.seamless_address,
      ...
      message: {
        ...
        subject: {
          data: '[Rechat] ' + results.room.title
        }
      },
      template_params:{
        uri:results.branch,
  ...
  var getMessages = (cb) => {
    Message.retrieve(room_id, {
      ...
      if(messages.length < 1)
        return cb('No message to send on seamless email to user ' + user_id);

      var cache = {};
      async.map(messages, (m, cb) => {
        Orm.populate(m, cache, cb, ['message.room'])
      }, (err, messages) => {
        if(err)
          return cb(err);

        messages.forEach( m => {
          m.time = moment(m.created_at*1000).format('lll');
          ...
          l.property.square_meters = Listing.getSquareFeet(l.property.lot_square_meters);
        });
  ...
  var renderEmail = (cb, results) => {
    var m = mustache.compileAndRender(__dirname + '/../html/message/body.html', results);
    var html = '';
    m.on('data', (rendered) => {
