...
Message.get = function(message_id, cb) {
  db.query(sql_get, [message_id], function(err, res) {
    ...
    async.auto({
      ...
      recommendation_counts: function(cb) {
        if(!message.recommendation)
          return cb();

        return Recommendation.getCounts(message.recommendation, cb);
      },
      ...
      if(results.recommendation_counts) {
        message.recommendation.comment_count = results.recommendation_counts.comment_count;
        message.recommendation.document_count = results.recommendation_counts.document_count;
        message.recommendation.video_count = results.recommendation_counts.video_count;
        message.recommendation.image_count = results.recommendation_counts.image_count;
      }
