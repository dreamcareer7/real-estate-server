                 ...
                 function(cb) {
                   db.query(sql_record, [message_room_id,
                            ...
                            function(err, res) {
                              ...
                              return cb(res.rows[0].id);
                            });
                 }],
    update_everything: ['message_id',
                        function(cb, results) {
      ...
      subjects: ['viewable_by',
                 ...
                 function(cb, results) {
                   ...
                   async.map(message.subjects, function(r, cb) {
                     return ObjectUtils.dereference(message.subject_class, r, ref, cb);
                   }, cb);
                 }],
      objects: ['viewable_by',
                'shortlist',
                function(cb, results) {
                  ...
                  async.map(message.objects, function(r, cb) {
                    return ObjectUtils.dereference(message.object_class, r, ref, cb);
                  }, cb);
                }]
    }, function(err, results) {
         if(err)
