...
Message.post = function(room_id, message, push, cb) {
  console.log('posting a message to:', room_id);
  async.auto({
    room: cb => {
...
Message.processSeamless = function(job, cb) {
  ...
  async.mapSeries(data.items, (r, cb) => {
    ...
    Job.redis.del(key, (err) => {
      ...
      params.uri = params.items[0] ? params.items[0].uri : config.webapp.base_url + '/dashboard/recents/' + data.room_id;
...
Message.cancelSeamless = function(user_id, room_id) {
  var job_key = 'notificaton:message:' + room_id + ':' + user_id;

  Job.redis.get(job_key, (err, seamless_job_id) => {
    if(err)
      return ;

    Job.kue.Job.get(seamless_job_id, (err, job) => {
      if(job)
        job.remove();
    })
  })
