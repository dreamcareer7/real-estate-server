...
Message.postSeamlessEmail = function (incoming, cb) {
  ...
  let attached_files
  try {
    attached_files = JSON.parse(incoming.attachments).filter(a => Attachment.isAllowed(a.name, a.size))
  } catch (e) {
    attached_files = []
  }
...
Message.get = function (message_id, cb) {
  db.query('message/get', [message_id], function (err, res) {
    if (err)
      return cb(err)
...
Message.retrieve = function (room_id, paging, cb) {
  Room.get(room_id, function (err, room) {
    ...
    db.query('message/retrieve', [
      ...
      async.map(message_ids, Message.get, (err, messages) => {
        if (err)
          return cb(err)
...
const sendStackedEmail = (user_id, room_id, first, last, cb) => {
  console.log('Sending email to', user_id, room_id)
  const getUser = cb => {
  ...
  const getMessages = (cb, results) => {
    ...
    Message.retrieve(room_id, {
      ...
      async.map(messages, (m, cb) => {
        ...
        console.log('Populated', messages.length)
        if (err)
          return cb(err)
        ...
        console.log('User generated', user_generated)
        if(user_generated < 1)
...
Message.sendEmailForUnread = (cb) => {
  db.query('message/unread', [], (err, res) => {
    if (err)
      return cb(err)
