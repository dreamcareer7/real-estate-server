...
var sql_ack                 = require('../sql/message/ack.sql');
    ...
    push: ['room',
           ...
           function(cb, results) {
             // if (push) {
               ...
               // var sub_aux = '';
               // var notification = {};
               // var address_line = (room.listing) ? Address.getLocalized(room.listing.property.address) : '';
               ...
               //   aux = (room.room_type === 'Comment') ? (' added a photo to ' + address_line): ' sent a photo';
               //   sub_aux = 'Photo';
               // }
               // else if(message.video_url) {
               //   aux = (room.room_type === 'Comment') ? (' added a video to ' + address_line): ' sent a video';
               //   sub_aux = 'Video';
               // }
               // else if(message.document_url) {
               //   aux = (room.room_type === 'Comment') ? (' added a document to ' + address_line): ' sent a document';
               //   sub_aux = 'Document';
               // }
               ...
               // var base_message = '#' + room.room.title + ': @' + user.first_name;
               ...
               // notification.listing = ((room.listing) ? room.listing.id : null);
               // notification.extra_object_class = (room.room_type === 'Comment') ? 'Recommendation' : null;
               // notification.extra_subject_class = sub_aux;

               // Notification.issueForRoomExcept(room_id, message.author, room.room.id, notification, cb);
             return cb();
             // } else {
               // return cb();
             // }
...
/**
 * Marks this `message` as read for the specified `user`
 * @name markAsRead
 * @function
 * @memberof Message
 * @instance
 ...
 * @returns {boolean}
 */
Message.markAsRead = function(message_id, user_id, cb) {
  db.query(sql_ack, [message_id, user_id], function(err, res) {
    if(err)
      return cb(err);
