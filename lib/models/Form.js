...
function extractRoles(form) {
  ...
  Object.keys(form.fields).forEach(name => {
    ...
    const role = field.assigned_to
    const num = field.assigned_to_index
    ...
    if (num > form.roles[role].max_count)
      form.roles[role].max_count = num
  })
}
...
Form.submit = ({deal_id, form_id, user_id, state, values}, cb) => {
  const getForm = cb => {
    Form.get(form_id, cb)
  }
