  ...
  if(contact.sub_contacts) {
    contact.sub_contacts.map(c => {
      if(c.attributes && c.attributes.name)
        names = names.concat(c.attributes.name)

      if(c.emails)
        emails = emails.concat(c.emails)

      if(c.phone_numbers)
        phone_numbers = phone_numbers.concat(c.phone_numbers)
...
Contact.publicize = function (model) {
  ...
  model.users &&
  model.users[0] &&
  model.users[0].attributes &&
  model.users[0].attributes.profile_image_url &&
  model.users[0].attributes.profile_image_url[0] &&
  model.users[0].attributes.profile_image_url[0].profile_image_url ?
  model.users[0].attributes.profile_image_url[0].profile_image_url : undefined

  const contact_p =
  model.attributes &&
  model.attributes.profile_image_url &&
  model.attributes.profile_image_url[0] &&
  model.attributes.profile_image_url[0] &&
  model.attributes.profile_image_url[0].profile_image_url ?
  model.attributes.profile_image_url[0].profile_image_url : undefined

  model.display_profile_image_url = contact_p || user_p || undefined
