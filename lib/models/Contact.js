...
var sql_string_search = require('../sql/contact/string_search.sql');
...
Contact.add = function (user_id, contact, cb) {
  ...
  if (contact.phone_number) {
    ...
    try {
      var formatted = ObjectUtil.formatPhoneNumberForDialing(contact.phone_number);
      if(!formatted)
...
Contact.stringSearch = function (user_id, terms, limit, similarity, all, cb) {
  terms = terms.join('|');

  db.query(sql_string_search, [user_id, terms, limit, similarity, all], (err, res) => {
    if (err)
      return cb(err);
    ...
    var contact_ids = res.rows.map(r => {
      return r.id;
    });
...
Contact.publicize = function (model) {
  ...
  if(model.contact_user & model.contact_user.profile_image_url)
    contact.display_profile_image_url = model.contact_user.profile_image_url || contact.profile_image_url;

  if(model.contact_user & model.contact_user.cover_image_url)
    contact.display_cover_image_url = model.contact_user.cover_image_url || contact.cover_image_url
