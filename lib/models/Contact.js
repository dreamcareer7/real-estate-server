...
Contact.addTag = function (contact_id, tag, cb) {
  //console.log(tag.length)
  for (var i = 0; i<tag.length;i++) {
    db.query(sql_add_tag, [contact_id, tag[i]], cb);
  }
};

Contact.getByTags = function (tags, cb) {

  var quoted_tags =(tags.replace(',','\',\''));
  console.log(tags);
  db.query(sql_get_by_tag, [tags], function (err, res) {
    if (err)
      return cb(err);
    ...
    var contacts = res.rows.map(function (r) {
      return r;
    });
...
Contact.getTags = function (contact_id, cb) {
  db.query(sql_get_tags, [contact_id], function (err, res) {
    if (err)
      return cb(err);
