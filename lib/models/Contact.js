...
Contact.addAttributes = function (contact_id, user_id, attributes, cb) {
  ...
  async.auto({
    ...
    emails: [
      ...
      (cb, results) => {
        ...
        expect(attributes.emails).to.be.a('array')

        return async.map(attributes.emails, (r, cb) => {
          Contact.addAttribute(contact_id, user_id, 'email', r, cb)
        }, cb)
      }
    ],
    phone_numbers: [
      ...
      (cb, results) => {
        ...
        expect(attributes.phone_numbers).to.be.a('array')

        return async.map(attributes.phone_numbers, (r, cb) => {
          console.log(r)
          Contact.addAttribute(contact_id, user_id, 'phone_number', r, cb)
        }, cb)
      }
    ],
    others: [
      'get',
      (cb, results) => {
        const others = attributes.attributes
        if(!others)
          return cb()

        expect(others).to.be.a('object')

        return async.map(Object.keys(others), (key, cb) => {
          expect(others[key]).to.be.a('array')

          async.map(others[key], (r, cb) => {
            Contact.addAttribute(contact_id, user_id, r.type, r, cb)
          }, cb)
        }, cb)
      }
    ],
