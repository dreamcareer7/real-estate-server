...
const sql_get = require('../sql/contact/get.sql')
const sql_user = require('../sql/contact/user.sql')
const sql_add = require('../sql/contact/add.sql')
const sql_delete = require('../sql/contact/delete.sql')
const sql_update = require('../sql/contact/update.sql')
const sql_string_search = require('../sql/contact/string_search.sql')
const sql_string_search_fuzzy = require('../sql/contact/string_search_fuzzy.sql')
const sql_patch_avatars = require('../sql/contact/patch_avatars.sql')
const sql_add_tag = require('../sql/contact/add_tag.sql')
const sql_get_by_tag = require('../sql/contact/get_by_tag.sql')
const sql_remove_tag = require('../sql/contact/remove_tag.sql')
const sql_remove_tags = require('../sql/contact/remove_tags.sql')
const sql_connected = require('../sql/contact/connected.sql')
const sql_dup = require('../sql/contact/is_dup.sql')
...
Contact.getDisplayName = function (contact) {
  if (!_u.isEmpty(contact.first_name) && !_u.isEmpty(contact.last_name))
    return contact.first_name + ' ' + contact.last_name

  if (!_u.isEmpty(contact.email))
    return contact.email

  if (!_u.isEmpty(contact.phone_number))
...
Contact.getAbbreviatedDisplayName = function (contact) {
  if (!_u.isEmpty(contact.first_name))
...
Contact.add = function (user_id, contact, cb) {
  if (typeof contact.email === 'string' && contact.email.length < 1)
    delete contact.email

  if (typeof contact.phone_number === 'string' && contact.phone_number.length < 1)
  ...
  if (contact.phone_number) {
    ...
    try {
      const formatted = ObjectUtil.formatPhoneNumberForDialing(contact.phone_number)
      if (!formatted)
...
Contact.patch = function (contact_id, user_id, contact, cb) {
  ...
  Contact.resolve(contact.email, contact.phone_number, (err, contact_user_id) => {
    if (err)
      return cb(err)
    ...
    validate(contact, function (err) {
      ...
      Contact.removeTags(contact_id, user_id, err => {
        if (err)
          return cb(err)

        if (contact.tags)
          Contact.addTag(contact_id, contact.tags, user_id, function (err) {
            if (err)
              return cb(err)
...
Contact.resolve = function (email, phone_number, cb) {
  User.getByEmail(email, (err, user_email) => {
    if (err)
      return cb(err)

    User.getByPhoneNumber(phone_number, (err, user_phone) => {
      if (err)
        return cb(err)
...
Contact.stringSearchFuzzy = function (user_id, terms, limit, similarity, all, cb) {
  terms = terms.join('|')

  db.query(sql_string_search_fuzzy, [user_id, terms, limit, similarity, all], (err, res) => {
    if (err)
      return cb(err)
...
Contact.stringSearch = function (user_id, terms, limit, cb) {
  ...
  db.query(sql_string_search, [user_id, terms, limit], (err, res) => {
    if (err)
      return cb(err)
...
Contact.getByTags = function (user_id, tags, cb) {
  ...
  db.query(sql_get_by_tag, ['Contact', tag_arr, user_id], (err, res) => {
    if (err)
      return cb(err)
...
Contact.removeTag = function (contact_id, tag, user_id, cb) {
  db.query(sql_remove_tag, [contact_id, tag, user_id], cb)
}
...
Contact.removeTags = function (contact_id, user_id, cb) {
  Contact.get(contact_id, err => {
    ...
    return db.query(sql_remove_tags, [contact_id, user_id], cb)
  })
}

Contact.isConnected = function (user_id, peer_id, cb) {
  db.query(sql_connected, [user_id, peer_id], (err, res) => {
    if (err)
      return cb(err)
