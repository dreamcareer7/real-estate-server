...
function getContacts(family, user_id, limit, offset, cb) {
  db.query(family, [user_id, limit, offset], function(err, res) {
    if(err)
      return cb(err);
    ...
    async.map(contact_ids, User.get, function(err, contacts) {
      if (contacts[0]) contacts[0].full_count = res.rows[0].full_count;
      cb(null, contacts);
    });
  });
}
...
Contact.getByUser = function(user_id, limit, offset, cb) {
  getContacts(sql_get, user_id, limit, offset, cb);
}

Contact.getShortlisted = function(user_id, limit, offset, cb) {
  getContacts(sql_get_shortlist, user_id, limit, offset, cb);
}
