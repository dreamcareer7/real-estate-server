...
Contact.add = function (user_id, contact, cb) {
  ...
  if (contact.phone_number) {
    ...
    try {
      contact.phone_number = ObjectUtil.formatPhoneNumberForDialing(contact.phone_number);
    } catch (err) {
  ...
  async.auto({
    ...
    notification: [
      ...
      'get',
      (cb, results) => {
        if (!contact.source_type || !results.get.id || contact.source_type == 'ExplicitlyCreated')
          return cb();
...
Contact.connect = function (user_id, peer_id, cb) {
  Contact.join(user_id, peer_id, (err) => {
    if (err)
      return cb(err);

    Contact.join(peer_id, user_id, (err) => {
      if (err)
        return cb(err);
