  ...
  db.query(sql_get, [contact_id], function (err, res) {
    ...
    async.parallel({
      ...
      tags: cb => {
        ...
        Contact.getTags(contact_id, process.domain.user.id, cb);
      }
    }, (err, results) => {
...
Contact.getTags = function (contact_id, user_id, cb) {
  if (user_id) {
    db.query(sql_get_tags, [contact_id, 'Contact', user_id], function (err, res) {
      if (err)
        return cb(err);
...
Contact.publicize = function (model) {
  if (model.contact_user) CompactUser.publicize(model.contact_user);
