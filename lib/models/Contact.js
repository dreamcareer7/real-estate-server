...
Contact.add = function (user_id, contact, cb) {
  ...
  async.auto({
    ...
    add_tag: ['resolve',
      ...
      function (cb, results) {
        if (contact.tags)
          return Contact.addTag(results.create, contact.tags, cb);
        else
          return cb();
      }],
    get: ['resolve',
...
Contact.stringSearch = function (user_id, strings, cb) {
  var regexps = strings.map(function (r) {
    return '.*' + r + '.*';
  });
...
var insert = function (contact_id, tag, cb) {
  db.query(sql_add_tag, [contact_id, tag], cb);
}

Contact.addTag = function (contact_id, tags, cb) {
  async.each(tags, insert.bind(null, contact_id), cb);
};

Contact.getByTags = function (tags, cb) {
  var tag_arr = tags.split(',');

  db.query(sql_get_by_tag, ['contact', tag_arr], function (err, res) {
    if (err)
      return cb(err);
...
Contact.removeTag = function (contact_id, tag, cb) {
  db.query(sql_remove_tag, [contact_id, tag, 'contact'], cb);
};
...
Contact.join = function (user_id, peer_id, cb) {
  Contact.isConnected(user_id, peer_id, function (err, connected) {
    ...
    if (connected) {
      console.log('user', user_id, 'is already connected to', peer_id);
      return cb();
    }
    ...
    User.get(peer_id, function (err, peer) {
      ...
      console.log('join user', user_id, 'with', peer_id);
