...
Agent.getAll = function(agent_ids, cb) {
  db.query('agent/get', [agent_ids], (err, res) => {
    if (err)
      return cb(err)
...
Agent.auditSecret = function (id, secret, cb) {
  Agent.get(id, (err, agent) => {
    ...
    console.log('==>', agent.phone_numbers, agent.emails, found)
    if (agent.phone_numbers)
...
Agent.getByOfficeId = function (office_mls, cb) {
  db.query('agent/office_mls', [office_mls], (err, res) => {
    ...
    Agent.getAll(agent_ids, (err, agents) => {
      if (err)
        return cb(err)
...
Agent.search = function (query, cb) {
  db.query('agent/search', [query], function (err, res) {
    ...
    Agent.getAll(res.rows.map(r => r.id), cb)
  })
}
...
Agent.publicize = function (model) {
  ...
  if (model.emails) {
    const valid_emails = model.phone_numbers.filter(Boolean)
