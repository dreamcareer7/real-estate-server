...
Agent.get = function(id, cb) {
  db.query(sql_get, [id], function(err, res) {
    ...
    var e = agent.emails || [];
    var p = agent.phone_numbers || [];

    agent.secret_questions = e.concat(p);
...
Agent.auditSecret = function(id, secret, cb) {
  Agent.get(id, (err, agent) => {
    ...
    if(_u.indexOf(agent.secret_questions, secret) != -1)
      return cb(null, true);
...
Agent.publicize = function(model) {
  ...
  if(model.emails) delete model.emails;
  if(model.phone_numbers) delete model.phone_numbers;
  if(model.secret_questions) {
    var obfs = model.secret_questions.map(ObjectUtil.obfuscateString);
    model.secret_questions = obfs;
  }
