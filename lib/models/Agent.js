  ...
  Agent.get(id, (err, agent) => {
    ...
    if(agent.phone_numbers)
      agent.phone_numbers.forEach(phone => {
        cleaned_phone  = phone.replace(/\D/g, '');
        cleaned_secret = secret.replace(/\D/g, '');
...
function obfuscatePhone(string) {
  ...
  var paranthesis = string.match(/^(\s){0,}\((\d){1,}\)/);
  if(!paranthesis)
    return obfuscated;

  return paranthesis[0] + obfuscated.slice(paranthesis[0].length, -2) + string.slice(-2);
}
