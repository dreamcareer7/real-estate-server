    ...
    const send = notification_user => {
      ...
      Socket.send('Notification', user.id, [airship], (err) => {
        ...
        getUserStatus(user, (err, status) => {
          ...
          /* In case of a multi-transport notifications, send them immediately
           * if the transports include SMS or push. Email transport must be
           * handled in corresponding model, internally. */
          const trans = notification.transports
          const forceSend = trans?.includes?.('sms') || trans?.includes?.('push')
          
          if (err || status === BACKGROUND || status === OFFLINE || notification.transports?.length || forceSend)
