...
const send = function ({
  ...
  const transports = (cb, results) => {
    ...
    Context.log(`[Debug:Notification] Sending ${notification.id} - Transports: ${notification.transports}`)
...
const sendForUnread = async () => {
  ...
  Context.log(`[Debug:Notifications] Sending unread: ${notification_ids}`)
