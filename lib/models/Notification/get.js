...
/**
 * Finds unread notifications over time.
 * @typedef {string} PostgresInterval
 * @typedef {import('../../../types/models/Notification').INotification} Notification
 * @typedef {import('../../../types/models/Notification').TNotificationObjectClass} NotificationObjectClass
 *
 * @param {object} options
 * @param {PostgresInterval} [options.minAge='1 min'] - minimum notification age
 * @param {PostgresInterval} [options.maxAge='30 min'] - maximum notification age
 * @param {NotificationObjectClass} options.objectClass - object class
 *
 * @returns {Promise<Notification[]>}
 */
const getUnreadOverTime = async function({
  minAge = '1 min',
  maxAge = '30 min',
  objectClass,
}) {
  const ids = await db.selectIds('notification/unread_over_time', [
    objectClass,
    maxAge,
    minAge,
  ])

  return promisify(getAll)(ids)
}
