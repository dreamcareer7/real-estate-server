...
const airshipConfig = require('../../config').push.rechat.config
  ...
  getUser(user_id).nodeify((err, user) => {
    ...
    db.query('notification/user_channels', [user_id], (err, res) => {
      ...
      async.each(
        res.rows,
        (r, calbk) => {
          airship.isTokenValid(airshipConfig, r.channel).nodeify((err, result) => {
...
const sendToDevice = function (notification_user, notification, channel_id, user_id, cb) {
  ...
  airship.send(airshipConfig, pushInfo)
    .then(data => {
