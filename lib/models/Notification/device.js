...
const getOneSignalDeviceTokensForUser = function (userId, onesignalConfig, cb) {
  return (async () => {
    ...
    const promises = res.rows
      .map(r => r.channel)
      .map(ch => onesignal.isTokenValid(onesignalConfig, ch).then(
...
const sendToOneSignal = function (notificationUser, notification, channelId, userId, onesignalConfig, cb) {
  ...
  onesignal.send(onesignalConfig, options)
    .then(data => {
