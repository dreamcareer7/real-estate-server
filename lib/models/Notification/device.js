...
const sendToAirship = function (notification_user, notification, channel_id, user_id, cb) {
  ...
  airship.send(notification.app, pushInfo)
    .then(data => {
      Context.log('<- (Airship-Transport) Successfully sent a push notification'.green, notification.id, channel_id, pushInfo)

      cb(null, data)      
    })
    .catch(err => {
      Context.log('<- (Airship-Transport) Error sending push notification'.red, notification.id, channel_id, err, pushInfo)
      cb(err)
    })
}
