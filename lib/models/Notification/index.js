...
Notification.create = function (notification, cb) {
  ...
  const push = (user, results, cb) => {
      ...
      Socket.send('Notification', user.id, [airship], (err) => {
        ...
        User.getStatus(user, (err, status) => {
          ...
          if (err || status === User.BACKGROUND || status === User.OFFLINE)
            return Notification.send(user.id, notification.room, airship, cb)
    ...
    insertForUser(populated.id, user.id, (err) => {
      if (err)
        return cb(err)
  ...
  const pushToAll = (cb, results) => {
    debug('>>> (Notification::create::pushToAll) Sending push to users:', results.users.map(u => u.id))
...
const sendAsPush = function(notification, user, room, tokens, delay, cb) {
  async.auto({
    ...
    message: (cb, results) => {
      ...
      Notification.formatForDisplay(notification, data, Notification.TRANSPORT_PUSH, (err, message) => {
        if(err)
          return cb(err)
