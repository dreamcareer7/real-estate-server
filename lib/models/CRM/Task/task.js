...
class CrmTask extends CrmTaskEventEmitter {
  ...
  async validate(task) {
    ...
    if (task.end_date && task.due_date) {
      expect(task.due_date <= task.end_date).to.be.equal(true)
    }
   ...
   * @returns {Promise<ITask>}
   */
  async create(task, _reason = 'direct_request') {
   ...
   * @param {string} _reason
   */
  async createMany(tasks, _reason = 'direct_request') {
   ...
   * @returns {Promise<any>}
   */
  async remove(task_ids, user_id, _reason = 'direct_request') {
   ...
   * @param {string=} _reason
   */
  async updateMany(tasks, user_id, _reason = 'direct_request') {
   ...
   * @returns {Promise<ITask>}
   */
  async update(task_id, task, user_id, _reason = 'direct_request') {
    await this.validate(task)
    ...
    if (task_changed || meta_changed) {
      ...
      this.emit('update', { task_ids: [task_id], brand_id: current.brand, user_id, reason: _reason })
    }
