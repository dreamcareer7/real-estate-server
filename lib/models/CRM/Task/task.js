...
class CrmTask extends CrmTaskEventEmitter {
  ...
  async filter(user_id, brand_id, options) {
    ...
    if (options.google_event_ids)
      q.where('google_event_id = ANY(?)', sq.SqArray.from(options.google_event_ids))

    if (options.microsoft_event_ids)
      q.where('microsoft_event_id = ANY(?)', sq.SqArray.from(options.microsoft_event_ids))
  ...
  async updateMany(tasks, user_id) {
    ...
    // Origin: for (const task of tasks) {
    ...
    // Origin: const for_update = tasks.map(t => _.pick(t, ['id', 'title', 'task_type', 'description', 'due_date', 'end_date', 'status', 'metadata']))
