...
const clone = async (new_task_data, original_task_id) => {
  ...
  const [original_task] = await Orm.populate({
    ...
    associations: ['crm_task.associations', 'crm_task.reminders', 'crm_task.assignees']
  })

  if (original_task?.associations?.length) {
    const assoc = await CrmAssociation.getAll(original_task.associations.map(({id}) => id))
    original_task.associations = assoc
  }
