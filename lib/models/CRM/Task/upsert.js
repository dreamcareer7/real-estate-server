...
const clone = async (new_task_data, original_task_id) => {
  ...
  original_task.assignees = original_raw_task.assignees

  if (new_task_data.associations) {
    new_task_data.associations = new_task_data.associations.map(a => ({ ...a, metadata: { origin: 'rechat' } }))
  }
  ...
  if (!task_to_create.associations) {
    delete task_to_create.associations
  }
  if (!task_to_create.assignees) {
    delete task_to_create.assignees
  }
  if (!task_to_create.reminders) {
    delete task_to_create.reminders
  }

  return create(task_to_create)
}
...
const updateMany = async (tasks, user_id, _reason = 'direct_request') => {
  ...
  await updateAssignees(Object.keys(current_tasks), tasks, user_id)
...
const update = async (task_id, task, user_id, _reason = 'direct_request') => {
  ...
  if (Array.isArray(task.assignees)) {
    await updateAssignees([updatedTask.id], [{ ...task, id: task_id }], user_id)
  }
 ...
 * @param {UUID[]} current_tasks The existing task data object
...
const updateAssignees = async (current_tasks, tasks, user_id) => {
  ...
  await Assignee.deleteForTasks(current_tasks, user_id)
