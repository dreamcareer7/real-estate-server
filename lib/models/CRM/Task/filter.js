...
const filter = async (user_id, brand_id, options) => {
  ...
  if (options.assignee) {
    ...
    q.where('cta.deleted_at IS NULL')
  }

  if (options.assignees) {
    q.where(`EXISTS(
      SELECT 1 FROM crm_tasks_assignees AS cta
      WHERE cta.crm_task = ct.id AND cta.deleted_at IS NULL AND
            cta.user = ANY(?::uuid[])
    )`, sq.SqArray.from(options.assignees))
  }
