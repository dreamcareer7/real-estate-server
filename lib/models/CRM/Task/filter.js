...
const filter = async (user_id, brand_id, options) => {
  ...
  } else if (options.type === 'Task' && options.task_type != null && options.task_type !== 'Other') {
    q.where('task_type = ?', options.task_type)
  } else if (options.type === 'Task' && options.task_type == null) {
    q.where('task_type != \'Other\'')
  } else if (options.type == null && options.task_type) {
    q.where('task_type = ?')
  }
