...
class TaskClass extends EventEmitter {
  ...
  async update(task_id, task, user_id) {
    ...
    if (task_changed || meta_changed) {
      ...
      this.emit('update', { task_id, brand: current.brand })
    }
  ...
  async updateAssociations(task, associations, user_id) {
    ...
    const toKeep = associations.filter(a => a.id)
    const toKeepIds = toKeep.map(a => a.id)
    ...
    await CrmAssociation.update(toKeep)
