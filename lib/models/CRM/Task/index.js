...
class TaskClass extends EventEmitter {
  ...
  async create(task, user_id, brand_id) {
    ...
    if (task.assignees) {
      await Assignee.create(added, task.assignees, user_id)
    }

    if (Array.isArray(task.associations)) {
      await this.Associations.createMany(task.associations, id, user_id, brand_id)
    }
  ...
  async updateAssociations(task, associations, user_id) {
    ...
    const toAdd = associations.filter(a => !a.id)
    ...
    await this.Associations.removeMany(toRemoveIds, task.id, user_id)
    await this.Associations.createMany(toAdd, task.id, user_id, task.brand)
  ...
  async updateAssignees(task, assignees = [], user_id) {
    ...
    const toAdd = _.difference(assignees, task.assignees)
    const toRemove = _.difference(task.assignees, assignees)

    await Assignee.create(task, toAdd, user_id)
...
TaskClass.prototype.Associations = AssociationsHelper('crm_task')
