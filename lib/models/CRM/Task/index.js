...
class TaskClass extends EventEmitter {
  ...
  async create(task, user_id, brand_id) {
    ...
    if (Array.isArray(task.associations)) {
      await this.Associations.createMany(task.associations, id)
    }
  ...
  async update(task_id, task, user_id) {
    ...
    if (task_changed) {
      ...
      Context.log('>>> (CrmTask::Update)', 'Creating UserEditedCrmTask notification on task', task.id)
      await promisify(Notification.issueForUsersExcept)(n, task.assignees, user_id, {})
    }
