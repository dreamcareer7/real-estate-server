...
class Task {
   ...
   */
  static async hasAccess(user_id, brand_id, op, task_ids) {
  ...
  static async getAll(ids, skip_associations = false) {
    ...
    if (!skip_associations) {
      ...
      const associations_index = await Task.Associations.getAllCategorizedByType(association_ids)
   ...
   * @returns {Promise<ITask[]>}
   */
  static async getForUser(user_id, brand_id, options) {
    const result = await Task.filter(user_id, brand_id, options)
   ...
   */
  static async filter(user_id, brand_id, options) {
    ...
    const q = squel.select()
      ...
      .where('check_task_read_access(crm_tasks, ?, ?)', user_id, brand_id)
    ...
    if (options.assignee)
      q.where('assignee = ?', options.assignee)
   ...
   * @returns {Promise<ITask>}
   */
  static async create(task, user_id, brand_id) {
    ...
    if (Array.isArray(task.associations)) {
      await Task.Associations.createMany(task.associations, id)
    }
   ...
   * @returns {Promise<ITask>}
   */
  static async update(task_id, task) {
    await Task.validate(task)
...
Task.Associations = AssociationsHelper('crm_task')
