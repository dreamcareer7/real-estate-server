...
class TaskClass extends EventEmitter {
   ...
   * @param {UUID} brand_id Owner of the task
   * @returns {Promise<ITask>}
   */
  async create(task, user_id, brand_id) {
    ...
    if (Array.isArray(task.associations)) {
      const associations = task.associations.map(a => ({ ...a, task: id }))
      await CrmAssociation.createMany(associations, user_id, brand_id)
    }
   ...
   * @param {UUID} brand_id 
   */
  async createMany(tasks, user_id, brand_id) {
    const fields = [
      'title',
      'description',
      'due_date',
    ...
    const data = tasks.map(t => ({
      ...belt.ensureFields(t, fields, { status: 'PENDING' }, { due_date: belt.epochToDate }),
      user_id,
      brand_id,
    }))

    const ids = await db.selectIds(
      sq.insert({ autoQuoteAliasNames: true, autoQuoteFieldNames: true })
        .into('crm_tasks')
        .setFieldsRows(data)
        .returning('id'),
      []
   ...
   * @param {ITask} task The existing task data object
   * @param {ICrmAssociationInput[]} associations Array of final associations to be replaced into the parent task
   * @param {UUID} user_id Current user
   */
  async updateAssociations(task, associations, user_id) {
    expect(associations).to.be.an('array')

    const toAdd = associations.filter(a => !a.id).map(a => ({ ...a, task: task.id }))
    ...
    await CrmAssociation.createMany(toAdd, user_id, task.brand)
  ...
  async updateAssignees(task, assignees = [], user_id) {
    ...
    await Assignee.create(toAdd, user_id)
