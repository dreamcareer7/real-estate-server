const db = require('../../utils/db.js')
const validator = require('../../utils/validator.js')

const { reminderSchema: schema } = require('./schemas.js')
const Notification = require('../Notification')

const validate = validator.promise.bind(null, schema)

function raiseNotFound(id) {
  throw Error.ResourceNotFound(`Reminder ${id} not found`)
}

/**
 * Calculates exact timestamp for when a reminder is relative to the task's due data
 * @param {IReminderInput} reminder Target reminder object
 * @param {ITask} task Parent task data object
 * @returns {Date} The calculated timestamp that's saved to the DB
 */
function calculateTimestamp(reminder, task) {
  if (reminder.is_relative) {
    return new Date(task.due_date.getTime() + reminder.time * 1000)
  }

  return new Date(reminder.timestamp)
}

const Reminder = {
  /**
   * Get a reminder by id
   * @param {UUID} id Reminder id to fetch
   * @returns {Promise<IReminder>}
   */
  async get(id) {
    const reminders = await Reminder.getAll([id])

    if (!reminders || reminders.length < 1) {
      raiseNotFound(id)
    }

    return reminders[0]
  },

  /**
   * Get multiple Reminders by id
   * @param {UUID[]} ids Array of Reminder ids to fetch
   * @returns {Promise<IReminder[]>}
   */
  async getAll(ids) {
    const res = await db.query.promise('crm/reminder/get', [
      ids,
    ])

    return res.rows
  },

  /**
   * Create a reminder
   * @param {IReminderInput} reminder Reminder object to add
   * @param {ITask} task Parent task of the Reminder
   * @return {Promise<UUID>}
   */
  async create(reminder, task) {
    await validate(reminder)

    const timestamp = calculateTimestamp(reminder, task)

    const res = await db.query.promise('crm/reminder/insert', [
      task.id,
      reminder.time,
      reminder.is_relative,
      timestamp
    ])

    return res.rows[0].id
  },

  /**
   * Remove a reminder
   * @param {UUID} reminder_id Reminder id to be removed
   * @return {Promise<any>}
   */
  async remove(reminder_id) {
    const reminder = await Reminder.get(reminder_id)

    const res = await db.query.promise('crm/reminder/delete', [
      reminder_id,
    ])

    if (reminder.notification)
      await Notification.remove(reminder.notification)

    return res
  },

  /**
   * Update a reminder
   * @param {IReminderInput} reminder Reminder object with id to be updated
   * @param {ITask} task Parent task data object of the reminder
   */
  async update(reminder, task) {
    await validate(reminder)

    const timestamp = calculateTimestamp(reminder, task)

    return db.query.promise('crm/reminder/update', [
      task.id,
      reminder.id,
      reminder.time,
      reminder.is_relative,
      timestamp
    ])
  },

  /**
   * Sets notification id on a reminder after it is created
   * @private
   * @param {IReminder} reminder Reminder object to patch its notification
   * @param {UUID} notification_id Notification id created for the reminder
   */
  async patchNotification(reminder, notification_id) {
    return db.query.promise('crm/reminder/patchNotification', [
      reminder.id,
      notification_id
    ])
  },

  publicize(data) {
    data.timestamp = new Date(data.timestamp).getTime()
  }
}

Orm.register('reminder', 'Reminder', Reminder)

module.exports = Reminder
