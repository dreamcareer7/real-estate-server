...
async function update_touch_times_for_synced_messages({ microsoft_credential, google_credential, timestamp, recipients }) {
  async function update(brand) {
    ...
    await increase_touch_times_for_contacts({ contacts: ids, brand, timestamp: timestamp / 1000, action: 'email' })
  }

  if (microsoft_credential) {
    const [{ brand }] = await MicrosoftCredential.getAll([microsoft_credential])
    await update(brand)
  } else if (google_credential) {
    const [{ brand }] = await GoogleCredential.getAll([google_credential])
    await update(brand)
  } else {
