...
async function update_next_touch_for_list_members({ list_id }) {
  ...
  await db.update(
    'crm/touch/minimize_contact_touch_freq_for_lists',
...
async function update_touch_times_for_tag({ tag, brand }) {
  ...
  const tagObj = await sql.selectOne(
    'SELECT touch_freq FROM crm_tags WHERE tag = $1 AND brand = $2 LIMIT 1',
    [tag, brand]
  )
  if (Number.isSafeInteger(tagObj?.touch_freq)) {
    await db.update(
      'crm/touch/minimize_contact_touch_freq',
      [ids, tagObj.touch_freq]
    )
  }
