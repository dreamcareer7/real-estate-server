...
async function update_touch_times_for_contacts({ contacts, brand }) {
  Context.log(`[Last Touch] Updating touch times for ${contacts.length} contacts`)
  const [start, ] = process.hrtime()
  await db.timed('crm/touch/update_touch_times_for_contacts', [ contacts ], 30000)
  const [end, ] = process.hrtime()

  if (end - start > 20) {
    Context.log(`[Last Touch] The operation took approximately ${end - start} seconds.`)
 ...
 */
async function update_touch_times_for_tags ({ tags, brand }) {
