...
class Touch {
   ...
   */
  static async get(id) {
    const result = await Touch.getAll([id])
  ...
  static async getAll(ids, skip_associations = false) {
    ...
    if (!skip_associations) {
      ...
      const associations_index = await Touch.Associations.getAllCategorizedByType(association_ids)
  ...
  static async getForUser(user_id, options) {
    const result = await Touch.filter(user_id, options)
    const touches = await Touch.getAll(result.ids)
   ...
   * @returns {Promise<ITouch>}
   */
  static async create(touch, user_id, brand_id) {
    await Touch.validate(touch)
    ...
    if (Array.isArray(touch.associations)) {
      await Touch.Associations.createMany(touch.associations, id)
    }
  ...
  static async remove(id) {
    return db.query.promise('crm/touch/delete', [
      id,
    ])
  }
   ...
   * @returns {Promise<ITouch>}
   */
  static async update(id, touch) {
    await Touch.validate(touch)

    const timestamp = belt.epochToDate(touch.timestamp)

    await db.query.promise('crm/touch/update', [
      id,
      touch.description,
      timestamp,
      touch.activity_type,
...
Touch.associations = associations
Touch.Associations = AssociationsHelper('touch')

Orm.register('touch', 'Touch', Touch)
