...
const getAll = async (model_name, ids, associations) => {
  ...
  const promised = (getAll.constructor.name === 'AsyncFunction') ? getAll : promisifyGetAll(getAll)

  return promised(ids, associations)
}
...
const populate = async ({models, associations, stash}) => {
  ...
  const loadAll = () => {
    const promises = Object.keys(toLoad).map(model_name => {
      return getAll(model_name, Array.from(toLoad[model_name]), associations)
    })
