...
Orm.register = (type, model_name, model) => {
  type_to_model[type] = model_name
  model_name_to_type[model_name] = type
  model_name_to_model[model_name] = model || global[model_name]
}

Orm.getModelFromType = (type) => {
...
function publicizeData (data, options) {
  ...
  const { select, omit } = Orm.getPublicFields()
...
const populate = async ({models, associations, stash}) => {
  ...
  const previousAssociations = Orm.getEnabledAssociations()
   ...
   */
  Orm.setEnabledAssociations(previousAssociations)
}
    ...
    if (definition.polymorphic) {
      if (definition.collection) {
        ...
        if (data[key]) {
          mapping[key] = {
            id: data[key].id,
            model: type_to_model[data[key].type]
          }
        } else {
          data[key] = null
        }
  ...
  const mount = model => {
    ...
    const getByReference = association => {
      ...
      if (!stash[type] || !stash[type][association.id] || !stash[type][association.id][0])
        throw Error.ResourceNotFound(`${association.model} ${association.id} not found`)
...
Orm.setEnabledAssociations = enabled => {
  Context.set({
    'Orm.Enabled-Associations': enabled
  })
}

Orm.enableAssociation = association => {
  const current = Orm.getEnabledAssociations()

  if (!current.includes(association)) {
    Orm.setEnabledAssociations(current.concat(association))
  }
}

Orm.getAssociationConditions = (association) => {
...
Orm.setAssociationConditions = conditions => {
  Context.set({
    'Orm.Associations-Conditions': conditions
  })
}
