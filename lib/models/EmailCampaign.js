...
class EmailCampaign {
  ...
  async create({ from, to, subject, html }, brand_id) {
    ...
    const tags = to.filter(/** @type {TIsTagPresent} */ (isPropertyPresent('tag'))).map(t => t.tag)
    const lists = to.filter(/** @type {TIsListPresent} */ (isPropertyPresent('list'))).map(t => t.list)
    ...
    const email_recipients = (to.filter(/** @type {TIsEmailPresent} */ (x => 
    ...
    const emails = recipients.map(/** @returns {IEmail} */rcp => ({
    ...
    const contact_emails = recipients
      .filter(/** @type {TRequireProp<IEmailRecipientEmailInput, 'contact'>} */ (isPropertyPresent('contact')))
      .map(/** @returns {IContactEmail} */(rcp, i) => ({
        user: from,
        contact: rcp.contact,
        email: email_ids[i]
      }))
