...
Listing.getInterestedUsers = function (user_id, cb) {
  db.query('listing/interested', [user_id], function (err, res) {
    if (err)
      return cb(err)
...
Listing.issueChangeNotifications = function (listing_id, before, after, cb) {
  ...
  if(after.price !== before.price)
    types.push = 'PriceDropped'

  if(after.status !== before.status)
    types.push = 'StatusChanged'
  ...
  async.auto({
    ...
    price_drop_trigger: [
      ...
      'interested',
      (cb, results) => {
        if (!_u.contains(types, 'PriceDropped'))
          return cb()
        ...
        async.map(results.interested, (interested, cb) => {
          ...
          if (!before.price || before.price === 0)
            ...
            notification.message = `[Price Change] ${results.adress} went from ${Listing.priceHumanReadable(before.price)} to ${Listing.priceHumanReadable(after.price)}`
    ...
    status_change_trigger: [
      ...
      'interested',
      (cb, results) => {
        if (!_u.contains(types, 'StatusChanged'))
          return cb()
