...
Listing.issueChangeNotifications = function(id, before, after, cb) {
  Listing.get(id, function(err, listing) {
    ...
    Listing.getInterestedRooms(id, function(err, rooms) {
      if(err)
        return cb(err);
          ...
          if(after.price != before.price) {
            async.map(rooms,
                      function(room_id, cb) {
                        ...
                        //     notification.message = '[New Price] #' + room.title + ': ' + address_line + ' is now $' + numeral(listing.price).format('0,0');
            ...
            async.map(rooms,
                      function(room_id, cb) {
                        Room.get(room_id, function(err, room) {
                          ...
                          // Notification.issueForRoom(room_id, notification, cb);
                          return cb();
                        });
                      },
                      function(err, results) {
