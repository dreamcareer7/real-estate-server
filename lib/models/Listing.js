                               ...
                               Room.get(room_id, function(err, room) {
                                 ...
                                 Recommendation.mapListingOnRoom(room_id, listing_id, function(err, recommendation_id) {
                                   ...
                                   console.log('Price Drop Notification:'.cyan,
                                               ('#' + results.liting.matrix_unique_id).red,
                                               '('.cyan, results.liting.id.yellow, ')'.cyan,
                                               'on Room with ID:'.cyan,
                                               room.id.yellow,
                                               '*'.blue, results.address, '*'.blue,
                                               'MLS#:'.white, results.liting.mls_number,
                                               'were:'.white, numeral(results.liting.last_price).format('0,0'),
                                               'is now:'.white, numeral(results.liting.price).format('0,0'));
                                   if (!results.results.liting.last_price || results.results.liting.last_price == 0) {
                                     notification.message = '[New Price] #' + room.title + ': ' + results.address
                                                          + ' is now $' + numeral(results.liting.price).format('0,0');
                                     return Notification.issueForRoom(notification, cb);
                                   } else {
                                     notification.message = '[Price Change] #' + room.title + ': ' + results.address + ' went from $'
                                                          + numeral(results.liting.last_price).format('0,0') + ' to $'
                                                          + numeral(results.liting.price).format('0,0');
                                     return Notification.issueForRoom(notification, cb);
                                   }
                                 });
                               });
