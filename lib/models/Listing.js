...
Listing.getAll = function(listing_ids, cb) {
  ...
  console.log('Getting all', listing_ids)
  db.query('listing/get', [listing_ids, user_id], (err, res) => {
    if (err)
      return cb(err)
    ...
    listings.forEach(l => {
      console.log(l.id, l.property.id, typeof l.property.address)
    })
...
Listing.create = function ({address, property, listing}, cb) {
  ...
  const notification = (cb, results) => {
    const id = results.listing.rows[0].id
  ...
  async.auto({
    ...
    address: saveAddress,
    geo: ['address', geocode],
    property: ['address', saveProperty],
    validate: ['address', 'property', cb => validate(listing, cb)],
    listing: ['property', saveListing],
