        ...
        if(after.price != before.price) {
          async.map(results.rooms, (interested, cb) => {
            ...
            if (!before.price || before.price === 0) {
              notification.message = '[New Price] #' + Room.getTitle({title: room.title, users: interested.users}) + ': ' + results.address
                + ' is now ' + Listing.priceHumanReadable(after.price);
              return Notification.issueForRoom(notification, cb);
            } else {
              notification.message = '[Price Change] #' + Room.getTitle({title: room.title, users: interested.users}) + ': ' + results.address + ' went from '
      ...
      (cb, results) => {
        if(after.status != before.status) {
          async.map(results.rooms, (interested, cb) => {
            ...
            notification.object = room.id;
            notification.object_class = 'Room';
            notification.message = '#' + Room.getTitle({title: room.title, users: interested.users}) + ': ' + results.address + ' is now ' + after.status;
            notification.recommendation = interested.recommendation;
            notification.room = room.id;
...
Listing.send = function(transport, data, cb) {
  ...
  var job_data = {
    ...
    subject: '[Rechat] '+ (data.room.title || 'New Listing'),
