...
Listing.issueHitNotifications = function (listing_id, cb) {
  ...
  async.auto({
    ...
    interestedUsers: [
      ...
      // 'alerts',
      (cb, results) => {
        console.log('yess', results.users)
        Alert.getUserAndStatus(results.users.user_ids, results.users.alert_ids, 
          Alert.userAlertSettingStatusEnum.Enabled, cb)
      }
    ],
    notification: [
      'listing',
    ...
    send: [
      ...
      'notification',
      (cb, results) => {
        console.log('Creating Notifications', results.interestedUsers)

        async.map(results.interestedUsers, (r, cb) => {
...
Listing.issueChangeNotifications = function (listing_id, before, after, cb) {
  ...
  async.auto({
    ...
    newUsersThatShouldBeNotified: cb => {
      Listing.matchingUsersByAlerts(listing_id, cb)
    },
    usersWithAlertEnabled: [
      'interested',
      'newUsersThatShouldBeNotified',
      // 'alerts',
      (cb, results) => {
        Alert.getUserAndStatus(results.interested, results.newUsersThatShouldBeNotified.alert_ids, Alert.userAlertSettingStatusEnum.Enabled, (err, res) => {
          if (err) {
            return cb(err)
          }
          Alert.getUserAndStatus(results.newUsersThatShouldBeNotified.user_ids, results.newUsersThatShouldBeNotified.alert_ids,
            Alert.userAlertSettingStatusEnum.Enabled, (err, res2) => {
              if (err) {
                return cb(err)
              }
              cb(null, {
    ...
    price_drop_trigger: [
      ...
      (cb, results) => {
        ...
        async.map(results.usersWithAlertEnabled.interested, (interested, cb) => {
          const notification = {}
