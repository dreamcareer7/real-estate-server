...
const sql_insert = require('../sql/listing/insert.sql')
const sql_update = require('../sql/listing/update.sql')
const sql_get = require('../sql/listing/get.sql')
const sql_get_compacts = require('../sql/listing/get_compacts.sql')
const sql_get_mui = require('../sql/listing/get_mui.sql')
const sql_get_mls_number = require('../sql/listing/get_mls_number.sql')
const sql_delete = require('../sql/listing/delete.sql')
const sql_matching = require('../sql/listing/matching.sql')
const sql_interested = require('../sql/listing/interested.sql')
const sql_string_search = require('../sql/listing/string_search.sql')
const sql_get_statuses = require('../sql/listing/get_statuses.sql')

const sql_get_area = require('../sql/listing/area/get_by_area.sql')
const sql_search_mls_area = require('../sql/listing/area/search.sql')
const sql_mls_area = require('../sql/listing/area/get.sql')
const sql_refresh_areas = require('../sql/listing/area/refresh.sql')

const sql_refresh_counties = require('../sql/listing/county/refresh.sql')
const sql_search_county = require('../sql/listing/county/search.sql')

const sql_refresh_subdiv = require('../sql/listing/subdivision/refresh.sql')
const sql_search_subdiv = require('../sql/listing/subdivision/search.sql')
...
Listing.issueChangeNotifications = function (listing_id, before, after, cb) {
  async.auto({
    ...
    rooms: [
      'interested',
      (cb, results) => {
        if ((after.price !== before.price) || (after.status !== before.status)) {
          async.map(results.interested, (r, cb) => {
            Room.get(r.room, (err, room) => {
              if (err)
                return cb(err)

              Room.getUsers(r.room, (err, users) => {
                if (err)
                  return cb(err)
    ...
    price_drop_trigger: [
      ...
      (cb, results) => {
        if (after.price !== before.price) {
          async.map(results.rooms, (interested, cb) => {
            const room = interested.room
            ...
            if (!before.price || before.price === 0) {
              notification.message = '[New Price] #' + room.proposed_title + ': ' + results.address + ' is now ' + Listing.priceHumanReadable(after.price)
              return Notification.issueForRoom(notification, cb)
            }

            notification.message = '[Price Change] #' + room.proposed_title + ': ' + results.address + ' went from '
      ...
      (cb, results) => {
        if (after.status !== before.status) {
          async.map(results.rooms, (interested, cb) => {
            ...
            notification.object = room.id
            notification.object_class = 'Room'
            notification.message = '#' + room.proposed_title + ': ' + results.address + ' is now ' + after.status
            notification.recommendation = interested.recommendation
            notification.room = room.id
