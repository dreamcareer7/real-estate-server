...
var sql_get_area       = require('../sql/listing/get_area.sql');
...
Listing.inquiry = function(user_id, listing_id, agent, brand, source_type, brand_id, external_info, cb) {
  var in_user = agent.user_id || brand.default_user;
  ...
  async.auto({
    room: cb => {
      return User.connectToUser(user_id, in_user, source_type, brand_id, cb);
    },
    recommendation: [
      'room',
      (cb, res) => {
        return Room.recommendListing(res.room, listing_id, external_info, cb);
      }
    ],
    message: [
      'room',
