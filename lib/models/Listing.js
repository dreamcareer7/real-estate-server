...
var get_sql = "SELECT\
 'listing' AS type,\
 listings.*,\
 EXTRACT(EPOCH FROM listings.created_at)::bigint AS created_at,\
 EXTRACT(EPOCH FROM listings.updated_at)::bigint AS updated_at\
 FROM listings\
 WHERE listings.id = $1";
      ...
      async.parallel({
        ...
        listing_agency: function(cb) {
          ...
          Agency.get(listing.listing_agency_id, cb);
           ...
           res_final.alerting_agent = results.alerting_agent || {};
           res_final.listing_agent = results.listing_agent || {};
           res_final.listing_agency = results.listing_agency || {};
           cb(null, res_final);
