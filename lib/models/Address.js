...
Address.getOSMGeocodingSearchString = function(address) {
  var standard = address.street_number + ' ' + address.street_name + ', ' + address.city + ', ' + address.state_code + '&format=json&addressdetails=0';
...
Address.updateGeoFromOSM = function(id, cb) {
  Address.get(id, function(err, address) {
    ...
    request(config.osm.url + encodeURIComponent(standard), function(err, response, body) {
      if(err)
        return cb(err);
