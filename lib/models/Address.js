...
Address.get = function(address_id, cb) {
  db.query(sql_get, [address_id], function(err, res) {
    if(err)
      return cb(err);
...
/**
 * Retrieves an `Address` object from database based on `Matrix_Unique_ID`
 * @name getByMUI
 * @function
 * @memberof Address
 * @instance
 * @public
 * @param {Listin#mui} mui - Matrix Unique ID of the referenced address
 * @param {callback} cb - callback function
 * @returns {Address#address} full address object
 */
Address.getByMUI = function(mui, cb) {
  db.query(sql_get_mui, [mui], function(err, res) {
    if(err)
      return cb(err);
...
Address.delete = function(address_id, cb) {
  db.query(sql_delete, [address_id], cb);
}
 ...
 * @returns {Address#address}
 */
Address.update = function(address_id, address, cb) {
  Address.get(address_id, function(err, data) {
    if(err)
      return cb(err);
...
Address.mapToListing = function(address_id, cb) {
  Address.get(address_id, function(err, address) {
    ...
    db.query(sql_map_to_listing, [address_id], function(err, res) {
      if(err)
        return cb(err);
...
Address.reschedule = function(address_id, cb) {
  Address.mapToListing(address_id, function(err, listing_id) {
    if(!listing_id)
      return cb(null, null);
    else
...
/**
 * We consider an address accurate if the following conditions are met:
 * 1. Street number matches with the formatted address
 * 1. Street name matches with the formatted address
 * 1. City matches with the formatted address
 * 1. State code matches with the formatted address
 * 1. Postal code matches with the formatted address
 * 1. Country code matches with the formatted address
 * @name isAccurate
 * @function
 * @memberof Address
 * @instance
...
Address.updateGeoFromGoogle = function(address_id, cb) {
  Address.get(address_id, function(err, address) {
    ...
    var standard = Address.getGeocodingSearchString(address);
        ...
        try {
          ...
          if(body.status === 'OK') {
            if(body.results[0]) {
              ...
              console.log('(Google-Direct) NOTICE: UPDATING LAT/LONG for address with id:', address_id, '(', standard, ') location:', location, 'confidence:', confidence);
              ...
              console.log('(Google-Direct) WARNING[1]: Error geocoding an address with id:', address_id);
              return Address.markAsCorruptedGoogle(address_id, cb);
            }
          } else {
            console.log('(Google-Direct) WARNING[2]: Error geocoding an address with id:', address_id);
            return Address.markAsCorruptedGoogle(address_id, cb);
          }
        }
        catch (e) {
          console.log('(Google-Direct) WARNING[3]: Error geocoding an address with id:', address_id, 'with exception:', e);
          return Address.markAsCorruptedGoogle(address_id, cb);
        }
      } else {
            ...
            if(body.resourceSets[0]) {
              if(body.resourceSets[0].resources) {
                if(body.resourceSets[0].resources[0]) {
                  ...
                  console.log('(Bing-Direct) NOTICE: UPDATING LAT/LONG for address with id:', address_id, '(', standard, ') location:', location, 'confidence:', confidence);
...
Address.markAsCorrupted = function(address_id, cb) {
  db.query(sql_mark_as_corrupted, [address_id], function(err, res) {
    if(err)
      return cb(err);
...
Address.markAsCorruptedGoogle = function(address_id, cb) {
  db.query(sql_mark_as_corrupted_google, [address_id], function(err, res) {
    if(err)
      return cb(err);
...
/**
 * Stripping the `Address` object off of unwanted content for public consumption
 * @name publicize
 * @function
 * @memberof Address
 * @instance
 * @public
 * @param {Address#address} model - address model to be modified
 * @returns {Address#address} modified address object
 */
