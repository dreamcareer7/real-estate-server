      ...
      if(body) {
        try {
          ...
          if(body[0]) {
            ...
            return Address.updateLatLong(id, location, 'OSM', {formatted_address: ''}, cb);
          } else {
...
Address.updateGeoFromGoogleDirect = function(id, cb) {
  Address.get(id, function(err, address) {
    ...
    request(config.google.url + '?key=' + config.google.api_key + '&address=' + standard, function(err, response, body) {
      if(err)
        return cb(err);
        ...
        try {
          ...
          if(body.status === 'OK') {
            if(body.results[0]) {
              ...
              if(!body.results[0].partial_match || Address.isAccurate(address, body.results[0].formatted_address)) {
                console.log('(Google-Direct) NOTICE: UPDATING LAT/LONG (EXACT-MATCH) for address with id:', id, 'location:', location);
                Address.updateLatLong(id, location, 'Google', {formatted_address: body.results[0].formatted_address}, function(err, address) {
                  if(err)
                    return cb(err);
                ...
                console.log('(Google-Direct) NOTICE: UPDATING LAT/LONG (PARTIAL-MATCH) for address with id:', id, 'location:', location);
                Address.updateLatLongPartial(id, location, 'Google', {formatted_address: body.results[0].formatted_address}, function(err, address) {
                  if(err)
                    return cb(err);
...
Address.updateGeoFromBingDirect = function(id, cb) {
  Address.get(id, function(err, address) {
    ...
    var standard = Address.getGoogleGeocodingSearchStringDirect(address);
    request(config.bing.url + '?q=' + standard + '&key=' + config.bing.api_key, function(err, response, body) {
      if(err)
        return cb(err);
              ...
              if(body.resourceSets[0].resources) {
                if(body.resourceSets[0].resources[0]) {
                  ...
                  if(body.resourceSets[0].resources[0].confidence === 'High') {
                    console.log('(Bing-Direct) NOTICE: UPDATING LAT/LONG (EXACT-MATCH) for address with id:', id, 'location:', location);
                    Address.updateLatLong(id, location, 'Bing', {formatted_address: body.resourceSets[0].resources[0].address.formattedAddress}, function(err, address) {
                      if(err)
                        return cb(err);
                    ...
                    console.log('(Bing-Direct) NOTICE: UPDATING LAT/LONG (PARTIAL-MATCH) for address with id:', id, 'location:', location);
                    Address.updateLatLongPartial(id, location, 'Bing', {formatted_address: body.resourceSets[0].resources[0].address.formattedAddress}, function(err, address) {
                      if(err)
                        return cb(err);
  ...
  Address.get(id, function(err, address) {
    ...
    gm.geocode({address: Address.getGoogleGeocodingSearchString(address)}, function(err, data) {
      ...
      if (data.status === 'OK') {
        ...
        Address.updateLatLong(id, location, 'Google', {formatted_address: data.results[0].formatted_address}, function(err, res) {
          if(err)
            return cb(err);
...
Address.updateLatLong = function(id, location, source, aux, cb) {
  Address.get(id, function(err, data) {
    if(err)
      return cb(err);
...
Address.updateLatLongPartial = function(id, location, source, aux, cb) {
  Address.get(id, function(err, data) {
    if(err)
      return cb(err);

    db.query(sql_update_latlong_partial, [location.longitude,
                                          location.latitude,
                                          id,
                                          source,
                                          aux.formatted_address],
