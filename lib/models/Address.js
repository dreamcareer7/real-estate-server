...
Address.updateGeoFromGoogleDirect = function(id, cb) {
  Address.get(id, function(err, address) {
    ...
    request(config.google.url + '?key=' + config.google.api_key + '&address=' + qs.escape(standard), function(err, response, body) {
      if(err)
        return cb(err);
...
Address.updateLatLongBing = function(id, location, aux, cb) {
  Address.get(id, function(err, data) {
    ...
    db.query(sql_update_latlong_bing, [location.longitude,
                                       location.latitude,
                                       aux.source,
                                       aux.partial,
                                       aux.formatted_address,
