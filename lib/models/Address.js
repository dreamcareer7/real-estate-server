...
var sql_update_latlong_google = require('../sql/address/update_latlong_google.sql');
var sql_update_latlong_bing = require('../sql/address/update_latlong_bing.sql');
var sql_update_latlong = require('../sql/address/update_latlong.sql');
var sql_mark_as_corrupted = require('../sql/address/mark_as_corrupted.sql');
var sql_mark_as_corrupted_google = require('../sql/address/mark_as_corrupted_google.sql');
var sql_mark_as_corrupted_bing = require('../sql/address/mark_as_corrupted_bing.sql');
...
Address.updateGeoFromGoogleDirect = function(id, cb) {
  Address.get(id, function(err, address) {
    ...
    request(config.google.url + (config.google.use_key ? ('?key=' + config.google.api_key + '&') : ('?')) + 'address=' + qs.escape(standard), function(err, response, body) {
      if(err)
        return cb(err);
        ...
        try {
          ...
          if(body.status === 'OK') {
            if(body.results[0]) {
              ...
              Address.updateLatLongGoogle(id, location, {
              ...
              return Address.markAsCorruptedGoogle(id, cb);
            }
          } else {
            console.log('(Google-Direct) WARNING[2]: Error geocoding an address with id:', id);
            return Address.markAsCorrupteGoogle(id, cb);
          }
        }
        catch (e) {
          console.log('(Google-Direct) WARNING[3]: Error geocoding an address with id:', id, 'with exception:', e);
          return Address.markAsCorruptedGoogle(id, cb);
        }
      } else {
    ...
    request(config.bing.url + '?q=' + qs.escape(standard) + '&key=' + config.bing.api_key, function(err, response, body) {
      ...
      if(body) {
        try {
            ...
            if(body.resourceSets[0]) {
              if(body.resourceSets[0].resources) {
                if(body.resourceSets[0].resources[0]) {
                  ...
                  var confidence = body.resourceSets[0].resources[0].confidence;
                  ...
                  Address.updateLatLongBing(id, location, {
                    formatted_address: formatted_address,
                    source: 'Bing',
                    partial: partial,
                    confidence: confidence,
                  ...
                  return Address.markAsCorruptedBing(id, cb);
                }
              } else {
          ...
          console.log('(Bing-Direct) WARNING[3]: Error geocoding an address with id:', id, 'with exception:', e);
          return Address.markAsCorruptedBing(id, cb);
        }
      } else {
        console.log('(Google-Direct) WARNING[4]: Error geocoding an address with id:', id);
  ...
  Address.get(id, function(err, address) {
    ...
    gm.geocode({address: Address.getGoogleGeocodingSearchString(address)}, function(err, data) {
      ...
      if (data.status === 'OK') {
        ...
        var partial = data.results[0].partial_match; // Not Available
        var confidence = data.results[0].geometry.location_type;
...
Address.updateGeo = function(id, cb) {
  Address.updateGeoFromGoogleDirect(id, function(err, address_id) {
    if(err)
      return cb(err);

    Address.updateGeoFromBingDirect(id, function(err, address_id) {
      if(err)
        return cb(err);

      Address.get(id, function(err, address) {
        var gsfa_google = address.geo_source_formatted_address_google || '';
        var gsfa_bing = address.geo_source_formatted_address_bing || '';
        var gc_google = address.geo_confidence_google || '';
        var gc_bing = address.geo_confidence_bing || '';
        ...
        if(gc_google === 'ROOFTOP')
          ...
          if(gsfa_google.indexOf(address.postal_code) != -1 &&
             gsfa_google.indexOf(address.street_name) != -1) {
            return Address.updateLatLong(id, address.location_google, {source: 'Google', approximate: true}, cb);
          }
          else if(gsfa_bing.indexOf(address.postal_code) != -1 &&
                  gsfa_bing.indexOf(address.street_name) != -1) {
            return Address.updateLatLong(id, address.location_google, {source: 'Bing', approximate: true}, cb);
          }
...
Address.markAsCorrupted = function(id, cb) {
  db.query(sql_mark_as_corrupted, [id], function(err, res) {
    if(err)
      return cb(err);
...
Address.markAsCorruptedGoogle = function(id, cb) {
  db.query(sql_mark_as_corrupted_google, [id], function(err, res) {
    if(err)
      return cb(err);
...
Address.markAsCorruptedBing = function(id, cb) {
  db.query(sql_mark_as_corrupted_bing, [id], function(err, res) {
    if(err)
      return cb(err);
...
Address.updateLatLongGoogle = function(id, location, aux, cb) {
  db.query(sql_update_latlong_google, [
    location.longitude,
    location.latitude,
    aux.partial,
...
Address.updateLatLongBing = function(id, location, aux, cb) {
  db.query(sql_update_latlong_bing, [
    location.longitude,
    location.latitude,
    aux.formatted_address,
    aux.confidence,
