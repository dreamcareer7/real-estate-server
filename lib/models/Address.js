          ...
          if(body.status === 'OK') {
            if(body.results[0]) {
              ...
              if(!body.results[0].partial_match || Address.isAccurate(address, body.results[0].formatted_address)) {
                ...
                Address.updateLatLong(id, location, function(err, address) {
                  if(err)
                    return cb(err);
                ...
                Address.updateLatLongPartial(id, location, function(err, address) {
                  if(err)
                    return cb(err);
...
Address.updateLatLongPartial = function(id, location, cb) {
  Address.get(id, function(err, data) {
    if(err)
      return cb(err);
