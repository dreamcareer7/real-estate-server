      ...
      if(body) {
        try {
          ...
          if(body[0]) {
            ...
            return Address.updateLatLong(id, location, 'OSM', cb);
          } else {
          ...
          if(body.status === 'OK') {
            if(body.results[0]) {
              ...
              if(!body.results[0].partial_match || Address.isAccurate(address, body.results[0].formatted_address)) {
                ...
                Address.updateLatLong(id, location, 'Google', function(err, address) {
                  if(err)
                    return cb(err);
                ...
                Address.updateLatLongPartial(id, location, 'Google', function(err, address) {
                  if(err)
                    return cb(err);
          ...
          if(body.statusCode == 200) {
            if(body.resourceSets[0]) {
              if(body.resourceSets[0].resources) {
                if(body.resourceSets[0].resource[0]) {
                  var location = {};
                  ...
                  if(body.resourceSets[0].resources[0].confidence === 'High') {
                    ...
                    Address.updateLatLong(id, location, 'Bing', function(err, address) {
                      if(err)
                        return cb(err);
                    ...
                    Address.updateLatLongPartial(id, location, 'Bing', function(err, address) {
                      if(err)
                        return cb(err);
  ...
  Address.get(id, function(err, address) {
    ...
    gm.geocode({address: Address.getGoogleGeocodingSearchString(address)}, function(err, data) {
      ...
      if (data.status === 'OK') {
        ...
        Address.updateLatLong(id, location, 'Google', function(err, res) {
          if(err)
            return cb(err);
...
Address.updateLatLong = function(id, location, source, cb) {
  Address.get(id, function(err, data) {
    if(err)
      return cb(err);
...
Address.updateLatLongPartial = function(id, location, source, cb) {
  Address.get(id, function(err, data) {
    if(err)
      return cb(err);
