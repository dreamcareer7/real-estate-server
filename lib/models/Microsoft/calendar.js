...
const { subscribeCalendar, unsubscribeCalendar, updateSubscriptions } = require('./workers/subscriptions/common')

const MicrosoftCalendar = {}


const subscribe = async (calendar) => {
  ...
  await subscribeCalendar(microsoft, calendar)

  return
}

const unsubscribe = async (calendar) => {
  const microsoft = await getClient(calendar.microsoft_credential, 'calendar')

  await MicrosoftCalendar.updateToSync([calendar.id], false)
  await unsubscribeCalendar(microsoft, calendar)
  ...
  for ( const remoteCalendar of remoteCalendars ) {
    ...
    */
    if ( !sameOwnerRemoteCalendarIds.includes(remoteCalendar.id) ) {
      console.log('--- remoteCalendar', remoteCalendar)
      ...
      console.log('--- currentRechatCalRemoteId', currentRechatCalRemoteId)
