...
MicrosoftMessage.getThread = async (microsoft_credential, thread_id) => {
  const messageIds = await db.selectIds('microsoft/message/get_by_thread', microsoft_credential, thread_id)

  if (messageIds.length < 1)
    throw Error.ResourceNotFound('No Any Message!')

  return await MicrosoftMessage.getAll([messageIds], microsoft_credential)
}
...
MicrosoftMessage.create = async (records) => {
  return await db.chunked(records, 17, (chunk, i) => {
    const q = squel
      .insert()
