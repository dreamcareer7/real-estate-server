...
const getClient = async (credential_id) => {
  ...
  if ( !credential.scope.includes(SCOPE_OUTLOOK_READ) && (process.env.NODE_ENV !== 'tests') )
...
const outlookBatchGetMessages = async function (message_ids, microsoft_credential) {
  const google = await getClient(microsoft_credential)

  try {
    return await fetchOutlookBody(google, message_ids)
  } catch (ex) {
