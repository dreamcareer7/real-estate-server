...
const { fetchOutlookBody, generateMMesssageRecord, generateGetByInternetMIdQuery, generateGetByConversationIdQuery } = require('./workers/messages/common')
...
MicrosoftMessage.getRemoteMessageByInternetMessageId = async (credential, internetMessageId) => {
  const url = generateGetByInternetMIdQuery(internetMessageId)

  const { outlookMessage } = await parseMessage(credential, url)
...
MicrosoftMessage.getRemoteMessageByConversationId = async (credential, conversationId, params) => {
  ...
  console.log('getRemoteMessageByConversationId url', url)
