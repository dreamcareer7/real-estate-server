 ...
 */
MicrosoftMessage.getDistinctCredentialByThread = async (thread_keys) => {
 ...
 * @param {UUID[]} ids
 */
MicrosoftMessage.getDistinctCredentialByMessage = async (ids) => {
 ...
 */
MicrosoftMessage.getByInternetMessageIds = async (microsoft_credential, internet_message_ids) => {
  const ids = await db.selectIds('microsoft/message/get_by_internet_message_id', [microsoft_credential, internet_message_ids])
  
  if ( ids.length > 0 ) {
    return await MicrosoftMessage.getAll(ids)
  }
 ...
 * @param {string[]} thread_keys
 */
MicrosoftMessage.getByThreadKeys = async (microsoft_credential, thread_keys) => {
 ...
 * @param {UUID[]} ids
 */
MicrosoftMessage.filterMessageIds = async (microsoft_credential, ids) => {
...
MicrosoftMessage.setCampaigns = async (records) => {
  if (records.length === 0) {
    return []
  }
 ...
 * @param {string[]} thread_keys
 */
MicrosoftMessage.deleteByThreadKeys = async (microsoft_credential, thread_keys) => {
  const ids = await MicrosoftMessage.getByThreadKeys(microsoft_credential, thread_keys)
 ...
 * @param {Array} ids
 */
MicrosoftMessage.batchArchive = async (credential, ids) => {
  const microsoft = await getClient(credential.id, 'outlook')
  const messages  = await MicrosoftMessage.getAll(ids)
