...
MicrosoftMessage.createAndSendMessage = async (params) => {
  const microsoft = await getMicrosoftClient(params.credential)
  Context.log('debug-sendEmail createAndSendMessage 1')

  let draftId = ''
  
  try {
    Context.log('debug-sendEmail createAndSendMessage 2')

    const { email, attachments } = await createEmailObject(params)
    Context.log('debug-sendEmail createAndSendMessage 3')

    const createMessageResult    = await microsoft.createMessage(email)
    draftId = createMessageResult.id
    Context.log('debug-sendEmail createAndSendMessage 4')

    Context.log('debug-sendEmail createAndSendMessage 5, attachments:', attachments)

    for (const attachment of attachments) {
      Context.log('debug-sendEmail createAndSendMessage, inside loop')
      await microsoft.addAttachmentNative(createMessageResult.id, attachment)
    }

    Context.log('debug-sendEmail createAndSendMessage 6, after loop')

    const updateMessageResult = await microsoft.updateMessage(createMessageResult.id, email.message)
    Context.log('debug-sendEmail createAndSendMessage 7, updateMessageResult', updateMessageResult)

    await microsoft.sendMessage(updateMessageResult.id)

    Context.log('debug-sendEmail createAndSendMessage 8')
    ...
    await microsoft.deleteDraft(draftId)
