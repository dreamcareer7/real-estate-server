    ...
    for (const microsoft_credential in by_credential) {
      ...
      for (const message of messages) {
        ...
        if (root) {
          ...
          const refineddAtts = attachments.map(att => {
          ...
          message.is_read     = root.isRead || true
          ...
          message.attachments = refineddAtts
...
MicrosoftMessage.createAndSendMessage = async (params) => {
  ...
  try {
    ...
    // Context.log('debug-sendEmail createAndSendMessage, before updateMessageResult')
    const updateMessageResult = await microsoft.updateMessage(createMessageResult.id, email.message)
    // Context.log('debug-sendEmail createAndSendMessage, after updateMessageResult', updateMessageResult)

    await microsoft.sendMessage(updateMessageResult.id)
    // Context.log('debug-sendEmail createAndSendMessage, after sendMessage')
    ...
    // Context.log('debug-sendEmail outlookSender, createAndSendMessage, ex:', ex)
...
MicrosoftMessage.sendReply = async (params) => {
  ...
  Context.log('debug-sendEmail sendReply internetMessageHeaders', JSON.stringify(params.internetMessageHeaders))
  ...
  Context.log('debug-sendEmail sendReply email', JSON.stringify(email))

  Context.log('debug-sendEmail sendReply createReplyResult', JSON.stringify(createReplyResult))
  ...
  try {
    ...
    Context.log('debug-sendEmail sendReply updateMessageResult', JSON.stringify(updateMessageResult))

    return await microsoft.sendMessage(updateMessageResult.id)
