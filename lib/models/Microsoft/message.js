 ...
 */
MicrosoftMessage.fetchFromMicrosoft = async function (microsoft_credential, message_ids) {
...
MicrosoftMessage.getAll = async (ids) => {
  ...
  if (shouldFetchBody) {
    ...
    for (const microsoft_credential in by_credential) {
      const bodies = await MicrosoftMessage.fetchFromMicrosoft(microsoft_credential, messages.map(m => m.message_id))
