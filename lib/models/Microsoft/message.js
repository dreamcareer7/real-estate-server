 ...
 */
const outlookBatchGetMessages = async function (microsoft_credential, message_ids) {
...
MicrosoftMessage.getAll = async (ids) => {
  ...
  if (shouldFetchBody) {
    ...
    for (const microsoft_credential in by_credential) {
      const bodies = await outlookBatchGetMessages(microsoft_credential, messages.map(m => m.message_id))
...
MicrosoftMessage.deleteByRemoteMessageIds = async (microsoft_credential, remote_message_ids) => {
  const threads = await db.map('microsoft/message/delete_by_remote_message_id', [
    microsoft_credential,
    remote_message_ids
  ], 'thread_key')
...
MicrosoftMessage.updateIsRead = async (id, status) => {
  const result = await db.update('microsoft/message/update_is_read', [id, status])

  await EmailThread.update([result[0].thread_key], 'microsoft')
}
