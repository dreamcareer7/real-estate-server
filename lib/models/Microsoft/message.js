...
const getMicrosoftClient = async (credential) => {
  ...
  if (credential.deleted_at)
    throw Error.BadRequest('Microsoft-Credential is releted!')
  ...
  if (message.has_attachments) {
    for (const attach of message.attachments) {
      ...
      const hash = Crypto.encrypt(JSON.stringify({
        credential_id: message.microsoft_credential,
      ...
      attach.link = `${process.env.API_HOSTNAME}/emails/microsoft/attachments/${hash}` 
    }
  }
...
MicrosoftMessage.downloadAttachment = async (mcid, mid, aid) => {
  ...
  // Validate message
  await MicrosoftMessage.getByMessageId(mid, credential.id)
