 ...
 * @param {UUID[]} ids ids
 */
MicrosoftMessage.deleteByIds = async (ids, microsoft_credential) => {
  const threads = await db.map('microsoft/message/delete_by_ids', [ids], 'thread_key')
...
MicrosoftMessage.batchTrash = async (credential, ids) => {
  ...
  return await MicrosoftMessage.deleteByIds(ids, credential.id)
}
 ...
 * @param {String?} next
 */
MicrosoftMessage.searchInThreads = async (mcid, query, next) => {
  const microsoft = await getClient(mcid, 'outlook')
