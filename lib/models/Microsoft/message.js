...
const handleBigAttachments = async function (microsoft, messageId, attachments) {
  for await (const att of attachments) {
    ...
    try {
      const resource = await microsoft.uploadSession(messageId, attachment)
      Context.log('--- outlook-attachment-big-upload-success', resource)
    } catch (ex) {
      Context.log('--- outlook-attachment-big-upload-failed', ex.statusCode, ex.message, ex.error)
    }
  }
}
...
const createEmailObject = async function (params) {
  ...
  const limit_alt = config.microsoft_integrationattachment_size_limit_alt // 3MB
  ...
  Context.log('--- outlook-attachment attachments', attachments)
  Context.log('--- outlook-attachment bigAttachments', bigAttachments)
