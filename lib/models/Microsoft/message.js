 ...
 * @param {UUID} id
 */
MicrosoftMessage.get = async (message_id) => {
  const messages = await MicrosoftMessage.getAll([message_id])
...
MicrosoftMessage.getByMessageId = async (message_id, microsoft_credential) => {
  const id = await db.selectId('microsoft/message/find_by_message_id', [message_id, microsoft_credential])

  return MicrosoftMessage.get(id)
}
...
/**
 * @param {UUID} mcid Google credential id
...
MicrosoftMessage.downloadAttachment = async (mcid, mid, aid) => {
  ...
  const microsoft        = await getMicrosoftClient(credential)
  const microsoftMessage = await MicrosoftMessage.getByMessageId(mid, credential.id)
...
MicrosoftMessage.getRemoteMessageByConversationId = async (credential, conversationId, params) => {
  ...
  if ( params.credential.email !== message.from.emailAddress.address )
    return null
