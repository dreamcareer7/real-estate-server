...
MicrosoftContact.getAll = async (remote_ids, microsoft_credential) => {
  const contacts = await db.select('microsoft/contact/get', [remote_ids, microsoft_credential])
...
MicrosoftContact.getAllBySource = async (remote_ids, microsoft_credential, source) => {
  const contacts = await db.select('microsoft/contact/get_by_source', [remote_ids, microsoft_credential, source])

  return contacts
}

MicrosoftContact.getMCredentialContactsAddress = async (credentialId) => {
  const contacts = await db.select('microsoft/contact/get_by_credential', [credentialId])
  const emails   = new Set()

  if ( contacts.length > 0 ) {
    for (const contact of contacts) {
      for (const address of contact.emailAddresses) {
...
MicrosoftContact.getMCredentialContactsNum = async (microsoft_credential) => {
