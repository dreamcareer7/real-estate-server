 ...
 * @param {UUID} brand
 */
MicrosoftCredential.getByBrand = async (cid, brand) => {
  ...
  const credentials = await MicrosoftCredential.getAll(ids)
...
MicrosoftCredential.forceSyncCalendar = async (id) => {
  ...
  const job = await UsersJob.getByMicrosoftCredential(credential.id, 'calendar')
