...
MicrosoftCredential.getAll = async (ids) => {
  ...
  for (const credential of credentials) {
    credential.access_token  = Crypto.decrypt(credential.access_token)
    credential.refresh_token = Crypto.decrypt(credential.refresh_token)
  }
...
MicrosoftCredential.create = async (body) => {
  return db.insert('microsoft/credential/insert',[
    ...
    Crypto.encrypt(body.tokens.access_token),
    Crypto.encrypt(body.tokens.refresh_token),
...
MicrosoftCredential.updateRechatMicrosoftCalendar = async (id, rechatMicrosoftCalendar) => {
  return db.update('microsoft/credential/update_rechat_mcalendar', [
    id,
    rechatMicrosoftCalendar
  ])
}

MicrosoftCredential.updateCalendarsLastSyncAt = async (id) => {
  return db.update('microsoft/credential/update_calendars_last_sync_at', [id])
}
