...
MicrosoftCalendarEvent.batchInsert = async (credential, calendar, events) => {
  ...
  if (failed.length > 0) {
    const text = 'SyncMicrosoftCalendar Batch Insert Failed'
    Context.log(text, failed.length, JSON.stringify(failed), JSON.stringify(events))
...
MicrosoftCalendarEvent.batchUpdate = async (credential, events) => {
  ...
  try {
    ...
    if (failed.length > 0) {
      const text = 'SyncMicrosoftCalendar Batch Update Failed'
      Context.log(text, failed.length, JSON.stringify(failed), JSON.stringify(events))
      Slack.send({ channel: 'integration_logs', text, emoji: ':skull:' })
    }
    ...
    if (failedIntegrations.length > 0) {
      const text = 'SyncMicrosoftCalendar Batch Update Failed - Integrations'
...
MicrosoftCalendarEvent.batchDelete = async (credential, events) => {
  ...
  if (failed.length > 0) {
    const text = 'SyncMicrosoftCalendar Batch Delete Failed'
    Context.log(text, failed.length, JSON.stringify(failed), JSON.stringify(events))
    Slack.send({ channel: 'integration_logs', text, emoji: ':skull:' })
  }
