...
const createRemote = async function (microsoftCalendar, resource) {
  const microsoft = await getClient(microsoftCalendar.microsoft_credential, 'calendar')
...
MicrosoftCalendarEvent.updateLocal = async function (id, updatedEvent) {
  return await db.selectIds('microsoft/calendar_events/update', [
    ...
    JSON.stringify(updatedEvent.event_start || updatedEvent.start) || null,
    JSON.stringify(updatedEvent.event_end || updatedEvent.end) || null,
