...
MicrosoftCalendarEvent.batchInsert = async (credential, calendar, events) => {
  ...
  if (failed.length > 0) {
    Context.log('SyncMicrosoftCalendar batchInsert failed', failed.length, JSON.stringify(failed))
  }
...
MicrosoftCalendarEvent.batchUpdate = async (credential, events) => {
  ...
  try {
    ...
    if (failed.length > 0) {
      Context.log('SyncMicrosoftCalendar batchUpdate failed', failed.length, JSON.stringify(failed))
    }
    ...
    if (failedIntegrations.length > 0) {
      Context.log('SyncMicrosoftCalendar batchUpdate failedIntegrations', failedIntegrations.length, JSON.stringify(failedIntegrations))
    }
...
MicrosoftCalendarEvent.batchDelete = async (credential, events) => {
  ...
  if (failed.length > 0) {
    Context.log('SyncMicrosoftCalendar batchDelete failed', failed.length, JSON.stringify(failed))
  }
