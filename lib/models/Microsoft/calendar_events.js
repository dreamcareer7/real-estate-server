...
MicrosoftCalendarEvent.batchInsert = async (credential, calendar, events) => {
  ...
  const result    = await microsoft.batchInsertEvent(events)

  const confirmed = result.responses.filter(record => ( record.status === 200 || record.status === 201 )).map(record => record.body)
  const toRetry   = result.responses.filter(record => ( record.status === 429 ))
  const failed    = result.responses.filter(record => ( record.status !== 200 && record.status !== 201 && record.status !== 429 ))

  console.log('---- confirmed', confirmed.length)
  console.log('---- toRetry', toRetry.length)
  console.log('---- failed', failed.length)

  const newEvents = confirmed.map(c => generateCalendarEvent(calendar, c))
