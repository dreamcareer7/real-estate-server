...
const helper = async function (microsoftCalendar) {
  const credential = await MicrosoftCredential.get(microsoftCalendar.google_credential)
...
const createRemote = async function (microsoftCalendar, resource) {
  const { microsoft } = await helper(microsoftCalendar)
