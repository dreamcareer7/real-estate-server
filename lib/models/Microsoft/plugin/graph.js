...
MGraph.prototype.uploadSession_old = async function(messageId) {
  const client = await this.getClient()
...
MGraph.prototype.uploadSession = async function(messageId, attachment) {
  ...
  const file = "./sample.zip"

  const options = {
    method: 'POST',
    url: `https://graph.microsoft.com/beta/me/messages/${messageId}/attachments/createUploadSession`,
    json: true,
    headers: {
      'Authorization': `Bearer ${this.tokens.access_token}`,
      'Content-Type': 'application/json' 
    },
    body: '{"item": {"@microsoft.graph.conflictBehavior": "rename", "name": "' + attachment.name + '"}}'
  }
  
  const response_1 = await request.post(options)
  console.log('---- response_1', response_1)

  // uploadFile(JSON.parse(response_1).uploadUrl)
}
