...
MGraph.prototype.batchInsertContacts = async function (chunks) {
  ...
  chunks = chunks.map((chunk, index) => ({ ...chunk, requestId: index + '' }))
  ...
  for (let chunk = chunks;;) {
    ...
    _(responses)
      ...
      .reject(r => r.status === 429 && toRetry.push(r.id + ''))
    ...
    retryAfter && await new Promise(r => setTimeout(r, retryAfter * 1000))
  ...
  const bodyBuilder = function(contacts) {
    const requests = contacts.map(contact => {
      return {
        ...
        // 'url': `/me/contactFolders/${contact.parentFolderId}/contacts/${contact.contactId}`,
        'body': contact.resource,
        'headers': {
          'Content-Type': 'application/json'
        }
