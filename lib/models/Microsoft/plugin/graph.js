...
MGraph.prototype.batchUpdateContacts = async function (chunks) {
  ...
  /**
   * Returns a suitable update URI for the `item`
   * @param {{ parentFolderId?: string?, id: string }} item
   * @returns {string}
   */
  const updateUriFor = item => item.parentFolderId
    ? `/me/contactFolders/${item.parentFolderId}/contacts/${item.id}`
    : `/me/contacts/${item.id}`
...
MGraph.prototype.batchGetContacts = async function (chunks) {
  /**
   * Returns suitable GET URI for the `item`
   * @param {{ parentFolderId?: string?, id: string }} item
   * @returns {string}
   */
  const getUriFor = item => item.parentFolderId
    ? `/me/contactFolders/${item.parentFolderId}/contacts/${item.id}`
    : `/me/contacts/${item.id}`

  /**
   * Converts the request `item` to a valid request object
   * @param {{ parentFolderId?: string?, id: string, requestId?: string? }}
   * @returns {object}
   */
...
MGraph.prototype.batchDeleteContacts = async function (chunks) {
  ...
  const deleteUriFor = c => c.parentFolderId
    ? `/me/contactFolders/${c.parentFolderId}/contacts/${c.id}`
    : `/me/contacts/${c.id}`
