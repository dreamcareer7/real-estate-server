...
MGraph.prototype.config = function(readOnly = true) {
  this.scope = SCOPES_READ_ONLY.contacts_and_messages
...
MGraph.prototype.discreteGeMessagessNative = async function* (filter, select) {
  let url  = `https://graph.microsoft.com/v1.0/me/messages?$top=10${filter}${select}`
  let next = null
...
// https://graph.microsoft.com/v1.0/me/messages/message-id/attachments?$select=id,lastModifiedDateTime,name,contentType,size,isInline


// https://graph.microsoft.com/v1.0/me/messages?$select=id,subject,hasAttachments&$expand=attachments($select=id,lastModifiedDateTime,name,contentType,size,isInline)