...
MGraph.prototype.listEvents = async function (calendarId) {
  ...
  return await client.api(`/me/calendars/${calendarId}/events`)
...
MGraph.prototype.batchInsertEvent = async function (events) {
  ...
  do {
    ...
    if (toRetry.length) {
      ...
      chunk = events.filter(record => { if (toRetry.includes(record.requestId) || toRetry.includes(String(record.requestId))) return record })
    }
...
MGraph.prototype.batchGetEvents = async function (events) {
  let confirmed = []
  let toRetry   = []
  let failed    = []
  ...
  do {
    ...
    if (toRetry.length) {
      ...
      chunk = events.filter(record => { if (toRetry.includes(record.id) || toRetry.includes(String(record.id))) return record })
    }
