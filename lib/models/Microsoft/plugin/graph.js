...
MGraph.prototype.geMessagessNative = async function() {
  let url      = 'https://graph.microsoft.com/v1.0/me/messages?$top=100'
  let messages = []
  let next     = null
...
MGraph.prototype.geMessagess= async function() {
  const client = await this.getClient()

  let url      = '/me/messages?$top=10'
  let messages = []
  let next     = null
...
MGraph.prototype.discreteGeMessagessNative = async function* (lastSyncDate) {
  const filter = lastSyncDate ? (`&$filter=createdDateTime ge ${new Date(lastSyncDate).toISOString()}`) : ''

  let url  = `https://graph.microsoft.com/v1.0/me/messages?$top=10${filter}`
  let next = null

  console.log('first shoot', url)
  ...
  do {
    ...
    console.log('in loop', next, data.value.length)
...
MGraph.prototype.discreteGeMessagess = async function* (lastSyncDate) {
  const client = await this.getClient()

  const filter = lastSyncDate ? (`&$filter=createdDateTime ge ${new Date(lastSyncDate).toISOString()}`) : ''

  let url  = `/me/messages?$top=10${filter}`
  let next = null
  ...
  do {
    ...
    console.log('in loop', next, res.value.length)
