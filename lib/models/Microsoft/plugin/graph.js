...
/** @typedef {import('../contact/defs.d').Microsoft.Contact} MicrosoftContact */
...
const MicrosoftGraph = {
  ...
  redirect_uri: CREDENTIALS.redirect_to_uri,
  ...
  setMicrosoftEmailAddress(email) {
    this.email = email
  },
  
  getAuthenticationLink(state, scopes) {
  ...
  async deltaWithImmutableId(url, timeZone = 'UTC') {
    let values = []
    let delta  = null
    let next   = null
  ...
  async batchRequestWithImmutableId(body) {
    const payload = [...body.requests]
   ...
   * @returns {Promise<Confirmed|Failed>}
   */
  async batchInsertContacts(chunks) {
   ...
   * @returns {Promise<Confirmed|Failed>}
   */
  async batchUpdateContacts(chunks) {
    chunks = ensureRequestIds(chunks)
   ...
   * @returns {Promise<Failed>}
   */
  async batchDeleteContacts(chunks) {
    // @ts-ignore
    chunks = ensureRequestIds(chunks)
   ...
   */
  async syncContacts(folders, projection) {
  ...
  async getContactsNative(lastSyncDate, folders, projection) {
  ...
  async getContacts(lastSyncDate, folders, projection) {
    const client = await this.getClient()
    ...
    for (let url of urls) {
      ...
      do {
        ...
        if ( loopCounter++ > 1 ) {
          ...
          res = await client.api(url).select(select).header('Prefer', 'IdType="ImmutableId"').get()
        }
        Metric.increment('microsoft.api.contacts')
  ...
  async *discreteGetMessagessNative(url) {
    let next = null
  ...
  async batchGetMessagesNative(messageIds, select, expand) {
    let counter = 1
  ...
  async getAttachmentNative(messageId, attachmentId) {
    const options = {
      method: 'GET',
  ...
  async sendMultipartMessage(email) {
  ...
  async updateMessageNative(messageId, message) {
    const options = {
      method: 'POST',
  ...
  async updateMessageExtensionsNative(messageId, data) {
    const options = {
      method: 'POST',
  ...
  async updateIsRead(messageIds, status) {
    let counter = 1
  ...
  async addAttachmentNative(messageId, attachment) {
    const options = {
      method: 'POST',
  ...
  async searchThreads(limit, query, next) {
  ...
  async uploadSession(messageId, attachment) {
    const client    = await this.getClient()
  ...
  async createSubscription(parmas) {
    const client = await this.getClient()
  ...
  async updateSubscription(id, parmas) {
    const client = await this.getClient()
  ...
  async batchDeleteSubscription(subIds) {
    let counter = 1
  ...
  // Calendar Events
  /**
 * @deprecated Since not used in project any more
 */
  async syncEvents(calendarId, timeMin, currentSyncToken = null) {
    const client = await this.getClient()
  ...
  async batchDeleteEvents(chunks) {
    let retryAfterArr = []
    let toRetry = []
    let failed  = []
  ...
  async getEventInstances(calendarId, eventId, startDateTime, endDateTime) {
    const client = await this.getClient()
  ...
  async updateIdToImmutableId(inputIds) {
...
module.exports.MicrosoftGraph = MicrosoftGraph

module.exports.api = function(scopes) {
  const mClient =  MicrosoftGraph
  return mClient
}
