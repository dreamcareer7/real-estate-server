...
MGraph.prototype.getClient = async function() {
  ...
  Context.log('SyncOutlookMessages - graph - getClient', JSON.stringify(this.tokens))
...
MGraph.prototype.getSubscription = async function(id) {
  Context.log('SyncOutlookMessages - graph - getSubscription', JSON.stringify(id))
  const client = await this.getClient()
  ...
  try {
    ...
    Context.log('SyncOutlookMessages - graph - getSubscription response', JSON.stringify(response))
    return response

  } catch (ex) {
    Context.log('SyncOutlookMessages - graph - getSubscription catch', JSON.stringify(ex))
    if ( ex.statusCode === 404 )
      return null
