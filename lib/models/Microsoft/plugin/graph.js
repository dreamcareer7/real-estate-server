...
MGraph.prototype.geMessagessNative = async function() {
  // let url = 'https://graph.microsoft.com/v1.0/me/mailFolders(\'Inbox\')/messages?$top=100'
  let url = `https://graph.microsoft.com/v1.0/me/messages?$top=2${filter}`
...
MGraph.prototype.discreteGeMessagessNative = async function* (lastSyncDate) {
  ...
  // let url  = `https://graph.microsoft.com/v1.0/me/mailFolders(\'Inbox\')/messages?$top=2${filter}`
  let url  = `https://graph.microsoft.com/v1.0/me/messages?$top=2${filter}`
  let next = null
...
MGraph.prototype.discreteSyncSentBox = async function* (lastSyncDate) {
  const filter = lastSyncDate ? (`&$filter=createdDateTime ge ${new Date(lastSyncDate).toISOString()}`) : ''

  let url  = `https://graph.microsoft.com/v1.0/me/mailFolders(\'SentItems\')/messages?$top=2${filter}`
  let next = null
