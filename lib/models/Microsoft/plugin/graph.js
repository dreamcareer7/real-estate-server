...
MGraph.prototype.geMessageByUrl = async function(url) {
  const responseString = await request.get({
    url: url,
    headers: { Authorization: `Bearer ${this.tokens.access_token}` }
  })
...
MGraph.prototype.discreteGeMessagessNative = async function* (url) {
  let next = null
...
MGraph.prototype.batchGetMessagesNative = async function (messageIds) {
  ...
  const requests = messageIds.map(message => ({
    'id': counter ++,
    'method': 'GET',
    'url': `/me/messages/${message}?&$select=bodyPreview,uniqueBody,body,isRead`
  }))
