...
const contactUri = (
  ...
  if (withExtensions) {
    const extId = config.microsoft_integration.openExtension.contact.name
    uri += `?$expand=extensions($filter=id eq '${extId}')`
  }
 ...
 * @param {object} $1 - options
 * @param {boolean} [$1.withExtensions=true] - Expand extensions, too?
 * @returns {Confirmed & Failed}
 */
