...
MGraph.prototype.uploadSession = async function(messageId, attachment) {
  ...
  const address = '/home/saeed/Downloads/MellatMobileBank_Android.apk' // '/home/saeed/Pictures/Zomorod.jpg'
  const stats   = fs.statSync(address) // filesize(stats.size, {round: 0})
  
  const size  = stats['size']
  const name  = 'MellatMobileBank_Android.apk' // 'Zomorodx.jpg'
  const attachmentType = 'file'
  const uploadSession  = { AttachmentItem: { attachmentType, name, size } }
  ...
  try {
    ...
    if (ex.response) console.log('ex.response.headers:', ex.response.headers)
