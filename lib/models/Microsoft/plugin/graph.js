...
MGraph.prototype.geMessagessNative = async function() {
  // `https://graph.microsoft.com/v1.0/me/mailFolders(\'Inbox\')/messages?$top=2${filter}`
  // `https://graph.microsoft.com/v1.0/me/mailFolders(\'SentItems\')/messages?$top=2${filter}`
...
MGraph.prototype.discreteGeMessagessNative = async function* (filter, select) {
  let url  = `https://graph.microsoft.com/v1.0/me/messages?$top=2${filter}${select}`
  let next = null
