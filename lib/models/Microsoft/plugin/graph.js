...
MGraph.prototype.syncContactFolders = async function(token) {
  ...
  let url     = token || '/me/contactFolders/delta'
  let folders = []
  let value   = []
  let next    = null
  ...
  do {
    ...
    value   = res.value
    folders = folders.concat(value)
    next    = res['@odata.deltaLink'] || null
    url     = next
...
/** 
 * @typedef {{folderId: string?, syncToken: string}} FolderSyncToken
 * @param {string[]} folders
 * @param {string[]} projection
 * @returns {Promise<{contacts: any[], next?: string?, syncTokens: FolderSyncToken[]}>}
 */
MGraph.prototype.syncContacts = async function(folders, projection) {
  ...
  // url ==> folder path
  for (const folderId of [null, ...folders]) {
    let url = folderId === null
      ? '/me/contacts/delta'
      : `/me/contactfolders/${folderId}/contacts/delta`
    
    do {
      const res = await client.api(url).select(projection.join(',')).get()

      value    = res.value
      contacts = contacts.concat(value)
      next     = res['@odata.deltaLink'] || null
      url      = next
  
    } while ( value.length !== 0 )

    syncTokens.push({ folderId, syncToken: next })
  }
