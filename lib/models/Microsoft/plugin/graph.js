 ...
 * @param {*} options 
 */
MGraph.prototype.retriableRequest = async function(options) {
  let tries = 0
  const retry = async () => {
    try {
      tries += 1
...
// Calendar
MGraph.prototype.listCalendars = async function(select = undefined) {
  const client = await this.getClient()

  try {
    const req = client.api('/me/calendars')
    if (Array.isArray(select) || typeof select === 'string') {
      req.select(select)
    }
...
MGraph.prototype.listEventsForThisWeek = async function (calendarId, select) {
  const moment = require('moment')
  const starts_gt = moment().startOf('week')
  const starts_lt = moment().startOf('week').add(1, 'week')
...
MGraph.prototype.listEvents = async function (calendarId, filters, select) {
  ...
  let req = client.api(`/me/calendars/${calendarId}/events`)
  if (filters) {
