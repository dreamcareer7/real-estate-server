...
const getMGraphClient = async (credential) => {
  try {
    ...
    if ( diff < 300000 ) {
      ...
      const text = `MTokens Refreshed, cid: ${credential.id}, expiredAt: ${credential.expires_in}, expiresAt: ${newTokens.expires_in}`
      ...
      const text = `MTokens Valid, cid: ${credential.id}, expiresAt: ${credential.expires_in}`
      Slack.send({ channel: 'integration_logs', text, emoji: ':bell:' })
