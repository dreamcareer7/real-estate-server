...
const extractPhoto = async (microsft, userId, brand, contact) => {
  ...
  try {
    const { imageData } = await microsft.getContactPhoto(contact.id)
...
const findNewAttributes = async (contact, oldAttributes) => {
  ...
  if ( rawData.givenName !== oldRawData.givenName ) {
    const firstNameAtt = refinedAtts.first_name.length ? refinedAtts.first_name[0].text : null
  ...
  if ( rawData.surname !== oldRawData.surname ) {
    const lastNameAtt = refinedAtts.last_name.length ? refinedAtts.last_name[0].text : null
  ...
  if ( rawData.middleName !== oldRawData.middleName ) {
    const middleNameAtt = refinedAtts.middle_name.length ? refinedAtts.middle_name[0].text : null
  ...
  if ( rawData.nickName !== oldRawData.nickName ) {
    const nickNameAtt = refinedAtts.nickname.length ? refinedAtts.nickname[0].text : null
  ...
  if ( rawData.title !== oldRawData.title ) {
    const titleAtt = refinedAtts.title.length ? refinedAtts.title[0].text : null
  ...
  if ( rawData.jobTitle !== oldRawData.jobTitle ) {
    const jobTitleAtt = refinedAtts.job_title.length ? refinedAtts.job_title[0].text : null
  ...
  if ( rawData.companyName !== oldRawData.companyName ) {
    const companyAtt = refinedAtts.company.length ? refinedAtts.company[0].text : null
  ...
  if ( rawData.birthday !== oldRawData.birthday ) {
    const birthdayAtt = refinedAtts.birthday.length ? refinedAtts.birthday[0].date : null
  ...
  if ( rawData.businessHomePage !== oldRawData.businessHomePage ) {
    const websiteeAtt = refinedAtts.website.length ? refinedAtts.website[0].text : null
  ...
  const oldPhoneNumbers = refinedAtts.phone_number.map(entry => entry.text)
  ...
  const map_1       = oldRawData.emailAddresses.map(entry => entry.address)
  const map_2       = rawData.emailAddresses.map(entry => entry.address)
  ...
  try {
    ...
    if (contacts.length) {
      ...
      if ( toUpdateContactIds.length ) {
        ...
        const refinedContactsAtts = _.groupBy(contactsAtts, function(entry) { return entry.contact})
  
        for (var key in refinedContactsAtts) {
          const newAttributes = await findNewAttributes(contactsMap[key], refinedContactsAtts[key])
