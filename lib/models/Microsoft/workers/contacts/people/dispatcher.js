...
async function handleCreated (microsoft, credential, contactIds) {
  ...
  const createdGContacts   = await MicrosoftContact.create(newGContacts)
  const integrationRecords = createdGContacts.map(mcontact => {
    return {
      google_id: null,
...
async function handleUpdated (microsoft, credential, contactIds) {
  ...
  const updatedGContacts   = await MicrosoftContact.update(toUpdateGContacts)
  const toUpdateIntRecords = updatedGContacts.map(umc => {
    return {
      google_id: null,
      microsoft_id: umc.id,
      contact: umc.contact,
