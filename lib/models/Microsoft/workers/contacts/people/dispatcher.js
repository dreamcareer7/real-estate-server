...
const { refineConnections, refineContacts }  = require('./helpers/refine')
...
async function handleUpdated (microsoft, credential, contactIds) {
  ...
  const refined  = records.map(record => {
    record.resource.etag = mContactsByContact[record.contact].etag

    return {
      resourceId: mContactsByContact[record.contact].resource_id,
      ...record
    }
  })
