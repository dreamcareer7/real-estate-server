const { groupBy, isEqual, difference, intersection } = require('lodash')
...
const findNewAttributes = (data, oldData, oldAttributes) => {
  ...
  const oldEmails   = refinedAtts?.email?.map?.(e => e.text) ?? []
  const map_1       = oldData?.emailAddresses?.map?.(e => e.address) ?? []
  const map_2       = data.emailAddresses?.map?.(e => e.address) ?? []
