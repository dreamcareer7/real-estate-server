const { groupBy, difference, chain, filter, isNil, map } = require('lodash')
...
const generateMContacts = (credential, contacts) => {
  return contacts.map(contact => {
    /**
     * A mapping object represents attributes groupbed by attribute-type. The
     * attributes are ordered by `is_primary` and `updated_at` in descending 
     * order.
     */
    const attrs = chain(contact.attributes)
      .orderBy(['is_primary', 'updated_at'], ['desc', 'desc'])
      .groupBy('attribute_type')
      .value()
    
    /**
     * Selects an a Microsoft-address from existing addresses
     * @param {string} label
     * @returns {MsAddress | null}
     */
    const chooseAddress = label => chain(contact.attributes)
      .filter(
        { label },
        attr => ADDRESS_RELATED_ATTR_TYPES.includes(attr.attribute_type)
      )
      .tap(addrs => {
        const index = addrs?.[0]?.index
        return isNil(index) ? [] : filter(addrs, { index })
      })
    ...
    const spouseName = filter(attrs.first_name, 'is_partner')?.[0]?.text
    const emailAddresses = map(attrs.email, toMsEmail)
    const birthday = singular('birthday', 'date')
    ...
    return {
      ...
      resource: {
        ...
        ...maybe('givenName', singular('first_name')),
        ...
        /* Misc.: */
        ...maybe('birthday', new Date(birthday).toISOString()),
