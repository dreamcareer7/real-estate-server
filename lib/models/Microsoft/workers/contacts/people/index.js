...
const processDeleted = async (credential, deleted, { microsoft, folderIds } = { microsoft: null, folderIds: [] } ) => {
  ...
  Context.log('syncContacts [Microsoft To Rechat] - processDeleted folderIds:', JSON.stringify(folderIds))
  if(microsoft) {
    // @ts-ignore
    const batchResponse = await microsoft.batchGetContacts(deleted)
    Context.log('syncContacts [Microsoft To Rechat] - deleted', JSON.stringify(deleted))
    Context.log('syncContacts [Microsoft To Rechat] - batchResponse', JSON.stringify(batchResponse))
...
const processConfirmed = async (credential, contacts) => {
  Context.log('syncContacts [Microsoft To Rechat] - processConfirmed:', JSON.stringify(contacts))
  ...
  */
  ...
  for (const mcontact of updatedMicrosoftContacts) {
    ...
    // TODO(Amin): Remove below log
    Context.log('syncContacts [Microsoft To Rechat] - findNewAttributes:', JSON.stringify({data, oldData, refinedAtts: refinedAtts[id], contactFolders}))
    const { attributes, deletedAtt } = findNewAttributes(data, oldData, refinedAtts[id], contactFolders)
    // TODO(Amin): Remove below log
    Context.log('syncContacts [Microsoft To Rechat] - after findNewAttributes:', JSON.stringify({ attributes, deletedAtt }))
  ...
  // TODO(Amin): Remove below logs
  Context.log('syncContacts [Microsoft To Rechat] - toDeleteAttributeIds:', JSON.stringify(toDeleteAttributeIds))
  Context.log('syncContacts [Microsoft To Rechat] - toUpdateContacts:', JSON.stringify(toUpdateContacts))
...
const microsoftToRechat = async (microsoft, credential) => {
  try {
    ...
    Context.log('syncContacts [Microsoft To Rechat] - Folders', JSON.stringify(folders))
    ...
    Context.log('syncContacts [Microsoft To Rechat] - contacts', JSON.stringify(contacts))
