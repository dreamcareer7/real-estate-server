...
const processDeleted = async (microsoft, credential, deleted) => {
  if ( deleted.length === 0 ) {
    return 0
  }

  const batchResponse = await microsoft.batchGetContacts(deleted);
  const availableIds = batchResponse.confirmed.map(res => res.body?.id ? res.body?.id : null ).filter(Boolean)
...
const microsoftToRechat = async (microsoft, credential) => {
  try {
    ...
    let deleted   = contacts.filter(f => f['@removed'])
    const confirmed = contacts.filter(f => !f['@removed'])
    const confirmedIds = confirmed.map(x => x.id)
    deleted = deleted.filter(x => !confirmedIds.includes(x.id))

    const deletedNum  = await processDeleted(microsoft, credential, deleted)
