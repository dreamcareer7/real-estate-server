...
const microsoftToRechat = async (microsoft, credential) => {
  ...
  try {
    ...
    const { contacts, syncTokens, next } = await microsoft.syncContacts(folders, projection)
    ...
    const oldRemoteContacts   = contacts.filter(c => oldMContactRemoteIds.includes(c.id) && oldMContactByRemoteId[c.remote_id] && (oldMContactByRemoteId[c.remote_id].etag !== c.etag))
    ...
    */
    ...
    const records = syncTokens
      .filterr(r => r.folderId)
      .map(r => ({ microsoft_credential: credentialId, folder_id: r.folderId, sync_token: r.syncToken }))

    await MicrosoftContact.bulkUpdateFolderSyncTokens(records)
    await MicrosoftCredential.updateContactsSyncToken(credentialId, next)
