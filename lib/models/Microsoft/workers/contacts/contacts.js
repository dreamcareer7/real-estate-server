...
const syncContacts = async (microsoft, credential, lastSyncAt) => {
  ...
  try {
    ...
    Context.log('SyncMicrosoft - syncContacts', credential.email)
    ...
    if (contacts.length) {
      ...
      if ( toUpdateContactIds.length ) {
        ...
        const refinedAtts  = _.groupBy(contactsAtts.filter(att => !att.deleted_at), function(entry) { return entry.contact})
