...
const syncContacts = async (microsoft, credential, lastSyncAt) => {
  ...
  try {
    ...
    if (contacts.length) {
      ...
      const refinedAtts  = groupBy(contactsAtts, function(entry) { return entry.contact}) // contactsAtts.filter(att => (!att.deleted_at && att.created_for === 'microsoft_integration'))
