...
const syncContacts = async (microsoft, credential, lastSyncAt) => {
  ...
  try {
    ...
    Context.log('SyncMicrosoftContacts - [Debug]-2', credential.id, contacts)
    ...
    if (contacts.length) {
      ...
      Context.log('SyncMicrosoftContacts - [Debug]-oldMicrosoftContactRemoteIds', credential.id, oldMicrosoftContactRemoteIds)
      ...
      for (const contact of contacts) {
        if ( oldMicrosoftContactRemoteIds.includes(contact.id) ) {
          ...
          if ( !oldMContact.deleted_at ) {
            ...
            Context.log('SyncMicrosoftContacts - [Debug] here')

            const result = await Contact.fastFilter(brand, [], { microsoft_id: oldMContact.id })

            Context.log('SyncMicrosoftContacts - [Debug] result', result)
      ...
      if ( toUpdateContactIds.length ) {
        ...
        const refinedAtts  = _.groupBy(contactsAtts.filter(att => (!att.deleted_at && att.created_for === 'microsoft_integration')), function(entry) { return entry.contact})
      ...
      Context.log('SyncMicrosoftContacts - [Debug]-3', credential.id, toUpdateRecords)
      Context.log('SyncMicrosoftContacts - [Debug]-4', credential.id, toUpdateContacts)
      Context.log('SyncMicrosoftContacts - [Debug]-5', credential.id, newContacts)
