...
const syncContacts = async (microsoft, credential, lastSyncAt) => {
  ...
  try {
    ...
    if (contacts.length) {
      ...
      // console.log('--- remoteIdsArr', remoteIdsArr, '\n')
      // console.log('--- oldMicrosoftContacts', oldMicrosoftContacts, '\n')
      // console.log('--- oldMContactRemoteIds', oldMContactRemoteIds, '\n')
      // console.log('--- oldMContactByRemoteId', oldMContactByRemoteId, '\n')

      const newRemoteContacts   = contacts.filter(c => !oldMContactRemoteIds.includes(c.id))
      const oldRemoteContacts   = contacts.filter(c => oldMContactRemoteIds.includes(c.id))

      // console.log('--- newRemoteContacts', newRemoteContacts, '\n')
      // console.log('--- oldRemoteContacts', oldRemoteContacts, '\n')

      const newRemoteContactIds = newRemoteContacts.map(c => c.id) // <== remote_id
      const toUpdateContactIds  = oldRemoteContacts.filter(c => (oldMContactByRemoteId[c.id] && oldMContactByRemoteId[c.id].contact && !oldMContactByRemoteId[c.id].deleted_at)).map(c => oldMContactByRemoteId[c.id].contact)
      const toUpdateMContacts   = oldRemoteContacts.map(c => ({ microsoft_credential: credentialId, remote_id: c.id, data: JSON.stringify(c) }) )

      // console.log('--- newRemoteContactIds', newRemoteContactIds, '\n')
      // console.log('--- toUpdateContactIds', toUpdateContactIds, '\n')
      // console.log('--- toUpdateMContacts', toUpdateMContacts, '\n')
      ...
      // console.log('--- createdContactIds', createdContactIds, '\n')
      ...
      // console.log('--- newMContacts', newMContacts, '\n')
      ...
      // console.log('--- createdMicrosoftContacts', createdMicrosoftContacts, '\n')
      // console.log('--- updatedMicrosoftContacts', updatedMicrosoftContacts, '\n')

      const contactsAtts = await ContactAttribute.getForContacts(toUpdateContactIds)        
      const refinedAtts  = groupBy(contactsAtts, function(entry) { return entry.contact}) // groupBy(contactsAtts.filter(att => (!att.deleted_at && att.created_for === 'microsoft_integration')), function(entry) { return entry.contact})

      // console.log('--- contactsAtts', contactsAtts, '\n')
      // console.log('--- refinedAtts', refinedAtts, '\n')
      ...
      // console.log('--- toUpdateContacts', toUpdateContacts, '\n')
