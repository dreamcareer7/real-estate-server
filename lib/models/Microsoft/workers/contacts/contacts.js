...
const syncAvatars = async (microsoft, credential) => {
  ...
  const helper = async (mcontact) => {
    ...
    toUpdateMicContacts.push({ id: mcontact.id, photo, processed_photo: photo ? true : false })
  ...
  try {
    ...
    console.log('microsoftContacts.length', microsoftContacts.length)
    ...
    for (const mcontact of microsoftContacts) {
      ...
      if ( promises.length && (promises.length % 5 === 0) ) {
        ...
        console.log('toUpdateMicContacts.length', toUpdateMicContacts.length)
        console.log('toUpdateContacts.length', toUpdateContacts.length)
      }
    }

    if (promises.length) {
      console.log('---- Promise.all last round')
      await Promise.all(promises)
      promises = []
    ...
    console.log('toUpdateMicContacts', toUpdateMicContacts)
