...
const syncContacts = async (microsoft, credential, lastSyncAt) => {
  ...
  try {
    ...
    Context.log('oldMContactRemoteIds', oldMContactRemoteIds)
    Context.log('oldMicrosoftContacts', JSON.stringify(oldMicrosoftContacts))
    Context.log('oldMContactByRemoteId', JSON.stringify(oldMContactByRemoteId))
    Context.log('contacts', JSON.stringify(contacts))
