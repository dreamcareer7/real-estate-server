...
const Outlook = {
  syncDue: async () => {
    ...
    const records = await db.select('users_job/microsoft/outlook_sync_due', [config.emails_integration.gap_hour])
    ...
    for (const record of records) {
      ...
      Context.log('SyncOutlookMessages - Job Pushed', record.id, record.microsoft_credential, record.job_name, record.start_at)
...
const Calendar = {
  syncDue: async () => {
    ...
    for (const record of records) {
      ...
      Context.log('SyncMicrosoftCalendar - Job Pushed', record.id, record.microsoft_credential, record.job_name, record.start_at)
...
const Contacts = {
  syncDue: async () => {
    ...
    for (const record of records) {
      ...
      Context.log('SyncMicrosoftContacts - Job Pushed', record.id, record.microsoft_credential, record.job_name, record.start_at)
