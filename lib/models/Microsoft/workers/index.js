...
const Calendar = {
  syncDue: async () => {
    ...
    for (const microsoftCredential of credentials) {
      ...
      const job = await UsersJob.getByMicrosoftCredential(microsoftCredential.id, 'calendar')

      if ( job && job.status === 'pending' ) {
        continue
      }

      await UsersJob.upsertByMicrosoftCredential(microsoftCredential, 'calendar', 'pending')
