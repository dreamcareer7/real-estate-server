...
const syncMicrosoftToRechat = async (microsoft, credential, userJob) => {
  ...
  Context.log('SyncMicrosoftCalendar - calendarEventsResult', JSON.stringify(calendarEventsResult))
...
const syncRechatToMicrosoft = async (credential, userJob, query) => {
  Context.log('SyncMicrosoftCalendar - enter: syncRechatToMicrosoft')
  const { user, calEvents } = await getEvents(credential, query)
  Context.log('SyncMicrosoftCalendar - afterGetEvents')
  ...
  Context.log('SyncMicrosoftCalendar - beforeRefineEvents')
  ...
  if ( !error && created.length !== 0 ) {
    Context.log('SyncMicrosoftCalendar - before handleCreatedEvents')
    const result = await handleCreatedEvents(credential, created)
    Context.log('SyncMicrosoftCalendar - after handleCreatedEvents')
    error = result.error
  }

  if ( !error && updated.length !== 0 ) {
    Context.log('SyncMicrosoftCalendar - before handleUpdatedEvents')
    const result = await handleUpdatedEvents(credential, updated)
    Context.log('SyncMicrosoftCalendar - after handleUpdatedEvents')
    error = result.error
  }

  if ( !error && deleted.length !== 0 ) {
    Context.log('SyncMicrosoftCalendar - after handleDeletedEvents')
