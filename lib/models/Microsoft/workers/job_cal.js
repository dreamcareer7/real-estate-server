...
const syncCalendar = async (data) => {
  ...
  Context.log('temp-check-QuotaExceededException - syncCalendar - credential:', credential.id, credential.revoked, credential.deleted_at)
  ...
  Context.log('temp-check-QuotaExceededException - syncCalendar - userJob:', userJob.id, userJob.deleted_at)
  if (!userJob) {
    return
  }

  if (shouldSkipJob(userJob)) {
