...
const syncCalendar = async (data) => {
  ...
  if (!credential.scope_summary.includes('calendar') || !credential.microsoft_calendar) {
    // Slack.send({ channel: 'integration_logs', text: `Microsoft Sync-Calendar Job Is Skipped - ${data.microsoftCredential.id} - ${data.microsoftCredential.email}`, emoji: ':bell:' })
  ...
  if ( duplicateCheck || currentCredential.revoked || currentCredential.deleted_at ) {
    // Slack.send({ channel: 'integration_logs', text: `Microsoft-Calendar Sync Job Is Skipped - ${credential.id} - ${credential.email}`, emoji: ':bell:' })
    await UsersJobs.upsertByMicrosoftCredential(credential, 'calendar', 'failed')
    return
  }
