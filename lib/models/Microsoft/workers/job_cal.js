...
const syncCalendar = async (data) => {
  ...
  if (!userJob) {
    Context.log('SyncMicrosoftCalendar - Job skipped due to a pending job', credential.id, credential.email)
    return
  }
...
const handleNotifications = async (data) => {
  try {
    ...
    */
    ...
    Context.log(`SyncMicrosoft - OutlookCalSub - Notifications process failed - subscription: ${data.subscriptionId} - Ex: ${ex.statusCode} ${ex}`)
