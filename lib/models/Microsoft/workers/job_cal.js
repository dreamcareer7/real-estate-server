...
const syncMicrosoftToRechat = async (microsoft, credential, userJob) => {
  ...
  Context.log('SyncMicrosoftCalendar - HandleCalendars', credential.id, credential.email)
...
const syncRechatToMicrosoft = async (credential, userJob, query) => {
  Context.log('SyncMicrosoftCalendar - getEvents', credential.id, credential.email)
  ...
  Context.log('SyncMicrosoftCalendar - refineEvents', credential.id, credential.email)
  ...
  if ( !error && created.length !== 0 ) {
    Context.log('SyncMicrosoftCalendar - handleCreatedEvents', credential.id, credential.email)
  ...
  if ( !error && updated.length !== 0 ) {
    Context.log('SyncMicrosoftCalendar - handleUpdatedEvents', credential.id, credential.email)
  ...
  if ( !error && deleted.length !== 0 ) {
    Context.log('SyncMicrosoftCalendar - handleDeletedEvents', credential.id, credential.email)
...
const syncCalendar = async (data) => {
  ...
  if ( credential.revoked || credential.deleted_at ) {
    Context.log('SyncMicrosoftCalendar - deleteByMicrosoftCredential', credential.id, credential.email)
    await UsersJob.deleteByMicrosoftCredential(credential.id)
    return
  }

  if ( !credential.microsoft_calendar ) {
    Context.log('SyncMicrosoftCalendar - deleteByMicrosoftCredentialAndJob', credential.id, credential.email)
    await UsersJob.deleteByMicrosoftCredentialAndJob(credential.id, JOB_NAME)
    return
  }

  // check to know if there is a pending job or not
  Context.log('SyncMicrosoftCalendar - checkLockByMicrosoftCredential', credential.id, credential.email)
