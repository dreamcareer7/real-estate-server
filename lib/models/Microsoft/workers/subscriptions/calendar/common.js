...
const fetchEvents = async function (microsoft, calendar, sameBrandEmails, timeZone) {
  ...
  try {
    ...
    Context.log('SyncMicrosoftCalendar - values:', JSON.stringify(values))
    Context.log('SyncMicrosoftCalendar - extendedById:', extendedById)
    ...
    /*
      This is a quick fix for the below error:
        410 - The sync state generation is not found

        more info: https://stackoverflow.com/questions/51933002/syncstatenotfound-error-how-to-fix-or-avoid

      There might be a better way to handle this error.
    */
    if ( ex.statusCode === 410 ) {
      Context.log('SyncMicrosoftCalendar - Handle 410 error', calendar.id, ex.statusCode, ex.code, ex.message)
