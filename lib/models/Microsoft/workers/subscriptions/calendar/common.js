...
const getExtended = async function (microsoft, calendar, values) {
  const select   = '?$select=id,subject'
  const expand   = `&$expand=extensions($filter=id eq '${openExt.calendar.name}')`
  const eventIds = values.map(event => event.id)
  ...
  const events = eventIds.map(id => {
    return {
      'id': counter++,
      'method': 'GET',
      'url': `/me/calendars/${calendar.calendar_id}/events/${id}/${select}${expand}`
    }
  })

  const result   = await microsoft.batchGetEvents(events)
...
const fetchEvents = async function (microsoft, calendar, timeZone) {
  const confirmed     = []
  const cancelled     = []

  const { delta, values } = await getEvents(microsoft, calendar, timeZone)
  const extendedById      = await getExtended(microsoft, calendar, values)
