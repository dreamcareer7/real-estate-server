...
function setTimezone(ts, type = null) {
...
const generateMicrosoftCalendarEvent = (event, timeZone, isInitialSync) => {
  ...
  // isAllDay
  let isAllDay = (event.object_type === 'crm_task') ? false : true

  if (event.full_crm_task && event.full_crm_task.metadata) {
    isAllDay = Boolean(event.full_crm_task.metadata.all_day)
  }
  ...
  start_date = setTimezone(start_date, event.event_type)
  end_date   = setTimezone(end_date)
...
const handleCreatedEvents = async (credential, events) => {
  ...
  console.log('---- microsoftCalEvents error', microsoftCalEvents, error)
  return {
    error
  }
