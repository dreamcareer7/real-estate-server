...
const refineEvents = async (credential, calEvents, timeZone, isInitialSync) => {
  ...
  Context.log('SyncMicrosoftCalendar - afterFirstLevel')
  ...
  Context.log('SyncMicrosoftCalendar - afterSecondLevel')
  // level #3 refine
  ...
  Context.log('SyncMicrosoftCalendar - afterThirdLevel')
  // flush memory
  events_level_1 = []
  events_level_2 = []
...
const handleCreatedEvents = async (credential, events) => {
  Context.log('SyncMicrosoftCalendar - enter handleCreatedEvents')
  const calendar = await MicrosoftCalendar.get(credential.microsoft_calendar)
  Context.log('SyncMicrosoftCalendar - after MicrosoftCalendar.get')
  let counter = 1
  ...
  Context.log('SyncMicrosoftCalendar - after batchInsert')
  if (microsoftCalEvents) {
    ...
    Context.log('SyncMicrosoftCalendar - after CalendarIntegration.insert')
    return {
      result
    }
  }
