...
const syncCalendarEvents = async (microsoft, credential, timeZone) => {
  let upserteIds = []
  let deletedIds = []
  Context.log('SyncMicrosoftCalendar - in syncCalendarEvents')
  try {
    const sameBrandEmails       = await getSameBrandEmails(credential.id, credential.brand)
    Context.log('SyncMicrosoftCalendar - after getSameBrandEmails', JSON.stringify({sameBrandEmails}))
    const toSyncRemoteCalendars = await getToSyncCalendars(credential.id)
    Context.log('SyncMicrosoftCalendar - after getToSyncCalendars', JSON.stringify({toSyncRemoteCalendars}))
    for ( const calendar of toSyncRemoteCalendars ) {
      ...
      Context.log('SyncMicrosoftCalendar - after fetchEvents', JSON.stringify({confirmed, cancelled, masters, delta}))
    ...
    Context.log('SyncMicrosoftCalendar - return syncCalendarEvents', JSON.stringify({upserteIds, deletedIds}))
    return  {
      status: true,
      ex: null,
      upserteIds,
    ...
    Context.log('SyncMicrosoftCalendar - return exseption:', JSON.stringify({ex, upserteIds, deletedIds}))
    return  {
      status: false,
      ex,
      upserteIds,
