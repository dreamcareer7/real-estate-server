...
const setupMapping = async (credential, calendar, confirmed, cancelled) => {
  ...
  const updatedEventRemoteIds = confirmed.filter(event => { if (oldEventsRemoteId.includes(event.id) && !oldEventsByEventId[event.id].deleted_at && (oldEventsByEventId[event.id].change_key !== event.change_key)) return true }).map(event => event.id)
