...
const setupMapping = async (credential, calendar, confirmed, cancelled) => {
  Context.log('SyncMicrosoftCalendar', calendar.id, 'confirmed.length', confirmed.length)
  Context.log('SyncMicrosoftCalendar', calendar.id, 'cancelled.length', cancelled.length)
 ...
 * @param {UUID} calendarId 
 * @param {*} masterSeries 
 * @returns 
 */
const handelMasterSeriesUpdate = async (credentialId, calendarId, masterSeries) => {
  const ids = []

  await Promise.all(masterSeries.map(async (master) => {
    const {id, recurrence: { range: { startDate, endDate } } } = master
    const fetchedIds = await MicrosoftCalendarEvent.getToBeRemovedByMasterDate(credentialId, calendarId, id, startDate, endDate)
    Context.log('SyncMicrosoftCalendar - object:', JSON.stringify({fetchedIds,credentialId, calendarId, id, startDate, endDate}))
    ids.push(...fetchedIds)
  }))
...
const syncCalendarEvents = async (microsoft, credential, timeZone) => {
  ...
  try {
    ...
    for ( const calendar of toSyncRemoteCalendars ) {
      ...
      // Get events which removed my master series date range update
      const removedBasedOnSeriesChange = await handelMasterSeriesUpdate(credential.id, calendar.id, masters)
      Context.log('SyncMicrosoftCalendar - removedBasedOnSeriesChange:', JSON.stringify(removedBasedOnSeriesChange))
      canceledEventRemoteIds.push(...removedBasedOnSeriesChange)
      Context.log('SyncMicrosoftCalendar - canceledEventRemoteIds:', JSON.stringify(canceledEventRemoteIds))
      const deletedTaskIds = await handleCanceledEvents(credential, calendar, canceledEventRemoteIds)
      Context.log('SyncMicrosoftCalendar - deletedTaskIds:', JSON.stringify(deletedTaskIds))
