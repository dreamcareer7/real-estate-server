...
const createNewSubscriptions = async (microsoft, data) => {
  ...
  const params = {
    ...
    notificationUrl: 'https://95843b42.ngrok.io/webhook/microsoft',
...
const handleSubscriptions = async (microsoft, data) => {
  try {
    ...
    if (subscription) {
      ...
      if (!liveSubscription) {
        Context.log('SyncMicrosoft - handle messages subscriptions, going to delete offlince record and create a new sub')
      ...
      if ( subscription.subscription_id !== liveSubscription.id ) {
        Context.log('SyncMicrosoft - handle messages subscriptions, going to delete offlince/remote records and create a new sub')
        await MicrosoftSubscription.delete(subscription.id)
      ...
      if ( (exp - now) / gap < 24 ) {
        Context.log('SyncMicrosoft - handle messages subscriptions, going to update sub')
        return await updateSubscriptions(microsoft, subscription)
      }
    }    
    
    if (!subscription) {
      Context.log('SyncMicrosoft - handle messages subscriptions, fresh sub')
      return await createNewSubscriptions(microsoft, data)
    }

    Context.log('SyncMicrosoft - handle messages subscriptions, nothing to do')
    ...
    Context.log('SyncMicrosoft - Mesaages - HandleSubscriptions failed', data.microsoftCredential.id, ex)
