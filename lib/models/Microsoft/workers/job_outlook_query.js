...
const syncOutlookByQuery = async (data) => {
  console.log('syncOutlookByQuery - Job Started', data)
  ...
  if (!userJob) {
    console.log('--------- syncOutlookByQuery - Job skipped due to a pending job')
    return
  }

  if (userJob.resume_at) {
    if ( new Date(userJob.resume_at).getTime() >= new Date().getTime() ) {
      console.log('--------- syncOutlookByQuery - Job skipped due to the paused job', credential.id)
      return
    }
  }
  ...
  try {
    ...
    console.log('------ syncOutlookByQuery - Job Finished')
