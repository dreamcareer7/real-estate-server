...
const listFolders = async (microsoft) => {
  ...
  if (vBeta.error) {
    Context.log('SyncOutlookMessages - ListFolders - V.Beta Failed', vBeta.error.message, vBeta.error)
  }

  if (vOne.error) {
    Context.log('SyncOutlookMessages - ListFolders - V.One Failed', vOne.error.message, vOne.error)
  }
