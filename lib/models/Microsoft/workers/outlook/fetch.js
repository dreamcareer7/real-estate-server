...
const fetchMessages = async function (microsoft, lastSyncAt, projection, folder, fullSync = false) {
  ...
  const filter_1 = lastSyncAt ? (`&$filter=isDraft eq false and receivedDateTime ge ${new Date(lastSyncAt).toISOString()}`) : '&$filter=isDraft eq false'
  ...
  /*
    If we add `&$orderby=receivedDateTime desc` to the url, we will get this error:
  ...
  */
  ...
  for await (const response of microsoft.discreteGetMessagessNative(url)) {
    ...
    const currentMsgCreatedDateTime  = new Date(fetchedMessages[fetchedMessages.length - 1]['receivedDateTime'])
