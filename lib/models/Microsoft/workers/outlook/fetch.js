...
const fetchMessages = async function (microsoft, lastSyncAt, projection, folder, fullSync = false) {
  ...
  if ( folder === 'inbox' ) {
    let url = `https://graph.microsoft.com/v1.0/me/MailFolders/Inbox/messages?$top=250${query}`
  }
  ...
  for await (const response of microsoft.discreteGetMessagessNative(url)) {
    ...
    Context.log('discreteGetMessagessNative', messages.length)
    ...
    Context.log('discreteGetMessagessNative messages.length >= max', messages.length >= max)
    Context.log('discreteGetMessagessNative toCheckTS <= UTS', toCheckTS <= UTS)
    Context.log('discreteGetMessagessNative toCheckTS, currentMsgModifiedDateTS, currentMsgCreatedDateTS', toCheckTS, currentMsgModifiedDateTS, currentMsgCreatedDateTS)
