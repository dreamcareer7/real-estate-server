...
const fetchMessages = async function (microsoft, lastSyncAt, projection, folder, fullSync = false) {
  ...
  let url = `https://graph.microsoft.com/v1.0/me/messages?$top=250${query}&$orderby=receivedDateTime desc`

  if ( folder === 'inbox' ) {
    url = `https://graph.microsoft.com/v1.0/me/MailFolders/Inbox/messages?$top=250${query}&$orderby=receivedDateTime desc`
  }
