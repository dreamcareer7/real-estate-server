...
const fetchSpamMessages = async function (microsoft, lastSyncAt) {
  ...
  try {
    ...
    if ( ex.statusCode === 404 ) {
      Context.log(`SyncOutlookMessages - fetchSpamMessages-failed - StatusCode: ${ex.statusCode}, Message: ${ex.message}`)
      return messages
    }
...
const syncMessages = async (microsoft, credential, lastSyncAt) => {
  ...
  try {
    ...
    Context.log(ex)
    Context.log(`SyncOutlookMessages - SyncMessages - catch ex => Email: ${credential.email}, StatusCode: ${ex.statusCode}, Message: ${ex.message}`)
