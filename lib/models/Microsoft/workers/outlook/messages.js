...
const syncMessages = async (microsoft, credential, lastSyncAt) => {
  ...
  try {
    Context.log('SyncOutlookMessages - SyncMessages 1', lastSyncAt)
    ...
    Context.log('SyncOutlookMessages - refinedFolders', JSON.stringify(refinedFolders))
    Context.log('SyncOutlookMessages - deletedMessages length', deletedMessages.length)
    Context.log('SyncOutlookMessages - spamMessages length', spamMessages.length)
    Context.log('SyncOutlookMessages - messages length', messages.length)
    Context.log('SyncOutlookMessages - bouncedIntMsgIds length', bouncedIntMsgIds.length)

    deletedMessages
      .filter(
        dm => typeof dm?.internetMessageId !== 'string' || !dm?.internetMessageId
      )
      .forEach(dm => {
        Context.log('SyncOutlookMessages - strange message object', dm)
      })
    ...
    Context.log('SyncOutlookMessages - intIds length', intIds.length)
