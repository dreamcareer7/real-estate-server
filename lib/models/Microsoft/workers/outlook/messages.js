...
const deduplicate = (rootMsgs, inboxMsgs) => {
  const rootMsgIds = rootMsgs.map(msg => msg.id)
  const temp = rootMsgs

  for(const msg of inboxMsgs) {
    if( !rootMsgIds.includes(msg.id) ) {
      temp.push(msg)
    }
  }
...
const syncMessages = async (microsoft, credential, lastSyncAt) => {
  ...
  try {
    ...
    */
    ...
    Context.log('syncMessages rootMsgs.length:', rootMsgs.length)
    Context.log('syncMessages inboxMsgs.length:', inboxMsgs.length)
    Context.log('syncMessages messages.length:', messages.length)
    Context.log('syncMessages deletedMessages.length:', deletedMessages.length)
    Context.log('syncMessages spamMessages.length:', spamMessages.length)
    Context.log('syncMessages bouncedIntMsgIds.length:', bouncedIntMsgIds.length)
