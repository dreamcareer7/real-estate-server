  ...
  try {
    ...
    if ( messages.length ) {
      ...
      for ( const message of messages ) {
        ...
        if ( !msgToPush.in_bound && !bouncedMessageIds.includes(message.id) ) {
      ...
      Context.log('SyncOutlookMessages - syncMessages 4', credential.email, 'microsoftMessages.length:', microsoftMessages.length)
    }
    ...
    await addEvents(deliveredMessageIds, 'delivered')
