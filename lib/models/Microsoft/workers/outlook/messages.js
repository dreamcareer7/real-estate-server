...
const handleEmailCampaigns = async function (records, processCredential) {
  ...
  for (const rec of records) {
    ...
    toStoreMicrosoftIds.push({
      ...
      microsoft_id: rec.microsoft_message_id
    })
    ...
    toSaveThreadKeys.push({
      id: campaign.id,
      thread_key: `${campaign.microsoft_credential}${rec.microsoft_conversation_id}`
    })

    toSetCampaigns.push({
      microsoft_credential: credential,
      message_id: rec.microsoft_message_id,
      campaign: rec.rechat_campaign_id
    })
  }

  Context.log('SyncOutlookMessages HandleEmailCampaigns ==> toStoreMicrosoftIds', toStoreMicrosoftIds)
  Context.log('SyncOutlookMessages HandleEmailCampaigns ==> toSaveThreadKeys', toSaveThreadKeys)
  Context.log('SyncOutlookMessages HandleEmailCampaigns ==> toSetCampaigns', toSetCampaigns)
