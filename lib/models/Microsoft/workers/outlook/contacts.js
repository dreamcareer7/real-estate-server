...
const processMessages = async (credential, messages) => {
  ...
  for ( const message of messages ) {
    ...
    if ( fromAddress !== userEmail && senderAddress !== userEmail ) {
      continue
    }
...
const chunkHandler = async (user, brand, temp) => {
  ...
  Context.log('SyncMicrosoftContacts - chunkHandler - emails', emails)
  Context.log('SyncMicrosoftContacts - chunkHandler - emails.length', emails.length)

  let {ids}     = await Contact.fastFilter(brand, [{ attribute_type: 'email', value: emails, operator: 'any' }], {})
  let result    = await Contact.getAll(ids)
  let duplicate = result.map(c => c.emails).flat()

  Context.log('SyncMicrosoftContacts - chunkHandler - duplicate', duplicate)
  Context.log('SyncMicrosoftContacts - chunkHandler - duplicate.length', duplicate.length)
  ...
  Context.log('SyncMicrosoftContacts - chunkHandler - temp', JSON.stringify(temp))

  let pruned = temp
    .map(c => {
      c.attributes = c.attributes.filter(att => ( att.attribute_type !== 'email' ) || ( att.attribute_type === 'email' && !duplicate.includes(att.text) ))
      return c
    })
  ...
  Context.log('SyncMicrosoftContacts - chunkHandler - pruned.length', pruned.length)
  Context.log('SyncMicrosoftContacts - chunkHandler - pruned', JSON.stringify(pruned))

  const createdContactIds = await Contact.create(pruned, user, brand, 'microsoft_integration', { activity: false, relax: true, get: false })

  Context.log('SyncMicrosoftContacts - chunkHandler - createdContactIds', createdContactIds)
...
const pruneAndPersist = async (contacts, user, brand) => {
  ...
  */
  ...
  for ( let i = 0; i < length; i ++ ) {
    ...
    if ( temp.length === limit ) {
      Context.log('SyncMicrosoftContacts - pruneAndPersist - calling chunkHandler', temp.length)
  ...
  if ( temp.length > 0 ) {
    Context.log('SyncMicrosoftContacts - pruneAndPersist - calling chunkHandler-latest', temp.length)
    const num = await chunkHandler(user, brand, temp)
    createdNum += num
...
const extractContacts = async (microsoft, credential, lastSyncAt) => {
  ...
  try {
    ...
    Context.log('SyncMicrosoftContacts - extractContacts - messages.length', messages.length)
    ...
    Context.log('SyncMicrosoftContacts - extractContacts - newContacts.length', newContacts.length)
    Context.log('SyncMicrosoftContacts - extractContacts - createdNum', createdNum)
