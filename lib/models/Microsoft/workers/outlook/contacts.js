...
const processMessages = async (credential, messages) => {
  ...
  Context.log('SyncMicrosoftContacts - credential', JSON.stringify(credential))
  Context.log('SyncMicrosoftContacts - messages', JSON.stringify(messages))
  for ( const message of messages ) {
    ...
    const recipientsArr = []
    recipientsArr.push(message.toRecipients)
    recipientsArr.push(message.ccRecipients)
    recipientsArr.push(message.bccRecipients)
  ...
  Context.log('SyncMicrosoftContacts - recipients', JSON.stringify(recipients))
  Context.log('SyncMicrosoftContacts - recipientsSet', JSON.stringify(recipientsSet))
  const validRecipients = recipients.filter(r => r.address)
  Context.log('SyncMicrosoftContacts - validRecipients', JSON.stringify(validRecipients))
...
const extractContacts = async (microsoft, credential, lastSyncAt) => {
  ...
  try {
    ...
    Context.log(`SyncMicrosoftContacts - createdNum:${createdNum} - newContacts:${JSON.stringify(newContacts)}`)
    return  {
      status: true,
      createdNum
    }
