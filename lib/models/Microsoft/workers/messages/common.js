 ...
 */
const fetchOutlookBody = async function (microsoft, message_ids) {
  ...
  const expand  = '&$expand=attachments($select=id,name,contentType,size,isInline,microsoft.graph.fileAttachment/contentId)'
  ...
  do {
    ...
    for (const message of result.responses) {
      ...
      messages[id] = {
        ...
        snippet: (message.body.bodyPreview) ? message.body.bodyPreview : ERR_MSG,
