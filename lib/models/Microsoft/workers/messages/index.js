...
const refineFolders = (fobj) => {
  ...
  return {
    byName: _.keyBy(fobj.vOne.folders, 'displayName'),
    byId: _.keyBy(fobj.vOne.folders, 'id')
  }
}
...
const syncMessages = async (microsoft, data) => {
  ...
  try {
    ...
    await MicrosoftMailFolder.upsertFolders(data.microsoftCredential.id, fobj.vOne.folders)
    const refinedFolders = refineFolders(fobj)
    ...
    if ( messages.length ) {
      ...
      for ( const message of messages ) {
        ...
        // const folder = refinedFolders[message.parentFolderId]
        // if ( folder.displayName === 'Archive' ) {}
