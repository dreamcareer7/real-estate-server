...
const fetchMessages = async function (microsoft, lastSyncAt) {
  const max = 5
  // const UTS = new Date().setDate(new Date().getDate() - 10)
  const UTS = new Date().setMonth(new Date().getMonth() - 5)

  let messages = []

  for await (const response of microsoft.discreteGeMessagessNative(lastSyncAt)) {
...
const syncSentBox = async function (microsoft, lastSyncAt) {
  const max = 5
  // const UTS = new Date().setDate(new Date().getDate() - 10)
  const UTS = new Date().setMonth(new Date().getMonth() - 5)

  const projection = '$select=id,createdDateTime,from,sender,toRecipients,ccRecipients,bccRecipients,subject'

  let messages = []

  for await (const response of microsoft.discreteSyncSentBox(lastSyncAt)) {
    if ( !response.value || response.value.length === 0 )
      break
...
const extractContacts = async (microsoft, data) => {
  ...
  try {
    ...
    const messages = await syncSentBox(microsoft, lastSyncAt)
    ...
    if ( messages.length ) {
      ...
      for (const recipient of recipients) {
        const remoteId = recipient.address + 'xxx'
        const data = {
          id: remoteId,
        ...
        records.push({ microsoft_credential: credentialId, remote_id: remoteId, data: JSON.stringify(data) })
      }
