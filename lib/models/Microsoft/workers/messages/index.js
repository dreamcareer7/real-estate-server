...
const fetchMessages = async function (microsoft, lastSyncAt, projection, fullSync = false) {
  ...
  const url = `https://graph.microsoft.com/v1.0/me/messages?$top=200${query}`

  for await (const response of microsoft.discreteGeMessagessNative(url)) {
...
const fetchDeletedMessages = async function (microsoft, lastSyncAt) {
  ...
  const url   = `https://graph.microsoft.com/v1.0/me/mailFolders/deleteditems/messages?$top=200${query}`
...
const syncMessages = async (microsoft, data) => {
  ...
  try {
    ...
    Context.log('SyncMicrosoft - syncMessages 2', data.microsoftCredential.email, 'messages.length:', messages.length)
    ...
    if ( messages.length ) {
      ...
      for ( const message of messages ) {
        ...
        /*
          ...
          if (fromAddress) {
            if ( fromAddress === data.microsoftCredential.email )
              inBound = false
          }
