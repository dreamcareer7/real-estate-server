...
const fetchMessages = async function (microsoft, lastSyncAt, projection, fullSync = false) {
  ...
  const expand   = `&$expand=attachments($select=id,name,contentType,size,isInline),extensions($filter=id eq '${config.microsoft_integration.openExtension}')`
      ...
      for ( const message of messages ) {
        ...
        if ( message.extensions ) {
          ...
          for ( const record of message.extensions ) {
            if ( record.id === config.microsoft_integration.openExtension ) {
...
const createOrUpdateMessage = async (microsoft, data) => {
