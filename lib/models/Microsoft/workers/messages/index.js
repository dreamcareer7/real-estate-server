...
const fetchMessages = async function (microsoft, lastSyncAt, projection, fullSync = false) {
  ...
  for await (const response of microsoft.discreteGeMessagessNative(query)) {
    ...
    Context.log('SyncMicrosoft - fetchMessages - response.value.length:', response.value.length)
...
const extractContacts = async (microsoft, data) => {
  ...
  try {
    ...
    Context.log('SyncMicrosoft - extractContacts - catch ex', data.microsoftCredential.email, ex)
...
const syncMessages = async (microsoft, data) => {
  ...
  try {
    ...
    Context.log('SyncMicrosoft - syncMessages - catch ex', data.microsoftCredential.email, ex)
