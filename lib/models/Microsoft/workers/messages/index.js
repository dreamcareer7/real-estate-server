...
const { syncMessagesPprojection, extractContactsProjection } = require('./static')
...
const extractContacts = async (microsoft, credential, lastSyncAt) => {
  ...
  try {
    ...
    Context.log('SyncMicrosoft - extractContacts 1', credential.email)
    ...
    Context.log('SyncMicrosoft - extractContacts 2', credential.email)

    const totalContactsNum = await MicrosoftContact.getMCredentialContactsNum(credentialId, ['sentBox', 'contacts'])

    Context.log('SyncMicrosoft - extractContacts 3', credential.email)
    ...
    Context.log(`SyncMicrosoft - extractContacts - catch ex => Email: ${credential.email}, Code: ${ex.statusCode}, Message: ${ex.message}`)
...
const syncMessages = async (microsoft, credential, lastSyncAt) => {
  ...
  try {
    ...
    Context.log('SyncMicrosoft - syncMessages 1', credential.email)
    ...
    Context.log('SyncMicrosoft - syncMessages 2', credential.email, 'messages.length:', messages.length, 'deletedMessages.length:', deletedMessages.length)
    ...
    if ( messages.length ) {
      ...
      Context.log('SyncMicrosoft - syncMessages 3', credential.email, 'microsoftMessages.length:', microsoftMessages.length)
      ...
      Context.log('SyncMicrosoft - syncMessages 4', credential.email, 'createdMessages.length:', createdMessages.length)
    ...
    Context.log('SyncMicrosoft - syncMessages 5', credential.email)
    ...
    Context.log(ex)
    Context.log(`SyncMicrosoft - syncMessages - catch ex => Email: ${credential.email}, Code: ${ex.statusCode}, Message: ${ex.message}`)
