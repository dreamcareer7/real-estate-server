...
const syncMessages = async (microsoft, data) => {
  ...
  try {
    ...
    if ( messages.length ) {
      ...
      for (const rechatEmail of rechatEmailsToUpdate) {
        await Email.storeMicrosoftId(rechatEmail.id, rechatEmail.microsoft_id)
        const currentEmmail = await Email.get(rechatEmail.id)
        await EmailCampaign.saveThreadKey(currentEmmail.campaign, 'threadKey_xxxxx')
      }
