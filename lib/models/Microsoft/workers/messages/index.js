  ...
  try {
    ...
    if ( messages.length ) {
      ...
      for ( const message of messages ) {
        ...
        // remove extra internetMessageHeaders

        const to  = (message.toRecipients.length) ? message.toRecipients.map(record => record.emailAddress) : []
        const cc  = (message.ccRecipients.length) ? message.ccRecipients.map(record => record.emailAddress) : []
        const bcc = (message.bccRecipients.length) ? message.bccRecipients.map(record => record.emailAddress) : []
        ...
        microsoftMessages.push({
          ...
          '"from"': (message.from) ? JSON.stringify(message.from.emailAddress) : JSON.stringify({}),
