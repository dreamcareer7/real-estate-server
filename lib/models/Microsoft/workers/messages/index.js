...
const { syncMessagesPprojection, extractContactsProjection, wellKnownFolders, generateMMesssageRecord } = require('./common')
...
const refineFolders = async (microsoft, credential) => {
  // do this part later
  ...
  await MicrosoftMailFolder.upsertFolders(credential.id, fobj.vOne)

  return {
    byName: _.keyBy(fobj.vOne, 'displayName'),
    byId: _.keyBy(fobj.vOne, 'id')
  }
}
...
const syncMessages = async (microsoft, data) => {
  ...
  try {
    ...
    const refinedFolders  = await refineFolders(microsoft, data.microsoftCredential)
    ...
    if ( messages.length ) {
      ...
      for ( const message of messages ) {
        ...
        const folder     = refinedFolders.byId[message.parentFolderId]
        const isArchived = (folder.displayName === 'Archive') ? true : false
