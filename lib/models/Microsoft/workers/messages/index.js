...
const fetchMessages = async function (microsoft, lastSyncAt, projection) {
  ...
  const filter = lastSyncAt ? (`&$filter=isDraft eq false and createdDateTime ge ${new Date(lastSyncAt).toISOString()}`) : '&$filter=isDraft eq false'
...
const extractContacts = async (microsoft, data) => {
  ...
  try {
    ...
    // console.log('\n\n---- ex.statusCode', ex.statusCode) // 504
    // console.log('\n\n---- ex.name', ex.name) // StatusCodeError
...
const syncMessages = async (microsoft, data) => {
  ...
  const projectionArr = [
    ...
    'hasAttachments', 'subject', 'bodyPreview', 'uniqueBody', 'body',
    'categories', 'importance', 'webLink',
    'isRead', 'isDraft'
  ]
...
// https://graph.microsoft.com/v1.0/me/messages?&$filter=lastModifiedDateTime ge 2019-07-22T16:55:00Z
// https://graph.microsoft.com/v1.0/me/messages?$top=100&$filter=isDraft eq false&$select=id,createdDateTime,from,sender,toRecipients,ccRecipients,bccRecipients,subject,isDraft