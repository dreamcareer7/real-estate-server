...
const syncMessages = async (microsoft, data) => {
  ...
  try {
    ...
    if ( messages.length ) {
      ...
      for (const rec of rechatEmailsToUpdate) {
        ...
        await EmailCampaign.saveThreadKey(currentCampaing.id, `${currentCampaing.microsoft_credential}${rec.microsoft_message_id}`)
      }
