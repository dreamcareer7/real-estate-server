...
// https://graph.microsoft.com/v1.0/me/mailFolders('SentItems')/messages?$select=sender,from,toRecipients,ccRecipients,bccRecipients,subject
// https://graph.microsoft.com/v1.0/me/mailFolders('Inbox')/messages?$select=sender,from,toRecipients,ccRecipients,bccRecipients,subject
...
const syncMessages = async (google, data) => {
  ...
  try {
    ...
    // const totalMessagesNum = await GoogleMessage.getGCredentialMessagessNum(credentialId)

    return  {
      status: true,
      createdNum: createdGoogleMessages.length
