...
const syncContacts = async (data) => {
  ...
  if ( credential.scope_summary.includes('contacts.read') ) {
    Context.log('SyncMicrosoftContacts - Sync Contact Folders', credential.id, credential.email)
    ...
    Context.log('SyncMicrosoftContacts - Sync Contacts', credential.id, credential.email)
  ...
  if ( credential.scope_summary.includes('contacts.read') ) {
    Context.log('SyncMicrosoftContacts - Handle Subscriptions', credential.id, credential.email)
  ...
  if ( credential.scope_summary.includes('mail.read') && (data.origin !== 'notifications') ) {
    ...
    if ( credential.scope_summary.includes('contacts.read') ) {
      Context.log('SyncMicrosoftContacts - Extract Contacts', credential.id, credential.email)
  ...
  if (num_of_synced_contacts) {
    ...
    Context.log('SyncMicrosoftContacts - Insert Contact AvatarJob', credential.id, credential.email)
