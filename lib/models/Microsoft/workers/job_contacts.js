...
const handleOtherContacts = async (microsoft, credential, userJob, origin) => {
  ...
  Context.log(`SyncMicrosoftContacts - last_start_at:${last_start_at} - userJob.start_at:${userJob.start_at}`)
...
const syncContacts = async (data) => {
  ...
  Context.log('SyncMicrosoftContacts - start', credential.id, credential.email)
