...
const syncMicrosoft = async (data) => {
  ...
  if ( microsoftJobDuplicateCheck || currentMicrosoftCredential.revoked || currentMicrosoftCredential.deleted_at ) {
    const text  = `Microsoft - Revoked: ${currentMicrosoftCredential.revoked} - Deleted_At: ${currentMicrosoftCredential.deleted_at} - Action: Job skipped`
    const emoji = ':skull:'
...
const getOfflineMessage = async (mid, cid) => {
  try {
...
const handleDeleteEvent = async (subscription, data) => {
  const credentialId = subscription.microsoft_credential
  const resourceId   = data.resourceData.id

  const offlineMessage = await getOfflineMessage(resourceId, credentialId)

  if (!offlineMessage)
    return

  if (offlineMessage.deleted_at)
    return

  if (!offlineMessage.deleted_at)
    return await MicrosoftMessage.deleteByRemoteMessageIds(credentialId, [resourceId])
}

const handleCUEvents = async (subscription, data) => {
  const credentialId = subscription.microsoft_credential
  const credential   = await MicrosoftCredential.get(credentialId)
  const microsoft    = await getMGraphClient(credential)

  await messageWorker.createOrUpdateMessage(microsoft, data)


  // const resource     = `https://graph.microsoft.com/v1.0/${data.resource}?$select=id,isDraft`
  // const credentialId = subscription.microsoft_credential

  // const credential = await MicrosoftCredential.get(credentialId)
  // const microsoft  = await getMGraphClient(credential)
  // const message    = await microsoft.geMessagesByUrl(resource)

  // console.log('--------------- changeType', data.changeType)
  // console.log('--------------- message', message)
  
  // if (!message) {
  //   Context.log('-------------- Message not found ==> Skipped')
  //   return
  // }

  // if (message.isDraft) {
  //   Context.log('-------------- Draft message ==> Skipped')
  //   return
  // }

  // await MicrosoftCredential.forceSync(credentialId)
}
...
const handleMessageNotif = async (data) => {
  ...
  try {
    ...
    // Outlook move event pushes deleted and created events. deleted will be handled in this part
    if ( data.changeType === 'deleted' )
      return await handleDeleteEvent(subscription, data)

    // Handle created/updated events
    return await handleCUEvents(subscription, data)

  } catch (ex) {

    if ( ex.message === 'Please wait until current sync job is finished.' ) {

      const text  = `SyncMicrosoft - Notifications process skipped - subscription.id: ${subscription.id} - Details: ${data.changeType} ${data.resourceData['@odata.type']}`
      const msg   = `SyncMicrosoft - Notifications process skipped - subscription.id: ${subscription.id}`
      const emoji = ':bell:'
      ...
      const text  = `SyncMicrosoft - Notifications process failed - subscription.id: ${subscription.id} - Details: ${ex.message}`
      const msg   = `SyncMicrosoft - Notifications process failed - subscription.id: ${subscription.id} - Ex: ${ex}`
      const emoji = ':skull:'
