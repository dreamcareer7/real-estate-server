...
const syncMicrosoft = async (data) => {
  ...
  const handleException = async function(msg, ex) {
    ...
    if (invalidGrant) {
      await MicrosoftSubscription.stop(data.microsoftCredential.id)
...
const handleMessageNotif = async (data) => {
  try {
    ...
    if ( credential.revoked || credential.deleted_at ) {
      Context.log('SyncMicrosoft - OutlookSub - Credential Is Not Active', credential.id)
    ...
    if ( data.lifecycleEvent === 'subscriptionRemoved' ) {
      Context.log('SyncMicrosoft - OutlookSub LifecycleEvent(removed) - Force Sync', credential.email, credential.id)
      await MicrosoftSubscription.stop(credential.id)
    ...
    if ( data.lifecycleEvent === 'missed' ) {
      Context.log('SyncMicrosoft - OutlookSub LifecycleEvent(missed) - Force Sync', credential.email, credential.id)
    ...
    Context.log(`SyncMicrosoft - OutlookSub - Notifications process failed - subscription.id: ${data.subscriptionId} - Ex: ${ex}`)
