...
const syncMicrosoft = async (data) => {
  ...
  const handleException = async function(msg, ex) {
    Context.log('SyncMicrosoft-Exception', msg, JSON.stringify(ex), JSON.stringify(data))
  ...
  if ( !syncContactsFromSentBoxResult.status ) {
    ...
    if ( syncContactsFromSentBoxResult.ex === Microsoft_Error ) {
  ...
  if ( syncContactsFromSentBoxResult.status ) {
    ...
    if ( !syncMessagesResult.status ) {
      ...
      if ( syncMessagesResult.ex === Microsoft_Error ) {
