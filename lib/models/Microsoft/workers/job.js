...
const syncMicrosoft = async (data) => {
  ...
  const disableEnableSync = async function(cid, status) {
    await MicrosoftCredential.disableEnableSync(cid, status)
  }
  ...
  const handleException = async function(msg, ex) {
    ...
    if (invalidGrant) {
      await disableEnableSync(data.microsoftCredential.id, 'disable')
    } else {
  ...
  if ( data.microsoftCredential.scope_summary.includes('calendar') ) {
    if (data.microsoftCredential.microsoft_calendar) {
      ...
      if ( !calendarSubscriptionResult.status && !calendarSubscriptionResult.skip ) {
