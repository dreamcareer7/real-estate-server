...
const syncMicrosoft = async (data) => {
  ...
  const Microsoft_Unknown_Error  = 'Microsoft-Unknown-Error'

  let extract_contacts_error = ''
  const sync_messages_error  = ''
  ...
  const handle504Exception = async function(msg) {
    const obj = {
      id: data.microsoftCredential.id,
      user: data.microsoftCredential.user,
      brand: data.microsoftCredential.brand,
  ...
  if ( !syncContactsFromSentBoxResult.status ) {
    ...
    if ( syncContactsFromSentBoxResult.ex === Microsoft_Unknown_Error ) {
      await handle504Exception('Job Error - Microsoft Sync Failed [extract-contacts-Err-504]')
      extract_contacts_error = Microsoft_Unknown_Error
  ...
  //     if ( syncMessagesResult.ex === Microsoft_Unknown_Error ) {
  //       await handle504Exception('Job Error - Microsoft Sync Failed [sync-messages-Err-504]')
  //       sync_messages_error = Microsoft_Unknown_Error
