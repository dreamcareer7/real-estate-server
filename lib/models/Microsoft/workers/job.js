...
const syncMicrosoft = async (data) => {
  ...
  const handleException = async function(msg, ex) {
    ...
    Context.log('SyncMicrosoft - handleException', data.microsoftCredential.email, msg, ex)
    ...
    Slack.send({ channel: 'integration_logs', text: `${msg} - Details: ${ex.message} - Info: ${JSON.stringify(obj)} - Exception: ${JSON.stringify(ex)}`, emoji: ':skull:' })

    const sync_finish_time = new Date() 
    sync_duration = sync_finish_time.getTime() - sync_start_time.getTime()

    await MicrosoftCredential.updateSyncStatus(data.microsoftCredential.id, null)
    await addLastSyncRecord(false)

    Context.log('SyncMicrosoft - handleException done', data.microsoftCredential.email)
  }
  ...
  const handleUnknownException = async function(msg, ex) {
    ...
    Context.log('SyncMicrosoft - handleUnknownException', data.microsoftCredential.email, msg, ex)

    // if (!data.microsoftCredential.last_sync_at)
    ...
    Context.log('SyncMicrosoft - handleUnknownException done', data.microsoftCredential.email)
  }
