...
const syncMicrosoft = async (data) => {
  ...
  if ( microsoftJobDuplicateCheck || currentMicrosoftCredential.revoked || currentMicrosoftCredential.deleted_at ) {
    const text = `Microsoft - Revoked: ${currentMicrosoftCredential.revoked} - Deleted_At: ${currentMicrosoftCredential.deleted_at} - Status: ${currentMicrosoftCredential.sync_status} - Action: Job skipped`
  ...
  if ( data.microsoftCredential.scope.some(entry => config.microsoft_scopes.basic.includes(entry)) ) {
    if ( new Date(sync_start_time).getTime() - new Date(data.microsoftCredential.last_sync_at).getTime() > 3 * 24 * 60 * 60 * 1000 ) {
