...
const handleMessageNotif = async (data) => {
  try {
    ...
    Context.log(`SyncMicrosoft - OutlookSub - Notifications process failed - subscription: ${data.subscriptionId} - Data: ${data} - Ex: ${ex}`)

    let text  = `SyncMicrosoft - Notifications process failed - subscription: ${data.subscriptionId} - Details: ${ex.message}`
    let emoji = ':skull:'

    if ( ex.message === 'Please wait until current sync job is finished.' ) {
      text  = `SyncMicrosoft - Notification Is skipped - subscription: ${data.subscriptionId}`
      emoji = ':bell:'
    }
