...
const syncContactsAvatars = async (data) => {
  ...
  try {
    ...
    // Report success
    Context.log('SyncMicrosoftContactsAvatars - updateJobStatus', credential.id, credential.email)
    await updateJobStatus(userJob.id, 'success')

    Context.log('SyncMicrosoftContactsAvatars - deleteById', credential.id, credential.email)
