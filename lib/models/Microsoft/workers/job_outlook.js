...
const syncOutlook = async (data) => {
  ...
  if (!userJob) {
    Context.log('SyncOutlookMessages - Job skipped due to a pending job', credential.id, credential.email)
    return
  }
  ...
  */
  ...
  // Sync Folders
  await messageWorker.syncFolders(microsoft, credential)

  // Sync Messages
  if ( credential.scope_summary.includes('mail.read') ) {
    const syncMessagesResult = await messageWorker.syncMessages(microsoft, credential, last_start_at)
    ...
    Context.log('SyncOutlookMessages - Messages', credential.email)
  }
  ...
  if ( credential.scope_summary.includes('mail.read') ) {
    ...
    Context.log('SyncOutlookMessages Subscription', credential.email)
  }
  ...
  Context.log('SyncOutlookMessages - job Finished', credential.email, sync_duration)
...
const handleNotifications = async (data) => {
  try {
    ...
    if ( data.lifecycleEvent === 'subscriptionRemoved' ) {
      // Context.log('SyncOutlookMessages - OutlookSub LifecycleEvent(removed)', credential.email, credential.id)
      return await MicrosoftSubscription.delete(subscription.id)
    }

    if ( data.lifecycleEvent === 'missed' ) {
      // Context.log('SyncOutlookMessages - OutlookSub LifecycleEvent(missed)', credential.email, credential.id)
    ...
    if ( subscription.resource === '/me/messages' ) {
      ...
      if ( data.changeType === 'deleted' ) {
        return await subscriptionWorkers.events.handleDeleteEvent(credential, data.resourceData.id)
      }
  
      if ( data.changeType === 'updated' ) {
        return await subscriptionWorkers.events.handleUpdateEvents(credential, data.resourceData.id)
      }
  
      if ( data.changeType === 'created' ) {
        const result = await subscriptionWorkers.events.handleCreateEvents(credential, data.resourceData.id)
