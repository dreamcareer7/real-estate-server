...
const syncOutlook = async (data) => {
  ...
  if ( credential.scope_summary.includes('mail.read') ) {
    Context.log('SyncOutlookMessages - Handle Subscriptions', credential.id, credential.email)
  ...
  // Sync Messages
  if ( credential.scope_summary.includes('mail.read') ) {
    Context.log('SyncOutlookMessages - Sync Messages', credential.id, credential.email)
