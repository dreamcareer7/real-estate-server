...
const syncOutlook = async (data) => {
  ...
  */
  ...
  // Sync Bounced
  if ( credential.scope_summary.includes('mail.read') ) {
    const syncBouncedResult = await outlookWorker.syncBouncedMessages(microsoft, credential, last_start_at)

    if ( !syncBouncedResult.status && !syncBouncedResult.skip ) {
      const message = 'Job Error - Microsoft Sync Failed [bounced]'
      await handleException(credential, message, syncBouncedResult.ex)
      return
    }

    Context.log('SyncOutlookMessages - Bounced', credential.email)
  }
