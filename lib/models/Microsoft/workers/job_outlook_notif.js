...
const wrapper = async (payload, cb) => {
  Context.log('HandleNotifications 3')
  try {
    Context.log('HandleNotifications 4')
    const { commit, rollback, run } = await createContext({
      id: `process-microsoftNotifications-${i++}`
    })

    Context.log('HandleNotifications 5')
    await run(async () => {
      Context.log('HandleNotifications 6')
      try {
        await promisify(handler)(payload)
        await commit()
      } catch(e) {
        rollback(e)
        throw e
