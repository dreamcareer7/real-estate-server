...
const timer = async (data) => {  
  ...
  Context.log('SyncOutlookNotifications - Invoking handler', data.payload.subscriptionId)
  await handler(data)
  Context.log('SyncOutlookNotifications - Invoking handler done', data.payload.subscriptionId)

  if (tid) {
    Context.log('SyncOutlookNotifications - Clearing timer', data.payload.subscriptionId)
    clearTimeout(tid)
  }
}
