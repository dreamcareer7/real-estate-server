...
const lockJob = async (credential, userJob) => {
  ...
  */
  ...
  await UsersJob.lockByMicrosoftCredential(credential.id, userJob.job_name)
...
const postpone = async (credential, userJob, ex) => {
  ...
  Context.log('Postpone-Microsoft-Job', credential.id, userJob.job_name, ex.statusCode, ex.message, ex)
  ...
  await UsersJob.postponeByMicrosoftCredential(credential.id, userJob.job_name, interval)
}
