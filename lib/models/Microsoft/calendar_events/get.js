 ...
 * @param {UUID} fromDate from date
 * @param {UUID} toDate to date
 */
const getToBeRemovedByMasterDate = async (credentialId, calendarId, masterSeriesId, fromDate, toDate) => {
  return await db.selectIds('microsoft/calendar_events/get_to_be_removed_by_master_date',
    [credentialId,calendarId,masterSeriesId,fromDate,toDate])
}
