 ...
 * @returns {Promise<number>} number of updated rows (should be <= records.length)
 */
const bulkUpdateFolderSyncToken = async (records) => {
  let rowCount = 0

  for (const record of records) {
    rowCount += await updateFolderSyncToken(record)
  }
