...
async function handleCalendarScope(credential) {
  ...
  */
  ...
  Context.log('Microsoft-Auth-Failed', credential.email, ' - Result:', JSON.stringify(result))
  ...
  Context.log('Microsoft-Auth-Failed', credential.email, ' - Done.')
...
MicrosoftAuthLink.grantAccess = async (data) => {
  ...
  try {
    ...
    //  ex.message =  A folder with the specified name already exists.
    Context.log('Microsoft-Auth-Failed - Message:', ex.message, ' - Ex:', ex)
