 ...
 * @param {() => Promise<any>} getTokens 
 */
async function setupCredential(user, brand, microsoft, getTokens) {
  try {
...
/**
 * @param {IMicrosoftCredential} cred 
 */
MicrosoftAuthLink.reconnectCredential = async (cred) => {
  const { user, brand } = cred
  const microsoft = await MicrosoftPlugin.setupClient(cred)

  return setupCredential(user, brand, microsoft, () => microsoft.refreshToken())
}

/**
 * @param {{ state: string; code: string; }} data 
 * @returns 
 */
MicrosoftAuthLink.grantAccess = async (data) => {
  ...
  const credential = await setupCredential(user, brand, microsoft, () => microsoft.tokenRequest(data.code))
