...
const createRechatRemoteCal = async (credential) => {
  ...
  Context.log('Microsoft-Auth-Failed - createRechatRemoteCal', credential.email, ' - body:', JSON.stringify(body))

  const rechatCalendarId = await create(credential.id, body)
  Context.log('Microsoft-Auth-Failed - createRechatRemoteCal', credential.email, ' - rechatCalendarId:', rechatCalendarId)

  await updateRechatMicrosoftCalendar(credential.id, rechatCalendarId)
  const offlineCal = await get(rechatCalendarId)
  Context.log('Microsoft-Auth-Failed - createRechatRemoteCal', credential.email, ' - offlineCal:', JSON.stringify(offlineCal))
...
const handleOldCalendar = async (credential) => {
  ...
  Context.log('Microsoft-Auth-Failed - handleOldCalendar', credential.email, ' - offlineCal:', JSON.stringify(offlineCal))
  Context.log('Microsoft-Auth-Failed - handleOldCalendar', credential.email, ' - remoteCal:', JSON.stringify(remoteCal))

  if (!remoteCal) {
    await resetPrimaryCalendar(credential)
    await createRechatRemoteCal(credential)

    Context.log('Microsoft-Auth-Failed - handleOldCalendar Done', credential.email)
...
const configureCalendars = async (credential, conf) => {
  Context.log('Microsoft-Auth-Failed', credential.email, ' - Conf:', JSON.stringify(conf))
  Context.log('Microsoft-Auth-Failed', credential.email, ' - credential:', JSON.stringify(credential))
  ...
  Context.log('Microsoft-Auth-Failed', credential.email, ' - handleToSync Done')
