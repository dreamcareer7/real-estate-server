...
const getGroupValues = ({deal, roles, group}) => {
  const items = group
    .map(annotation => {
  ...
  const value = getValue({deal, roles, instruction})
  const annotations = items.map(i => i.annotation)

  return wordWrap(annotations, value)
}

const getValues = ({deal, roles, groups}) => {
  let values = {}

  for(const group of groups) {
    const groupValues = getGroupValues({deal, roles, group})
