...
Deal.create = function (deal, cb) {
  ...
  const tasks = (cb, results) => {
    const saved = results.deal

    DealChecklist.addChecklists(saved).nodeify(cb)
  }

  const finalize = (cb, results) => {
    Deal.updateTitle(results.saved).nodeify(cb)
  }
  ...
  async.auto({
    ...
    tasks: ['deal', tasks],
    scrape: ['deal', scrape],
    saved: ['scrape', 'roles', 'tasks', get],
    final: ['saved', finalize],
...
const generateTitle = async deal => {
  const mls_address = deal.mls_context ? deal.mls_context.full_address : null

  if (mls_address)
    return mls_address

  const context_address = Deal.getContext(deal, 'full_address')

  if (context_address)
    return context_address

  const client_role = deal.deal_type === 'Buying' ? 'Buyer' : 'Seller'

  // Address not found. We need to generate a name using client names

  const all_roles = await promisify(DealRole.getAll)(deal.roles)
  const client_roles = all_roles.filter(role => {
    return role.role === client_role
  })

  if (client_roles.length < 1)
    return '[Draft]'

  return client_roles.map(r => r.legal_full_name).join(', ')
}
