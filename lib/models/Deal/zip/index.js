...
const zipEnvelopes = async ({deal, task, checklist, zip, envelopes, envelope_documents}) => {
  ...
  for(const doc of related_documents) {
    ...
    const name = `${checklist.title}/${task.title}/Signed Copies/${envelope.title} (${envelope.status})`
    const stream = await AttachedFile.downloadAsStream(file)
    zip.file(encode(name), stream, { binary: true })
  }
}

const zipUploadedFiles = async ({checklist, task, zip}) => {
  const room = await promisify(Room.get)(task.room)
  const files = await AttachedFile.getAll(room.attachments)
...
const zip = async deal => {
  ...
  for(const checklist of checklists) {
    ...
    const a = tasks.map(task => zipTask({zip, task, checklist, deal, envelopes, envelope_documents})).flat()
    promises.push(...a)
  }
...
const generateZipLink = async ({deal, user}) => {
  const data = {
    deal: deal.id,
    user: user.id,
    date: new Date()
