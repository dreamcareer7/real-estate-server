...
const getPalette = async deal => {
  ...
  for(const { marketing_palette } of settings)
    if (marketing_palette) {
      const bg = Color(marketing_palette['inverted-container-bg-color']).rgb().array()
      const text = Color(marketing_palette['inverted-container-text-color']).rgb().array()
      return {
  ...
  return {
    'inverted-container-text-color': [0, 0, 0],
    'inverted-container-bg-color': [0, 178, 134],
