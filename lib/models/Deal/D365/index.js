...
const getLeaseAttributes = ({deal, roles}) => {
  ...
  const MonthlyRent = Deal.getContext(deal, 'lease_price')
...
const sync = async ({deal, roles, property_type}) => {
  ...
  const details = await getAgentDetails(user_ids)
  console.log(details)
  const agents = _.chain(roles)
    .filter(isAgent)
    .filter(isInternal)
    .map(role => {
      const { AgentId, BusinessLocation } = _.find(details, {user: role.user})
...
const considerSync = async (deal, brands) => {
  if (!brands.includes('a1daa4ea-6fb4-4a6f-81a4-5fe9ea7f6e83'))
    return
  ...
  await sync({deal, property_type, roles})
