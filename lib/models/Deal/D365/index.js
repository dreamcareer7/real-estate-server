...
const sync = async (deal, brand_ids) => {
  ...
  const mapExternal = role => {
    const isReferral = role.role === 'BuyerReferral' || role.role === 'SellerReferral'
    const AgentType = isReferral ? 'Referral' : 'ReferralAgent'
    const Feebase = isReferral ? 'Off_the_top' : null
