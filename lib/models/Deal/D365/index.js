...
const getLeaseAttributes = ({deal, roles}) => {
  ...
  return {
    ...
    DealCreatedBy: 'N/A',
    ProjectedClosingDate: moment.utc(deal.created_at).format('YYYY-MM-DD'),
...
const sync = async (deal, brand_ids) => {
  ...
  const ListingId = Deal.getContext(deal, 'deal_number') ?? Deal.getContext(deal, 'mls_number') ?? `Hippocket-${deal.number}`
  ...
  /*
   * On an office double ender deal we would have 2 separate deals. One on the sell side, one on the buy side.
   * If a deal is set to Both, then DE API would expect listing data to be present there as well (Like ListSideCommissionRate)
   * But we only have those data points on the deal that was on the List side.
   * So we only say Both on the Sell type deal where we have all the data points DE needs
   */
  if (isDoubleEnded(deal) && deal.deal_type === Deal.SELLING)
    DealSide = 'Both'
