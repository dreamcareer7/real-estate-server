...
const contextQuery = (term, query, expr) => {
  ...
  expr.or(`deals.id IN(SELECT deal FROM current_deal_context cdc
    WHERE cdc.deleted_at IS NULL AND cdc.text IS NOT NULL
    AND cdc.searchable @@ to_tsquery('english', ?))`, tsquery(term))
}
