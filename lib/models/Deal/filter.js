...
const status_query = `
  (
    ...
    SELECT (
          (?::boolean = is_active   AND ?::boolean IS NOT NULL)
      OR  (?::boolean = is_pending  AND ?::boolean IS NOT NULL)
      OR  (?::boolean = is_archived AND ?::boolean IS NOT NULL)
...
const statusFilter = (q, filter) => {
  ...
  const { is_active, is_pending, is_archived } = status

  q.where(status_query, is_active, is_active, is_pending, is_pending, is_archived, is_archived)
}
