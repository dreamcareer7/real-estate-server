...
const brandFilter = (q, filter) => {
  ...
  expr.and(`deals.brand IN (SELECT brand_children(?))
            OR deals_roles.brand IN(SELECT brand_children(?))`, filter.brand, filter.brand)
...
const accessFilter = (q, user) => {
  ...
  expr.or('deals_roles.brand IN(SELECT brand_children(?))', user.id)
...
Deal.filter = async ({filter, user}) => {
  const q = squel
    ...
    .where('deals.deleted_at IS NULL AND deals_roles.deleted_at IS NULL')
    .left_join('deals_roles', null, 'deals.id = deals_roles.deal')
