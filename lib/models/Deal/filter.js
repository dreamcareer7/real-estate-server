...
const accessFilter = (q, user) => {
  const expr = squel.expr()
  expr.and('deals.brand IN (SELECT user_brands(?))', user.id)

  q.where(expr)
}

const listingQuery = (term, query, expr) => {
  expr.or('deals.listing IN (SELECT id FROM search_listings(?))', term)
}

const contextQuery = (term, query, expr) => {
  query.join('current_deal_context', 'cdc', 'deals.id = cdc.deal')

  const q = `to_tsvector('english', cdc.text) @@ plainto_tsquery('english', ?)`
  expr.or(q, term)
}
...
const roleQuery = (term, query, expr) => {
  ...
  query.join('deals_roles', 'dr', 'deals.id = dr.deal')
...
const roleFilter = (query, filter) => {
  ...
  expr.and('deals_roles.deleted_at IS NULL')
  expr.and('deals_roles.user IN ?', user)
...
Deal.filter = async ({filter, user}) => {
  const q = squel.select().from('deals').field('deals.id').where('deals.deleted_at IS NULL')
