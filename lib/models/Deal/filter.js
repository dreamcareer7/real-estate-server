...
const statusFilter = (q, filter) => {
  ...
  const { is_active, is_pending, is_archived, is_null } = status
  ...
  if (isBoolean(is_active))
    q.where('deal_statuses.is_active = ?', is_active)

  if (isBoolean(is_pending))
    q.where('deal_statuses.is_pending = ?', is_pending)

  if (isBoolean(is_archived))
    q.where('deal_statuses.is_archived = ?', is_archived)

  if (is_null === true)
    q.where('deal_statuses.deal IS NULL')

  if (is_null === false)
    q.where('deal_statuses.deal IS NOT NULL')
