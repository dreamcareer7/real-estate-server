...
const contextQuery = (query, expr) => {
  ...
  q.where(`to_tsvector('english', value) @@ plainto_tsquery('english', ?)`, query)
...
const roleQuery = (query, expr) => {
  ...
  users.where(`to_tsvector('english', first_name || ' ' || last_name || ' ' || email) @@ plainto_tsquery('english', ?)`, query)
