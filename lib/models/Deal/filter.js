...
const statusFilter = (q, filter) => {
  ...
  const active = statuses.is_active || null
  const pending = statuses.is_pending || null
  const archived = statuses.is_pending || null

  q.where(statuses_query, active, active, pending, pending, archived, archived)
}
