...
DealChecklist.addChecklists = async ({deal_id, brand_id, flags}) => {
  const res = await db.query.promise('deal/checklist/match', [
    brand_id,
    flags
  ])

  for(const checklist of res.rows) {
    checklist.deal = deal_id
    await DealChecklist.create(checklist)
  }
}
