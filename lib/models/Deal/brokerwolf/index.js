...
Deal.BrokerWolf.considerSync = async old => {
  if(!config.brokerwolf.enabled)
    return log('Brokerwolf integration is not enabled')
  ...
  if (isTraining)
    return log('Deal is in training mode')
  ...
  if (!shouldUpdate)
    return log('Attributes that are changed are not sensitive')
...
Deal.BrokerWolf.sync = async deal => {
  ...
  if (!SellPrice)
    return log('SellPrice is not defined')
  ...
  if (!closing_date)
    return log('CloseDate is not defined')
