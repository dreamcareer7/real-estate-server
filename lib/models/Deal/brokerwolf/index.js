...
Deal.BrokerWolf.queue = peanar.job({
...
Deal.BrokerWolf.sync = async deal => {
   ...
   */
  let StreetNumber = Deal.getContext(deal, 'street_number') || '0'
  ...
  try {
    ...
    const { statusCode } = e
    if (statusCode === 404) {
      log(deal, 'Deal already updated in Brokerwolf')
      return
    }
  ...
  await Deal.BrokerWolf.setMetadata({
    deal,
    transaction: saved
  })
...
Deal.BrokerWolf.setMetadata = async ({deal, transaction}) => {
