...
const log = (deal, reason) => {
  console.log(`BrokerWolf sync refused on ${deal.id}: ${reason}`)
}
...
Deal.BrokerWolf.considerSync = async old => {
  if(!config.enabled)
    return log(old, 'Brokerwolf integration is not enabled')
  ...
  if (isTraining)
    return log(updated, 'Deal is in training mode')
  ...
  if (!shouldUpdate)
    return log(updated, 'Attributes that are changed are not sensitive')
...
Deal.BrokerWolf.sync = async deal => {
  ...
  if (!SellPrice)
    return log(deal, 'SellPrice is not defined')
  ...
  if (!closing_date)
    return log(deal, 'CloseDate is not defined')
  ...
  const updateRoles = async type => {
    ...
    for (const i in Saved) {
      ...
      const role = _.find(personnel[type], {
        Id: Role.Id
      })
