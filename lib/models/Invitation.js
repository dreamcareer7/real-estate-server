...
Invitation.patch = function(invitation_id, action, cb) {
  Invitation.get(invitation_id, (err, invitation) => {
    if(err)
      return cb(err);

    db.query(sql_patch, [invitation_id, action], (err, res) => {
      if(err)
        return cb(err);

      if(action === true) {
        if(invitation.room) {
          Room.addUser(invitation.invited_user.id, invitation.room.id, false, err => {
            if(err)
              return cb(err);
