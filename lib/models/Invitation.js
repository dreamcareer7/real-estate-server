  ...
  async.auto({
                        ...
                        function(cb, results) {
                          if(results.shortlist) {
                            if(!results.is_member && results.user) {
                              ...
                              notification.auxiliary_object = invitation.resource;
    ...
    send_email: ['invitation', 'inviting_user', 'user', 'shortlist',
                 function(cb, results) {
                   if(results.shortlist) {
                     if(results.user) {
                       // Sending email to a user already on our service for a specific shortlist
                       ...
                       // Sending email to a user not on our service for a specific shortlist
                       SES.sendMail({from: config.email.from,
                                     to: [ invitation.email ],
                                     source: config.email.source,
                                     html_body: html_body,
