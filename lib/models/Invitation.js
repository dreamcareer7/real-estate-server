  ...
  db.query(sql_get, [id], function(err, res_base) {
    ...
    async.parallel({
      ...
      referring_user: function(cb) {
        if(!invitation.referring_user)
          return cb();

        User.get(invitation.referring_user, cb);
      }
...
Invitation.create = function(invitation, cb) {
  validate(invitation, function(err) {
    ...
    User.getByEmail(invitation.email, function(err, user) {
      ...
      invitation.referring_user = (user === false) ? null : user.id;
      db.query(sql_insert, [invitation.referring_user, invitation.email, invitation.resource], function(err, res) {
        if(err)
          return cb(err);
...
Invitation.patch = function(invitation_id, action, cb) {
  db.query(sql_patch, [invitation_id, action], function(err, res) {
    ...
    Invitation.get(invitation_id, function(err, invitation) {
      ...
      if(action === true) {
        console.log(invitation.referring_user.id, invitation.resource.id);
        Shortlist.addUser(invitation.referring_user.id, invitation.resource.id, function(err, res) {
          if(err)
            return cb(err);
