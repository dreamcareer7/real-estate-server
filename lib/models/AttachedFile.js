...
//     file.url = AttachedFile.getSignedUrl(file.url)
    setPreview(file)
...
// AttachedFile.signedUrl = url => {
//   const options = {
//     keypairId: config.cloudfront.keypair.id,
//     privateKeyString: config.cloudfront.keypair.private,
//     expireTime: Number(new Date()) + (600 * 1000) //Valid for an hour.
//   }
//
//   return cf.getSignedUrl(url, options)
// }

AttachedFile.getCookies = path => {
  const url = config.cdns.private + path
