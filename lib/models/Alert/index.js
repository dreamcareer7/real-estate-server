...
Alert.create = function (room_id, alert, cb) {
  async.auto({
    ...
    owner: cb => {
      return User.get(alert.created_by).nodeify(cb)
    },
    validate: cb => {
      validate(alert, err => {
        if (err)
    ...
    insert: [
      ...
      (cb, results) => {
        ...
        db.query('alert/insert', [
          ...
          alert.furnished,
          alert.fenced_yard,
          alert.master_bedroom_in_first_floor
        ],
        (err, res) => {
          if (err)
            return cb(err)
    ...
    update: [
      ...
      (cb, results) => {
        ...
        db.query('alert/patch', [
          ...
          next.furnished,
          next.fenced_yard,
          next.master_bedroom_in_first_floor,
          alert_id
        ], cb)
      }],
    alert: [
  ...
  async.auto({
    ...
    compact_listings: [
      ...
      (cb, results) => {
        ...
        Listing.getCompacts(listing_ids).nodeify((err, listings) => {
          if (err)
            return cb(err)
