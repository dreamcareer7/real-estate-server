...
const agents = async (alert) => {
  const filters = buildQuery(alert)
  ...
  const list_agents = squel.select().from('agents')
    .fields('listings.mls,agents.id as list_agent,count(listings.id),sum(price) as sum_price'.split(','))
    .join('listings', undefined, 'agents.mls=listings.mls and agents.matrix_unique_id IN (listings.list_agent_mui, listings.co_list_agent_mui)')
    .group('agents.id').group('listings.mls')

  const selling_agents = squel.select().from('agents')
    .fields('listings.mls,agents.id as selling_agent,count(listings.id) as selling_count,sum(price) as selling_sum_price'.split(','))
    .join('listings', undefined, 'agents.mls=listings.mls and agents.matrix_unique_id IN (listings.selling_agent_mui, listings.co_selling_agent_mui)')
    .group('agents.id').group('listings.mls')
  ...
  const q = squel.select()
    ...
    .from('selling_agents')
    .full_join('list_agents', undefined, 'selling_agents.mls=list_agents.mls')
