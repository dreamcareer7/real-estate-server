...
const sendStackedEmail = (user_id, rooms, cb) => {
  const getUser = cb => {
  ...
  const saveDeliveries = (cb, results) => {
    async.forEach(rooms, (room, cb) => {
      saveDelivery(results.user, room.room, cb)
    }, cb)
  }
  ...
  const prepareRoom = (user, room, cb) => {
    ...
    const notification_branches = cb => {
    ...
    async.auto({
      messages,
      room_branch,
      notification_branches: ['messages', notification_branches]
    }, done)
  }
  ...
  const getRooms = (cb, results) => {
    Room.getAll(rooms.map(r => r.room), (err, loaded) => {
      ...
      async.map(loaded, prepareRoom.bind(null, results.user), cb)
    })
  }
  ...
  async.auto({
    ...
    deliveries: ['html', saveDeliveries],
...
Message.sendEmailForUnread = (cb) => {
  db.query('message/unread', [
    ...
    // Groups them by recipient user
    const users = _.groupBy(res.rows, 'user')

    async.forEach(Object.keys(users), (u, cb) => {
      const rooms = users[u]

      sendStackedEmail(u, rooms, cb)
    }, cb)
  })
}