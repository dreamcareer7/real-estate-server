 ...
 * @property {string=} download_base_url - URL for downloading the app
 */
...
SES.sendMail = function(email, cb) {
  queue.create('email', email).save(cb);
};
...
SES.callSES = function(email, cb) {
  ...
  ses.sendEmail({
    ...
    if(err)
      ...
      console.log('-> (SES-Transport) Successfully sent a message to'.cyan, email.to[0].yellow);
