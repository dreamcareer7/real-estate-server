...
Transaction.get = function (transaction_id,  cb) {
  db.query(sql_get, [transaction_id, process.domain.user.id || null], function (err, res) {
    if(err)
      return cb(err);
    ...
    async.parallel({
      ...
      tasks: cb => {
        ...
        async.map(transaction.tasks, Task.get, cb);
      },
      ...
      notes: cb => {
        Note.get(transaction_id, process.domain.user.id, cb);
      }
    }, function (err, results) {
      if (err)
        return cb(err);
      ...
      transaction.tasks = results.tasks || null;
