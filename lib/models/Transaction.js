...
Orm.register('transaction', Transaction)
...
const schema_transaction_roles = {
  type: 'object',
  properties: {
    contact: {
      type: 'string',
  ...
  async.auto({
    ...
    create: [
      ...
      (cb, results) => {
        ...
        const listing = (results.recommendation) ? results.listing.id : (
          (results.listing) ? results.listing.id : null)
        const transaction_status = (results.listing) ? results.listing.status : 'Active'
        for (const i in transaction.listing_data)
          listing_data[i] = transaction.listing_data[i]

        db.query('transaction/insert', [
          transaction.user,
          transaction.title,
  ...
  Transaction.get(transaction_id, function (err, transaction) {
    ...
    async.auto({
      ...
      patch: [
        ...
        'recommendation',
        function (cb, results) {
          const listing_data = (transaction.listing_data) ? transaction.listing_data : results.listing
          db.query('transaction/patch', [
            transaction.title,
            transaction.recommendation,
            transaction.listing,
            listing_data,
...
Transaction.get = function (transaction_id, cb) {
  ...
  db.query('transaction/get', [transaction_id, (current_user) ? current_user.id : null], function (err, res) {
    if (err)
      return cb(err)
...
Transaction.getForUser = function (user_id, cb) {
  db.query('transaction/user', [user_id], function (err, res) {
    if (err)
      return cb(err)
    ...
    const transaction_ids = res.rows.map(function (r) {
      return r.id
    })
...
Transaction.delete = function (transaction_id, cb) {
  Transaction.get(transaction_id, function (err) {
    ...
    db.query('transaction/delete', [transaction_id], function (err) {
      if (err)
        return cb(err)
...
Transaction.assign = function (transaction_id, contact_id, cb) {
  async.auto({
    ...
    assign: [
      ...
      'contact',
      (cb, results) => {
        return db.query('transaction/assign', [transaction_id, contact_id], cb)
      }
    ],
    user: cb => {
...
Transaction.withdraw = function (transaction_id, contact_id, cb) {
  async.auto({
    ...
    withdraw: [
      ...
      'contact',
      (cb, results) => {
        return db.query('transaction/withdraw', [transaction_id, contact_id], cb)
      }
    ]
  }, (err, results) => {
    if (err)
...
Transaction.getTransactionContact = function (transaction_id, contact_id, cb) {
  Transaction.get(transaction_id, function (err) {
    ...
    Contact.get(contact_id, function (err) {
      if (err)
        return cb(err)
      db.query('transaction/get_transaction_contact', [transaction_id, contact_id], function (err, res) {
        if (err)
          return cb(err)
...
const insert = function (transaction_id, contact_id, role, cb) {
  Transaction.getTransactionContact(transaction_id, contact_id, function (err, transaction_contact_id) {
    if (err)
      return cb(err)

    db.query('transaction/add_contact_role', [transaction_contact_id, role], cb)
  })
}
...
Transaction.removeContactRole = function (transaction_id, contact_id, role, cb) {
  Transaction.getTransactionContact(transaction_id, contact_id, function (err, transaction_contact_id) {
    ...
    db.query('transaction/remove_contact_role', [transaction_contact_id, role], cb)
  })
}

Transaction.getContactRole = function (transaction_id, contact_id, cb) {
  db.query('transaction/get_contact_role', [transaction_id, contact_id], function (err, res) {
    if (err)
      return cb(err)
    ...
    const roles = res.rows.map(function (r) {
      return r.role
    })
...
Transaction.getAssignees = function (transaction_id, cb) {
  Transaction.get(transaction_id, err => {
    ...
    db.query('transaction/assignees', [transaction_id], (err, res) => {
      if (err)
        return cb(err)
