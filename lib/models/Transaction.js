...
var schema_transaction_contact = {
  type: 'object',
  properties: {
    id: {
      type: 'string',
    ...
    contacts: ['create',
      (cb, results) => {
        ...
        async.map(transaction.contacts, (r, cb) => {
          validator(schema_transaction_contact, r, (err) => {
            if (err)
              return cb(err);

            Transaction.assign(results.create, r.id, (err) => {
              if (err)
                return cb(err);

              Transaction.addContactRole(results.create, r.id, [r.role], cb);
            });
          });
        }, cb);
      }]
...
Transaction.get = function (transaction_id, cb) {
  db.query(sql_get, [transaction_id], function (err, res) {
    ...
    async.parallel({
      ...
      contact_roles: function (cb) {
        if (!transaction.contacts)
          return cb();

        return async.map(transaction.contacts, (r, cb) => {
          return Transaction.getContactRole(transaction_id, r, cb);
        }, cb);
      },
      ...
      attachments: function(cb) {
        ...
        return async.map(transaction.attachments, Attachment.get, cb);
      }
    }, function (err, results) {
      if (err)
        return cb(err);
      ...
      transaction.attachments = results.attachments || null;

      if (results.contacts) {
        for (var i in results.contacts) {
          results.contacts[i].roles = results.contact_roles[i];
        }

        transaction.contacts = results.contacts;
