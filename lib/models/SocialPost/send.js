...
const send = async (id) => {
  try {    
    ...
    // TODO how to handle not to try again
    // TODO check deleted_at of all these get methods
    if (!post || post.deleted_at) {
      throw new Error(`Facebook page id ${id} is disconnected`)
    }   

    await lock(post.id)

    Context.log(`Getting Template for Social Post with template id ${post.template}`)
    ...
    Context.log('Error during posting on instagram', error)
    await saveError(id, error.message)
  }
}
