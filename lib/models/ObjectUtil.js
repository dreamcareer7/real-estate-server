...
ObjectUtil.formatPhoneNumberForDialing = function(phone_number) {
  ...
  phone_number = phone_number.replace(/^0/,'+1');

  // Apparently, phone numbers can start without a zero or a plus in the US as well or so Dan says.
  if (phone_number.match(/^[0-9]/))
    phone_number = '+1' + phone_number;

  var n = pnu.parse(phone_number);
