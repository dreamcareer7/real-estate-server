...
Stripe.charge = ({user, amount, customer}, cb) => {
  ...
  const getUser = cb => User.get(user).nodeify(cb)
...
const captureCharge = async id => {
  const { charge } = await promisify(Stripe.getCharge)(id)

  return promisify(stripe.charges.capture)(charge.id)
}

Stripe.captureCharge = peanar.job({
  handler: captureCharge,
  queue: 'capture_charge',
  error_exchange: 'capture_charge.error',
  exchange: 'capture_charge',
  name: 'captureCharge'
})
