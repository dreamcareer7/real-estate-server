...
Template.generateThumbnailForBrand = async ({id, brand, template}) => {
...
Template.generateThumbnails = async template => {
  ...
  const indexed = keyBy(brands, 'id')

  for(const row of rows) {
    const brand = indexed[row.brand]
...
Template.updateThumbnails = async () => {
  ...
  for(const row of rows) {
    ...
    promises.push(Template.generateThumbnailForBrand({id, brand, template}))
...
Template.invalidateForBrand = async brand_id => {
