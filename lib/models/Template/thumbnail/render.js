...
const get = (brand, name) => {
  return _.get(brand, `settings.palette.palette.${name}`, _.get(default_palette, name))
}
...
const render = async ({html, template, brand, variables = {}}) => {
  ...
  const palette = template.mjml ? `${__dirname}/palette.mjml` : `${__dirname}/palette.html`
  data.palette = await env.render(palette, {
    brand,
    ...data
  })
