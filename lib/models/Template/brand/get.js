...
const getCategoriesForBrand = async ({types = null, mediums = null, brands, filter = ''}) => {
  ...
  let s1 = new Date()
  const data = await db.query.promise('template/brand/category', [brands, types, mediums, filterByTemplate, filterByAssets])
  
  Context.log(`get-category query ${new Date() - s1}`)
  ...
  Context.log(`get-category hash ${new Date() - s1}`)
