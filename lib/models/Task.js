...
Task.patch = function(task_id, data, cb) {
  Task.get(task_id, function(err, task) {
    ...
    var transaction_id = (data.transaction) ? data.transaction: undefined;
    ...
    async.auto({
      transaction: cb => {
        ...
        Transaction.get(transaction_id, cb);
      },
      patch: ['transaction',
              (cb, results) => {
                db.query(sql_patch, [
  ...
  async.auto({
    ...
    notification: [
      ...
      (cb, results) => {
        ...
        notification.message = results.user.first_name + ' removed you from task: ' + results.task.title;

        Notification.issueForUser(notification, results.contact_user.id, cb);
      }
    ]
  }, (err, results) => {
    if(err)
  ...
  async.auto({
    ...
    notification: [
      ...
      (cb, results) => {
        ...
        notification.auxiliary_object_class = 'User';
        notification.auxiliary_object = results.contact_user.id;
        notification.message = results.user.first_name + ' added you to task: ' + results.task.title;
