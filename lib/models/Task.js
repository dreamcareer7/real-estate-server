...
Task.deleteByGoogleId = function (google_id, cb) {
  console.log('deleting ' + google_id)
            ...
            if (err) {
              ...
              if (last_sync_id != null) {
                for (var i = 0, len = response.items.length; i < len; i++) {
                  switch (response.items[i].status){
                    case 'cancelled':
                      Task.deleteByGoogleId(response.items[i].id, function(err, result){
                        if(err){
                          return cb(err);
                        }else{
                          console.log('A task removed '  + response.items[i].id);
                        }
                      });
                      break;
                    case 'confirmed':
                      Task.createOrUpdateFromCalendar(user_id,response.items[i].summary,response.items[i].start.date,response.items[i].id,function(err, result){
                        if(err){
                          console.log(err);
                        } else{
                          console.log('A task created or updated '  + response.items[i].id);
                        }
                      });
                      break;
                  }
...
Task.createOrUpdateFromCalendar = function (user_id, title, due_date, google_id, cb) {
  db.query(sql_get_by_google_id, [google_id], function (err, res) {
    ...
    if (res.rows.length > 0) {
      ...
      console.log(moment(due_date).unix());
