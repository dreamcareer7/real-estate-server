...
class QueryBuilder {
  ...
  _getDrilldownFields(drilldown_key) {
    ...
    if (!level) {
      dd_fields.push({ name: dim.name, expression: dim.name })
    }
    else {
      ...
      do {
        cur = dim.levels[i]
        dd_fields.push({
          name: dim.name + '_' + cur.name,
          expression: cur.expression || dim.name + '_' + cur.name
        })
  ...
  _addSelectFields() {
    ...
    for (const dd of this.drilldowns) {
      ...
      const levels = this._getDrilldownFields(dd)

      for (const lvl of levels) {
        if (dim.type === 'date')
          this.q.field(`EXTRACT(epoch FROM ${lvl.expression})`, lvl.name)
        else
          this.q.field(lvl.expression, lvl.name)
      }
    }
  ...
  _addGroupBy() {
    ...
    for (const dd of this.drilldowns) {
      for (const lvl of this._getDrilldownFields(dd)) {
        this.q.group(lvl.expression)
      }
    }
