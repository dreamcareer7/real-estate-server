const _ = require('lodash')
const db = require('../../../../utils/db')

const QueryBuilder = require('../query')

const AttributeDef = require('../../../Contact/attribute_def')
const Contact = require('../../../Contact')

function keyByReducer(key) {
  return (s, x) => { s[x[key]] = x; return s }
}

class ContactExportQueryBuilder extends QueryBuilder {
  constructor(drilldowns, filters, user_id, brand_id) {
    // @ts-ignore
    super({}, drilldowns, filters, user_id, brand_id)
  }

  addFilter(filter) {
    this.filters.push(filter)
  }

  async getMaxIndices(defs) {
    const res = await db.select('contact/export/max_indices', [
      this.brand_id,
      defs.filter(d => d.section !== 'Addresses').map(d => d.id)
    ])

    return res.reduce((index, row) => {
      if (Array.isArray(index[row.attribute_def])) {
        for (let i = index[row.attribute_def].length; i < row.max_index; i++) {
          index[row.attribute_def].push(i + 1)
        }
      }
      else {
        index[row.attribute_def] = []
        for (let i = 0; i < row.max_index; i++) {
          index[row.attribute_def].push(i + 1)
        }
      }

      return index
    }, {})
  }

  async headerMapper(row) {
    const def_ids = await AttributeDef.getForBrand(this.brand_id)
    const defs = await AttributeDef.getAll(def_ids)
    const defs_by_id = _.keyBy(defs, 'id')

    const keys = Object.keys(row)

    function header_map(key) {
      if (key === 'id') return 'ID'

      const parts = key.split(':')
      const def = defs_by_id[parts[0]]

      if (def.name === 'email' && parts[1] === '1') {
        return 'E-mail Address'
      }

      if (parts.length === 1) {
        return def.label
      }

      if (def.section === 'Addresses') {
        return parts[1] + ' ' + def.label
      }

      if (parts.length === 2) {
        return def.label + ' ' + parts[1]
      }

      console.warn('Strange column name', key)
      return def.label
    }

    return keys.map(header_map)
  }

  async facts(options = {}) {
    const def_ids = await AttributeDef.getForBrand(this.brand_id)
    const defs = await AttributeDef.getAll(def_ids)

    const singular = defs.filter(d => d.singular)
    const non_singular_with_label = defs.filter(d => !d.singular && d.has_label && d.section !== 'Addresses')
    const non_singular_without_label = defs.filter(d => !d.singular && !d.has_label)
    const addresses = defs.filter(d => d.section === 'Addresses')
    const max_indices = await this.getMaxIndices(non_singular_with_label)

    let ids

    if ((Array.isArray(this.filters) && this.filters.length > 0) || Object.keys(options).length > 0) {
      const res = await Contact.fastFilter(this.brand_id, this.filters, options)
      ids = Array.from(res.ids)
    }

    const ctsql = `
      WITH attrs AS (
        SELECT
          ca.*,
          (CASE 
            WHEN ca.date < '1801-01-01'::date THEN
              
          END) AS date,
          c.id::text || ':' || ca.is_partner AS idp
        FROM
          contacts c
          JOIN contacts_attributes ca
            ON ca.contact = c.id
        WHERE
          c.deleted_at IS NULL
          ${ids ? `AND c.id = ANY('{${ids.join(', ')}}'::uuid[])` : ''}
          AND c.brand = '${this.brand_id}'::uuid
          AND ca.deleted_at IS NULL
          AND ca.attribute_def = ANY('{${def_ids}}'::uuid[])
      ),
      summaries AS (
        (
          SELECT
            idp AS id,
            attribute_def::text AS attribute_def,
            COALESCE(
              text,
              number::text,
              date::text
            ) AS "value"
          FROM
            attrs
          WHERE
            attribute_def = ANY('{${singular.map(d => d.id)}}'::uuid[])
        )
        UNION ALL
        (
          SELECT
            idp AS id,
            attribute_def::text,
            array_agg(text ORDER BY is_primary desc, updated_at desc)::text AS "value"
          FROM
            attrs
          WHERE
            attribute_def = ANY('{${non_singular_without_label.map(d => d.id)}}'::uuid[])
          GROUP BY
            idp,
            attribute_def
        )
        UNION ALL
        (
          SELECT
            idp AS id,
            attribute_def::text || ':' || (row_number() OVER (w)),
            text AS "value"
          FROM
            attrs
          WHERE
            attribute_def = ANY('{${non_singular_with_label.map(d => d.id)}}'::uuid[])
          WINDOW w AS (
            PARTITION BY
              idp, attribute_def
          )
          ORDER BY
            idp,
            attribute_def,
            is_primary DESC
        )
        UNION ALL
        (
          SELECT
            idp AS id,
            attribute_def::text || ':' || attrs.label,
            text AS "value"
          FROM
            attrs
          WHERE
            attribute_def = ANY('{${addresses.map(d => d.id)}}'::uuid[])
          ORDER BY
            idp,
            attribute_def,
            index
        )
      )
      SELECT
        id,
        attribute_def,
        "value"
      FROM
        summaries
      ORDER BY
        id,
        attribute_def
    `

    function type_map(def) {
      const map = {
        'date': 'timestamptz',
        'text': 'text',
        'number': 'double precision'
      }

      return map[def.data_type] + (def.singular || def.has_label ? '' : '[]')
    }

    function def_name(def) {
      if (def.singular) return [def.id]
      if (!def.has_label) return [def.id]
      if (def.section === 'Addresses') return ['Home', 'Work', 'Other'].map(l => `${def.id}:${l}`)
      if (Array.isArray(max_indices[def.id])) return max_indices[def.id].map(i => `${def.id}:${i}`)

      return []
    }

    function name_map(def) {
      return def_name(def).map(name => `('${name}')`)
    }

    function column_map(def) {
      return def_name(def)
        .map(name => `"${name}" ${type_map(def)}`)
    }

    function fields(defs) {
      const global_defs = defs.filter(d => d.global).reduce(keyByReducer('name'), {})
      const primary = _(global_defs).pick([
        'title',
        'first_name',
        'middle_name',
        'last_name',
        'marketing_name',
        'source',
        'company',
        'job_title',
        'birthday',
        'email',
        'phone_number',
        'website',
        'tag'
      ])
        .values()
        .flatMap(d => def_name(d))
        .map(f => `"${f}"`)
        .value()

      const address = ['Home', 'Work', 'Other'].map(label => _(global_defs).pick([
        'state',
        'city',
        'street_prefix',
        'street_name',
        'street_suffix',
        'street_number',
        'unit_number',
        'postal_code'
      ])
        .values()
        .flatMap(d => `${d.id}:${label}`)
        .map(f => `"${f}"`)
        .value()
      ).reduce((s, a) => s.concat(a), [])

      return primary.concat(address)
    }

    const ct_values = _.flatMap(defs, name_map)
      .filter(x => x.length > 0)
      .join(', ')

    const ct_columns = _.flatMap(defs, column_map)
      .filter(x => x.length > 0)
      .join(', ')

    const sql = `
      SELECT
        ${fields(defs)}
      FROM
        crosstab($$${ctsql}$$, $$
          VALUES
            ${ct_values}
        $$) AS ct(
          id text,
          ${ct_columns}
        )
    `
    const res = await db.executeSql.promise(sql, [])
    return res.rows
  }
}

module.exports = ContactExportQueryBuilder
