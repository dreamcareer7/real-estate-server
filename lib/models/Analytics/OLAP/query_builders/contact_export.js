...
class ContactExportQueryBuilder extends QueryBuilder {
  ...
  async facts(options = {}) {
    ...
    function def_name(def, is_partner) {
      ...
      console.warn('Unknown def', def.name)
      return []
    }

    function name_map(is_partner) {
      return def => def_name(def, is_partner).map(name => `('${name}')`).join(', ')
    }

    function column_map(is_partner) {
      return def => def_name(def, is_partner).filter(x => x.length > 0).map(name => `"${name}" ${type_map(def)}`).join(', ')
    }
    ...
    function fields(defs) {
      ...
      return primary.concat(partner).concat(address)
    }
    ...
    const sql = `
      ...
      FROM
        crosstab($$${ctsql}$$, $$
          VALUES
            ${defs.map(name_map(false)).filter(x => x.length > 0).join(', ')},
            ${singular.map(name_map(true)).filter(x => x.length > 0).join(', ')},
            ${non_singular_with_label.map(name_map(true)).filter(x => x.length > 0).join(', ')}
        $$) AS ct(
          id uuid,
          ${defs.map(column_map(false)).filter(x => x.length > 0).join(', ')},
          ${singular.map(column_map(true)).filter(x => x.length > 0).join(', ')},
          ${non_singular_with_label.map(column_map(true)).filter(x => x.length > 0).join(', ')}
        )
    `
