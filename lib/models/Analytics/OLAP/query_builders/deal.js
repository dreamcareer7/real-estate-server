...
class DealsBaseQueryBuilder extends QueryBuilder {
  ...
  _deal_info() {
    return sq.select()
      ...
      .field('d.brand')
      .field('d.deal_type')
      .field('bc.checklist_type')
      .field('pt.id', 'property_type')
      .field('pt.is_lease', 'is_lease')
      .field('dc.id', 'checklist')

      .from('deals', 'd')
      .join('deals_checklists', 'dc', 'd.id = dc.deal')
      .join('brands_checklists', 'bc', 'bc.id = dc.origin')
      .join('brands_property_types', 'pt', 'bc.property_type = pt.id')
  ...
  _agent_field(role) {
    return sq.select()
      ...
      .where('ai.deal = di.id')
      .where(sq.expr()
        .or('ai.checklist IS NULL')
        .or('ai.checklist = di.checklist')
      )
      .limit(1)
  }
  ...
  _mini_deals() {
    return sq.select()
      ...
      .from('deal_info', 'di')
      .join('brands_relations', 'br', 'di.brand = br.id')
  }
