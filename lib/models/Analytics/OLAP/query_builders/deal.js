...
class DealsBaseQueryBuilder extends QueryBuilder {
  ...
  _agent_info() {
    return sq.select()
      ...
      .from('deals_roles', 'dr')
      .join('deal_info', 'd', 'd.id = dr.deal')

      .where('dr.deleted_at IS NULL')
      .where('dr.role = ANY(\'{BuyerAgent,SellerAgent}\'::deal_role[])')
      .where(sq.expr().or('dr.checklist IS NULL').or('dr.checklist = d.checklist'))
  }
  ...
  _role_info() {
    return sq.select()
      ...
      .where('dr.deleted_at IS NULL')
      .where('dr.role = ANY(\'{Buyer,Seller}\'::deal_role[])')
      .where(sq.expr().or('dr.checklist IS NULL').or('dr.checklist = d.checklist'))
  ...
  _context_info() {
    return sq.select()
      .field('cdc.*')
      .from('current_deal_context', 'cdc')
      .join('deal_info', 'di', 'cdc.deal = di.id')
  }
