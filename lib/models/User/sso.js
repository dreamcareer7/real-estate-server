...
const createBrandForUser = async (user, parent) => {
...
User.connectSso = async ({
  ...
  const sso_provider = SsoProvider.getByIdentifier(provider)

  if (sso_provider.brand)
    await createBrandForUser(sso_provider.brand)
