 ...
 * @param {boolean=} [throwIfExists=false]
...
async function getOrCreate (
  ...
  const exists = () => Error.Validation('The user already exists (are you trying to upload an avatar for an existing user?)')

  if (user) {
    const userObj = await User.get(user)
    if (throwIfExists) { throw exists() }
  ...
  const byPhone = phone_number ? await getUserByPhoneNumber(phone_number) : null
  const bothFound = byEmail && byPhone
  const oneFound = Boolean(byEmail) !== Boolean(byPhone)
  const areTheSame = byEmail?.id === byPhone?.id

  if ((oneFound || bothFound) && throwIfExists) {
    throw exists()
  }

  if (oneFound || (bothFound && areTheSame)) {
    return byEmail?.id ?? byPhone?.id ?? assert.fail('Impossible state')
  }

  if (bothFound && !areTheSame) {
    throw Error.Conflict('Provided email and phone number belong to different users')
  }
