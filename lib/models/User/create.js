...
const sendActivation = function(user_id, context, cb) {
  async function contacting_agent() {
    Context.log(`debug contacting_agent1 ${user_id} , ${context.agent}, ${typeof context.agent}`)
    if (context.agent) return Agent.get(context.agent)

    const brand = Brand.getCurrent()

   
    Context.log(`debug contacting_agent5 brand is ${brand}`)
    if (!brand) {
      Context.log('debug contacting_agent2 brand is not exist')
      return
    }

    const agents = await Brand.proposeAgents(brand.id, user_id)
    if (agents.length < 1) {
      Context.log(`debug contacting_agent3 length ${agents.length}`)
      return
    }

    Context.log(`debug contacting_agent4 agent id is ${agents[0].agent}`)
    Context.log(`debug contacting_agent5 ${JSON.stringify(agents)}`)
