 ...
 * @param {{email: string}} user - full user object
 * @param {Callback<void>} cb - callback function
 */
 ...
 * @param {Callback<IUser>} cb - callback function
 */
User.getByPhoneNumber = function(phone, cb) {
  ...
  db.query('user/get_by_phone', [p], (err, res) => {
    ...
    if (res.rows.length < 1) return cb()
...
User.patchAvatars = function(user_id, type, link, cb) {
  if (type !== 'Profile' && type !== 'Cover' && type !== 'EmailSignaturePic')
...
User.sendActivation = function(user_id, context, cb) {
  async function contacting_agent() {
    ...
    const agents = await Brand.getAgents(brand.id, user_id, 1)
