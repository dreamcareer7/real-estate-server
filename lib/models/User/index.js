...
User.emailAvailable = function (user, cb) {
  User.getByEmail(user.email).nodeify((err, user) => {
    if (err)
      return cb(err)
...
User.getAll = async function(ids) {
  const res = await db.query.promise('user/get', [ids])
  ...
  return res.rows.map(user => {
    ...
    if (typeof SocketServer === 'undefined')
      user.online_state = User.OFFLINE
    else
      user.online_state = SocketServer.getUserStatus(user)
...
User.getByPhoneNumber = function (phone, cb) {
  ...
  db.query('user/get_by_phone', [p], (err, res) => {
    ...
    return User.get(res.rows[0].id).nodeify(cb)
  })
}
...
User.create = function (user, cb) {
  ...
  async.auto({
    ...
    email_available: [
      'validate',
      cb => {
        User.emailAvailable(user, cb)
      }
    ],
    phone_available: [
      'validate',
      cb => {
        User.phoneAvailable(user, cb)
      }
    ],
    hash_password: [
      'email_available',
...
User.patch = function (user_id, user, cb) {
  ...
  const phone_owner = cb => {
    ...
    User.getByPhoneNumber(user.phone_number, (err, phone_owner) => {
      if (err)
        return cb(err)
...
User.connectToUser = function (user_id, peer_id, source_type, brand, cb) {
  ...
  User.get(peer_id).nodeify((err, user_connect) => {
    if (err)
      return cb(err)
...
User.initiatePasswordReset = function (email, cb) {
  User.getByEmail(email).nodeify((err, user) => {
    ...
    if (user.is_shadow)
      return cb(Error.Forbidden('Cannot request a reset password for a shadow user. You must complete your sign up through activation process first.'))

    const token = (process.env.NODE_ENV === 'tests') ? 'a' : crypto.randomBytes(20).toString('hex')
...
User.resetPassword = function (email, token, password, cb) {
  async.auto({
    ...
    html: [
      'update_password',
      (cb, results) => {
        const template = __dirname + '/../../html/user/password_recovery_done.html'
        render(template, results, cb)
      }
    ],
    email: [
...
User.combineUserReferences = function (user_id, users, emails, phones, cb) {
  ...
  async.auto({
    ...
    emails: cb => {
      ...
      async.map(emails, (e, cb) => User.getByEmail(e).nodeify(cb), (err, results) => {
        if (err)
          return cb(err)
...
User.sendActivation = function (user_id, context, cb) {
  const contacting_agent = cb => {
    if (context.agent)
      return Agent.get(context.agent).nodeify(cb)
    ...
    Brand.getAgents(brand.id, user_id, 1).nodeify((err, agents) => {
      if (err)
        return cb(err)
      ...
      Agent.get(agents[0].agent).nodeify(cb)
    })
  }
  ...
  async.auto({
    ...
    agent: ['user', (cb, results) => Agent.matchByEmail(results.user.email).nodeify(cb)],
...
User.getStatus = function (user, _cb) {
  ...
  const job = queue.create('socket_user_status', {user}).removeOnComplete(true)
