...
const { mjml: render_mjml, html: render_html } = require('../../utils/render')
...
/**
  ...
  */
User.getActivationLink = async ({user, agent}, context) => {
  const params = {
    uri: '/branch'
  }
  ...
  if (context && context.action) {
    ...
    if (context.action === 'create_alert') {
      if (!context.alert)
        throw new Error.Validation('You must supply alert id for proper redirection')

      params.query.action = 'create_alert'
      params.query.alert_id = context.alert
    } else if (context.action === 'listing_inquiry') {
      if (!context.room)
        throw new Error.Validation('You must supply room id for proper redirection')

      params.query.action = 'listing_inquiry'
      params.query.room_id = context.room
    } else if (context.action === 'favorite_listing') {
      if (!context.listing)
        throw new Error.Validation('You must supply listing id for proper redirection')
  ...
  const type = User.getLogicalType(user)
...
User.sendActivation = function(user_id, context, cb) {
  ...
  const renderHTML = (cb, results) => {
    if (results.contacting_agent) {
      const template = __dirname + '/../../html/user/activation_brand.html'
      render_html(template, results, cb)
    } else {
      const template = __dirname + '/../../mjml/user/activation.mjml'
