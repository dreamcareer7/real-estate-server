...
const { rstr, insert } = require('../../../utils/squel_extensions')
...
const update = async (user_id, brand_id, key, value) => {
  ...
  const upsert_query = insert()
    ...
    .onConflict('"user", brand', {
      [mapped_key]: value_expr,
      updated_at: rstr('clock_timestamp()'),
    })
 ...
 * @param {UUID} brand 
 */
const touch = (user, brand) => {
  return db.selectId('user/settings/touch', [ user, brand ])
}
