 ...
 */
async function getForBrand(brand_id, attribute_filters, options) {
  const filter_res = await fastFilter(brand_id, attribute_filters, options)
 ...
 */
async function getHavingDateAttribute (brandId, attrType) {
  return db.selectIds('contact/having_date_attribute', [brandId, attrType])
}
