...
async function createContacts(contacts, user_id, brand_id, _reason = 'direct_request') {
  ...
  Context.log('data',JSON.stringify(data), null, 2)
  Context.log('sql', JSON.stringify(await sql.select('SELECT  id, parked FROM contacts WHERE created_at > NOW() - INTERVAL \'2 SECONDS\' '), null, 2))
