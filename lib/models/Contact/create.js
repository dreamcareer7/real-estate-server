...
async function createContacts(user_id, contacts) {
  ...
  for (let i = 0; i < contacts.length; i++) {
    data[i] = {
      ...
      // created_at: new Date(contactInsertionTime += i).toISOString()
    }
  }

  const q = squel.insert({autoQuoteFieldNames: true, nameQuoteCharacter: '"'})
    .into('contacts')
    .setFieldsRows(data)
    .returning('id')
    .toParam()
...
function createContactAttributes(user_id, contact_ids, contacts, relax = false) {
  let attributes = []

  for (let i = 0; i < contacts.length; i++) {
    if (Array.isArray(contacts[i].attributes)) {
      for (const attr of contacts[i].attributes) {
        attr.contact = contact_ids[i]
        attr.created_by = user_id
      }

      attributes = attributes.concat(contacts[i].attributes)
    }
  }
...
async function createActivityForContacts(contact_ids) {
  for (const id of contact_ids) {
    ...
    // FIXME: this stops the test suite
