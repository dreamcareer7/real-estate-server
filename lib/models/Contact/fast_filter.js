 ...
 */
async function attributeFilterQuery(q, attribute_filters, user_id) {
  if (!Array.isArray(attribute_filters)) return null

  const def_ids_to_get = await AttributeDef.getForUser(user_id)
...
async function contactFilterQuery(user_id, attribute_filters = [], options = {}) {
  ...
  if (Array.isArray(terms)) {
    for (const term of terms) {
      if (term && term.length > 2)
        q.where('search_field @@ to_tsquery(\'english\', ?)', `${term.replace(/([:&\*\|\\])/, '\\$1')}:*`)
    }
  }
  ...
  if (Array.isArray(attribute_filters) && attribute_filters.length > 0) {
    await attributeFilterQuery(q, attribute_filters, user_id)
  }
