...
async function contactFilterQuery(
  ...
  if (!Array.isArray(options.users)) {
    q.where('check_contact_read_access(contacts, ?, ?)', brand_id, user_id)
  } else if (user_id && options.users.includes(user_id)) {
    const checkUserAndBrand = sq.expr()
      .and('brand = ?', brand_id)
      .and('"user" = ANY(?)', sq.SqArray.from(options.users))

    const condition = sq.expr()
      .or(checkUserAndBrand)
      .or('check_contact_read_access(contacts, ?, ?)', brand_id, user_id)

    q.where(condition)
  } else {
    q.where('brand = ?', brand_id)
      .where('"user" = ANY(?)', sq.SqArray.from(options.users))
  }
 ...
 *  is possible here
 * @param {UUID | undefined} brand_id Brand id requesting filter
 * @param {UUID | undefined | null} user_id User id requesting filter
 * @param {IContactAttributeFilter[]} attribute_filters
 * @param {IContactFilterOptions & PaginationOptions | undefined} options
 */
async function fastFilter(brand_id, user_id, attribute_filters = [], options = {}) {
