...
async function contactFilterQuery(
  ...
  /** expr to filter contacts accessible for the user. mutates `q`. */
  const matchCurrentUser = (matchBrand = true) => {
    ...
    const expr = sq.expr().or('contacts.id = ANY(assigned_contacts.assigned_contacts)')
    if (matchBrand) {
      expr.or('contacts.brand = ?::uuid', brand_id)
    }
  ...
  if (!options.users?.length) {
    ...
    q.where(sq.expr().or(matchCurrentUser(false)).or(matchUsers()))
  } else {
    q.where(matchUsers())
  }
