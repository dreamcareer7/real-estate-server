...
class ContactAttributeDef extends EventEmitter {
   ...
   * @returns {Promise<UUID>}
   */
  async create(user_id, attribute_def) {
  ...
  async delete(id) {
    /** @type {UUID[]} */
    const rows = await db.select('contact/attribute_def/delete', [
      id
    ])

    const affected_contacts = new Set(rows)

    this.emit('delete', {
      id,
      affected_contacts: Array.from(affected_contacts)
    })
