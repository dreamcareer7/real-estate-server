...
class ContactAttributeDef extends EventEmitter {
   ...
   * @param {UUID} brand_id
   */
  async getDefsByName(brand_id) {
    const def_ids = await this.getForBrand(brand_id)
    const defs = await this.getAll(def_ids)
