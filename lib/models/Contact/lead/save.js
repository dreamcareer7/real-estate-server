 ...
 */
async function _createContact(user, brand, source, lead) {
 ...
 * @param {IContact} contact
 */
async function sendNotification(link_metadata, agent_mlsid, note, contact) {
  const user = await findTeamMember(link_metadata, agent_mlsid)
 ...
 * @param {IContact} contact
 */
async function sendEmailToAgent(link_metadata, agent_mlsid, note, contact) {
 ...
 * @param {UUID} user 
 * @param {UUID} brand 
 * @returns {Promise<{ contact: IContact; lead: import('./types').ILtsLead; }>}
 */
async function saveContact(protocol, payload, source, user, brand) {
  ...
  if (contact) {
    ...
    contact = await _createContact(user, brand, source, lead)
  }
...
async function saveAndNotify(link_metadata, payload) {
  const { user, brand, source } = link_metadata
  const { lead, contact } = await saveContact(link_metadata.protocol, payload, source, user, brand)

  await sendNotification(link_metadata, lead.agent_mlsid, lead.note, contact)
