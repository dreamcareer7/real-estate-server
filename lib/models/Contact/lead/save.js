...
async function _findContact(email, user, brand) {
  ...
  const res = await Contact.fastFilter(brand, user, [{
    attribute_type: 'email',
    value: email
...
async function findTeamMember(link_metadata, agent_mlsid) {
  ...
  if (!agent) return User.get(link_metadata.user)

  const user = await User.getByAgentId(agent.id)
  if (!user) return User.get(link_metadata.user)

  const access_table = await Brand.hasAccessToBrands(link_metadata.brand, user.id, null)
  if (!access_table[link_metadata.brand]) return User.get(link_metadata.user)
...
async function saveAndNotify(link_metadata, payload) {
  ...
  const { lead, contact } = await saveContact(link_metadata.protocol, payload, source, user_id, brand)
