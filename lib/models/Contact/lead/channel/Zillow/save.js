...
const mapZillowToContact = (zillowBody) => {
  ...
  const contact = {
    ...
    on property ${zillowBody?.ContactMessage?.PropertyInfo?.MLSNumber}, ${zillowBody?.ContactMessage?.PropertyInfo?.MLSNumber}`,
    address: '',
    message: '',
    lead_source: 'Zillow',
...
/**
 * @param {string} zillowBody
 */

const save = async (zillowBody) => {
  try {
    Context.log(`start processing Zillow response '${zillowBody}'`)
    /** @type {import('../types').Zillow_Sns} */
    let zillowJSON

    /** @type {import('../types').Zillow} */
    let zillowMessage
    try {
      zillowJSON = JSON.parse(zillowBody)
      zillowMessage = JSON.parse(zillowJSON.Message)
    } catch (error) {
      const err = Error.Generic(`Can not parse Zillow Json with value '${zillowBody}'`)
      err.retry = false
      throw err
    }
    ...
    if (!leadChannelID) {
      const err = Error.Generic(`Lead Channel ID is not set in Zillow data '${zillowBody}'`)
      err.retry = false
      throw err
    }
    ...
    await LeadChannel.upsertZillowJson(contact.id, zillowBody)
  } catch (error) {
    Context.error(error)
