...
helpers.getInsertEmailsQuery = async function (contact, contactID, emailArr, errored) {
...
helpers.getInsertPhonesQuery = async function (contact, contactID, phoneNumbers, errored) {
...
helpers.getInsertAttributesQuery = async function (contact, contactID, attrsArr, errored) {
  for (const key of Object.keys(contact.attributes)) {
    ...
    for (const a of attrs) {
      ...
      attrsArr.push({
        ...
        attribute: JSON.stringify(_.omit(a, ['label', 'is_primary', 'type']))
      })
...
helpers.getInsertErrorNotesQuery = function (errorNotes, contactID, insertionNotesArray) {
  errorNotes.forEach(err => {
    insertionNotesArray.push({
      contact: contactID,
      attribute_type: 'note',
      is_primary: false,
      label: '',
...
helpers.getAttributesQuery = async function (contactIDs, contacts) {
  ...
  for (const [indx, c] of contacts.entries()) {
    ...
    await helpers.getInsertEmailsQuery(c, contactID, emailsArray, errors)
    await helpers.getInsertPhonesQuery(c, contactID, phonesArray, errors)
    await helpers.getInsertAttributesQuery(c, contactID, attrsArray, errors)
    const mappedErrors = errors.map(err => ({
      type: 'note',
      note: `Rechat was unable to store information for ${err.type} with value ${err.subject}`
    }))
    helpers.getInsertErrorNotesQuery(mappedErrors, contactID, insertionErrors)
  }

  return {
    refsQuery: updateRefsQuery.length ? updateRefsQuery.join(';') : {},
    emailsQuery: emailsArray.length ? pgsql.insert().into('contacts_emails').setFieldsRows(emailsArray).toParam() : {},
    phoneNumbersQuery: phonesArray.length ? pgsql.insert().into('contacts_phone_numbers').setFieldsRows(phonesArray).toParam() : {},
    attrsQuery: attrsArray.length ? pgsql.insert().into('contacts_attributes').setFieldsRows(attrsArray).toParam() : {},
    errorNotesQuery: insertionErrors.length ? pgsql.insert().into('contacts_attributes').setFieldsRows(insertionErrors).toParam() : {},
  }

}

function getContacts(family, user_id, paging, cb) {
  db.query(family, [user_id, paging.type, paging.timestamp, paging.limit, paging.start], (err, res) => {
    if (err)
      return cb(err)
...
Contact.extractNameInfo = function (contact) {
  ...
  if (contact.sub_contacts) {
    contact.sub_contacts.map(c => {
      if (c.attributes) {
        ...
        if (c.attributes.companies)
          companies = companies.concat(c.attributes.companies)
  ...
  companies = _u.chain(_u.sortBy(companies, r => r.updated_at)).reverse().value()
  ...
  const candidate_company = companies.find(item => item.is_primary) || companies[0] || {}
  ...
  return {
    ...
    nickname: candidate_name.nickname,
...
Contact.summarize = function (contact) {
  ...
  return {
    ...
    profile_image_url: _(contact.sub_contacts[0].attributes.profile_image_urls).sortBy('updated_at').map('profile_image_url').last()
  }
}

Contact.getDisplayName = function (contact) {
  const {nickname, first_name, last_name, email, phone_number, company} = Contact.extractNameInfo(contact)
...
Contact.getAbbreviatedDisplayName = function (contact) {
  const {nickname, first_name, email, phone_number, company} = Contact.extractNameInfo(contact)
  if (!_u.isEmpty(nickname))
    return nickname
...
Contact.getAll = function (contact_ids, cb) {
  ...
  db.query('contact/get', [contact_ids, user_id], (err, res) => {
    if (err)
      return cb(err)
...
Contact.addBulk = async function (user_id, contacts, options = {}) {
  ...
  !_.isEmpty(q.refsQuery) && await executeSql(q.refsQuery, [])
  !_.isEmpty(q.emailsQuery) && await executeSql(q.emailsQuery.text, q.emailsQuery.values)
  !_.isEmpty(q.phoneNumbersQuery) && await executeSql(q.phoneNumbersQuery.text, q.phoneNumbersQuery.values)
  !_.isEmpty(q.attrsQuery) && await executeSql(q.attrsQuery.text, q.attrsQuery.values)
  !_.isEmpty(q.errorNotesQuery) && await executeSql(q.errorNotesQuery.text, q.errorNotesQuery.values)
...
Contact.getByAttribute = async function (user_id, attribute, values, includeDeleted = false) {
...
Contact.delete = async function (userID, arrayOfIDsToDelete) {
  return promisify(db.query)('contact/delete', [arrayOfIDsToDelete])
}

Contact.checkAccess = async function (userID, ids) {
  const result = await db.query.promise('contact/check_access', [userID, ids])
  const length = Number(result.rows[0].count)
  if (length < ids.length) {
    throw Error.Forbidden('Unauthorized access to contact resource')
  }
}

Contact.getIDsByTagFilters = async function (userID, tagsArray) {
  return promisify(db.query)('contact/filter-by-tags', [userID, tagsArray])
}

Contact.getByTagFilters = async function (userID, tagsArray) {
  const res = await Contact.getIDsByTagFilters(userID, tagsArray)
  if (res.rows.legnth < 1)
    return null
...
Contact.addTags = async function (userID, contactIDs, tags) {
  // To prevent any redundant tags
  await db.query.promise('/contact/remove_tag_attributes', [tags, contactIDs])
  const insertArray = []
  for (const contactID of contactIDs) {
    for (const tag of tags) {
      insertArray.push({
        ...
        attribute_type: 'tag',
        is_primary: false,
        attribute: JSON.stringify({ tag })
      })
    }
  }

  const query = pgsql.insert()
    .into('contacts_attributes')
    .setFieldsRows(insertArray)
    .toParam()
  const executeSql = promisify(db.executeSql)
  return executeSql(query.text, query.values)
}
