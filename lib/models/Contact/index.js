...
class ContactClass extends EventEmitter {
  async validate(contacts) {
    for (const contact of contacts) {
      expect(contact.user).to.be.uuid
  ...
  async update(user_id, brand_id, contacts) {
    ...
    /** @type {Record<'id' | 'ios_address_book_id' | 'user', string>[]} */
    const toChangeContacts = []

    for (const contact of contacts) {
      if (contact.hasOwnProperty('ios_address_book_id') || contact.hasOwnProperty('user')) {
        toChangeContacts.push(_.pick(contact, ['id', 'ios_address_book_id', 'user']))
      }
    ...
    if (toChangeContacts.length > 0) {
      const q = squel
        ...
        .table('contacts')
        .set('ios_address_book_id = COALESCE(uv.ios_address_book_id, ios_address_book_id)')
        .set('"user" = COALESCE(uv."user", "user")')
