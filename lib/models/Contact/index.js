...
Contact.tt2 = async function () {
  return db.query.promise('contact/test2', [])
}

Contact.tt3 = async function () {
  return db.query.promise('contact/test3', [])
}
...
helpers.getInsertEmailsQuery = function (contact, contactID, queryArray) {
  ...
  emails.forEach(e => {
    emailArr.push({
      ...
      data: pgsql.str(`'${JSON.stringify(_.omit(e, ['email', 'label', 'is_primary', 'type']))}'`)
    })
    emailIsPrimary = emailIsPrimary || Boolean(e.is_primary)
  }) // End of emails forEach
  
  emails.length && queryArray.push(
    pgsql.insert()
...
helpers.getInsertPhonesQuery = function (contact, contactID, queryArray) {
  ...
  phones.forEach(p => {
    phoneNumbers.push({
      ...
      data: pgsql.str(`'${JSON.stringify(_.omit(p, ['phone_number', 'label', 'is_primary', 'type']))}'`)
    })
    phoneIsPrimary = phoneIsPrimary || Boolean(p.is_primary)
  }) // End of phones forEach
  
  phones.length && queryArray.push(
    pgsql.insert()
...
helpers.getInsertAttributesQuery = function (contact, contactID, queryArray) {
  Object.keys(contact.attributes).forEach(key => {
    ...
    attrs.forEach(a => {
      attrsArr.push({
        ...
        attribute: pgsql.str(`'${JSON.stringify(_.omit(a, ['label', 'is_primary', 'type'])).replace("'", "''")}'`)
      })
      attrIsPrimary = attrIsPrimary || Boolean(a.is_primary)
    }) // End of attrs forEach
    
    attrs.length && queryArray.push(
      pgsql.insert()
        .into('contacts_attributes')
        .setFieldsRows(attrsArr)
        .toString()
    )
  }) // End of other keys forEach
}

helpers.getAttributesQuery = function (contactIDs, contacts) {
  ...
  contacts.forEach((c, indx) => {
    ...
    helpers.getInsertEmailsQuery(c, contactID, insertEmailsQuery)
    helpers.getInsertPhonesQuery(c, contactID, insertPhoneNumbersQuery)
    helpers.getInsertAttributesQuery(c, contactID, insertAttrsQuery)
  
  }) // End of contacts forEach
  
  return {
    refsQuery: updateRefsQuery.join(';'),
    emailsQuery: insertEmailsQuery.join(';'),
    phoneNumbersQuery: insertPhoneNumbersQuery.join(';'),
    attrsQuery: insertAttrsQuery.join(';')
  }
...
Contact.addBulk = async function (user_id, contacts, options = {}) {
  ...
  const mainTableData = contacts.forEach(c => {
    mainInsert.push({
      user: user_id,
  ...
  q = helpers.getAttributesQuery(dbRes, contacts)
  console.log(q.attrsQuery)
  await executeSql(q.refsQuery + ';' + q.emailsQuery + ';' + q.phoneNumbersQuery + ';' + q.attrsQuery, [])
  ...
  //
  // const saved = await promisify(Contact.get)(contact_id)
  // saved.device_contact_id = contacts.device_contact_id
  // return saved
}
