...
helpers.getEmailsQuery = function (contactIDs, contacts) {
  ...
  contacts.forEach((c, indx) => {
    ...
    updateRefsQuery.push(
      pgsql.update()
        .table('contacts')
        .set('refs', pgsql.str((`ARRAY['${contactIDs[indx]}']::uuid[]`)))
        .where('id = ?', contactIDs[indx])
        .toString()
    )
    
    const emails = _.get(c, 'attributes.emails', [])
    let emailIsPrimary = false
    const emailArr = []
    emails.forEach(e => {
      emailArr.push({
        contact: contactIDs[indx],
        email: e.email,
    ...
    emails.length && insertEmailsQuery.push(
      pgsql.insert()
    ...
    const phones = _.get(c, 'attributes.phone_numbers', [])
    ...
    phones.length && insertPhoneNumbersQuery.push(
      pgsql.insert()
    ...
    Object.keys(c.attributes).forEach(key => {
      ...
      const attrs = _.get(c, `attributes.${key}`, [])
      ...
      attrs.length && insertAttrsQuery.push(
        pgsql.insert()
...
Contact.addBulk = async function (user_id, contacts, options = {}) {
  ...
  q = helpers.getEmailsQuery(dbRes, contacts)
