...
Contact.addBulk = async function (user_id, contacts, options = {}) {
  const attributes = contacts.attributes ? contacts.attributes : {}
  
  // await validate(contacts)
  
  let contactInsertionTime = Date.now()
  const mainTableData = contacts.map(c => ({
    user: user_id,
  ...
  const q = pgsql.select()
    ...
    .with('newrows',
      pgsql.insert({autoQuoteFieldNames: true, nameQuoteCharacter: '"'})
        .into('contacts')
        .setFieldsRows(mainTableData)
        .returning('*'))
    .toString()
  
  console.log(q)
  return promisify(db.executeSql)(q, [])
  
  // const contact_id = (await db.query.promise('contact/add', [
  //   user_id,
  //   contacts.ios_address_book_id,
  //   contacts.android_address_book_id
  // ]))
  //   .rows[0]
  //   .id
  //
