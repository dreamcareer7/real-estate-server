...
class Contact {
  ...
  static extractNameInfo(contact) {
    ...
    function first_attribute_of_type(attribute_type) {
      const attr = _(contact.sub_contacts[0].attributes)
    ...
    return {
      ...
      job_title: best_attribute_of_type('job_title'),
      stage: best_attribute_of_type('stage'),
      source_type: first_attribute_of_type('source_type'),
      profile_image_url: first_attribute_of_type('profile_image_url')
    }
  }
  ...
  static async filter(user_id, attribute_filters = [], options = {}) {
    ...
    // We need to unique by parent, a parent with two sub-contacts doesn't appear twice.
    return {
      ids: _(rows).uniqBy('parent').map('id').value(),
      total: rows[0].total
    }
  }
