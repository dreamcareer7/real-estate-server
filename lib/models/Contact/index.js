...
class Contact {
  ...
  static async getAll(ids, user_id = undefined) {
    ...
    const attributes = await ContactAttribute.getForContacts(ids)
  ...
  static async filter(user_id, attribute_filters, options) {
    ...
    q.where('deleted_at IS NULL')
      .where('check_contact_read_access(contacts, ?)', user_id)
   ...
   */
  static async update(contact_id, user_id, contact) {
    await Contact.validate([contact])

    const [toUpdate, toAdd] = _.partition(contact.attributes, 'id')

    for (const attr of toAdd) {
      attr.created_by = user_id
      attr.contact = contact_id
    }

    await ContactAttribute.create(toAdd)
    await ContactAttribute.update(contact_id, toUpdate)

    return db.update('contact/patch', [
      contact_id,
  ...
  static async getAllTags(user_id) {
    const rows = await db.select('tag/get_all', [user_id])
