...
class ContactClass extends EventEmitter {
  ...
  async filter(brand_id, attribute_filters = [], options = {}) {
    ...
    return {
      ids: _.difference(rows.map(r => r.id), options.excludes || []),
      total: rows[0].total
    }
  }
   ...
   * @returns {Promise<UUID[]>}
   */
  async create(contacts, user_id, brand_id, _reason = 'direct_request', options = {}) {
    ...
    await Summary.update(contact_ids)
   ...
   * @param {IContactFilterOptions & PaginationOptions | undefined} options
   */
  async undelete(brand_id, attribute_filters = [], options = {}) {
    const { ids } = await this.fastFilter(brand_id, attribute_filters, options)

    const result = await db.update('contact/undelete', [
      ids,
    ])

    await Summary.update(ids)
   ...
   * @param {TContactActionReason} _reason
   */
  async addAttributes(contact_id, attributes, user_id, brand_id, _reason = 'direct_request') {
    ...
    await Summary.update(affected_contact_ids)
   ...
   * @returns {Promise<UUID[]>}
   */
  async update(contacts, user_id, brand_id, _reason = 'direct_request') {
    ...
    for (const contact of contacts) {
      if (contact.id && (contact.hasOwnProperty('ios_address_book_id') || contact.hasOwnProperty('user'))) {
  ...
  async updateSummary(contact_ids) {
   ...
   * @param {UUID} brand_id 
   */
  async merge(contact_ids, parent_id, user_id, brand_id) {
    ...
    await Summary.update([parent_id])
