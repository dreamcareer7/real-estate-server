...
Contact.add = function (user_id, contact, options, cb) {
  if (!cb)
    cb = options
  ...
  async.auto({
    ...
    emails: [
      ...
      (cb, results) => {
        ...
        if (results.create) {
          return async.mapSeries(attributes.emails, (r, cb) => {
    ...
    phone_numbers: [
      ...
      (cb, results) => {
        ...
        if (results.create) {
          return async.mapSeries(attributes.phone_numbers, (r, cb) => {
    ...
    attributes: [
      ...
      (cb, results) => {
        ...
        if (results.create) {
          return async.mapSeries(keys, (key, cb) => {
            expect(attributes[key]).to.be.a('array')

            async.mapSeries(attributes[key], (r, cb) => {
    ...
    activity: [
      ...
      (cb, results) => {
        ...
        const activity = {
          ...
          object: results.create,
          object_class: 'contact'
        }

        Activity.add(results.create, 'Contact', activity, cb)
      }
    ],
    get: [
      'validate',
      ...
      'attributes',
      (cb, results) => {
        if (!results.create || options.get === false)
          return cb()
