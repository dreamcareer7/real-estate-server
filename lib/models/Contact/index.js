...
Contact.add = async (user_id, contact, options = {}) => {
  ...
  for (const type in attributes) {
    for (const attr of attributes[type]) {
      promises.push(promisify(Contact.addAttribute)(contact_id, user_id, attr))
      attrs.push(attr)
    }
  }
  const results = await Promise.all(promises.map(p => p.catch(e => new Error(e.toString()))))
  results.forEach((r, idx) => {
    if (_u.isError(r)) {
      if (!options.relax) {
        throw r
      }
      errored.push(attrs[idx])
    }
  })
