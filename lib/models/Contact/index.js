...
helpers.getInsertPhonesQuery = function (contact, contactID, queryArray, errored) {
...
helpers.getAttributesQuery = function (contactIDs, contacts) {
  ...
  contacts.forEach((c, indx) => {
    ...
    helpers.getInsertPhonesQuery(c, contactID, insertPhoneNumbersQuery, errors)
    
    const mappedErrors = errors.map(err => ({
      type: 'note',
      note: `Rechat was unable to store information for ${err.type} with value ${err.subject}`
    }))
    c.attributes.notes = Object.assign(_.get(c, 'attributes.notes', []), mappedErrors)
