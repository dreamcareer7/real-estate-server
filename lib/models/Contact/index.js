...
class Contact {
  ...
  static async filter(user_id, attribute_filters = [], options = {}) {
    ...
    const strFn = function(...args) {
      const inst = new squel.cls.FunctionBlock({
        rawNesting: true
      })
      inst.function(...args)
      return inst
    ...
    if (Array.isArray(terms)) {
      q.from(strFn('score_contacts_with_search_term(contacts, ?)', [terms]), 'contacts_scores(score)')
      q.where('score > 0')
    }
  ...
  static publicize(contact) {
    contact.merged = Array.isArray(contact.sub_contacts) && contact.sub_contacts.length > 1
  }
}
