  ...
  contacts.forEach((c, indx) => {
    ...
    phones.forEach(e => {
      phoneNumbers.push({
        ...
        is_primary: phoneIsPrimary? false : Boolean(e.is_primary),
        label: _.isUndefined(e.label)? null : e.label,
        data: pgsql.str(`'${JSON.stringify(_.omit(e, ['phone_number', 'label', 'is_primary', 'type']))}'`)
      })
      phoneIsPrimary = phoneIsPrimary || Boolean(e.is_primary)
    })
...
Contact.addBulk = async function (user_id, contacts, options = {}) {
  ...
  const mainTableData = contacts.forEach(c => {
    mainInsert.push({
      ...
      refs: pgsql.str(`ARRAY['${user_id}']::uuid[]`)  
    })
    // const emails = _.get(c, 'attributes.emails', [])
  ...
  await executeSql(q.emailsQuery + ';' + q.phoneNumbersQuery, [])
  ...
  //       contact: dbRes[indx],
  //       email: e.email,
  //       is_primary: isPrimary? false : Boolean(e.is_primary),
  //       label: _.isUndefined(e.label)? null : e.label,
  //       data: pgsql.str(`'${JSON.stringify(_.omit(e, ['email', 'label', 'is_primary', 'type']))}'`)
  //     })
  //     isPrimary = isPrimary || Boolean(e.is_primary)
  //   })
  ...
  // q = insertEmailsQuery.join(';')
  // console.log("koery is ", q)
  // await executeSql(q, [])
  // try {
  //   dbRes = await promisify(db.executeSql)()
  // }
  // catch (error) {
  //   return Promise.reject(error)
  // }
