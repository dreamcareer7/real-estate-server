...
class ContactClass extends EventEmitter {
   ...
   * @param {UUID} brand_id 
   */
  async addAttributes(contact_id, attributes, user_id, brand_id) {
   ...
   * @returns {Promise<UUID[]>}
   */
  async update(contacts, user_id, brand_id) {
    ...
    const { affected_contact_ids: affected_add } = await ContactAttribute.create(toAdd, user_id, brand_id)
