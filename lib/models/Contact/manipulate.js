...
async function update(contacts, user_id, brand_id, _reason = 'direct_request') {
  ...
  for (const contact of contacts) {
    if (contact.id && (
      ...
      contact.hasOwnProperty('touch_freq')
    )) {
  ...
  if (toChangeContacts.length > 0) {
    const q = sq
      ...
      .set('touch_freq', sq.rstr('COALESCE(uv.touch_freq::int, contacts.touch_freq)'))
    ...
    const touchFreqUpdated = _.reject(toChangeContacts, { touch_freq: undefined })
    if (touchFreqUpdated.length) {
      emitter.emit('update:touch_freq', {
        contact_ids: _.map(touchFreqUpdated, 'id'),
        brand_id,
      })
    }
  }
