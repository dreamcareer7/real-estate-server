...
const { call_webhooks } = require('./worker/webhooks')
 ...
 */
function onCreateContacts({ brand_id, contact_ids, reason }) {
  ...
  call_webhooks(contact_ids, 'create', { reason })
}
...
function onUpdateContacts({ event_type, user_id, brand_id, contact_ids, reason }) {
  update_contact_memberships(contact_ids)
  call_webhooks(contact_ids, 'update', { reason, event_type })
 ...
 */
function onDeleteContacts({ event_type, contact_ids, reason }) {
  delete_contact_memberships(contact_ids)
  call_webhooks(contact_ids, 'delete', { reason, event_type })
...
function onDeleteAttribute({ attribute_ids, affected_contacts, reason }) {
  Contact.emit('update', {
...
function onDeleteAttributeDef({ id, affected_contacts, reason }) {
  Contact.emit('update', {
    contact_ids: affected_contacts,
 ...
 * @param {IBrand['id'][]} args.removed_brands
 */
function onContactsAssigned ({ contact_ids, removed_brands }) {
  update_contact_memberships(contact_ids, removed_brands)
}
...
module.exports = function attachEventHandlers() {
  ...
  Contact.on('assigned', onContactsAssigned)
