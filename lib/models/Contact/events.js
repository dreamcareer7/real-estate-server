...
function onSummaryUpdated(e) {
  enqueueJob('contact_lists', 'update_contact_memberships', {
...
function onCreateContacts(e) {
  enqueueJob('contact_duplicates', 'add_vertices', {
    brand_id: e.brand_id,
    contact_ids: e.contact_ids
  })
  enqueueJob('contacts', 'update_summary', { contact_ids: e.contact_ids })
}
...
function onUpdateContacts(e) {
  ...
  if (e.event_type !== 'merge') {
    enqueueJob('contact_duplicates', 'update_edges', {
      brand_id: e.brand_id,
      contact_ids: e.contact_ids
    })
  }
 ...
 */
function onUpdateContactsBrand(e) {
  enqueueJob('contact_lists', 'delete_contact_memberships', { contact_ids: e.contact_ids })
  enqueueJob('contact_lists', 'update_contact_memberships', {
    contact_ids: e.contact_ids
  })

  enqueueJob('contact_duplicates', 'remove_vertices', {
    contact_ids: e.contact_ids
  })
  enqueueJob('contact_duplicates', 'update_edges', {
    brand_id: e.brand_id,
...
function onBrandCreated(brand) {
  enqueueJob('contact_lists', 'create_default_lists', { brand_id: brand.id })
}
...
function onListDeleted(list_ids) {
  enqueueJob('contact_lists', 'delete_list_memberships', { list_ids })
}
...
function onFlowStopped({ brand_id, flow_id }) {
  enqueueJob('contact_lists', 'remove_flow_from_lists', { brand_id, flow_id })
}

function onCrmTaskDeleted({ brand_id, task_ids }) {
  enqueueJob('contact_lists', 'remove_crm_tasks_from_lists', { brand_id, task_ids })
}

module.exports = function attachEventHandlers() {
  Contact.on('update:summary', onSummaryUpdated)
