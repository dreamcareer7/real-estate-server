...
class ListMemberClass extends EventEmitter {
  ...
  getAll(pairs) {
    ...
    q
      ...
      .join('pairs', undefined, 'clm.contact = pairs.contact AND clm.list = pairs.list')
  ...
  async findNonMembers(pairs) {
    ...
    q
      .from('contact_lists_members', 'clm')
      .left_join('pairs', undefined, 'clm.contact = pairs.contact AND clm.list = pairs.list')
      .where('deleted_at IS NULL')
  ...
  async add(membership_records) {
    ...
    Contact.emit('list:add', affected_contacts)
  ...
  async remove(membership_records) {
    ...
    if (manuals.length > 0) {
      ...
      const q_update = squel
        .update()
        .withValues('update_values', to_skip.map(mr => _.pick(mr, ['list', 'contact'])))
        .table(TABLE, 'clm')
        .set('is_manual = FALSE')
      ...
      await db.query.promise(q_update, [])
    }
    ...
    const q = squel
      .update()
      .withValues('update_values', to_remove)
      .table(TABLE, 'clm')
      .set('deleted_at = now()')
      .from('update_values', 'uv')
      .where('clm.list = uv.list::uuid')
    ...
    Contact.emit('list:remove', affected_contacts)

    this.emit('member:remove', removed)
  }
