...
class ListMemberClass extends EventEmitter {
  ...
  getAll(pairs) {
    ...
    if (Array.isArray(pairs))
      q.withValues('pairs', pairs.map(mr => _.pick(mr, ['list', 'contact'])))
    else
      q.with('pairs', pairs)

    q
      .from('contact_lists_members', 'clm')
      .join('pairs', undefined, 'clm.contact = pairs.contact::uuid AND clm.list = pairs.list::uuid')
  ...
  async findNonMembers(pairs) {
    ...
    q
      .from('contact_lists_members', 'clm')
      .left_join('pairs', undefined, 'clm.contact = pairs.contact::uuid AND clm.list = pairs.list::uuid')
      .where('deleted_at IS NULL')
  ...
  async findByListId(list_id) {
    const members_by_list = await this.findByListIds([list_id])
  ...
  async add(membership_records) {
    ...
    if (Array.isArray(membership_records)) {
      ...
      const q = squel.insert()
        .into(TABLE)
        .fromQuery(['list', 'contact', 'is_manual'], membership_records)
        .onConflict(['list', 'contact'], {
          deleted_at: null,
          is_manual: squel.case()
            .when('contact_lists_members.deleted_at IS NULL')
  ...
  async deleteByContactIds(contact_ids) {
    ...
    q.name = 'contact/list_member/find_by_contact_ids'
    const affected_lists = await db.selectIds(q, [contact_ids])
   ...
   * @param {UUID[]} list_id Deleted list id
   */
  async deleteByListId(list_id) {
    if (!list_id)
    ...
    q.name = 'contact/list_member/find_by_list_id'
    const affected_contacts = await db.selectIds(q, [list_id])
