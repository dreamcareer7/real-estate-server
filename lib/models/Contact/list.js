...
class ContactListClass extends EventEmitter {
   ...
   * @param {UUID} brand_id Brand of the lists
   * @param {UUID[]} user_ids Owners of the lists
   * @returns {Promise<IContactList[]>}
   */
  async getForBrand(brand_id, user_ids) {
    const ids = await this.getForBrands([brand_id], user_ids)
  ...
  async createDefaultListsForBrand(brand_id) {
    ...
    /** @type {Record<string, UUID>} */
    const defs_by_name = defs.reduce((idx, def) => {
      idx[def.name] = def.id
      return idx
    }, {})

    const lists = ['Warm List', 'Hot List', 'Past Client'].map(/** @returns {IContactListInput} */(name) => ({
      name,
      filters: [{
        attribute_def: defs_by_name['tag'],
        value: name
      }]
    })).concat([{
      name: 'iOS',
    ...
    return belt.asyncMap(lists, list => this.create(undefined, brand_id, list))
  }
}
