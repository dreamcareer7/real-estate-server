...
class ContactListClass extends EventEmitter {
  async create(userID, data) {
    const id = await db.insert('contact/list/create', [userID, JSON.stringify(data.filters), data.name, data.is_pinned || false])
  ...
  async update(id, {filters, name, is_pinned}) {
    const result = await db.select('contact/list/update', [id, JSON.stringify(filters), name, is_pinned])
