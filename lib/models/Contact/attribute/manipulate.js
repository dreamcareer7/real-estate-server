 ...
 * @param {TContactActionReason} _reason
 */
async function create(attributes, user_id, brand_id, _reason) {
 ...
 * @param {TContactActionReason} _reason
 */
async function deleteContact(attribute_ids, user_id, _reason = 'direct_request') {
  ...
  emit('delete', { attribute_ids, affected_contacts, user_id })
...
async function deleteForContact(contact_id, attribute_id, user_id, _reason = 'direct_request') {
  ...
  await deleteContact([attr.id], user_id, _reason)
  return 1
}
