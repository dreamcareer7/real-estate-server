...
async function deleteAttribute(attribute_ids, user_id, _reason = 'direct_request') {
  ...
  Emitter.emit('delete', { attribute_ids, affected_contacts, user_id })
  ...
  if (deletedDateAttrs.length) {
    Emitter.emit('delete:dateAttribute', { attributes: deletedDateAttrs })
  }
...
async function update(attributes, user_id, _reason = 'direct_request') {
  ...
  try {
    ...
    /* XXX: Clearing a date attribute value is an update or delete?
     *      For now, lets assume thats a delete... */
    const updatedDateAttrs = merged_attributes.filter(a => !isNil(a.date))
    if (updatedDateAttrs.length) {
      Emitter.emit('update:dateAttribute', { attributes: updatedDateAttrs })
    }
