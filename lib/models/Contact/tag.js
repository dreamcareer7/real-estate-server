...
class ContactTag extends EventEmitter {
   ...
   * @param {UUID=} [user]
   * @param {string[]=} [tags]
   * @returns {Promise<IContactTag[]>}
   */
  async getAll(brand_id, users_filter, user, tags) {
    // TODO: accept arguments as options
  ...
  async update_touch_frequency(brand_id, user_id, tag, touch_freq) {
    ...
    this.emit('update:touch_freq', { brand: brand_id, user: user_id, tag })
  }
