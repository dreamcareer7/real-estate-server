...
/**
 * Updates contact display names
 * @param {UUID[]} contact_ids
 */
function set_display_name_for_contacts(contact_ids) {
...
class ContactAttribute extends EventEmitter {
   ...
   * @returns {Promise<IContactAttribute[]>}
   */
  async getForContacts(contact_ids) {
  ...
  static async filterQuery(attribute_filters) {
    const used_def_ids = attribute_filters.map(af => af.attribute_def).filter(x => x)
    const global_def_ids = await AttributeDef.getGlobalDefs()
    const def_ids_to_get = _.union(used_def_ids, global_def_ids)
    const defs = await AttributeDef.getAll(def_ids_to_get)

    return attributeFilterQuery(attribute_filters, defs)
  }
  ...
  async create(attributes, user_id) {
    ...
    try {
      ...
      await set_display_name_for_contacts(Array.from(affected_contact_ids))
   ...
   * @param {UUID} user_id
   */
  async createForContact(attributes, contact_id, user_id) {
    ...
    const ids = await this.create(attributes, user_id)
  ...
  async delete(attributes) {
    ...
    const result = await db.select('contact/attribute/delete', [attribute_ids])
    const affected_contacts = result.map(r => r.contact)
    await db.update('contact/update_with_searchable_field', [affected_contacts])
    await set_display_name_for_contacts(affected_contacts)

    this.emit('delete', { attribute_ids, affected_contacts })
  ...
  async update(attributes) {
    ...
    try {
      ...
      await set_display_name_for_contacts(Array.from(contact_ids))
