 ...
 */
function set_updated_at_for_contacts(contact_ids, user_id, update_searchable_field) {
  const query = update_searchable_field ?
    'contact/update_with_searchable_field' :
    'contact/set_updated_at'

  return db.update(query, [contact_ids, user_id])
}
...
class ContactAttribute extends EventEmitter {
  ...
  async create(attributes, brand_id, user_id) {
    ...
    try {
      ...
      await set_updated_at_for_contacts(Array.from(affected_contact_ids), user_id, true)
  ...
  async delete(attributes, user_id) {
    ...
    await db.update('contact/update_with_searchable_field', [
      affected_contacts,
      user_id
    ])
  ...
  async update(attributes, user_id) {
    ...
    try {
      ...
      await set_updated_at_for_contacts(Array.from(contact_ids), user_id, defs.some(def => def.searchable))
