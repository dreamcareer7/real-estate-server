...
/**
 *
 * @param {IContactAttributeInput} attr
...
function get_def_from_attribute(attr, defs_by_id) {
  ...
  if (attr.attribute_def) def = defs_by_id[attr.attribute_def]
  else if (attr.attribute_type) def = defs_by_name[attr.attribute_type]
...
class ContactAttribute extends EventEmitter {
  ...
  async delete(attributes, user_id) {
    ...
    const non_editable_attrs = attributes.filter(
      a => !defs_by_id[a.attribute_def].editable
    )

    if (non_editable_attrs.length > 0) {
      throw Error.BadRequest(
        'You cannot delete non-editable attributes of type ' +
          non_editable_attrs.map(a => a.attribute_type).join(', ')
      )
    }
  ...
  async update(attributes, user_id) {
    ...
    const merged_attributes = attributes
      .filter(
        /** @type {TIsRequirePropPresent<IContactAttributeInput, 'id'>} */ (attr =>
