...
class ContactAttribute {
  ...
  static async create(attributes, relax = false) {
    ...
    try {
      const created_ids = await db.selectIds(q)

      const affected_contact_ids = new Set(attrsArray.map(a => a.contact))
      await db.update('contact/set_updated_at', [Array.from(affected_contact_ids)])
  ...
  static async update(attributes, relax = false) {
    ...
    try {
      const updated_ids = await db.selectIds(q)
      await db.update('contact/set_updated_at', [Array.from(contact_ids)])
