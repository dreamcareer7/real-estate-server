...
class ContactAttribute {
   ...
   * @param {IContactAttribute[]} attributes
   */
  static async delete(attributes) {
    ...
    const result = await db.select('contact/attribute/delete', [attributes.map(a => a.id)])
    const contact_ids = result.map(r => r.contact)
    await db.update('contact/update_with_searchable_field', [contact_ids])
    await set_display_name_for_contacts(contact_ids)
  }
   ...
   */
  static async deleteForContact(contact_id, attribute_id) {
    const attr = await ContactAttribute.get(attribute_id)
    if (!attr || attr.contact !== contact_id) return 0

    await ContactAttribute.delete([attr])
    return 1
  }
