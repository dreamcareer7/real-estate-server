...
const formatters = {
  ...
  phone_number: function(attr) {
    try {
      attr.text = ObjectUtil.formatPhoneNumberForDialing(
        ObjectUtil.cleanPhoneNumber(attr.text)
      )
      expect(attr.text).to.be.phone
    } catch (ex) {
      throw new InvalidPhoneException(attr.text)
    }
  }
}
...
class InvalidPhoneException extends Error {
  constructor(phone_number) {
    super(`Phone number ${phone_number} is not valid.`)
    this.slack = false
 ...
 * @param {boolean} relax
 */
function normalizeAttributes(attributes, defs_by_id, relax = false) {
  for (let i = 0; i < attributes.length; i++) {
    const attr = attributes[i]
    const def = defs_by_id[attr.attribute_def]

    try {
      ContactAttribute.validate(attr, defs_by_id, relax)

      if (def.name === 'email') {
        formatters.email(attr)
      }
      if (!relax && def.name === 'phone_number') {
        formatters.phone_number(attr)
      }
    } catch (ex) {
...
class ContactAttribute {
  static validate(attribute, defs_by_id, relax) {
  ...
  static async create(attributes, relax = false) {
    ...
    normalizeAttributes(attributes, defs_by_id, relax)
