...
async function normalizeAttributes(attributes, defs_by_id, relax = false) {
  const note_attr_def = Object.values(defs_by_id).find(
    def => def.name === 'note'
  )

  return asyncMap(attributes, async attr => {
    ...
    try {
      ...
      if (relax && ex instanceof InvalidPhoneException) {
        attr = {
          ...
          attribute_def: note_attr_def.id,
          text: ex.message
        }
      } else {
        console.error(ex)
    ...
    const formatter = formatters[defs_by_id[attr.attribute_def].name]
    if (formatter) {
      attr = formatter(attr)
    }
...
class ContactAttribute {
    ...
    if (defs_by_id) {
      ...
      if (!def) {
        throw Error.Validation(
          `Attribute ${attribute.attribute_def} definition not found.`
        )
      }
   ...
   * @param {IContactAttribute[]} attributes
   */
  static async update(contact_id, attributes) {
    ...
    const contact_attrs = await ContactAttribute.getForContacts([contact_id])
    ...
    let attrsArray = await normalizeAttributes(attributes, defs_by_id)
    ...
    const q = squel
      ...
      .where('contacts_attributes.id = (uv.id)::uuid')
      .returning('contacts_attributes.id')
      .toParam()
