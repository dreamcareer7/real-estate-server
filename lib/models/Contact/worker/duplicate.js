...
async function remove_vertices(contact_ids) {
  ...
  Context.log(`disbanded ${_.uniq(clusters)?.length} clusters, affecting ${affected_contacts?.length} contacts`)

  await db.query.promise('contact/duplicate/update_duplicate_clusters_for_contacts', [ _.uniq(affected_contacts) ])
}
