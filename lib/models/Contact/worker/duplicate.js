...
async function merge(job) {
  ...
  for (const cl of clusters) {
    await Contact.merge(job.data.user_id, cl.sub_contacts, cl.parent)
  }
}

async function add_vertices(job) {
  return db.query.promise('contact/duplicate/add', [
    job.data.user_id,
...
async function update_edges(job) {
  return db.query.promise('contact/duplicate/update', [
    job.data.user_id,
    job.data.contact_ids
  ])
}
...
async function remove_vertices(job) {
  return db.query.promise('contact/duplicate/remove', [
    job.data.contact_ids
  ])
}
