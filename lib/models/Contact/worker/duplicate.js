...
async function add_vertices(brand_id, contact_ids) {
  ...
  if (contact_ids?.length < 500) {
    ...
    await db.query.promise('contact/duplicate/update_duplicate_pairs_for_brand', [ brand_id ])
    await db.query.promise('contact/duplicate/update_duplicate_clusters_for_brand', [ brand_id ])
  }
}
...
async function remove_vertices(contact_ids) {
  ...
  if (contact_ids?.length >= 500) {
    ...
    await db.query.promise('contact/duplicate/remove/remove_pairs_for_brand', [ brand ])
    await db.query.promise('contact/duplicate/remove/remove_clusters_for_brand', [ brand ])
    await db.query.promise('contact/duplicate/update_duplicate_pairs_for_brand', [ brand ])
    await db.query.promise('contact/duplicate/update_duplicate_clusters_for_brand', [ brand ])
  } else {
    await db.query.promise('contact/duplicate/remove/remove_pairs_for_contacts', [contact_ids])
    ...
    await db.query.promise('contact/duplicate/update_duplicate_clusters_for_contacts', [ affected_contacts_unique ])
  }
}
