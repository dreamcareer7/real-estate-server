...
/**
 * Trims all of the tags and omits duplicate tags.
 * Mutates `contacts`.
 * @param {IContactInput[]} contacts
 * @returns {IContactInput[]}
 */
function normalize_tags (contacts) {
  contacts
    .filter(con => con?.attributes?.length)
    .forEach(con => {
      /** @type {Set<string>} */
      const uniqTags = new Set()

      con.attributes = con.attributes.filter(attr => {
        if (attr?.attribute_type !== 'tag') { return true }

        attr.text = (attr.text || '').trim()
        const lower = attr.text.toLowerCase()

        if (!attr.text || uniqTags.has(lower)) { return false }
...
async function import_json(contacts, user_id, brand_id) {
  contacts = normalize_tags(contacts)
