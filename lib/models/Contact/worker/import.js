  ...
  for (let i = 0; i < header.length; i++) {
    ...
    if (def.name === 'full_address') {
      ...
      try {
        ...
        Context.log(`Unable to parse '${fieldValue}' as a full address`)
...
async function do_import_csv(user_id, brand_id, file_id, owner, mapping_defs) {
  ...
  const {total_contacts, empty_lines, contact_ids} = await import_csv_stream(stream, user_id, brand_id, owner, mapping_defs)
