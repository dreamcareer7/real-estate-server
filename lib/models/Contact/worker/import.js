...
async function prepareMappingDefs (brandId, mappingDefs) {
  ...
  const mappings = _.mapValues(mappingDefs, (m, csvField) => ({
    ..._.pick(m, 'label', 'index', 'is_partner', 'multivalued'),
    def: getDef(m, csvField),
  }))
...
async function do_import_csv(user_id, brand_id, file_id, owner, mapping_defs) {
  ...
  const { total_contacts, empty_lines, contact_ids } = import_csv_stream(
...
const xlsx = (function xlsxModule () {
   ...
   * @param {Record<string, ICSVImporterMappingDef>} mappingDefs
   * @returns {Promise<Record<'totalContacts' | 'emptyLines' | 'contactIds', any>>}
   */
  ...
  async function doImport (userId, brandId, fileId, ownerId, mappingDefs) {
    ...
    const { totalContacts, emptyLines, contactIds } = importStream(
      stream, userId, brandId, ownerId, mappinfDefs
    )
  ...
  async function importXlsx (userId, brandId, fileId, ownerId, mappingDefs) {
    try {
      ...
      sendSlackSupportMessage(userId, brandId, totalContacts, 'XLSX')
