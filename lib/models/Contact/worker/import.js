...
async function prepareMappingDefs (brandId, mappingDefs) {
  ...
  function def ({ attribute_type, attribute_def }, csvField) {
  ...
  const entries = _.mapValues(mappingDefs, (mapping, csvField) => ({
    ..._.pick(mapping, 'index', 'label', 'is_partner', 'multivalued'),
    def: def.bind(null, mapping, csvField),
  }))
  
  /** @type {Map<string, ICSVImporterMapping>} */
  const mappings = new Map(Object.entries(entries))
...
async function importXlsxStream (stream, userId, brandId, owner, mappingDefs) {
  ...
  function hasContent (sheet) {
    if (sheet.rowCount && sheet.columnCount) { return true }
    
    Context.warn('WARN: Skip empty worksheet: ${sheet.name}')
    return false
  }
  ...
  const nonEmptySheets = spreadsheet.worksheets.filter(hasContent)
  if (!nonEmptySheets.length) {
    Context.warn('WARN: Empty spreadsheet?! seems impossible.')
    return { total_contacts: 0, empty_lines: 0, contact_ids: [] }
  }
  
  const promises = nonEmptySheets.flatMap(importSheet)
