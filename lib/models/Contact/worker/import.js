  ...
  return new Promise(function(resolve, reject) {
    ...
    Papa.parse(stream, {
      ...
      async chunk(results, parser) {
        ...
        const res = await Contact.create(contacts, job.data.user_id, job.data.brand_id, {
          activity: false,
          relax: true,
          get: false
        })
...
async function import_json(job) {
  ...
  const res = await Contact.create(job.data.contacts, job.data.user_id, job.data.brand_id, {
    activity: false,
    relax: true,
    get: false
  })
...
async function sendSlackSupportMessage(job, total_contacts, import_type) {
  ...
  const text = format(
    '%s imported %d contacts into %s from %s',
    user.display_name,
    total_contacts,
    brand.name,
    import_type
