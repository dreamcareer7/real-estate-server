...
const debug = require('debug')('rechat:contacts:import')
...
async function import_csv_stream(stream, job) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    Papa.parse(stream, {
      ...
      skipEmptyLines: true,
      beforeFirstChunk() {
        debug(`Job #${job.id}: ` + 'Reading the CSV file...')
      },
      async chunk(results, parser) {
        ...
        debug(`Job #${job.id}: Chunk #${chunkIndex++} contains ${contacts.length} contacts. ${res.length} contacts were created.`)

        parser.resume()
      },

      complete() {
        debug(`Job #${job.id}: Total of ${empty_lines} empty lines were ignored.`)
...
async function sendSlackSupportMessage(job, total_contacts, import_type) {
  ...
  debug(`Job #${job.id}: Finished importing ${total_contacts} from ${import_type} for user ${user.email}.`)
