  ...
  return new Promise(function(resolve, reject) {
    Papa.parse(stream, {
      ...
      step(results, parser) {
        ...
        _.each(
          mappedFields,
          ({ attribute_def: def_id, label, index = 0 }, csvField) => {
            if (!def_id) {
              debug(
...
async function import_csv(job) {
  ...
  await import_json({ data: { user_id, brand_id, contacts } })

  debug('Created the contacts...')
}

async function import_json(job) {
  const res = await Contact.create(job.data.user_id, job.data.brand_id, job.data.contacts, {
    activity: false,
    relax: true,
    get: false
  })
