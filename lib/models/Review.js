...
Review.getAll = (review_ids, cb) => {
  db.query('review/get', [review_ids], (err, res) => {
    if (err)
      return cb(err)
...
const notifyOwner = (options, cb) => {
  ...
  if (!comment) {
    ...
    comment += ` marked as ${options.review.state}`
  }
...
const notifyRoom = (review, message, cb) => {
  ...
  const room = (cb, results) => {
    if (!results.brand)
      return cb()

    if (results.brand.room)
      return cb(null, results.brand.room)

    Brand.getRoom({
      brand: results.brand.id,
