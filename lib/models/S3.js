...
S3.upload = function(bucket, file, cb) {
  ...
  if(!bucket_name || !cdn) {
    return cb('Could find bucket or cdn information');
  }
    ...
    url: (cb) => {
      s3.putObject(params, function(err, data) {
        if (err) {
          ...
          console.log('<- (S3-Transport) Successfully uploaded a file'.blue, key.yellow, 'URL:'.white , url.yellow);
          return cb(null, url);
        }
      });
    },
...
S3.parseSingleFormData = function(req, cb) {
  ...
  form.parse(req, function(err, fields, files) {
    ...
    fs.readFile(fpath, function(err, body) {
      ...
      } catch(err) {console.log('Error parsing Exif info');}
      ...
      fs.unlink(fpath, function(err, ok) {
        ...
        return cb(null, {user: req.user.id, body: body, ext: ext, name: name, metadata: metadata, attachment: true});
      });
    });
  });
};
