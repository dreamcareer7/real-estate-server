...
const getUserInfo = (user_id, cb) => {
  db.query('envelope/user/get', [user_id], (err, res) => {
    ...
    if (res.rows.length < 1)
      return cb(permissionError('Cannot find document authentication'))
    ...
    request(r, (err, res, body) => {
      ...
      fetchTokens({
        ...
        saveUser({
          ...
          request(r, cb) //Retry with the newly acquired token.
        })
      })
    })
  })
...
Envelope.update = (id, cb) => {
  const updateRecipients = (cb, results) => {
    const params = {
      url: `/envelopes/${results.envelope.docusign_id}/recipients`,
      headers,
      json: true
    }
  ...
  const updateEnvelope = (cb, results) => {
    const params = {
      url: `/envelopes/${results.envelope.docusign_id}`,
      headers,
      json: true
    }
...
Envelope.getDocuments = (id, doc, cb) => {
  Envelope.get(id, (err, envelope) => {
    ...
    const params = {
      url: `/envelopes/${envelope.docusign_id}/documents/${doc}`,
      headers,
      encoding: null
    }
...
Envelope.getSignUrl = ({envelope_id, user}, cb) => {
  ...
  const getSignUrl = (cb, results) => {
    const params = {
      url: `${config.docusign.baseurl}/envelopes/${results.envelope.docusign_id}/views/sender`,
      headers,
      method: 'POST',
      body: {
        returnUrl: Url.api({
...
Envelope.resend = (envelope_id, cb) => {
  ...
  const resend = (cb, results) => {
    const params = {
      url: `/envelopes/${results.get.docusign_id}?resend_envelope=true`,
      method: 'PUT',
      headers,
      body: {},
      json: true
