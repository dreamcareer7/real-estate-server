...
Envelope.create = function (envelope, cb) {
  ...
  const docusign = (cb, results) => {
    ...
    results.recipients.forEach((recipient => {
      ...
      results.submissions.forEach((rev, doc_id) => {
        const form = rev.form
        ...
        Object.keys(form.fields).forEach(field_name => {
          ...
          const tab = {
            documentId: doc_id + 1, //Must start from 1
  ...
  async.auto({
    ...
    docusign: ['loadPdfs', 'insert', 'recipients', 'loadForms', docusign],
