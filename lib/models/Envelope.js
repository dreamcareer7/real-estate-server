    ...
    results.recipients.forEach((recipient => {
      ...
      results.submissions.forEach(rev => {
        ...
        Object.keys(form.fields).forEach(field_name => {
          ...
          if (parseInt(field.assigned_to_index) !== roles_counts[recipient.role])
            return
...
Envelope.void = (envelope_id, cb) => {
  async.auto({
    ...
    remote_void: [
      'get',
      (cb, results) => {
        if(!results.get.docusign_id)
          return cb(Error.Generic('Envelope is not synced with Docusign'))
