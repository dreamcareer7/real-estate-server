  ...
  const create = cb => {
    ...
    User.get(user_id, (err, user) => {
      ...
      request.post(params, (err, res, body) => {
        ...
        if (body.newUsers[0].errorDetails)
          return cb(Error.Generic(body.newUsers[0].errorDetails))
...
Envelope.update = (id, cb) => {
  const updateRecipients = (cb, results) => {
    const params = {
      url: `${config.docusign.baseurl}/envelopes/${results.envelope.docusign_id}/recipients`,
      headers,
      json: true
    }
  ...
  const updateEnvelope = (cb, results) => {
    const params = {
      url: `${config.docusign.baseurl}/envelopes/${results.envelope.docusign_id}`,
      headers,
      json: true
    }
...
Envelope.resend = (envelope_id, cb) => {
  ...
  const resend = (cb, results) => {
    const params = {
      url: `${config.docusign.baseurl}/envelopes/${results.get.docusign_id}?resend_envelope=true`,
      headers,
      body: {},
      json: true
    }
