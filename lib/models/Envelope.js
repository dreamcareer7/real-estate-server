...
const fetchTokens = (form, cb) => {
  ...
  const params = {
    url: `${config.docusign.baseurl}/oauth/token`,
    headers,
    form,
    json: true
  }
...
const getLoginInfo = (user_id, token, cb) => {
  ...
  const params = {
    url: `${config.docusign.baseurl}/oauth/userinfo`,
    headers,
    json: true
  }
...
//         emailBlurb: body,
  ...
  const getRoles = (cb, results) => {
    async.map(results.deal.roles, DealRole.get, cb)
  }
  ...
  const saveRoles = (cb, results) => {
    ...
    results.recipients.forEach(recipient => {
      let found = false
      results.roles.forEach(deal_role => {
        if (deal_role.user === recipient.user.id)
          return
    ...
    async.forEach(toSave, saveRecipientAsRole, cb)
  }
  ...
  async.auto({
    ...
    docusign: ['loadPdfs', 'insert', 'recipients', 'loadForms', 'documents', 'title', 'user', docusign],
    updateEnvelope: ['docusign', updateEnvelope],
    deal: cb => Deal.get(envelope.deal, cb),
    roles: ['deal', getRoles],
    saveRoles: ['roles', 'recipients', saveRoles]
  }, done)
}

const updateRecipient = (envelope, recipient, cb) => {
  console.log('Updating recipient', envelope, recipient)

  const issueNotification = (envelope, user, recipient, cb) => {
    console.log('Issuing notification', envelope, user, recipient)
  ...
  EnvelopeRecipient.get(recipient.id, (err, old) => {
    console.log('Getting recipient', err, old, recipient)
    if (err)
      return cb(err)

    console.log('Statuses', old.status. recipient.status)
    if (old.status === fixStatus(recipient.status))
      return cb()

    console.log('Recipient Status check passed')
    ...
    db.query('envelope/recipient/update', [
      ...
      console.log('Updating recipient entry', err)
      if (err)
        return cb(err)
...
Envelope.updateStatus = (id, xml, cb) => {
  console.log('Envelope.updateStatus called', xml)
  ...
  const updateStatus = (cb, results) => {
    console.log('updateStatus closure', results.data)
    update({
  ...
  const updateRecipients = (cb, results) => {
    console.log('updating recipients', results.data.recipients)
