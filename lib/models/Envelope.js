...
Envelope.update = (id, cb) => {
  const updateRecipients = (cb, results) => {
    ...
    request(params, (err, res, body) => {
      ...
      if (res.statusCode !== 200)
        return cb(Error.Generic('Cannot fetch recipient list'))

      const recipients = body.signers.map(signer => {
        const status = signer.status
        return {
          id: signer.customFields[0],
          status: status.charAt(0).toUpperCase() + status.slice(1), // completed => Completed
  ...
  const updateEnvelope = (cb, results) => {
    ...
    request(params, (err, res, body) => {
      ...
      if (res.statusCode !== 200)
        return cb(Error.Generic('Cannot fetch envelope'))
  ...
  const getSignUrl = (cb, results) => {
    const params = {
      ...
      body: {
        ...
        returnUrl: Url.api(`/envelopes/${envelope_id}/signed`, {
          query: {
            token: results.envelope.token
          }
        })
      },
      json: true
