...
Orm.register('envelope', 'Envelope', Envelope)

const EnvelopeRecipient = {}
EnvelopeRecipient.CARBON_COPY = 'CarbonCopy'
EnvelopeRecipient.SIGNER = 'Signer'

Orm.register('envelope_recipient', 'EnvelopeRecipient', EnvelopeRecipient)
...
Orm.register('envelope_document', 'EnvelopeDocument', EnvelopeDocument)

const DocusignAccount = {}

DocusignAccount.get = (id, cb) => {
  DocusignAccount.getAll([id], (err, accounts) => {
    if(err)
      return cb(err)
...
DocusignAccount.getAll = (account_ids, cb) => {
...
Orm.register('docusign_account', 'DocusignAccount', DocusignAccount)
...
Envelope.create = function (envelope, cb) {
  ...
  async.auto({
    ...
    docusign: ['loadPdfs', 'insert', 'recipients', 'forms', 'assignments', 'documents', 'user', docusign],
