...
Envelope.create = function (envelope, cb) {
  ...
  const getRevision = (deal, user, task, cb) => {
    updateTask({deal, user, task}, (err, submission) => {
      ...
      Form.getRevision(submission.last_revision).nodeify(cb)
    })
  }
  ...
  const loadOriginalSubmissionPdf = (rev, cb) => {
  ...
  const loadSubmissionPdf = (rev, cb) => {
    Form.flatSubmissionRevision(rev).nodeify((err, buffer) => {
      if (err)
        return cb(err)
  ...
  const loadPdfs = (cb, results) => {
    async.parallel([
      cb => async.forEach(results.submissions, loadOriginalSubmissionPdf, cb),
  ...
  const _loadAssignments = async submissions => {
    ...
    for (const submission of submissions) {
      const pdf = original_pdfs[submission.id]
  ...
  const addDocuments = (cb, results) => {
    ...
    const prepareSubmissionDocument = d => {
      const submission_id = tasks_revisions[d.task]
      const submission = submissions[submission_id]

      const document = {}
      document.title = submission.title + '.pdf'
      document.task = d.task
      document.submission_revision = submission_id
  ...
  const docusign = async (cb, results) => {
    ...
    results.documents.forEach(document => {
      const documentBase64 = document.submission_revision && pdfs[document.submission_revision].toString('base64')
  ...
  async.auto({
    ...
    tasks: getTasks,
    submissions: ['token', 'tasks', 'user', 'deal', getSubmissions],
