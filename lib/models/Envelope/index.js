...
const getRelevantTasks = async envelope_id => {
  ...
  for(const task of tasks) {
    ...
    if (_.intersection(indexed[task.room].attachments, envelope_attachments).length > 0) {
      relevant.push(task)
    }
  }
...
const updateRecipient = (envelope, recipient, cb) => {
  const issueActivity = (role, cb) => {
    const formatter = submission => {
      const activity = {
        action: 'DealRoleReactedToEnvelopeForSubmission',
        object_class: 'envelope_activity',
        object: {
          type: 'envelope_activity',
          recipient,
...
Envelope.updateStatus = (id, xml, cb) => {
  ...
  const submit = (cb, results) => {
    if (results.data.envelop_status !== 'Completed')
      return cb()
    ...
    getRelevantTasks(results.envelope.id).nodeify((err, tasks) => {
      ...
      async.series(tasks, submitTask, cb)
    })
  }
