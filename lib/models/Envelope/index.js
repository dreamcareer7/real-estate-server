  ...
  const docusign = async (cb, results) => {
    ...
    const promises = results.recipients.map((async recipient => {
      ...
      results.documents.forEach(document => {
        ...
        const rev = submissions[document.submission_revision]
        const form = rev.form
        ...
        Object.keys(form.fields).forEach(field_name => {
          ...
          if (field.assigns.type === 'Checkbox') {
            tab.xPosition -= 5
            tab.yPosition -= 3
...
Envelope.saveDocuments = async envelope => {
  const user = await promisify(User.get)(envelope.created_by)

  const saveDocument = async document => {
    const pdf = await promisify(Envelope.getDocuments)(envelope.id, document.document_id)

    const saved = await promisify(AttachedFile.saveFromBuffer)({
  ...
  const promises = envelope.documents.map(saveDocument)
...
const getRelevantTasks = async envelope_id => {
  ...
  const envelope_submissions = new Set(envelope.documents.map(d => d.submission))
  const envelope_attachments = new Set(envelope.documents.map(d => d.file))
...
const addEnvelopeActivity = async ({envelope_id, formatter, user_id, deal_role, set_author = true}) => {
  ...
  if (set_author && user)
    user_id = await promisify(User.get)(user_id)
...
Envelope.updateStatus = (id, xml, cb) => {
  ...
  const saveDocuments = (results, cb) => {
    Envelope.saveDocuments(results.envelope).nodeify(cb)
  }
