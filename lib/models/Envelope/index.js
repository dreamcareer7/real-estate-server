...
Envelope.create = function (envelope, cb) {
  ...
  const docusign = async (cb, results) => {
     ...
     */
    const recipient_ids = new Set(envelope.recipients.map(r => r.role))
    const isRecipient = role => {
      return recipient_ids.has(role.id)
    }
    const grouped_roles = _.chain(results.roles).filter(isRecipient).groupBy('role').value()
