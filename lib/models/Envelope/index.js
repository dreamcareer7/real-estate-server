...
const addEnvelopeActivity = async ({envelope_id, activity}) => {
  const addActivity = (envelope, task, cb) => {

  }

  const addActivities = (cb, results) => {
    async.map(results.tasks, cb => addActivity(envelope, task, cb))
  }

  const getTasks = (cb, results) => {
    console.log(results.deal)
    Task.getAll(results.deal.tasks).nodeify(cb)
  }
  ...
  const tasks = await Task.getAll(deal.tasks)

  for(const task of tasks) {
    if (!task.submission || !submissions.has(task.submission))
      continue

    activity.object = task.submission

    await promisify(Activity.postToRoom)(task.room, activity)
  }
}
    ...
    db.query('envelope/recipient/update', [
      ...
      User.get(res.rows[0].user, (err, user) => {
        ...
        issueNotification(envelope, user, recipient, err => {
          ...
          issueActivity(envelope, recipient, cb)
        })
      })
    })
  })
