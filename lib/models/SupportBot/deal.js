...
function getListingStatus(deal) {
  ...
  const listing_status = Deal.getContext(deal, 'listing_status')

  if (listing_status) {
    const icon = getStatusColorClass(listing_status)
    return `${icon} ${listing_status}`
  }
}
...
async function moveDealsToBrand(user_id, from_brand, to_brand) {
  Context.log(`moveDealsToBrand: ${user_id} - ${from_brand} - ${to_brand}`)
