...
function getListingStatus(deal) {
  if (!deal.mls_context) return

  const icon = getStatusColorClass(deal.mls_context.listing_status)
  return `${icon} ${deal.mls_context.listing_status}`
}
...
function getMainPart(deal) {
  ...
  if (primaryAgent) {
    const pa_email = primaryAgent.user ? primaryAgent.user.email : primaryAgent.email
    main.author_name = 
