...
const {
  ...
  getPrimaryAgent,
  dealLink,
  getDealStatus: getListingStatus
} = require('./deal')
...
function getEnderType(deal) {
  let ender_type
  if (deal.context.ender_type) {
    ender_type = deal.context.ender_type.text
  } else {
    ender_type = 'Normal'
  }
...
async function getChangeBrandPart(deal) {
  ...
  try {
    pa_brands = await getBrandsForUser(pa.user.id)
  } catch (ex) {
    if (ex instanceof NotFound) return
    throw ex
  }

  const options = pa_brands.map(b => ({
    value: b.id,
...
function formatMailto(email, name) {
  if (!email || email.length === 0) return name
...
function getMainPart(deal) {
  ...
  if (deal.created_by) {
    main.footer =
      'Created by ' +
      formatMailto(deal.created_by.email, deal.created_by.display_name) +
  ...
  if (primaryAgent) {
    const pa_email = primaryAgent.user
      ? primaryAgent.user.email
      : primaryAgent.email
    main.author_name =
      getListingStatus(deal) +
      ' - ' +
      formatMailto(pa_email, primaryAgent.legal_full_name)
  ...
  if (deal.context && deal.context.photo) {
    main.image_url = deal.context.photo.text
  }
