...
async function deleteActiveTriggers ({ flows, steps, eventType, userId }) {
  ...
  for (const t of triggers) {
    ...
    if (knownFlows.has(t.flow)) {
      Context.log('Warning: scheduling a flow (step) that already has got active trigger: ' + t.flow)
      continue
    }
...
async function scheduleSteps(step_ids, origin, user_id, brand_id) {
  ...
  const deleteFailures = await deleteActiveTriggers({
    steps,
    flows,
  ...
  ;[steps, flows] = adaptWithFailures(deleteFailures, steps, flows)

  // TODO: append schedule failures as well
  if (origin.event) {
