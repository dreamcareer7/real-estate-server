...
async function getCampaignBody(origin, flows, user_id, brand_id) {
  if (origin.email) {
    ...
    return Array(flows.length).fill(body)
  }

  if (origin.template_instance) {
    const body = getHtmlFromTemplateInstance(origin.template_instance)
    return Array(flows.length).fill(body)
  }

  if (origin.template) {
    return getHtmlFromTemplate(flows, origin.template, user_id, brand_id)
  }
...
async function getHtmlFromTemplate(flows, template_id, user_id, brand_id) {
  ...
  if (!template.inputs.includes('contact')) {
    ...
    return Array(flows.length).fill(rendered)
  }
