...
async function scheduleSteps(step_ids, origin, user_id, brand_id) {
  ...
  if (origin.event) {
    await scheduleEventTrigger(flows, steps, origin, brand_id, user)
  } else {
    const sch = await scheduleEmailTrigger(flows, steps, origin, brand_id, user)
 ...
 * @param {IUser} user
 */
async function scheduleEventTrigger(flows, steps, origin, brand_id, user) {
  ...
  return Trigger.create(steps.map(step => {
    ...
    return {
      ...
      time: origin.time,
      effective_at: new Date(moment.tz(user.timezone).startOf('day').valueOf()).toISOString(),
    }
  }))
}
 ...
 */
async function scheduleEmailTrigger(flows, steps, origin, brand_id, user) {
  const {
    campaignIds,
    filteredFlows,
    failures: flowFailures,
  } = await createDraftCampaigns(flows, origin, user.id, brand_id)
