...
class FlowStep {
   ...
   */
  async get(id) {
    const steps = await this.getAll([id])
   ...
   * @param {UUID[]} step_ids
   */
  async delete(step_ids, user_id, brand_id) {
    const deleted_steps = await db.select('flow/step/delete', [
      step_ids,
      user_id
    ])

    const events = deleted_steps.filter(s => s.crm_task).map(s => s.crm_task)
    const emails = deleted_steps.filter(s => s.email).map(s => s.email)

    for (const ev of events) {
      await CrmTask.remove(ev, user_id)
    }
...
FlowStep.prototype.associations = {
  crm_task: {
    model: 'CrmTask',
    collection: false,
    enabled: false,
