...
class Flow {
  ...
  async _createSteps(
    ...
    await FlowStep.create(brand_id, user_id, moment.unix(starts_at).startOf('day').unix(), brand_steps, steps)
  }
  ...
  async enrollContacts(brand_id, user_id, brand_flow_id, starts_at, brand_steps, contact_ids) {
    ...
    await this._createSteps(brand_id, user_id, flow_ids, starts_at, brand_steps, contact_ids)
  ...
  //   const ids_to_remove = steps.filter(s => s.)
  // }
  ...
  async disableSteps(user_id, flow_id, brand_steps) {
    ...
    await FlowStep.delete(user_id, existing_steps)
  }
}
