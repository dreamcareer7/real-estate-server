...
class Flow {
  ...
  async stop(user_id, flow_id) {
    /**
     * 1. Set deleted_at and deleted_by for the flow
     * 2. 
     */
  }

  /**
   * Enables a step from flow template
   * @param {UUID} user_id 
   * @param {UUID} flow_id 
   * @param {UUID} brand_flow_step_id 
   */
  async enableStep(user_id, flow_id, brand_flow_step_id) {
