...
BrandChecklist.createAll = async checklists => {
  const defaults = {
    brand: null,
    title: null,
    deal_type: null,
    ...
    is_terminatable: null,
    tab_name: null,
  }

  const q = sq.insert({ autoQuoteFieldNames: true, nameQuoteCharacter: '"' })
    .into('brands_checklists')
    .setFieldsRows(checklists.map(c => ({...defaults, ...c})))

  // @ts-ignore
  q.name = 'brand/checklist/create_all'
