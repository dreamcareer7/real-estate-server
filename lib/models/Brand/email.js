const db = require('../../utils/db')
const Orm = require('../Orm')

const BrandEmail = {}
global['BrandEmail'] = BrandEmail

BrandEmail.get = async id => {
  const emails = await BrandEmail.getAll([id])
  if (emails.length < 1)
    throw Error.ResourceNotFound(`Brand Email ${id} not found`)

  return emails[0]
}

BrandEmail.getAll = async ids => {
  const res = await db.query.promise('brand/email/get', [ids])
  return res.rows
}

BrandEmail.create = async email => {
  const res = await db.query.promise('brand/email/insert', [
    email.created_by,
    email.brand,
    email.name,
    email.goal,
    email.subject,
    email.include_signature,
    email.body
  ])

  return BrandEmail.get(res.rows[0].id)
}

BrandEmail.update = async email => {
  await db.query.promise('brand/email/update', [
    email.id,
    email.name,
    email.goal,
    email.subject,
    email.include_signature,
    email.body
  ])

  return BrandEmail.get(email.id)
}

BrandEmail.delete = async id => {
  await db.query.promise('brand/email/remove', [
    id
  ])
}

BrandEmail.getByBrand = async brand => {
  const res = await db.query.promise('brand/email/by_brand', [brand])

  const ids = res.rows.map(r => r.id)

  return BrandEmail.getAll(ids)
}

BrandEmail.associations = {}

Orm.register('brand_email', 'BrandEmail')

module.exports = BrandEmail
