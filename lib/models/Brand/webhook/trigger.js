...
const handler = async ({ webhook, payload, event, topic, timestamp }) => {
  ...
  try {
    ...
    // Context.log(`Calling webhook endpoint failed with status code ${ex.statusCode}. URL: ${webhook.url}`)
    throw new Error(`Calling webhook endpoint failed with status code ${ex.statusCode}. URL: ${webhook.url}`)
  }
}
