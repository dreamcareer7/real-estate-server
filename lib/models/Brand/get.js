...
const BrandConstant = require('./constants')
...
const getFirstBrokerageInBrandHierarchy = async (brandId) => {
  const parent_ids = await getParents(brandId)
  const parents = await getAll(parent_ids)

  let brand
  for(const parent of parents) {
    if (parent.brand_type !== BrandConstant.BROKERAGE)
      continue
...
module.exports = {
  ...
  getByParent,
  getParents,
  getFirstBrokerageInBrandHierarchy
}
