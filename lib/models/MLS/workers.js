...
const cargo = async.cargo((photos, cb) => {
  Photo.create(photos).nodeify(cb)
})

const processPhoto = async data => {
...
async function mls_listing_validate(data) {
  return promisify(Listing.updateStatus)({
    muis: data.muis,
    mls: data.mls,
    status: 'Cancelled'
  })
...
module.exports = {
  ...
  mls_photo: peanar.job(jobDef({ handler: mls_photo, name: 'mls_photo', queue: 'MLS.Photo', contexr: false })),
