...
async function retriveAndSave(data) {
  ...
  Socket.send(data.doneEvent, data.userID,
    err => console.log(`Error: Sending data to socket event ${data.doneEvent} failed ==> ${err}`))
}
...
module.exports = {
  redirectHandler: function (expressApp) {
    ...
    expressApp.get('/authorize-ms-graph', async (req, res) => {
      ...
      Socket.send(state.authSuccessEvent, state.user,
        err => console.log(`Error: Sending data to socket event ${state.authSuccessEvent} failed ==> ${err}`))
