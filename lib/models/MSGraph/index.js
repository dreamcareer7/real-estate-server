...
async function authorizationDone(data, res) {
  try {
...
async function retriveAndSave(data, res) {
  ...
  console.log('Adding new contacts done')
...
module.exports = {
  redirectHandler: function (expressApp) {
    require('./web-routes')(expressApp, authorizationDone)
    ...
    expressApp.get('/authorize-ms-graph', async (req, res) => {
      ...
      accessToken && await retriveAndSave({
        userID: state.user,
