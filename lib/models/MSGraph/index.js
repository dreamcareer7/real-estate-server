...
async function authorizationDone(data) {
  try {
    ...
    const currentGraphContacts = await dbAdapter.getCurrentMSGraphContacts(data.authInfo.user)
    ...
    await dbAdapter.saveContacts(data.authInfo.user, totallyNew)
    await dbAdapter.updateContactForUser(data.authInfo.user, changed)
    await dbAdapter.addAttributeToContactForUser(data.authInfo.user, addAttribute)
