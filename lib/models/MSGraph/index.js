...
module.exports = {
  redirectHandler: function (expressApp) {
    ...
    expressApp.post(MSRedirectPath.pathname, bp, async (req, res) => {
      ...
      try {
        ...
        const status = {
          ...
          clientMessage: 'Something went wrong when we tried to fetch your data.'
        }
        if (e.statusCode && e.statusCode === 404) {
          status.logMessage = 'Outlook sent a 404, now sending failEvent to socket and terminating the connection'
          status.clientMessage = 'Outlook does not give us your data. It seems you are using an organizational account.'
          status.errorCode = 404
        }
        console.log(status.logMessage, e)
    ...
    expressApp.get('/authorize-ms-graph', async (req, res) => {
      ...
      //     Socket.send(state.failEvent, state.user, [404, 'Outlook does not give us your data. It seems you are using an organizational account.'], err => err && console.error(`Error: Sending data to socket event ${state.failEvent} failed ==> ${err}`))
      //     end()
      //     return res.end('Outlook does not give us your data. It seems you are using an organizational account.')
      //   }
      // }
      // end()
      // res.end('All done. Please close this window.')
