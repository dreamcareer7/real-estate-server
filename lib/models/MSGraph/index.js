...
module.exports = {
  redirectHandler: function (expressApp) {
    ...
    expressApp.post(MSRedirectPath.pathname, bp, async (req, res) => {
      ...
      Socket.send(stateData.authSuccessEvent, stateData.user, [], err => console.log(`Error: Sending data to socket event ${stateData.authSuccessEvent} failed ==> ${err}`))
      await authorizationDone({code, stateData})
      Socket.send(stateData.doneEvent, stateData.user, [], err => console.log(`Error: Sending data to socket event ${stateData.doneEvent} failed ==> ${err}`))
      end()
    ...
    expressApp.get('/authorize-ms-graph', async (req, res) => {
      ...
      Socket.send(state.doneEvent, state.user, [], err => console.log(`Error: Sending data to socket event ${state.doneEvent} failed ==> ${err}`))
      end()
      res.end('All done. Please close this window.')
    })
  }
