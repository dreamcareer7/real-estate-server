...
const MS_GRAPH_ERROR_CODE = 70000
...
async function retrieveAndSave(data) {
  ...
  return dbAdapter.addAttributeToContactForUser(data.userID, addAttribute)
}
...
module.exports = {
  redirectHandler: function (expressApp) {
    ...
    expressApp.post(MSRedirectPath.pathname, bp, async (req, res) => {
      ...
      console.log('Sending Done event using socket after outlook authorization')
    ...
    expressApp.get('/authorize-ms-graph', async (req, res) => {
      ...
      try {
        ...
        if (e.error_codes && e.error_codes.includes(MS_GRAPH_ERROR_CODE)) {
      ...
      res.end('All done. Please close this window.')
      console.log('Sending Done event using socket without outlook authorization')
