...
async function updateContactForUser(userID, contactProps) {
  contactProps.forEach(x => {
    if (x.prop === 'name') {
      Contact.patchAttribute(x.id, userID, x.propID, x.prop, {type: x.prop, ...x.value}, (err, res) => {
        if (err) {
          return console.log('Error updating attribute ', x.prop, ' with id', x.id)
        }
        console.log('Attributes updated successfully', x.prop)
      })
    } else if (x.prop === 'address') {
      // TODO-JAVAD: This should be fixed since is not updating addresses
      Contact.patchAttribute(x.id, userID, x.propID, x.prop, {[x.prop]: x.value}, (err, res) => {
        if (err) {
          return console.log('Error updating attribute ', x.prop, ' with id', x.id)
        }
        console.log('Attributes updated successfully', x.prop)
...
async function addAttributeToContactForUser(userID, contactProps) {
  contactProps.forEach(x => {
    Contact.addAttribute(x.id, userID, {type: x.prop, [x.prop]: x.value}, (err, res) => {
      if (err) {
