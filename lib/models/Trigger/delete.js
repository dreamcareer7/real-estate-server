...
const BrandTrigger = require('./brand_trigger/exclusions')
...
async function deleteTriggers(ids, user_id) {
  Context.log(`Deleting ${ids.length} triggers...`)
  ...
  const triggersByBrands = _.groupBy(triggers, 'brand')
  for (const brandId in triggersByBrands) {
    const triggersByEventType = _.groupBy(
      triggersByBrands[brandId],
      'event_type'
    )
    for (const eventType in triggersByEventType) {
      await BrandTrigger.deleteExclusion(
        brandId,
        eventType,
        triggersByEventType[eventType].filter(trigger=>trigger.origin).map(trigger => trigger.contact)
      )
    }
  }
