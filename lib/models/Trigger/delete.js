...
async function deleteTriggers(ids, user_id) {
  Context.log(`Deleting ${ids.length} triggers: ${ids.slice(0, 50).join(', ')}${ids.length > 50 ? '...' : ''}`)

  let ctx = Context.getActive()
  function logActiveQueriesLater () {
    if (ctx !== Context.getActive()) { return }
    
    setTimeout(function () {
      if (ctx !== Context.getActive()) { return }
      
      sql.select(
        'SELECT * FROM pg_stat_activity WHERE application_name = $1::text',
        [Context.getActive()?.id]
      )
        .then(r => Context.log(`Active queries: ${JSON.stringify(r)}`))
        .catch(Context.error)
        .finally(logActiveQueriesLater)
    }, 1000)
  }
  logActiveQueriesLater()
