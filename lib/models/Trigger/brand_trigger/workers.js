...
async function updateTriggersHandler(brandTriggerId, overrideManualTriggers) {
  ...
  const triggerIdsToRemove = overrideManualTriggers ?
    await Trigger.filter({
      ...COMMON_TRIGGER_FILTERS,
      brand: bt.brand,
...
async function dateAttributesCreated({ brand: brandId, attributes }) {
  ...
  const exclusions = await BrandTrigger.getExclusions(bts.map(bt=>bt.id))

  const attributesToCreateTriggersAndCampaigns = 
    attributes.filter(attr => !exclusions.find(exclusion => attr.contact === exclusion.contact))
  const brandTriggerMap = new Map(Object.entries(keyBy(bts, 'event_type')))

  const contactIdMap = 
    attributesToCreateTriggersAndCampaigns.reduce((map, { attribute_type, contact }) => {
...
async function triggerEffectivelyExecuted(brandId, triggerId, contactId) {
  const bts = await BrandTrigger.getForBrand(brandId)
  if (!bts.length) {
    return
  }
  ...
  await createCampaignsAndTriggers(bt, [contactId])
}
...
module.exports = {
  ...
  triggerEffectivelyExecuted: peanar.job({
    handler: triggerEffectivelyExecuted,
