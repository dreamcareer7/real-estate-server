 ...
 */
async function getContactsHavingAttribute (brandId, attrType, options) {
  return Contact.getForBrand(
    brandId,
    [{ attribute_type: attrType, invert: true, value: null }],
    options
  )
}
...
async function updateTriggersHandler (brandTriggerId, overrideManualTriggers) {
  ...
  /** @type {UUID[]} */
  let excludingContactIds = []

  if (!overrideManualTriggers) {
    excludingContactIds = await Trigger
      .filter({ ...filters, origin_ne: bt.id })
      .then(Trigger.getAll)
      .then(res => res.map(t => /** @type {any} */(t).contact))
  }

  const contacts = await getContactsHavingAttribute(
    bt.brand,
    bt.event_type,
    { excludes: excludingContactIds }
  )
  if (!contacts.length) { return }
