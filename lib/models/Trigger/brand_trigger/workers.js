...
/** @type {(_: any[]) => IContactTrigger[]} */
const asContactTrigger = triggers => triggers
...
function findContactsWithAttr (contacts, attrType) {
  return contacts.filter(c => c.attributes.some(
    a => a.attribute_type === attrType
  ))
}
...
async function filterAttributesHavingNoTrigger (brandId, attributes) {
  ...
  const uniqueKey = attr => `${attr.contact}#${attr.attribute_type}`
  const uniqueKeys = new Set(attributes.map(uniqueKey))

  return attributes.filter(a => !uniqueKeys.has(uniqueKey(a))) 
}
...
async function contactsMerged ({ brand_id, contact_ids }) {
  ...
  const triggers = await Trigger.getAll(triggerIds).then(asContactTrigger)
  ...
  function toContactIdsMap (map, bt) {
    ...
    if (!cids.length) { return map.set(bt, cids) }
