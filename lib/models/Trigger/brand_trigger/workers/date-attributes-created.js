...
async function dateAttributesCreated({ brand: brandId, attributes }) {
  Context.log(
    `
      date-attributes-created : brand: ${brandId}
  ...
  attributes = await filterAttributesHavingNoFlowTrigger(brandId, attributes)
  ...
  for (const [bt, contactIds] of contactIdMap.entries()) {
    const exclusions = await BrandTriggerExclusion.getExcludedContactIds(bt.brand, bt.event_type)
    const notExcludedContactIds = contactIds.filter((contactId) => 
      !exclusions.includes(contactId)
    )
    contactsToExclude = []
    contactsToCreateTriggersFor = []
    notExcludedContactIds.forEach(contactId => {
