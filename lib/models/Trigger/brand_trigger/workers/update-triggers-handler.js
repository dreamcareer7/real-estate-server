...
async function updateTriggersHandler(brandTriggerId, overrideManualTriggers) {
  ...
  if (overrideManualTriggers) {
    ...
    const excludedContactIds = await BrandTriggerExclusion.getExcludedContactIds(bt.brand, bt.event_type)
    await BrandTriggerExclusion.create(
      bt.brand,
      bt.event_type,
      manualTriggers
        .filter(trigger=>!excludedContactIds.includes(trigger.contact))
