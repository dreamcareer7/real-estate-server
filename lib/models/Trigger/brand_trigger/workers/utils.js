...
async function createCampaigns(brandTrigger, contacts) {
  ...
  if (brandTemplate && brandTemplate?.template) {
    return createCampaignsByTemplate(brandTrigger, contacts, brandTemplate)
  } 
 ...
 * @param {BrandTrigger} brandTrigger
 * @param {IContact[]} contacts
 * @param {object} brandTemplate
 * @returns {Promise<UUID[]>}
 */
async function createCampaignsByTemplate(brandTrigger, contacts, brandTemplate) {
