...
async function createCampaigns(brandTrigger, contacts) {
  if (brandTrigger.template) {
    return createCampaignsByTemplate(brandTrigger, contacts)
  } 
  return createCampaignsByTemplateInstance(brandTrigger, contacts)
 ...
 * @returns {Promise<UUID[]>}
 */
async function createCampaignsByTemplate(brandTrigger, contacts) {
 ...
 * @returns {Promise<UUID[]>}
 */
async function createCampaignsByTemplateInstance(brandTrigger, contacts) {
  const campaignInfos = contacts.map((contact) => ({
    created_by: brandTrigger.created_by,
    brand: brandTrigger.brand,
    individual: true,
    due_at: null,
 ...
 * @param {{effective_at?: string}} options
 * @returns {Promise<UUID[]>}
 */
async function createTriggers(brandTrigger, contactIds, campaignIds, options = {}) {
  assert.equal(contactIds.length, campaignIds.length)

  /** @type {(_: [UUID, UUID]) => ITriggerInput} */
  const mapper = ([contactId, campaignId]) => {
    const trigger = {
      /** @type {'schedule_email' | 'create_event'} */ action: 'schedule_email',
    ...
    if (options.effective_at) {
      console.log(new Date(options.effective_at))
      trigger.effective_at = options.effective_at
    }
    return trigger
  }
 ...
 * @param {{effective_at?: string}} options
 */
async function createCampaignsAndTriggers (bt, contactIds, options = {}) {
  ...
  return createTriggers(bt, contactIds, campaignIds, options)
}
