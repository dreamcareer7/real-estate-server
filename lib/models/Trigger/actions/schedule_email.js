...
async function clone(trigger) {
  ...
  if (!Array.isArray(populated.recipients)) {
    // FIXME: Why are we here?!
    return null
  }
  ...
  const [cloned_campaign] = await EmailCampaign.createMany([{
    ...
    due_at: null,
    from: campaign.from,
    to: populated.recipients.filter(r => r.send_type === TO),
    cc: populated.recipients.filter(r => r.send_type === CC),
    bcc: populated.recipients.filter(r => r.send_type === BCC),
