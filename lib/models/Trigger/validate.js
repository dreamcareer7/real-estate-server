...
async function validateForCreate(trigger) {
  ...
  if (trigger.contact) {
    const accessIndex = await Contact.hasAccess(trigger.brand, 'read', [trigger.contact])
    expect(accessIndex.get(trigger.contact), 'Contact is not accessible from the specified brand').to.be.true
  }
}
...
async function validateForUpdate(data, trigger) {
  ...
  expect(trigger.executed_at, 'unable to update an already executed trigger').to.be.null
