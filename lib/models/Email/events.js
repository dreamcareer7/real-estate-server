...
const addEvents = async function (messageIds, event, origin) {
  ...
  if ( origin === 'outlook' ) {
    emailIdss = await getByMicrosoftMessageIds(messageIds)
  }

  if ( origin === 'gmail' ) {
    emailIdss = await getByGoogleMessageIds(messageIds)
  }

  const records = await EmailCampaignEmail.getAll(emailIdss)
