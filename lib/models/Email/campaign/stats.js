...
const rebuildCampaignStats = async (campaign_id) => {
  ...
  const { rows } = await db.query.promise('email/campaign/update-stats', [campaign_id])
...
const updateStats = async (delay = config.email.stat_update_delay) => {
  ...
  for (const id of ids) {
    ...
    Context.log('Debug-Notifs step-7 key', key)
    Context.log('Debug-Notifs step-8 status', status)
