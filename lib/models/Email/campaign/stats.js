...
const updateStats = async (delay = config.email.stat_update_delay) => {
  ...
  for(const member of members) {
    const obj = JSON.parse(member) // { campaign_id, email_event_id }

    const { rows } = await db.query.promise('email/event/get', [obj.email_event_id])

    await rebuildCampaignStats(member)

    if ( rows[0].event === email_events.opened ) {
      await handleNotifications(rows[0].object, obj.event)
    }
  }
}

const touch = async (campaign_id, email_event_id) => {
  Context.log('Touching campaign', campaign_id, email_event_id)

  return zadd(REDIS_KEY, Date.now(), JSON.stringify({ campaign_id, email_event_id }))
}
