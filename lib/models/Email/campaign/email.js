...
EmailCampaignEmail.getFailedEmails = async id => {
  const ids = db.select('email/campaign/email/get_failed', [id])

  return await EmailCampaignEmail.getAll(ids)
}
...
EmailCampaignEmail.saveError = async (email, ex) => {
  ...
  Context.log(`Failed-to-send-email Ex: ${JSON.stringify(ex)}`)
