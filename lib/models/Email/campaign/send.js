...
const sendIndividualCampaign = async ({email, recipients = [], campaign, user}) => {
  ...
  const cdnized_html = cdnize(email.html)
  ...
  const renderer = recipient => {
    ...
    const html = render(cdnized_html, {
      recipient: contact,
      sender: user,
      email: { url }
    })
