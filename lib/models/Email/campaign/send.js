...
/**
 * @template {{ contact: UUID? }} R
 * @param {R[]} allRecipients
 * @returns {Promise<R[]>}
 */
async function removeUnsubscribedRecipients (allRecipients) {
  const allContactIds = /** @type {UUID[]} */(allRecipients.map(r => r.contact).filter(Boolean))
  if (!allContactIds.length) { return allRecipients }

  const { ids: subscribedContactIds } = await Contact.fastFilter(
    undefined,
    [{ attribute_type: 'Tag', invert: true, value: 'Unsubscribed' }],
    { ids: allContactIds },
  )

  const subscribedContactsSet = new Set(subscribedContactIds)
  return allRecipients.filter(r => !r.contact || subscribedContactsSet.has(r.contact))
}
...
const send = async id => {
  ...
  const recipients = await removeUnsubscribedRecipients(allRecipients)
