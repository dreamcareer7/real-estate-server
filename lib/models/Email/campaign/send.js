...
const sendIndividualCampaign = async ({email, recipients = [], campaign, user}) => {
  ...
  Context.log('gmailSender-3 emails', JSON.stringify(emails))
...
const send = async id => {
  ...
  Context.log('gmailSender-1 campaign', JSON.stringify(campaign))
  ...
  Context.log('gmailSender-2 email', JSON.stringify(email))
