...
async function omitUnsubscribedRecipients (brandId, allRecipients) {
  ...
  const subsEmails = rows.flatMap(r => r.email).map(e => e.toLowerCase())
  const subsEmailsSet = new Set(subsEmails)

  return allRecipients.filter(r => subsEmailsSet.has(r.email.toLowerCase()))
}
