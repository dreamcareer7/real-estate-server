...
const sendNotification = async ({ event, campaign_id, email_id, recipient, count }) => {
  ...
  try {
    ...
    Context.log('Sending-Notification', event, campaign_id, email_id, recipient, count, recipientName)
...
const singleNotification = async (members, event, campaign_id) => {
  ...
  Context.log('Sending-Notification process-single', event, campaign_id, email_id, recipient)
...
const processNotification = async (member) => {
  ...
  /*
    No need to have this logic!
    If members length is zero, then nothing will be executed but redis key deletion at the end of method
  ...
  */
  ...
  if ( members.length > 1 ) {
    ...
    */
    ...
    if (count > 1) {
      Context.log('Sending-Notification process-multi', event, campaign_id, emailCampaign.opened, val)
