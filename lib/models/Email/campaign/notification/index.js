...
const sendNotification = async (origin, event, email, recipient = null) => {
  ...
  let emailCampaignEmail
  let emailCampaign

  try {
    emailCampaignEmail = await getByEmail(email_id)
    emailCampaign      = await getCampaign(emailCampaignEmail.campaign)
  } catch (ex) {
  ...
  if ( !emailCampaignEmail || !emailCampaign ) {
    return
  }
