...
const singleNotification = async (members, event, campaign_id) => {
  try {
    ...
    Context.log('send-single-notification-failed', event, campaign_id, email_id, recipient)
    await sendNotification({ event, campaign_id, email_id, recipient, count: 1 })

  } catch (ex) {

    const emoji = ':skull:'
    const text  = `send-single-notification-failed', ${ex.message}, ${members}, ${event}, ${campaign_id}`
