...
const debounceOpenEvents = async (member, sent) => {
  ...
  const rate  = easeOutQuad(index)
  const res   = ( sent <= 500 ) ? (rate * 2 * 10000) : (rate * 10000) // res : result of calculation
...
const processNotification = async (object, event, email_id) => {
  ...
  try {
    ...
    Context.log('processNotification=> emailCampaign', emailCampaign.id)

    Context.log('processNotification=> isRecipientOwner', isRecipientOwner(emailCampaign, recipient))

    if (isRecipientOwner(emailCampaign, recipient)) {
      return
    }
    ...
    Context.log('processNotification=> isCampaignOlderThanAMonth', isCampaignOlderThanAMonth(emailCampaign.created_at))
