...
const set    = promisify(redis.sadd.bind(redis))
...
const touch = async (rows, event) => {
  ...
  */
  ...
  Context.log('Debug-Notifs step-1 rows', JSON.stringify(rows))
  ...
  Context.log('Debug-Notifs step-2 records', JSON.stringify(records))
  ...
  Context.log('Debug-Notifs step-3 eventHash', eventHash)

  const duplicateEvent = await exists(eventHash) // true if there is at least one open event.

  Context.log('Debug-Notifs step-4 duplicateEvent', duplicateEvent)
  ...
  Context.log('Debug-Notifs step-5 status', status)

  if ( event === email_events.opened && !duplicateEvent ) {
    const key = `opened_notifications_${campaign_id}`

    Context.log('Debug-Notifs step-6 status', status)
