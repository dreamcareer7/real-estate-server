...
async function handleUnsubscribe(object, event) {
  ...
  const recipient = (() => {
    if (!email.to?.length) {
      Context.warn('Email recipients are empty. Handling unsubscription based on mailgun recipient...')
      return object.recipient
    }

    const rec = object.recipient
    return rec ? email.to.find(e => e === rec) : email.to[0]
  })()
  if (!recipient) {
    return Context.warn(`Unable to extract recipient email from payload: ${JSON.stringify(object)}`)
  }
  ...
  const { ids: contactIds } = await Contact.fastFilter(
    brandId,
    userId,
    [{ attribute_type: 'email', operator: 'any', value: recipient }],
    { skipTotal: true },
  )
