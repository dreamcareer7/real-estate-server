...
const touch = async (rows, event) => {
  ...
  */
  ...
  const created_event = records[0]

  const eventHash = Crypto.encrypt(JSON.stringify({
    'event': created_event.object,
    'origin': created_event.origin,
    'message': created_event.message,
    'recipient': created_event.recipient,
    'client-info': created_event['client-info'],
    'geolocation': created_event.object,
  }))
