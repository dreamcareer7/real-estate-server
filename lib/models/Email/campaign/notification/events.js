...
const { getAll: usersGetAll } = require('../../../User/get')
...
const { isCampaignOlderThanAMonth } = require('./helper')
...
const checkForNotification = async (campaign_id, email_event_id) => {
  /*
    ...
    const createdEvent = emailEvents[0]

    const eventHashObj = {
      'event': createdEvent.event,
      'origin': createdEvent.object.origin,
      'recipient': createdEvent.recipient,
      'message': {},
      'client-info': {}
    }

    if ( createdEvent.object.origin === 'outlook' ) {
      eventHashObj.message = createdEvent.object.email
      eventHashObj['client-info'] = createdEvent.object.headers['user-agent']
    }

    if ( createdEvent.object.origin === 'gmail' ) {
      eventHashObj.message = createdEvent.object.email
      eventHashObj['client-info'] = createdEvent.object.headers['user-agent']
    }

    if ( createdEvent.object.origin === 'mailgun' ) {
      eventHashObj.message = createdEvent.object.message
      eventHashObj['client-info'] = createdEvent.object['client-info']
    }

    if ( event === 'opened' || event === 'clicked' ) {
      Context.log('eventHashObj', JSON.stringify(eventHashObj))
    }

    const eventHash = Crypto.encrypt(JSON.stringify(eventHashObj))
    ...
    if ( event === email_events.opened && !duplicateEvent ) {
      ...
      // sadd key event_1 event_2 event_3 ... - ttl: 1 Day ???
  ...
  */
  ...
  const owners = await usersGetAll([emailCampaign.created_by, emailCampaign.from])
  const ownersEmails = owners.map(user => user.email)

  Context.log('Campaign:', campaign_id, 'ownersEmails:', ownersEmails, 'email_event_id:', email_event_id, 'recipient:', emailEvents[0].recipient)

  if (ownersEmails.includes(emailEvents[0].recipient)) {
    return
  }
