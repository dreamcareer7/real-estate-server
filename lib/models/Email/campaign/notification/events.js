...
const touch = async (rows, event) => {
  ...
  */
  ...
  if ( event === 'opened' ) Context.log('Debug-Notifs step-1 rows', JSON.stringify(rows))
  ...
  if ( event === 'opened' ) Context.log('Debug-Notifs step-2 records', JSON.stringify(records))
  ...
  if ( event === 'opened' ) Context.log('Debug-Notifs step-3 eventHash', eventHash)

  const duplicateEvent = await exists(eventHash) // true if there is at least one open event.

  if ( event === 'opened' ) Context.log('Debug-Notifs step-4 duplicateEvent', duplicateEvent)

  if (!duplicateEvent) {
    set(eventHash, '1', 'EX', 5) // expire in 5 seconds
  }

  // temp
  const status = await exists(eventHash)
  if ( event === 'opened' ) Context.log('Debug-Notifs step-5 status', status)

  if ( event === email_events.opened && !duplicateEvent ) {
    const key = `opened_notifications_${campaign_id}`

    if ( event === 'opened' ) Context.log('Debug-Notifs step-6 status', status)
