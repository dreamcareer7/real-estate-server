...
async function getEmailId (object) {
  // gmail / outlook
  if (object?.email && typeof object?.email === 'string') { return object.email }
  ...
  return Email.getByMailgunId(mailgunId)
}
...
async function handleUnsubscribe(object, event) {
  if (event !== email_events.unsubscribed) { return }
  ...
  const recipient = (() => {
    if (typeof object.recipient === 'string') { return object.recipient }
    if (email.to?.length === 1) { return email.to[0] }
    Context.warn(`Unable to extract recipient from email ${email.id} or payload: ${JSON.stringify(object)}`)
  })()
  if (!recipient) { return }

  const campaign = await getCampaign(recipient)
  const { brand: brandId, created_by: userId } = campaign

  // TODO: take take of contact roles (lead assignments)
  const emailFilter = [{ attribute_type: 'email', value: email.to[0] }]
  const { ids: contactIds } = await Contact.fastFilter(brandId, emailFilter)
