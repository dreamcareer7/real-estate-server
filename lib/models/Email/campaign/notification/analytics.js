...
const handleEmailNotification = async (object, event) => {
  ...
  if ( origin === 'mailgun' ) {
    email = (object.message && object.message.headers) ? object.message.headers['message-id'] : null
  }
  ...
  if ( event === config.email_events.opened ) {
    await debounceOpenEvents(email)
  }
  ...
  if ( event === config.email_events.clicked ) {
    ...
    */
    await sendNotification(event, email, recipient)
  }
}

const addEvent = async ({ object, event, created_at }) => {
  Context.log('sendNotification ==> step 1 ==>', event, JSON.stringify(object))
