const { email_events, codeTwoIPs } = require('./constants')
...
const filterOutCodeTwo = (object) => {
  ...
  return codeTwoIPs.includes(ip)
}

const addEvent = async ({ object, event, created_at }) => {   
  if (filterOutGoogleBot(object) || filterOutCodeTwo(object)) {
    return
  }
