...
const sendIndividualCampaign = async ({email, recipients = [], campaign, user}) => {
  Context.log('------ sendIndividualCampaign campaign', campaign)
  Context.log('------ sendIndividualCampaign recipients', recipients)
  Context.log('------ sendIndividualCampaign email', email)
  Context.log('------ sendIndividualCampaign user', user)
  ...
  Context.log('sendIndividualCampaign emails', emails)
  Context.log('sendIndividualCampaign viaMicrosoft', viaMicrosoft)
  Context.log('sendIndividualCampaign viaGoogle', viaGoogle)
  Context.log('sendIndividualCampaign viaMailgun', viaMailgun)

  let saved
  
  if(viaMailgun)
    saved = await Email.createAll(emails)

  if(viaGoogle)
    saved = await sentViaGmail(emails)

  if(viaMicrosoft)
    saved = await sentViaOutlook(emails)

  Context.log('sendIndividualCampaign saved', saved)
...
const sendCampaign = async ({email, recipients = [], campaign, user}) => {
  ...
  const promises = [
    renderSubject({email, user}),
    renderHtmlBody({email, user}),
    renderTextBody({email, user}),
  ]
