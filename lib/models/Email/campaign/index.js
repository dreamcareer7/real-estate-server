...
function notifyBrandUsers(socket_event, user, brand, ids) {
  Socket.send(
    socket_event,
    brand,
    [{ user, brand, ids }],
    ...
    err => {
      if (err) Context.error(`>>> (Socket) Error sending ${socket_event} socket event.`, err)
    }
  )
}

EmailCampaign.on('create', ({ ids, user, brand }) => {
  notifyBrandUsers('email_campaign:create', user, brand, ids)
})
EmailCampaign.on('send', ({ ids, user, brand }) => {
  notifyBrandUsers('email_campaign:send', user, brand, ids)
})
EmailCampaign.on('delete', ({ ids, user, brand }) => {
  notifyBrandUsers('email_campaign:delete', user, brand, ids)
})
