...
const sendSpecial = async ({email, recipients = [], campaign, user}) => {
  const grouped = _.groupBy(recipients, 'send_type')
...
EmailCampaign.send = async (campaign) => {
  ...
  for (const attachment of attachments) {
    attachment.file_name = filesById[attachment.file].name
    attachment.url  = filesById[attachment.file].url
    attachment.type = filesById[attachment.file].mime
  }
