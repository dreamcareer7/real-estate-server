...
const handleRemoteAttachments = async (campaign) => {
  ...
  for (const att of remoteAttachments) {
    ...
    // temp.host === host || 
    if ( att.url.indexOf('/emails/google/attachments/') > 0 || att.url.indexOf('/emails/microsoft/attachments/') > 0 ) {
      const hash   = att.url.split('/').pop()
      const attObj = JSON.parse(Crypto.decrypt(decodeURIComponent(hash)))
    
      const { attachment } = await GoogleMessage.downloadAttachment(attObj.credential_id, attObj.message_remote_id, attObj.attachment_id)
