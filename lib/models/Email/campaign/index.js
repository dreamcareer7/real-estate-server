 ...
 * @param {UUID} eceid email_campaign_email_id
 * @param {String} origin gmail/outlook
 */
const handleTracking = async function (html, eceid, origin) {  
  if ( origin === 'mailgun' ) {
    return html
  }
  ...
  const enc   = Crypto.encrypt(JSON.stringify({ eceid, origin }))
...
const sendIndividualCampaign = async ({email, recipients = [], campaign, user}) => {
  ...
  const origin = ( !campaign.google_credential && !campaign.microsoft_credential ) ? 'mailgun' : ( campaign.google_credential ? 'gmail' : 'outlook' )
  
  const renderer = recipient => {
    const contact  = indexed[recipient.contact] || { email: recipient.email }
    const entities = { recipient: contact, sender: user }

    const subject = render(email.subject, entities)
    const text    = render(email.text, entities)
    const html    = handleTracking(render(email.html, entities), 'eceid', origin)
...
const saveError = async (campaign, error) => {
  return db.query.promise('email/campaign/save-error', [
    campaign.id,
    Context.getId(),
    error,
  ])
...
EmailCampaign.getAll = async ids => {
  ...
  const { contact, limit } = conditions || {}
...
EmailCampaign.send = async (campaign) => {
  ...
  try {
    ...
    await saveError(campaign, error)
 ...
 * @param {string} key
 */
EmailCampaign.saveThreadKey = async (id, key) => {
  return db.update('email/campaign/save_thread_key', [id, key])
}

EmailCampaign.publicize = campaign => {
  delete campaign.failure
}
