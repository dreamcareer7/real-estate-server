...
const validateAttachmentsSize = async (ids, limit) => {  
  ...
  for (const file of files) {
    const { chars } = await AttachedFile.getFileSize(file)
  ...
  for(const campaign of campaigns) {    
    if (campaign.attachments) {
      ...
      for (const attachment of campaign.attachments) {
        if (!attachment.file)
          throw Error.Validation('Attachment.file is missed!')

        if (!viaMailgun) {
          if (attachment.is_inline) {
            if (!attachment.content_id)
              throw Error.Validation('Attachment.content_id is missed!')
          }
        }
      ...
      if (viaGoogle)
        limit = config.google_integration.attachment_size_limit
...
const validate = async campaign => {
  ...
  if ( viaMicrosoft || viaGoogle ) {
    ...
    if ( credential.user !== campaign.created_by )
      throw 'Invalid user!'
  
    if (credential.revoked)
      throw 'Credential is revoked!'
  
    if ( credential.deleted_at )
      throw 'Credential is deleted!'
  }
