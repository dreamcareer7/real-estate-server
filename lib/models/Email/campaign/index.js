...
const handleFileAttachments = (campaign) => {
  ...
  const fileAttachments = campaign.attachments.filter(att => { if (att.file) return true })
...
const handleRemoteAttachments = async (campaign) => {
  ...
  const remoteAttachments = campaign.attachments.filter(att => { if (att.url) return true })
  ...
  if (campaign.attachments) {
    for (const attachment of campaign.attachments) {
      if ( !attachment.file && !attachment.url )
        throw Error.Validation('Attachment\'s file or url is missed!')

      if ( attachment.file && attachment.url )
        throw Error.Validation('It is not allowed to send both attachments\'s file and url')
