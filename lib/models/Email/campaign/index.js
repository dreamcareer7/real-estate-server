 ...
 * @param {IEmailCampaign[]} campaigns 
 */
const insertEmailCampaignAttachments = async (campaigns) => {
  const attachments = []

  for(const campaign of campaigns) {

    const ids   = campaign.attachments.map(att => att.file)
    const files = await AttachedFile.getAll(ids)

    for (const attachment of campaign.attachments) {
      for (const file of files) {
        if ( attachment.file === file.id ) {
          attachment.file_name = file.name
          attachment.link = file.url
          attachment.type = file.mime
        }
      }
    }
...
EmailCampaign.createMany = async (campaigns) => {
  ...
  await insertEmailCampaignAttachments(/** @type {IEmailCampaign[]} */(campaigns))
...
EmailCampaign.send = async (campaign) => {
  ...
  // const emailCampaignAttachments = await EmailCampaignAttachment.getByCampaign(campaign.id)
  ...
  const email = {
    ...
    campaign_attachments: campaign.attachments,
