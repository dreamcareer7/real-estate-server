...
const regexp = /{{\s*((recipient|sender)+)\.([\w]+)\s*(or "?(([\w])+)")*\s*}}/gi

const render = async (template, entities) => {
  ...
  for(const match of matches) {
    ...
    const value = model[attribute] || default_value
...
const sendIndividualCampaign = async ({email, recipients = [], campaign, user}) => {
  ...
  const renderer = async recipient => {
    ...
    const text = await render(email.text, {
      recipient: contact,
      sender: user
    })

    const html = await render(email.html, {
      recipient: contact,
      sender: user
    })
...
const validate = async campaign => {
  ...
  for (const attachment of campaign.attachments) {
    expect(Boolean(!attachment.file) && Boolean(!attachment.url), 'Attachment\'s file or url is miss!').to.equal(false)
