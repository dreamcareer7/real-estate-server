...
EmailCampaign.send = async (campaign) => {
  ...
  try {
    ...
    if ( campaign.individual ) {
      // outlook => 30 messages per minute (per minute limit. will be handled in outlook-queue)
      // gmail   => 2000 daily


      // Lines 726-732 are temporary, Lines 736-750 are main logic
      campaign.headers = {}
      campaign.microsoft_credential = ''
      campaign.google_credential = ''
      ...
      /*
      if ( (campaign.google_credential && recipients.length > 750) || (campaign.microsoft_credential && recipients.length > 750) ) {
        campaign.headers = {}
        campaign.microsoft_credential = ''
        campaign.google_credential = ''
        email.microsoft_credential = ''
