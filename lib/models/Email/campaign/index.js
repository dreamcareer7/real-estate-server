...
EmailCampaign.createMany = async (campaigns) => {
  ...
  await insertEmailCampaignAttachments(campaigns)
...
EmailCampaign.send = async (campaign) => {
  ...
  let emailCampaignAttachments = []

  if (!viaMailgun) {
    emailCampaignAttachments = await EmailCampaignAttachment.getByCampaign(campaign.id)
  }
  ...
  const email = {
    ...
    attachments: viaMailgun ? campaign.attachments : emailCampaignAttachments,
