...
const renderSubject = ({email, user, recipient}) => {
  const { subject = '' } = email
...
const renderHtmlBody = ({email, user, recipient}) => {
  const { html = '' } = email
...
const renderTextBody = ({email, user, recipient}) => {
  const { text = '' } = email
...
const sendIndividualCampaign = async ({email, recipients = [], campaign}) => {
  ...
  const emails = recipients.map(recipient => {
    return {
      ...email,
      subject: renderSubject({email, recipient, user}),
      html: renderHtmlBody({email, recipient, user}),
      text: renderTextBody({email, recipient, user}),
      tags: [campaign.brand],
      to: [recipient.email]
    }
  })
