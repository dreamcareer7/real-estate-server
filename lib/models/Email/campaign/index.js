...
EmailCampaign.send = async (campaign) => {
  ...
  try {
    ...
    if ( campaign.individual ) {
      ...
      if ( campaign.google_credential && recipients.length > 750 ) {
        campaign.headers = {}
        campaign.microsoft_credential = ''
        campaign.google_credential = ''
        ...
        // outlook => per minute limit (will be handled in outlook-queue)
        // mailgun => no any limit
        await sendIndividualCampaign({campaign, recipients, email, user})
      }
      ...
      if ( (campaign.google_credential && recipients.length > 100) || (campaign.microsoft_credential && recipients.length > 500) ) {
        campaign.headers = {}
        campaign.microsoft_credential = ''
        campaign.google_credential = ''
        email.microsoft_credential = ''
...
EmailCampaign.associations = {
  ...
  from: {
    model(c, cb) {
      if (c.google_credential) return cb('GoogleCredential')
      if (c.microsoft_credential) return cb('MicrosoftCredential')
      return cb('User')
    },
    id(c, cb) {
      if (c.google_credential) return cb(c.google_credential)
      if (c.microsoft_credential) return cb(c.microsoft_credential)
      return cb(c.from)
    },
    enabled: false
  },
