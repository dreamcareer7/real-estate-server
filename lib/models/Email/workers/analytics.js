...
const CLICK_PROCESS_DELAY = 5
const OPEN_PROCESS_DELAY  = 60
...
const debounceEvents = async (object, event, created_at) => {
  let redis_key = CLICK_DEBOUNCE_KEY
  let delay     = CLICK_PROCESS_DELAY
  
  if ( event === config.email_events.opened ) {
    redis_key = OPEN_DEBOUNCE_KEY
    delay     = OPEN_PROCESS_DELAY
  }

  const score = Math.round(Date.now() / 1000) + delay
  await zadd(redis_key, score, object.email)
}

const addEvent = async ({ object, event, created_at }) => {
  if ( event === config.email_events.clicked || event === config.email_events.opened ) {
    await debounceEvents(object, event, created_at)
  }
