...
const handleGmailOUtlook = async (object, event, created_at) => {
  const url = object.headers['Referer'] || null
  const ip  = object.headers['X-Forwarded-For'] || null
  ...
  Context.log(`Adding email event: ${object.origin} ${event} ${recipient} - email: ${email}`)
