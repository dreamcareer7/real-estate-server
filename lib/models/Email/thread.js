...
ThreadMessage.checkByThread = async (thread_key, user, brand) => {
  const messages = await db.select('email/thread/check_by_thread', [thread_key])
...
ThreadMessage.getByThread = async (thread_key) => {
  const messages = await db.select('email/thread/get_by_thread', [thread_key])
