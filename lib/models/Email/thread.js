  ...
  async getAll(ids) {
    ...
    if (shouldFetchSnippet) {
      ...
      for (const [gc, ts] of google_threads.entries()) {
        const bodies = await GoogleMessage.fetchFromGoogle(gc, ts.map(t => t.last_message_id))

        for (const t of ts) {
          if (bodies[t.message_id]) t.snippet = bodies[t.message_id].snippet
        }
      }

      for (const [mc, ts] of microsoft_threads.entries()) {
        const bodies = await MicrosoftMessage.fetchFromMicrosoft(mc, ts.map(t => t.last_message_id))
  ...
  async prune(ids, credentials) {
    ...
    if (deleted_ids.length > 0) {
      sendSocketEvent('email_thread:delete', credential.user, {
        ...credentials,
        threads: deleted_ids.slice(0, 50)
      })
