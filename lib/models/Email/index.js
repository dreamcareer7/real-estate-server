...
const sendViaOutlook = async email => {
  try {
    ...
    await EmailCampaignEmail.saveError(email, ex)
    throw ex
  }
}
...
Email.publicize = (model, options) => {
  /*
   * It would make our responses huge as sometimes we send thousands of these to clients.
   */
  if (Array.isArray(options.select['email'])) {
    if (!options.select['email'].includes('html')) delete model.html
    if (!options.select['email'].includes('text')) delete model.text
  }
}
