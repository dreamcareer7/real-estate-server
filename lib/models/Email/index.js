...
const sendViaOutlook = async email => {
  ...
  Context.log('Sent an outlook message'.cyan, recipient, email.id)
}

const sendViaGmail = async email => {
  const sentMessage = await gmailSender(email)

  await Email.storeGoogleId(email.id, sentMessage.message_id)
  await EmailCampaign.saveThreadKey(email.campaign, `${email.google_credential}${sentMessage.thread_id}`)

  const recipient = email.to || email.cc || email.bcc
  Context.log('Sent a gmail message'.cyan, recipient, email.id)
}
