...
const sendViaOutlook = async email => {
  try {
    const pixelTrackingId = `${email.microsoft_credential}_${uuid.v4()}`

    await outlookSender(email, pixelTrackingId)
    await Email.storePixelTrackingId(email.id, pixelTrackingId)
...
const sendViaGmail = async email => {
  try {
    ...
    const pixelTrackingId = `${email.google_credential}_${sentMessageId}`
