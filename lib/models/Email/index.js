...
Email.storeMicrosoftMessageId = async (id, microsoft_message_id) => {
  await db.query.promise('email/microsoft_message_id', [id, microsoft_message_id])
}
...
const sendViaOutlook = async email => {
  try {
    const sentMessageId = await outlookSender(email)

    await Email.storeMicrosoftMessageId(email.id, sentMessageId)

    Context.log('Sent an outlook-message'.cyan, sentMessageId, email.google_credential)
