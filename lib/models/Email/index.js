...
const addEvent = async ({ email, event, created_at, recipient, object, origin }) => {
  Context.log('Adding email event', origin, event, recipient, email)
  ...
  let promise = db.query.promise('email/event/rechat_add', data)
  
  if ( origin === 'mailgun' ) {
    promise = db.query.promise('email/event/mailgun_add', data)
  }
...
Email.getByGoogleMessageIds = async (message_ids) => {
  return await db.selectIds('email/get_by_google_message_id', [message_ids])
}

Email.getByMicrosoftMessageIds = async (message_ids) => {
