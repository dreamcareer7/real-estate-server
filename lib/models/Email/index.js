...
Email.storePixelTrackingId = async (id, pixel_tracking_id) => {
  await db.query.promise('email/store_pixel_tracking_id', [id, pixel_tracking_id])
}

Email.storeGoogleMessageId = async (id, google_message_id) => {
  await db.query.promise('email/store_google_message_id', [id, google_message_id])
}

Email.storeMicrosoftMessageId = async (records) => {
  return await db.chunked(records, 2, (chunk, i) => {
    const q = squel
      .insert()
      .into('emails')
      .setFieldsRows(chunk)
      .onConflict(['id'], {
        microsoft_message_id: squel.rstr('EXCLUDED.microsoft_message_id')
      })
      .returning('id')

    q.name = 'email/store_microsoft_message_id'
...
const sendViaOutlook = async email => {
  try {
    ...
    Context.log('Sent a outlook-message'.cyan, email.microsoft_credential)

  } catch (ex) {

    const text = `Outlook-Send-Email-Failed - credential: ${email.microsoft_credential} - Ex: ${ex.message}`
...
const sendViaGmail = async email => {
  try {
    ...
    await Email.storeGoogleMessageId(email.id, sentMessageId)

    Context.log('Sent a gmail-message'.cyan, sentMessageId, email.google_credential)

  } catch (ex) {

    const text = `Gmail-Send-Email-Failed - credential: ${email.google_credential} - Ex: ${ex.message}`
