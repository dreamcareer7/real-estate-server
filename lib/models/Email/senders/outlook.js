...
const outlookSender = async (email) => {
  Context.log('debug-sendEmail outlookSender 1, email:', email.id, email.to)
  ...
  Context.log('debug-sendEmail outlookSender 2, recipientsNum:', recipientsNum)

  const isReply = email.headers ? ( email.headers.message_id ? true : false ) : false

  Context.log('debug-sendEmail outlookSender 3, isReply:', isReply)
  ...
  Context.log('debug-sendEmail outlookSender 4, before params.attachments', email.attachments)

  if (isReply) {
    await MicrosoftMessage.sendReply(params)
  } else {
    Context.log('debug-sendEmail outlookSender 5, go to createAndSendMessage')
    await MicrosoftMessage.createAndSendMessage(params)
  }

  Context.log('debug-sendEmail outlookSender 6, forceSync')
