...
/*
  ...
  const Return_Path = `<outlook-bounces@${config.mailgun.domain}?`
...
const outlookSender = async (email) => {
  ...
  const params = {
    ...
    'body': {
      'text': email.text,
      'html': handleTracking(email.html, email.id, 'outlook')
    },
    ...
    'extensions': [
      {
        ...
        'rechatCredential': credential.id,
        'rechatEmailId': email.id,
        'rechatCampaignId': email.campaign,
  ...
  try {
    ...
    try {
      ...
      // do nothing
      Context.log('Microsoft outlookSender-Failed', err)
    }
