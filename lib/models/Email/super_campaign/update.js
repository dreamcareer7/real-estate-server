...
const { updateEligibility, updateEnrollments } = require('./eligibility')
...
async function updateTags(super_campaign, tags) {
  ...
  // TODO: fetch eligible brands from the DB
  const eligibleBrands = []
  if (!eligibleBrands.length) { return result }
  ...
  await updateEnrollments(super_campaign, [], eligibleBrands)
