...
async function updateTags(super_campaign, tags) {
  const { tags: oldTags } = await get(super_campaign)
  ...
  const attachedEnrollments = await Enrollment.filter({
    super_campaign,
    detached: false,
    tags: oldTags,
  })
  if (!attachedEnrollments.length) { return }

  await Enrollment.upsertMany(
    attachedEnrollments.map(
      ae => ({ ...pick(ae, 'super_campaign', 'brand', 'user'), tags })
    )
  )
}
