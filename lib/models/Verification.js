  ...
  async.auto({
    ...
    verify: ['user',
             function(cb, results) {
               ...
               db.query(sql_phone_verify, [code, results.user.phone_number], function (err, res) {
                 if (err)
                   return cb(err);
...
PhoneVerification.get = function (verification_id, cb) {
  db.query(sql_phone_get, [verification_id], function (err, res) {
    if (err)
      return cb(err);
  ...
  async.auto({
    ...
    email: ['branch',
            ...
            function(cb, results) {
              ...
              console.log('HHHHHHEREEE');
              console.log(html_email_verification);
              console.log(text_email_verification);
              console.log(text_subject_email_verification);
 ...
 */
EmailVerification.audit = function (user_id, code, cb) {
  async.auto({
    user: function(cb) {
      return User.get(user_id, cb);
    },
    verify: ['user',
             function(cb, results) {
               if(!results.user.email)
                 return cb(Error.Forbidden('No email are registered for this user'));

               console.log(code, results.user.email);
               db.query(sql_email_verify, [code, results.user.email], function (err, res) {
                 if (err)
                   return cb(err);
