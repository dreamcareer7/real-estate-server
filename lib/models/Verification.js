...
PhoneVerification.create = function (verification, notify, cb) {
  ...
  async.auto({
    ...
    branch: ['validate',
             function(cb, results) {
               var token_plain = verification.phone_number + ':' + verification.code;
...
PhoneVerification.audit = function (phone_number, code, cb) {
  ...
  db.query(sql_phone_verify, [code, phone_number], function (err, res) {
    if (err)
      return cb(err);
...
EmailVerification.create = function (verification, notify, cb) {
  ...
  async.auto({
    ...
    branch: ['validate',
             function(cb, results) {
               var token_plain = verification.email + ':' + verification.code;
               var token = Crypto.encrypt(token_plain);
    ...
    email: ['user',
            ...
            function(cb, results) {
              ...
              return Email.send({from: config.email.from,
...
EmailVerification.audit = function (email, code, cb) {
  db.query(sql_email_verify, [code, email], function (err, res) {
    ...
    var date = new Date(res.rows[0].created_at);
    var now = new Date();
    var diff = now.getTime() - date.getTime();

    if(diff > 86400000) //Tokens are valid for a day
      return cb(Error.Forbidden('Token has expired'));
