...
PhoneVerification.create = function (verification, notify, cb) {
  verification.code = (process.env.NODE_ENV === 'tests') ? '12345' : Math.floor((Math.random() * 9000) + 1000).toString()
  ...
  async.auto({
    ...
    render: [
      'branch',
      (cb, results) => {
        Template.render(__dirname + '/../templates/verifications/phone.tmpl', {
 ...
 * @returns {PhoneVerification#phone_verification}
 */
PhoneVerification.get = function (id, cb) {
  PhoneVerification.getAll([id], (err, verifications) => {
    if(err)
      return cb(err)

    if (verifications.length < 1)
      return cb(Error.ResourceNotFound(`Phone Verification ${id} not found`))
...
PhoneVerification.getAll = function(verification_ids, cb) {
  db.query('verification/phone_get', [verification_ids], (err, res) => {
    if (err)
      return cb(err)

    const verifications = res.rows

    return cb(null, verifications)
  })
}
/**
...
EmailVerification.create = function (verification, notify, cb) {
  verification.code = (process.env.NODE_ENV === 'tests') ? 'a' : crypto.randomBytes(16).toString('hex')
 ...
 * @returns {EmailVerification#email_verification}
 */
EmailVerification.get = function (id, cb) {
  EmailVerification.getAll([id], (err, verifications) => {
    if(err)
      return cb(err)

    if (verifications.length < 1)
      return cb(Error.ResourceNotFound(`Email Verification ${id} not found`))
...
EmailVerification.getAll = function(verification_ids, cb) {
  db.query('verification/email_get', [verification_ids], (err, res) => {
    if (err)
      return cb(err)
