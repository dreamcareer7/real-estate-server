...
EmailVerification.create = function (user_id, cb) {
  ...
  async.auto({
    ...
    agent: [
      ...
      'validate',
      (cb, results) => {
        Agent.matchByEmail(results.user.email, cb)
      }
    ],
    branch: [
      'user',
    ...
    email: [
      ...
      (cb, results) => {
        ...
        return Email.sane({
          ...
          html: results.html,
          subject: 'Rechat Email Verification'
        }, cb)
        ...
        //     first_name: results.user.first_name ? results.user.first_name : results.user.email,
        //     base_url: Url.web({}),
