...
Shortlist.recommendListing = function(shortlist_id, listing, cb) {
  console.log('recommending to:', shortlist_id);
  var recommendation = {};
  var message_room = {};
  ...
  recommendation.object = listing.id;
  ...
  message_room.listing = listing.id;
  message_room.shortlist = shortlist_id;
  ...
  async.auto({
    ...
    dup: ['shortlist',
          function(cb) {
            db.query(sql_dup, [listing.id, shortlist_id], function(err, res) {
              if (err)
                return cb(err);

              if (res.rows[0].bool_or == false)
    ...
    recommendations: ['users', 'comment_room',
                      function(cb, results) {
                        async.map(results.users, function(id, cb) {
                          ...
                          recommendation.message_room = results.comment_room.id;
