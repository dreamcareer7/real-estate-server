...
var sql_unpin_orphaned_recs = require('../sql/shortlist/unpin_orphaned_recs.sql');
...
Shortlist.removeOrphanedRecommendationsMessageRooms = function(shortlist_id, cb) {
  db.query(sql_remove_orphaned_recs_message_rooms, [shortlist_id], function(err, res) {
    ...
    async.mapSeries(message_room_ids, MessageRoom.delete, function(err, results) {
      if(err)
        return cb(err);
...
Shortlist.unpinOrphanedRecommendations = function(shortlist_id, cb) {
  db.query(sql_unpin_orphaned_recs, [shortlist_id], function(err, res) {
    if(err)
      return cb(err);
