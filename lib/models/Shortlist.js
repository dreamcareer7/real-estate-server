...
var sql_invited = require('../sql/shortlist/invited.sql');
...
Shortlist.inviteList = function(shortlist_id, cb) {
  Shortlist.get(shortlist_id, function(err, shortlist) {
    if(err)
      return cb(err);

    db.query(sql_invited, [shortlist_id], function(err, res) {
      if(err)
        return cb(err);

      var invitees = res.rows;
      async.map(invitees, invitationDecode, function(err, results) {
        return cb(null, results);
      });
    });
  });
...
function invitationDecode(invite, cb) {
  async.parallel({
    inviting_user: function(cb) {
      if(!invite.inviting_user)
        return cb();

      User.get(invite.inviting_user, cb);
    },
    invited_user: function(cb) {
                    if(!invite.invited_user)
                      return cb();

      User.get(invite.invited_user, cb);
    }
  }, function(err, results) {
       if (invite.inviting_user) invite.inviting_user = User.publicize(results.inviting_user);
       if (invite.invited_user) invite.invited_user = User.publicize(results.invited_user);
