...
var sql_hide_orphaned_recs = require('../sql/shortlist/hide_orphaned_recs.sql');
var sql_deactivate_orphaned_recs_message_rooms = require('../sql/shortlist/deactivate_orphaned_recs_message_rooms.sql');
var sql_archive = require('../sql/shortlist/archive.sql');
var sql_get_recommendations = require('../sql/shortlist/get_recommendations.sql');
var sql_update_lead_agent = require('../sql/shortlist/update_lead_agent.sql');

function recommendationRestrict(recs, user) {
  for (var i in recs) {
    if(recs[i].referred_user.id != user)
      recs.splice(i, 1);
  }
  ...
  db.query(sql_get, [id], function(err, res_base) {
    ...
    async.parallel({
      owner: function(cb) {
        ...
        return User.get(shortlist.owner, cb);
      },
      users: function(cb) {
        Shortlist.getUsers(id, cb);
      },
      lead_agent: function(cb) {
        if(!shortlist.lead_agent)
          return cb();

        return User.get(shortlist.lead_agent, cb);
      }
    }, function(err, results) {
         if(err)
           return cb(err);

         shortlist.owner = results.owner;
         shortlist.users = results.users;
         shortlist.lead_agent = results.lead_agent;
...
Shortlist.addUser = function(user_id, shortlist_id, cb) {
  async.auto({
    ...
    join_default: ['add_user',
                   function(cb) {
                     Shortlist.joinDefaultRoom(user_id, shortlist_id, cb);
                   }],
    join_comments: ['join_default',
                    function(cb) {
                      Shortlist.joinComments(user_id, shortlist_id, cb);
                    }],
    others: ['join_comments',
             function(cb) {
              Shortlist.others(shortlist_id, user_id, cb);
             }],
    private_rooms: ['others',
                    function(cb, results) {
                      async.mapSeries(results.others, function(other_id, cb) {
    ...
    lead_agent: ['user',
                 'shortlist',
                 function(cb, results) {
                   if(!results.shortlist.lead_agent && results.user.user_type == 'Agent') {
                     return Shortlist.updateLeadAgent(shortlist_id, user_id, cb);
                   } else {
                     return cb();
                   }
                 }],
    notification: ['user',
                   ...
                   function(cb, results) {
                     ...
                     notification.message = '#' + results.shortlist.title + ': @' + results.user.first_name + ' just joined, let\'s say hi';
...
Shortlist.deleteRooms = function(shortlist_id, cb) {
  Shortlist.getRoomIds(shortlist_id, function(err, message_room_ids) {
    ...
    async.mapSeries(message_room_ids, MessageRoom.delete, function(err, message_rooms) {
      if(err)
        return cb(err);
    ...
    Shortlist.get(shortlist_id, function(err, shortlist) {
      ...
      db.query(sql_get_one_to_ones, [user_id, shortlist_id], function(err, res) {
        ...
        if(res.rows.length < 0)
          return cb(Error.ResourceNotFound('No one to one rooms found on this shortlist'));
...
Shortlist.getDefaultRoom = function(shortlist_id, cb) {
  Shortlist.get(shortlist_id, function(err, shortlist) {
    ...
    db.query(sql_get_default, [shortlist_id], function(err, res) {
      ...
      if(res.rows.length < 0)
        return cb(Error.ResourceNotFound('No default room found for this shortlist'));
...
Shortlist.deactivateOrphanedRecommendationsMessageRooms = function(shortlist_id, cb) {
  db.query(sql_deactivate_orphaned_recs_message_rooms, [shortlist_id], function(err, res) {
    if(err)
      return cb(err);
    ...
    async.map(message_room_ids, MessageRoom.deActivate, function(err, results) {
      if(err)
        return cb(err);
...
Shortlist.hideOrphanedRecommendations = function(shortlist_id, cb) {
  Shortlist.deactivateOrphanedRecommendationsMessageRooms(shortlist_id, function(err, ok) {
    if(err)
      return cb(err)

    db.query(sql_hide_orphaned_recs, [shortlist_id], function(err, res) {
      if(err)
        return cb(err);
...
Shortlist.recommendListing = function(shortlist_id, listing_id, external_info, cb) {
  ...
  Listing.get(listing_id, function(err, listing) {
    ...
    Shortlist.isDup(shortlist_id, listing_id, function(err, dup) {
      ...
      if (dup) {
        async.auto({
          add_alert_to_recommendations: function(cb) {
            Recommendation.addAlertToRecommendations(shortlist_id, listing_id, alert_id, cb);
          },
          unhide_listing_on_shortlist: ['add_alert_to_recommendations',
                                        function(cb) {
                                          Recommendation.unhideListingOnShortlist(shortlist_id, listing_id, cb);
                                        }],
          recommendations: ['unhide_listing_on_shortlist',
                            function(cb) {
                              Shortlist.getRecommendations(shortlist_id, listing_id, cb);
                            }],
          recommendations_deref: ['add_alert_to_recommendations',
                                  'unhide_listing_on_shortlist',
                                  'recommendations',
                                  function(cb, results) {
                                    async.map(results.recommendations, function(r, cb) {
                                      Recommendation.getOnShortlist(r, shortlist_id, cb);
                                    }, function(err, recs) {
                                         if(err)
                                           return cb(err);
        ...
        async.auto({
          ...
          comment_room: function(cb) {
            return MessageRoom.create(message_room, cb);
          },
          listing: function(cb) {
          ...
          notifications: ['users',
                          ...
                          function(cb, results) {
                            if (external_info.enable_push) {
                              ...
                              notification.object =  shortlist_id;
                              notification.object_class = 'Shortlist';
                              notification.message = '#' + results.shortlist.title + ': ' + address_line + ' just hit the market';
          ...
          recommendations_deref: ['recommendations',
                                  function(cb, results) {
                                    async.map(results.recommendations, function(r, cb) {
                                      return Recommendation.getOnShortlist(r, shortlist_id, cb);
                                    }, function(err, recs) {
                                         if(err)
                                           return cb(err);
...
Shortlist.archive = function(shortlist_id, cb) {
  db.query(sql_archive, [shortlist_id], function(err, res) {
    if(err)
      return cb(err);
...
Shortlist.getRecommendations = function(shortlist_id, listing_id, cb) {
  db.query(sql_get_recommendations, [shortlist_id, listing_id], function(err, res) {
    if(err)
      return cb(err);
...
Shortlist.updateLeadAgent = function(shortlist_id, agent_id, cb) {
  db.query(sql_update_lead_agent, [shortlist_id, agent_id], function(err, res) {
    if(err)
      return cb(err);
