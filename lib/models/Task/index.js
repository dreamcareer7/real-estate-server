...
Task.sendNotifications = async () => {
  ...
  for(const deal_id in grouped) {
    ...
    const deal_users = Object.keys(grouped_users)
      .filter(room_id => {
        return deal_rooms.indexOf(room_id) > -1
      })
      .map(room_id => grouped_users[room_id])
      .reduce((all, room_users) => {
        return all.concat.room_users
      })
      .filter(room_user => {
        return room_user.notification_setting === 'N_ALL'
      })
...
const sendDealNotification = async ({id, users, tasks}) => {
  ...
  if (counts[Review.APPROVED])
    message += `${counts[Review.APROVED]} Approved\n`
