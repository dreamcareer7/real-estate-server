...
Task.setSubmission = async (task_id, submission) => {
  ...
  const saveActivity = submission => {
    ...
    if (submission.status === 'Fair')
      activity.action = 'UserCompletedSubmission'

    return promisify(Activity.postToRoom)(task.room, activity)
  }
...
Task.setReview = async (task_id, review) => {
  ...
  const actions = {}
  actions[Review.APPROVED] = 'UserApprovedReview'
  actions[Review.REJECTED] = 'UserRejectedReview'
  actions[Review.PENDING] = 'UserSubmittedReview'

  const activity = {
    object_class: 'Review',
    object: task.review,
    action: actions[review.status]
  }
