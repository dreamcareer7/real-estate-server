...
Task.FORM = 'Form'
Task.GENERAL_COMMENTS = 'GeneralComments'
Task.YARD_SIGN = 'YardSign'
Task.OPEN_HOUSE = 'OpenHouse'
Task.MEDIA = 'Media'
...
Task.setSubmission = async (task_id, submission, log = true) => {
  ...
  if (task.submission) {
    ...
    saved = await Form.updateSubmission(submission)
  } else {
    saved = await Form.submit(submission)
...
Task.sendNotifications = async (delay = config.task_notification_delay) => {
...
Task.publicize = task => {
  ...
  if (task.task_type !== Task.FORM || !task.form)
    return
...
Task.setAttention = async ({task, attention_requested, user}) => {
  const deal = await promisify(Deal.get)(task.deal)
...
const notifySeamlessToBackoffice = async({message, room, user}) => {
  ...
  const task = await Task.get(row.id)

  await Task.setAttention({
    task,
    user,
...
Message.addSeamlessMiddleware(notifySeamlessToBackoffice)
