...
async function getEvents(credential, query) {
  Context.log('SyncMicrosoftCalendar - enter: getEvents')
  ...
  Context.log('SyncMicrosoftCalendar - afterFilter')
  Context.log('SyncMicrosoftCalendar - beforePopulate',JSON.stringify({ modelLength: models.length, associations }))
  const calEvents = await Orm.populate({ models, associations })

  Context.log('SyncMicrosoftCalendar - afterPopulate')
