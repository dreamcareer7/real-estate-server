...
class CalendarWorker {
  ...
  async getUnreadNotifications() {
    ...
    const notifications_by_user = _.groupBy(models, 'auxiliary_subject')
  ...
  async sendEmailForUnread() {
    ...
    Context.log(`Got ${unread_notifications.length} unread notification. Sending emails...`)

    for (const n of unread_notifications) {
      /** @type {IUser} */
      const user = n.auxiliary_subject
      Context.log(`Sending email for notification ${n.id} to ${user.email}`)
