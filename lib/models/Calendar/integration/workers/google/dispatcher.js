...
const generateGoogleCalendarEvent = (event, timezone, contactsById) => {
  ...
  return {
    ...
    recurrence: event.recurring ? ['RRULE:FREQ=YEARLY;COUNT=100'] : [],
...
const handleCreatedEvents = async (credential, events) => {
  ...
  const googleCalEvents = await GoogleCalendarEvent.bulkInsert(calendar, remoteEvents)
  ...
  return await CalendarIntegration.insert(records)
}
