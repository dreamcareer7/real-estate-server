...
const handleBatchCreatedEvents = async (credential, events) => {
  const promises = []
  const calendar = await GoogleCalendar.get(credential.google_calendar)

  for (const event of events) {
    promises.push(GoogleCalendarEvent.create(calendar, event.resource))
  }

  const objects = await Promise.all(promises)

  const records = objects.map((ob, index) => {
    return {
      google_id: ob.id,
      microsoft_id: null,
    
      crm_task: (events[index].object_type === 'crm_task') ? events[index].id : null,
      contact: (events[index].object_type === 'contact') ? events[index].id : null,
  ...
  const result = await CalendarIntegration.insert(records)
  console.log('---- handleBatchCreatedEvents result', result)
}
...
const handleUpdatedEvents = async (events) => {
  ...
  for (const event of events) {   
    ...
    promises.push(GoogleCalendarEvent.update(googleCalEvent.id, googleCal, event.resource))
  }
