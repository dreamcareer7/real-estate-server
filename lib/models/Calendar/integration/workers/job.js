...
const syncGoogleCalendar = async (data) => {
  ...
  if ( created.length !== 0 ) {
    const { result, error } = await handleCreatedEvents(credential, created.slice(0, 45))
    console.log('---- handleCreatedEvents result', result.length, result)
  ...
  // if ( updated.length !== 0 ) {
  //   const { error } = await handleUpdatedEvents(credential, updated)
  // }
