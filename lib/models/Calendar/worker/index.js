 ...
 * @returns {ICalendarGlobalNotificationSettingInput[]}
 */
function uniqSettings (settings) {
  const groupsMap = groupBy(settings, s => `${s.event_type}@${s.object_type}`)
  return minBy(Object.values(groupsMap), 'reminder')
}
...
async function set_default_notification_settings({ role, user }) {
  ...
  await NotificationSetting.setGlobalSettings(uniqSettings(settings), user, brand)
}
