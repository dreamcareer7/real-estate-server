...
const conditional_event_type_filter = (cond, true_value, false_value) => event_types => Array.isArray(event_types) ? _.uniq(event_types.map(evt => evt === cond ? true_value : false_value)).filter(x => x) : [true_value, false_value].filter(x => x)
...
const OBJECT_TYPE_VIEW_MAP = {
  ...
  email_campaign: conditional_event_type_filter('executed_email', 'calendar.email_campaign_executed', 'calendar.email_campaign_scheduled'),
  email_campaign_email: conditional_event_type_filter('executed_email', 'calendar.email_campaign_email_executed', null),
  deal_context: conditional_event_type_filter('home_anniversary', 'calendar.home_anniversary', 'calendar.deal_context'),
}
