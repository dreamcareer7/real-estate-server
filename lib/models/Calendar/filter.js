...
const filter = async (filter, query, parallel = DEFAULT_PARALLEL) => {
  ...
  if (parallel) {
    ...
    /**
     * This is used to make sure recurring events are compared using only day
     * and month, instead of the full timestamp that includes year.
     * @param {{ timestamp: number; recurring: boolean; }} a 
     * @param {{ timestamp: number; recurring: boolean; }} b 
     * @returns {number}
     */
    const compareFn = (a, b) => {
      if (a.recurring || b.recurring) {
        const d_a = new Date(a.timestamp * 1000)
        const d_b = new Date(b.timestamp * 1000)

        const md_a = d_a.getMonth() * 100 + d_a.getDay()
        const md_b = d_b.getMonth() * 100 + d_b.getDay()

        if (md_a === md_b) {
          return d_a.getFullYear() - d_b.getFullYear()
        }
    ...
    const merged = events.flat(1).sort(compareFn)
