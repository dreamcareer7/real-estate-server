...
Activity.add = function(reference_id, reference_type, activity, cb) {
  ...
  async.auto({
    ...
    reference_check: [
      'reference_fetch',
      (cb, results) => {
        if (Array.isArray(results.reference_fetch))
          return cb(results.reference_fetch[0])
    ...
    reference_fetch: [
      ...
      cb => {
        if (reference_type === 'Contact')
          return Orm.getAll('Contact', [reference_id]).nodeify(cb)
        if (reference_type === 'User')
          return Orm.getAll('User', [reference_id]).nodeify(cb)
        if (reference_type === 'DealRole')
          return Orm.getAll('DealRole', [reference_id]).nodeify(cb)
