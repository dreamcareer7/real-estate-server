...
Activity.parse = function (activity, cb) {
  ...
  async.auto({
    model: cb => {
      ...
      Orm.getAll(activity_class, [activity_data]).nodeify((err, results) => {
        if(err)
          return cb(err)
...
Activity.add = function(reference_id, reference_type, activity, cb) {
  ...
  async.auto({
    ...
    get: [
      ...
      'record',
      (cb, results) => {
        const [object, ,] = results.parse

        Activity.get(results.record, (err, activity) => {
          if(err)
            return cb(err)
...
Activity.postToRoom = function({room_id, activity, user_id}, cb) {
  if (!user_id)
  ...
  async.auto({
    ...
    message: [
      ...
      'create',
      (cb, results) => {
        Activity.formatForDisplay(results.create, results.user, cb)
      }
    ],
    post: [
      'create',
      ...
      (cb, results) => {
        ...
        Message.post(room_id, message, false, cb)
      }
    ]
  }, (err, results) => {
    if(err)
