 ...
 */
Activity.contactTimeline = async function(contact_id, pagination) {
  expect(contact_id).to.be.uuid

  /** @type {IParentContact} */
  const contact = await promisify(Contact.get)(contact_id)
...
const enqueueIntercomReport = (event, metadata) => {
  ...
  if (action) {
    if (!Array.isArray(process.domain.intercom))
      process.domain.intercom = []

    process.domain.intercom.push(Object.assign({
      event: action,
      timestamp: Date.now(),
    }, metadata))
  }
...
Activity.postToRoom = function({room_id, activity, user_id, push = false, set_author = true}, cb) {
  if (!user_id)
