...
CalendarIntegration.insert = async (records) => {
  ...
  return db.select('calendarIntegration/insert', [JSON.stringify(records)])
}
...
CalendarIntegration.getAll = async (ids) => {
  return await db.select('calendarIntegration/get', [ids])
}
...
CalendarIntegration.getByGoogleIds = async (google_ids) => {
  const ids = await db.selectIds('calendarIntegration/get_by_google_ids', [google_ids])
...
CalendarIntegration.getByMicrosoftIds = async (microsoft_ids) => {
  const ids = await db.selectIds('calendarIntegration/get_by_microsoft_ids', [microsoft_ids])
...
CalendarIntegration.getByCrmTasks = async (crm_task_ids) => {
  const ids = await db.selectIds('calendarIntegration/get_by_crm_tasks', [crm_task_ids])
...
CalendarIntegration.getByContacts = async (contact_ids) => {
  const ids = await db.selectIds('calendarIntegration/get_by_contacts', [contact_ids])
...
CalendarIntegration.getByContactAttributes = async (contact_attribute_ids) => {
  const ids = await db.selectIds('calendarIntegration/get_by_contact_attributes', [contact_attribute_ids])
...
CalendarIntegration.getByDealContexts = async (deal_context_ids) => {
  const ids = await db.selectIds('calendarIntegration/get_by_deal_contexts', [deal_context_ids])
...
CalendarIntegration.getByHomeAnniversaries = async (deal_context_ids) => {
  const ids = await db.selectIds('calendarIntegration/get_by_home_anniversaries', [deal_context_ids])
...
CalendarIntegration.deleteMany = async function (ids) {
  await db.select('calendarIntegration/delete', [ids])
}
