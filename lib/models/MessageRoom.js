...
MessageRoom.getForUser = function(message_room_id, user_id, cb) {
  MessageRoom.get(message_room_id, function(err, message_room) {
    ...
    MessageRoom.getNewCounts(message_room_id, user_id, function(err, counts) {
      message_room.new_comments_count = counts.new_comments_count || 0;
      message_room.new_videos_count = counts.new_videos_count || 0;
      message_room.new_images_count = counts.new_images_count || 0;
      message_room.new_documents_count = counts.new_documents_count || 0;
...
MessageRoom.recordAcks = function(room_id, message_id, cb) {
  MessageRoom.get(room_id, function(err, message_room) {
    if(err)
      return cb(err);

    MessageRoom.getUserIds(room_id, function(err, users) {
