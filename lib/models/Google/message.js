...
GoogleMessage.fetchFromGoogle = async function (gcid, googleMessageIds) {
...
GoogleMessage.getAll = async (ids) => {
  ...
  if (shouldFetchBody) {
    ...
    for (const google_credential in by_credential) {
      const bodies = await GoogleMessage.fetchFromGoogle(google_credential, messages.map(m => m.message_id))
