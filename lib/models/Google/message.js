 ...
 */
GoogleMessage.setCampaign = async (id, campaign) => {
 ...
 */
GoogleMessage.getRemoteMessage = async (gcid, messageId, campaign = null) => {
  const google  = await getClient(gcid, 'gmail')
  ...
  if (campaign) {
    await GoogleMessage.setCampaign(id, campaign)
  }

  return await GoogleMessage.get(id)
}
