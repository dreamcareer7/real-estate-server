...
GoogleMessage.getAll = async (ids) => {
  ...
  const conditions = Orm.getAssociationConditions('email_thread.messages')
  const shouldFetchBody = conditions && conditions.body
...
GoogleMessage.publicize = async (message) => {
  ...
  if (message.has_attachments) {
    for (const attach of message.attachments) {
      ...
      attach.link = `${process.env.API_HOSTNAME}/emails/google/attachments/${hash}` 
    }
  }
...
GoogleMessage.downloadAttachment = async (gcid, mid, aid) => {
  const googleMessage = await GoogleMessage.getByMessageId(mid, gcid)
...
GoogleMessage.updateReadStatus = async (credential, id, status) => {
  ...
  return await GoogleMessage.updateIsRead(googleMessage.id, status)
}
