...
const createEmailObject = async function (params) {
  ...
  for (const att of params.attachments) {
    ...
    attachments.push({
      ...
      'contentId': att.contentId || att.cid || null, // <img src="cid:some-image-cid" alt="img" />
      'content': base64,
      'encoding': 'base64',
      'type': att.type
    })
...
GoogleMessage.getById = async (id, google_credential) => {
  const messages = await db.select('google/message/get_by_id', [id, google_credential])
...
GoogleMessage.create = async (records) => {
  const res = await db.chunked(records, 23, (chunk, i) => {
    const q = squel
      .insert()
...
GoogleMessage.updateReadStatus = async (credential, messageId, status) => {
  const google = await getGoogleClient(credential)

  if (!google)
    throw Error.BadRequest('Google-Client Failed!')

  const googleMessage = await GoogleMessage.getById(messageId, credential.id)

  if ( !googleMessage )
    throw Error.ResourceNotFound(`Google-Message ${messageId} Not Found!`)
