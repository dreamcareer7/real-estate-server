...
const updateIsRead = async (id, status) => {
  return db.update('google/messages/update_is_read.sql', [id, status])
}
...
GoogleMessage.updateReadStatus = async (credential, messageId, status) => {
  ...
  const googleMessage = await GoogleMessage.get(messageId, credential.id)

  if ( !googleMessage )
    throw Error.ResourceNotFound(`Google-Message ${messageId} Not Found!`)

  if (status) {
    await google.markAsRead(messageId)
  } else {
    await google.markAsUnRead(messageId)
  }
