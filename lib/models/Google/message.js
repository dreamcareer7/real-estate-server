...
GoogleMessage.sendEmail = async (gcid, tid) => {
  const credential = await GoogleCredential.get(gcid)
  const google     = await getGoogleClient(credential)
  ...
  const email = {
    ...
    subject: 'this-is-subject xxxx',
    textBody: 'this-is-body xxxx',
    htmlBody: '<b>this-is-body xxxx</b>'
  }

  return await google.sendMessage(email, tid)
}
