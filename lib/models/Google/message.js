...
const getClient = async (credential_id) => {
  ...
  if (credential.deleted_at)
    throw Error.BadRequest('Google-Credential is releted!')
...
const gmailBatchGetMessages = async function (googleMessageIds, gcid) {
...
GoogleMessage.deleteByMessageIds = async (google_credential, message_ids) => {
  ...
  await EmailThread.update(threads, 'google')
  ...
  if (message.has_attachments) {
    for (const attach of message.attachments) {
      ...
      const hash = encodeURIComponent(Crypto.encrypt(JSON.stringify({
        credential_id: message.google_credential,
      ...
      attach.url = `https://${process.env.API_HOSTNAME}/emails/google/attachments/${hash}` 
    }
  }
...
GoogleMessage.create = async (records) => {
  ...
  await EmailThread.update(_.uniq(res.map(r => r.thread_key)), 'google')
...
GoogleMessage.sendEmail = async (params) => {
  const google = await getClient(params.credential.id)
...
GoogleMessage.getRemoteMessage = async (gcid, messageId) => {
  ...
  const gmailMessage     = await GoogleMessage.getAsThreadMember(gcid, message.id)
 ...
 */
GoogleMessage.updateIsRead = async (id, status) => {
  return db.update('google/message/update_is_read', [id, status])
}

GoogleMessage.updateReadStatus = async (gcid, id, status) => {
  const google  = await getClient(gcid)
  const message = await GoogleMessage.get(id)

  if (status) {
    await google.markAsRead(message.message_id)
  } else {
    await google.markAsUnRead(message.message_id)
  }

  return await GoogleMessage.updateIsRead(message.id, status)
}
