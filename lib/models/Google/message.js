...
const { generateGMesssageRecord } = require('./workers/gmail/common')
...
GoogleMessage.batchTrash = async (gcid, ids) => {
  ...
  return await GoogleMessage.deleteByMessageIds(gcid, remote_message_ids)
}
 ...
 * @param {String?} next
 */
GoogleMessage.searchInThreads = async (gcid, query, next) => {
  const google = await getClient(gcid, 'gmail')
