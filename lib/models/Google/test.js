...
const randomException = function() {
...
const test = async (req, res) => {
  ...
  if (!GOOGLE_STACK[email]) {
    ...
    GOOGLE_STACK[email].rate ++
  }

  const time = new Date().getTime()

  for (const key in GOOGLE_STACK) {
    console.log('key:', key, 'val:', GOOGLE_STACK[key])

    if ( (time - GOOGLE_STACK[key].time < 0) ) {
      console.log('----- Fire sync process')
      delete GOOGLE_STACK[key]
