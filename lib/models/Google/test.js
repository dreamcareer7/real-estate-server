...
const setupMapping = async (googleCredential, calendar, confirmed, cancelled) => {
  ...
  const associations = ['crm_task.associations', 'crm_task.assignees', 'crm_task.reminders']
  ...
  const tasksByGoogleId = _.keyBy(models, 'google_event_id')

  console.log('------------ CrmTask.filter ==>', resutl)
  console.log('------------ CrmTask.getAll ==>', models)
  console.log('------------ Orm.populate   ==>', tasks)
  ...
  try {
    ...
    for ( const calendar of [toSyncRemoteCalendars[2]] ) {      
      ...
      const toUpdateTasks = confirmed.filter(event => { if (updatedEventRemoteIds.includes(event.id)) return true }).map(event => {
        ...
        task.id = tasksByGoogleId[gid].id
      ...
      /***  Handle new crm_tasks (Completed)  ***/
