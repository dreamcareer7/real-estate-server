...
const handleToSync = async (credential, toSync) => {
  const result    = await GoogleCalendar.persistRemoteCalendars(credential, toSync)
...
GoogleCalendar.configureCalendars = async (credential, conf) => {
  ...
  if (conf.toStopSync) {
    await handleToStopSync(credential)
  }

  if (conf.toSync) {
    await handleToSync(credential)
  }

  try {
    await GoogleCredential.forceSyncCalendar(credential.id)
  } catch (ex) {
    // do nothing
  }
