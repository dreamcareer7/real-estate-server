...
GoogleCalendar.persistRemoteCalendars = async (googleCredentialId, toSyncRemoteCalendarIds = []) => {
  ...
  for ( const remoteCalendarId of toSyncRemoteCalendarIds ) {
    ...
    if (!remoteCal) {
      ...
      if (offlineCal) {
        deletedCalendarIds.push(offlineCal.id)
      ...
      if (!offlineCal) {
        const id = await GoogleCalendar.persistRemoteCalendar(googleCredentialId, remoteCal)
    ...
    */
    if ( !sameOwnerRemoteCalendarIds.includes(remoteCalendar.id) ) {
      ...
      const cal = {
        ...
        // selected: remoteCalendar.selected,
...
GoogleCalendar.configureCalendars = async (credential, conf) => {
  ...
  if (conf.toSync) {
    const result    = await GoogleCalendar.persistRemoteCalendars(credential.id, conf.toSync)
