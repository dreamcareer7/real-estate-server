...
GoogleCalendar.watch = async (id) => {
  ...
  const google = await googleClient(calendar.google_credential)
  ...
  const options = {
    ...
    requestBody: {
      ...
      type: 'web_hook',
      address: 'https://boer.api.rechat.com/webhook/google/calendars',
    }
  }
...
GoogleCalendar.getRemoteGoogleCalendars = async (googleCredential) => {
  const sameBrandRemoteCalendarIds = await db.select('google/calendar/get_same_brand_rechat_gcalendars', [googleCredential.id])
