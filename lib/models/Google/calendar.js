...
GoogleCalendar.getByRemoteCalendarId = async (googleCredentialId, remoteCalendarId) => {
  const id = await db.select('google/calendar/get_by_remote_cal', [googleCredentialId, remoteCalendarId])

  return await GoogleCalendarEvents.get(id)
}

GoogleCalendar.getByWatcherChannelId = async (channelId) => {
  const calendars = await db.select('google/calendar/get_by_channel_id', [channelId])

  if (calendars.length < 1)
    throw Error.ResourceNotFound(`Google Calendar GetByWatcherChannelId ${channelId} not found.`)
...
GoogleCalendar.stopSync = async (googleCredentialId, remoteCalendarIds = []) => {
  return await db.select('google/calendar/stop_sync', [googleCredentialId, remoteCalendarIds])
}
...
GoogleCalendar.listEvents = async (googleCredentialId) => {
  ...
  */
  ...
  return await google.syncEvents('saeed.uni68@gmail.com', '2019-12-01T00:00:00-00:00')
}
