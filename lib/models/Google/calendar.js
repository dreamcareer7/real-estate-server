...
GoogleCalendar.configureCaledars = async (credential, conf) => {
  ...
  if (conf.toStopSync)
    await GoogleCalendar.stopSync(credential.id, conf.toStopSync)

  if (conf.toSync)
    await GoogleCalendar.persistRemoteCalendars(credential.id, conf.toSync)
...
GoogleCalendar.stopWatch = async (id) => {
  ...
  const result = await google.stopWatchCalendar(options)

  return await GoogleCalendar.updateWatcher(calendar.id, result.id, 'stopped')
}

GoogleCalendar.syncCalendar = async (id, channelId) => {
