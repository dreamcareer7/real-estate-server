...
const handleException = async function(credential, ex) {
  ...
  Slack.send({ channel: '7-server-errors',  text: `${msg} - StatusCode: ${ex.statusCode} - Message: ${ex.message} - Info: ${JSON.stringify(obj)}`, emoji: ':skull:' })
  Slack.send({ channel: 'integration_logs', text: `${msg} - StatusCode: ${ex.statusCode} - Message: ${ex.message} - Info: ${JSON.stringify(obj)}`, emoji: ':skull:' })
}
