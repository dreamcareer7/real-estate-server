...
Google.prototype.listConnections = async function (currentSyncToken = null) {
  ...
  const personFields = 'metadata,memberships,addresses,phoneNumbers,photos,birthdays,emailAddresses,names,nicknames,organizations,occupations,urls,biographies'
  ...
  try {
    do {
      ...
      const multipart = temp.map((contact, index) => ({
        'Content-Type': 'application/http',
        'Content-ID': (counter * 25) + index,
        'body': `PATCH /v1/${contact.resource_id}:updateContact?updatePersonFields=${contact.updatePersonFields} HTTP/1.1\n`
              + 'Content-Type: application/json\n\n'
              + JSON.stringify(contact.resource)
      }))
...
Google.prototype.batchDeleteContacts = async function (contacts) {
  let counter   = 0
  let confirmed = []
  let error     = null
  ...
  try {
    do {
      ...
      const multipart = temp.map((contact, index) => ({
        'Content-Type': 'application/http',
        'Content-ID': (counter * 25) + index,
        'body': `DELETE /v1/people/${contact.resource_id}:deleteContact HTTP/1.1\n`
      }))
      ...
      console.log('---- batchDeleteContacts responseString', responseString)
