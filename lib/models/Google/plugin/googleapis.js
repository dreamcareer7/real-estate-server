...
Google.prototype.bulkInsertEvent = async function (events) {
  let confirmed = []
  let error     = null
  ...
  try {
    do {
      ...
      const dataArr = result.filter(res => res.status === 200).map(res => res.data)
      confirmed     = confirmed.concat(dataArr)
  
    } while ( events.length > 0 )
  } catch (ex) {
    error = ex.message
    Context.log('bulkInsertEvent-failed', ex.message)
  }
  ...
  try {
    do {
      ...
      const multipart  = temp.map((event, index) => ({
        ...
        'body': `POST /calendar/v3/calendars/${event.calendarId}/events HTTP/1.1\n`
  ...
  try {
    do {
      ...
      const multipart = temp.map((event, index) => ({
        'Content-Type': 'application/http',
        'Content-ID': (counter * 25) + index,
        'body': `PUT /calendar/v3/calendars/${event.calendar.calendar_id}/events/${event.eventId} HTTP/1.1\n`
              + 'Content-Type: application/json\n\n'
              + JSON.stringify(event.resource)
      }))
  ...
  try {
    do {
      ...
      const multipart = temp.map((event, index) => ({
        'Content-Type': 'application/http',
        'Content-ID': (counter * 25) + index,
        'body': `DELETE /calendar/v3/calendars/${event.calendarId}/events/${event.eventId} HTTP/1.1\n`
      }))
