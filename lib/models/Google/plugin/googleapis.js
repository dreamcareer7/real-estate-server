...
const SCOPES_FULL      = config.google_scopes_full
...
Google.prototype.getAuthenticationLink = async function(state, readOnly) {
...
Google.prototype.getContacts = async function (path) {
  ...
  do {
    ...
    console.log('****** getContacts in-plugin-loop letngth', contacts.length)
    next     = false
...
Google.prototype.getContacts_test = async function (path) {
  const authHeader = await this.oAuth2Client.getRequestHeaders()
