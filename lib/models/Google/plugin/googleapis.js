...
Google.prototype.listCalendarEvents = async function(calendarId, currentSyncToken = null) {
  let items         = []
  ...
  do {
    try {
      ...
      console.log('----- listCalendarEvents', ex)
    }
...
Google.prototype.testCalendar = async function () {
  let option
  ...
  // const calendars = await this.calendar.calendars.get(option)
  ...
  const primaryCalEvents = await this.calendar.events.list(option)
  console.log('----------- primaryCalEvents.data', primaryCalEvents.data)


  option = {
    calendarId: 'primary',
    eventId: '6gmdqmru424g24rp9ecbrb4o7k',
  }

  const event = await this.calendar.events.get(option)
