...
const GOOGLE_CREDENTIALS = {
  'installed': {
    ...
    'redirect_uris': [
      ...
      'https://chappar.rechat.com/webhook/google/grant',
      'https://irish.rechat.com/webhook/google/grant',
      'https://rechat.com/webhook/google/grant'
    ]
  }
}
...
const CREDENTIALS = config.GOOGLE_CREDENTIALS || GOOGLE_CREDENTIALS
const SCOPES      = config.GOOGLE_SCOPES || GOOGLE_SCOPES
...
Google.prototype.config = function(mode) {
  if (mode === 'cli') {
    ...
    if ( process.env.NODE_ENV === 'alpine' )
      this.redirectTo = CREDENTIALS.installed.redirect_uris[2]

    if ( process.env.NODE_ENV === 'boer' )
      this.redirectTo = CREDENTIALS.installed.redirect_uris[3]

    if ( process.env.NODE_ENV === 'bagot' )
      this.redirectTo = CREDENTIALS.installed.redirect_uris[4]
  }
...
Google.prototype.refreshToken = async function() {
  ...
  await this.oAuth2Client.setCredentials(tokens.credentials)
...
Google.prototype.batchGetMessages = async function(messageIds) {
  ...
  request({
    ...
    multipart: [{
      ...
      'Content-ID': '16b17b3c672d3617',
      'body': 'GET ' + 'gmail/v1/users/me/messages/16b17b3c672d3617' + '\n'
    }],
