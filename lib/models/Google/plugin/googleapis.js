...
Google.prototype.config = function (mode) {
  this.redirectTo = CREDENTIALS.redirect_to_uri || 'https://boer.api.rechat.com/webhook/google/grant'
...
Google.prototype.getAuthenticationLink = async function (state, scopes) {
  ...
  const authUrl = await this.oAuth2Client.generateAuthUrl({
    ...
    // login_hint: 'heshmat.zapata@gmail.com'
  })
...
Google.prototype.batchUpdateContacts = async function (events) {
  let counter   = 0
  let confirmed = []
  let error     = null
...
Google.prototype.batchDeleteContacts = async function (events) {
  let counter = 0
  let error   = null
