...
Google.prototype.listContactGroups = async function (currentSyncToken = null) {
  ...
  do {
    try {
      ...
      /*
        Some time due to an unknown reason, we got the below response.
        Then we have to reset tokens.
      ...
      */
      ...
      if ( ex?.response?.data?.error.status === 'INVALID_ARGUMENT' ) {
        invalidArgument = true
      }

      if ( ex.message === 'Request contains an invalid argument.' ) {
        invalidArgument = true
      }

      if ( (ex.message !== expTokenMsg) && !invalidArgument ) {
        throw ex
      }
      ...
      if (!response.data) {
        Context.log('listContactGroups-failed - response:', response, ' - body:', body)
      }
