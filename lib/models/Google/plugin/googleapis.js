...
Google.prototype.batchInsertEvent = async function (events) {
  ...
  try {
    do {
      ...
      console.log('--- batchInsertEvent loop#', counter++)
  ...
  try {
    do {
      ...
      const multipart  = temp.map((event, index) => ({
        'Content-Type': 'application/http',
        'Content-ID': (counter * 25) + index,
        'body': `PUT calendar/v3/calendars/${event.calendarId}/events/${event.eventId} HTTP/1.1\n`
              + 'Content-Type: application/json\n\n'
              + JSON.stringify(event.resource)
      }))
      ...
      console.log('--- batchUpdateEvent loop#', counter++)
