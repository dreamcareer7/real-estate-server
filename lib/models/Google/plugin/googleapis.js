...
Google.prototype.bulkInsertEvent = async function (events) {
  ...
  try {
    do {
      ...
      console.log('--- bulkInsertEvent loop#', counter++)
...
Google.prototype.bulkUpdateEvent = async function (events) {
  const batch = this.client.newBatch()
