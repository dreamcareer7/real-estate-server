...
Google.prototype.listContactGroups = async function (currentSyncToken = null) {
  ...
  do {
    try {
      ...
      Context.log('syncGoogleContacts','in api try body', JSON.stringify(body))
      response = await this.people.contactGroups.list(body)

    } catch (ex) {
      Context.log('syncGoogleContacts','in api catch expTokenMsg:', expTokenMsg)
      Context.log('syncGoogleContacts','in api catch ex.message:', ex.message)
      Context.log('syncGoogleContacts','in api catch', JSON.stringify(ex))
      if ( ex.message !== expTokenMsg ) {
        throw Error.Conflict({ details: {
          attributes: { email: currentGmailAddress },
          info: {
            method: 'google.listContactGroups',
            currentSyncToken: currentSyncToken,
            message: ex.message
          }
        }})
      }
      ...
      Context.log('syncGoogleContacts','in api catch body', JSON.stringify(body))
      response = await this.people.contactGroups.list(body)
      isSyncTokenRefreshed = true
      Context.log('syncGoogleContacts','in api catch response:', JSON.stringify(response))
    } finally {
