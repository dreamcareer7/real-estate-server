...
const syncContacts = async (data, testFlag = false) => {
  ...
  const _google_credential     = await GoogleCredential.getByUser(user, brand)
  const _result                = await Contact.filter(brand, [], { google_id: _resource_name })
  const _rechat_contact        = await Contact.get(_result.ids[0])

  const _google_contact        = await GoogleContact.get(_resource_name)
