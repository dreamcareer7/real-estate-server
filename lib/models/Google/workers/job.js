...
const syncGoogle = async (data) => {
  ...
  const handleException = async function(msg, ex) {
    Context.log('SyncGoogle', data.googleCredential.email, data.googleCredential.id, ex)
  ...
  Context.log('SyncGoogle - start job', data.googleCredential.email, data.googleCredential.id)
  ...
  if ( data.googleCredential.scope.some(entry => config.google_scopes_map.profile.includes(entry)) ) {
    ...
    Context.log('SyncGoogle - profileResult', data.googleCredential.email, profileResult)
  }
  ...
  if ( data.googleCredential.scope.some(entry => config.google_scopes_map.contacts.includes(entry)) ) {
    ...
    Context.log('SyncGoogle - contactGroupsResult', data.googleCredential.email, contactGroupsResult)
    ...
    Context.log('SyncGoogle - contactsLastRsult', data.googleCredential.email, contactsLastRsult)
  ...
  if ( data.googleCredential.scope.some(entry => config.google_scopes_map.gmail.includes(entry)) ) {
    ...
    Context.log('SyncGoogle - gmailProfileResult', data.googleCredential.email, gmailProfileResult)
    ...
    if (!data.googleCredential.messages_sync_history_id) {
      ...
      Context.log('SyncGoogle - syncMessagesResult', data.googleCredential.email, syncMessagesResult)
      ...
      Context.log('SyncGoogle - partialSyncResult', data.googleCredential.email, partialSyncResult)
  ...
  Context.log('SyncGoogle - job Finish', data.googleCredential.email, sync_duration)
