...
const syncGoogle = async (data) => {
  ...
  const handleException = async function(msg, ex) {
    ...
    if ( ex.message === 'Invalid Credentials' || ex.message === 'invalid_grant' ) {
  ...
  if ( data.googleCredential.scope_summary.includes('mail.read') ) {
    ...
    const watchResult = await messageWorker.watchMailBox(google, data)
    Context.log('SyncGoogle watchMailBox:', data.googleCredential.id, watchResult.historyId, watchResult.expiration)
  }
