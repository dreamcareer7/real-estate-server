...
const syncGoogle = async (data) => {
  ...
  if ( googleJobDuplicateCheck || currentGoogleCredential.revoked || currentGoogleCredential.deleted_at ) {
    const text = `Google - Revoked: ${currentGoogleCredential.revoked} - Deleted_At: ${currentGoogleCredential.deleted_at} - Status: ${currentGoogleCredential.sync_status} - Action: Job skipped`
  ...
  const handleException = async function(msg, ex) {
    ...
    if ( ex.message === 'Invalid Credentials' )
      Context.log('SyncGoogle - Invalid Credentials', data.googleCredential.id, data.googleCredential.email, ex)
  ...
  if ( data.googleCredential.scope.some(entry => config.google_scopes_map.profile.includes(entry)) ) {
    if ( new Date(sync_start_time).getTime() - new Date(data.googleCredential.last_sync_at).getTime() > 3 * 24 * 60 * 60 * 1000 ) {
  ...
  // Socket.send('Google.Account.Connected', data.googleCredential.user, []) // Should be happened just once
