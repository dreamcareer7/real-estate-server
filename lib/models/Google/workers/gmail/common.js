...
const parser = function (message) {
  ...
  if (message.labelIds) {
    ...
    Context.log('SyncGoogle generateGMesssageRecord message, payload', JSON.stringify(message.payload))
  }
...
const generateGMesssageRecord = function (credentialId, message) {
  try {
    ...
    return {
      ...
      label_ids: message.labelIds ? JSON.stringify(message.labelIds) : JSON.stringify([]),
    ...
    Context.log('SyncGoogle generateGMesssageRecord message,ex', JSON.stringify(message), ex)
    return null
  }
}
...
const fetchHistory = async function (google, messages_sync_history_id) {
  ...
  for await (const response of google.discreteHistory(50, messages_sync_history_id)) {
    ...
    for ( const history of response.data.history ) {
      ...
      if ( history.messagesAdded ) {
        for ( const msg of history.messagesAdded ) {
          messageIds = messageIds.add(msg.message.id)
        }
      }
    ...
    const batchRawResultAded = await google.batchGetMessages(Array.from(messageIds), fields)
