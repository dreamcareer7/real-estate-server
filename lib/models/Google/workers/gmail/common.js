...
const fetchMessages = async function (google) {
  ...
  for await (const response of google.discreteSyncMessages(50)) {
    ...
    */
    ...
    Context.log('---- fetchMessages rawMessages.length', rawMessages.length)

    if (batchRawResult.error) {

      Context.log('---- fetchMessages batchRawResult', batchRawResult)

      if ( batchRawResult.error.code === 429 || batchRawResult.error.code === 430 ) {

        Context.log('---- fetchMessages throw batchRawResult.error.message', batchRawResult.error.message)
