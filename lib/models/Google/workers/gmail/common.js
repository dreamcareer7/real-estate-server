...
const parser = function (message) {
  ...
  const inBound = (message.labelIds.includes('SENT')) ? false : true
...
const generateGMesssageRecord = function (credentialId, message) {
  try {
    ...
    const {
      recipientsArr, attachments, internetMessageId, inReplyTo, subject, inBound,
    ...
    Context.log('SyncGoogle generateGMesssageRecord message,ex', message, ex)
    return null
  }
}
