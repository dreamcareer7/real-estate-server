...
const bodyParser = function (message) {
  ...
  const result = {
    snippet: _.unescape(message.snippet),
    text: '',
    html: ''
  }
  ...
  while (parts.length) {
    ...
    if ( part.mimeType === 'text/plain' && !result.text ) {
      if(part.body.data) {
        result.text = _.unescape(decode(part.body.data))
      }
    }
...
const parser = function (message) {
  ...
  if (message.labelIds) {
    ...
    Context.log('SyncGoogle generateRecord message-has-not-labels, payload', JSON.stringify(message.payload))
  }
...
const generateRecord = function (credentialId, message, gmailLabels) {
  try {
...
const processLabels = async function (credentialId, messages) {
  ...
  for (const msg of messages) {
    msg.is_archived = (msg.label_ids.some(r => mainLabels.indexOf(r) >= 0) || msg.label_ids.some(r => gmailLabels.userLabels.indexOf(r) >= 0)) ? false : true
