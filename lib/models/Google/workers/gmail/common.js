...
const parser = function (message) {
  ...
  const to    = to_raw.filter(rec => { if (rec.address) return true }).map(rec => rec.address )
  const cc    = cc_raw.filter(rec => { if (rec.address) return true }).map(rec => rec.address )
  const bcc  = bcc_raw.filter(rec => { if (rec.address) return true }).map(rec => rec.address )
  ...
  if (message.labelIds) {
    ...
    Context.log('SyncGoogle generateGMesssageRecord message, payload', message, JSON.stringify(message.payload))
  }
...
const bodyParser = function (message) {
  // https://stackoverflow.com/questions/37445865/gmail-api-where-to-find-body-of-email-depending-of-mimetype/57361821
...
const generateGMesssageRecord = function (credentialId, message) {
  try {
    ...
    Context.log('SyncGoogle generateGMesssageRecord message,ex', message, JSON.stringify(message), ex)
    return null
  }
}
