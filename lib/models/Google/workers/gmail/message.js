...
const insertMessages = async (credential, messages) => {
  await processLabels(credential.id, messages)
  await GoogleMessage.create(messages, credential.id)

  Context.log('SyncGoogleMessages - syncMessages createdNum', messages.length)
}
  ...
  try {
    ...
    for ( const message of result.rawMessages ) {
      ...
      if ( messages.length === 50 ) {
        await insertMessages(credential, messages)
        messages = []
      }
    }
