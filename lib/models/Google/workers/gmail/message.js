...
const syncMessages = async (google, data, hasLostState = false) => {
  ...
  try {
    ...
    Context.log('SyncGoogle syncMessages syncedMessagesNum', credentialId, syncedMessagesNum[0]['count'])

    if ( syncedMessagesNum[0]['count'] !== 0 ) {
      hasSyncedMessages = true
    }

    Context.log('SyncGoogle syncMessages hasSyncedMessages', credentialId, hasSyncedMessages)

    if ( hasLostState || !hasSyncedMessages ) {
      toSyncNum = 500
    }

    Context.log('SyncGoogle syncMessages toSyncNum', credentialId, toSyncNum)
  
    const messages = await fetchMessages(google, toSyncNum)

    if (!messages) {
      // in case of rateLimitExceeded error
      return  {
        status: false,
        ex: { message: 'rateLimitExceeded', status: 429, statusCode: 429 },
        createdNum,
        totalNum: 0,
        hasSyncedMessages,
        historyId
