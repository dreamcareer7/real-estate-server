...
const { listLables, isArchived } = require('./labels')
...
const partialSync = async (google, data) => {
  ...
  try {
    ...
    const labels = await listLables(google, credentialId)
    ...
    if ( rawMessages.length ) {
      ...
      for ( const message of rawMessages ) {
        ...
        if ( message.labelIds && message.labelIds.includes('SPAM')) {
          ...
          if(record) {
            // record.is_archived = isArchived(message.labelIds, labels)
            googleMessages.push(record)

            console.log('record.is_archived', record.message_id, record.subject, isArchived(message.labelIds, labels))
          }
        }        
      }
