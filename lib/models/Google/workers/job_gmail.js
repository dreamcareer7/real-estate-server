...
const GoogleWorker = require('./index')
...
const syncGmail = async (data) => {
  Context.log('SyncGoogleMessages-handleWebhooks - worker', data)
...
const handleWebhooks = async (data) => {
  ...
  for (const credential of credentials) {
    try {
      ...
      Context.log('SyncGoogleMessages-handleWebhooks - push', data)

      // @ts-ignore
      await GoogleWorker.Gmail.syncGmail.immediate(data)

    } catch (ex) {
      // do nothing
      Context.log('SyncGoogleMessages-handleWebhooks - ex', ex)
    }
  }
