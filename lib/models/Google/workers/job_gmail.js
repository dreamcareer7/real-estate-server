...
const syncGoogle = async (data) => {
  ...
  if ( duplicateCheck || currentCredential.revoked || currentCredential.deleted_at ) {
    // Slack.send({ channel: 'integration_logs', text: `Google Sync Job Is Skipped - ${data.googleCredential.id} - ${data.googleCredential.email}`, emoji: ':bell:' })
    return
  }
  ...
  const handleException = async function(msg, ex) {
    ...
    if ( ex.statusCode === 401 || ex.message === 'invalid_grant' ) {
      await GoogleCredential.disconnect(data.googleCredential.id)
    }
...
const handleWebhooks = async (data) => {
  ...
  for (const credential of credentials) {
    try {
      await GoogleCredential.forceSyncGmail(credential.id)
    } catch (ex) {
      // do nothing
    }
  }
