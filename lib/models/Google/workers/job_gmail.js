...
const handleWebhooks = async (data) => {
  ...
  Context.log('SyncGoogle-debouncer handleWebhooks-jobHandler result.length', result.length)

  for (const credential of credentials) {
    try {
      Context.log('SyncGoogle-debouncer handleWebhooks-jobHandler forceSync', credential.id, credential.email)
      await GoogleCredential.forceSync(credential.id)
    } catch (ex) {
      Context.log('SyncGoogle-debouncer handleWebhooks-jobHandler forceSync-ex', credential.id, credential.email, ex.message)
      // do nothing
    }
  }
