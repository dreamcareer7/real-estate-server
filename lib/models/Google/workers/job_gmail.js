...
const handleException = async (credential, msg, ex) => {
  ...
  await addLastSyncRecord(credential)

  return
}

const addLastSyncRecord = async function(credential, info = {}) {
  return await GoogleSyncHistory.addSyncHistory({
    user: credential.user,
    brand: credential.brand,
    google_credential: credential.id,
    synced_messages_num: info.synced_messages_num || 0,
    messages_total: info.messages_total || 0,
    synced_contacts_num: 0,
    contacts_total: 0,
    sync_duration: info.sync_duration  || 0,
...
const syncGoogle = async (data) => {
  ...
  */
  ...
  if ( credential.scope_summary && credential.scope_summary.includes('mail.read') ) {
    Context.log('SyncGoogleMessages - syncLabels', credential.id, credential.email)
    ...
    Context.log('SyncGoogleMessages - syncLabels done', credential.id, credential.email)
  }
    ...
    if (dailySync) {
      ...
      if (credential.messages_sync_history_id) {
        ...
        if ( partialSyncResult.needsFullSync ) {
          ...
          Context.log('SyncGoogleMessages - partialSyncResult Needs-FullSync', credential.id, credential.email)
          await messagesFullSync(true)
          ...
          Context.log('SyncGoogleMessages - partialSyncResult', credential.id, credential.email)
  ...
  await addLastSyncRecord(credential, info)
  await UsersJob.upsertByGoogleCredential(credential, 'calendar', 'success')

  Context.log('SyncGoogle - job Finish', credential.id, credential.email, sync_duration)
