...
const checkForUpdate = async (brand, conn) => {
  ...
  const gContactObj = await GoogleContact.get(conn.resourceName)

  const gap         = Math.floor(contactObj.updated_at) - Math.floor(gContactObj.updated_at.getTime() / 1000)

  console.log('\n\ncontactObj.id', contactObj.id, contactObj.display_name)
  console.log(contactObj.updated_at, gContactObj.updated_at.getTime() / 1000)
  console.log('conds', gap, contactObj.deleted_at)
  console.log('if', gap < 2 && (contactObj.deleted_at === null))
  ...
  try {
    ...
    for (const conn of connections) {
      ...
      if ( conn.metadata.deleted ) {
        ...
        if (record) {
          const ids = await Contact.filter(brand, [], { google_id: conn.resourceName })

          contactIdsToDelete = contactIdsToDelete.concat(ids)
          gContactsToDelete.push(conn.resourceName)
    ...
    await Contact.update(user, brand, updatedContacts)


    // delete
    await GoogleContact.bulkDelete(gContactsToDelete)
    await Contact.delete(contactIdsToDelete, user)
