...
const getSingularAttribute = async (attributes, attribute_type) => {
  let target = null
...
const getMultipleAttribute = async (attributes, attribute_type) => {
  let targets = []
...
const updateRechatContact = async (user, brand, conn) => {
  ...
  const localGetSingularOldAttribute = async (attribute_type) => {
    return await getSingularAttribute(oldAttributes, attribute_type)
  }

  const localGetSingularGoogleAttribute = async (attribute_type) => {
    return await getSingularAttribute(googleAttributes, attribute_type)
  }

  const localGetMultipleOldAttribute = async (attribute_type) => {
    return await getMultipleAttribute(oldAttributes, attribute_type)
  }

  const localGetMultipleGoogleAttribute = async (attribute_type) => {
    return await getMultipleAttribute(googleAttributes, attribute_type)
  }

  for (const oldAttribute of oldAttributes) {
    for (const keyWord of singulars) {
      if ( oldAttribute['attribute_type'] === keyWord ) {
        const googleAttribute = await localGetSingularGoogleAttribute(keyWord)
    ...
    for (const keyWord of multiples) {
      if ( oldAttribute['attribute_type'] === keyWord ) {
        const googleAttributesArr = await localGetMultipleGoogleAttribute(keyWord)
  ...
  for (const googleAttribute of googleAttributes) {
    for (const keyWord of singulars) {
      if ( googleAttribute['attribute_type'] === keyWord ) {
        const oldAttribute = await localGetSingularOldAttribute(keyWord)
    ...
    for (const keyWord of multiples) {
      if ( googleAttribute['attribute_type'] === keyWord ) {
        const oldAttributesArr = await localGetMultipleOldAttribute(keyWord)
