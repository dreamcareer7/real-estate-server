...
const singulars  = ['first_name', 'last_name', 'cover_image_url', 'profile_image_url', 'birthday', 'website'] 
const multiples  = [ 'country', 'postal_code', 'city', 'street_name', 'email', 'phone_number']
const mixed      = ['company', 'job_title'] 
...
const parseConnection = (key, conn, contactGroups) => {
...
const getMultipleAttribute = (attributes, attribute_type, label) => {
  let target = null

  for (const att of attributes) {
    if ( att.attribute_type === attribute_type && att.label === label ) {
      target = att
      break
    }
  }
...
const updateRechatContact = async (user, brand, conn, contactGroups) => {
  ...
  for (const key in conn) {
    if (targetKeys.indexOf(key) >= 0) {
      const attributes = parseConnection(key, conn, contactGroups)
  ...
  const localGetMultipleOldAttribute = (attribute_type, label) => {
    return getMultipleAttribute(oldAttributes, attribute_type, label)
  }

  const localGetMultipleGoogleAttribute = (attribute_type, label) => {
    return getMultipleAttribute(googleAttributes, attribute_type, label)
  }

  for (const oldAttribute of oldAttributes) {

    for (const keyWord of mixed) {
    ...
    for (const keyWord of multiples) {
      if ( oldAttribute.attribute_type === keyWord ) {
        const googleAttribute = localGetMultipleGoogleAttribute(keyWord, oldAttribute.label)
  ...
  for (const googleAttribute of googleAttributes) {
    ...
    for (const keyWord of multiples) {
      if ( googleAttribute.attribute_type === keyWord ) {
        const oldAttribute = localGetMultipleOldAttribute(keyWord, googleAttribute.label)
