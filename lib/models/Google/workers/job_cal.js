...
const syncGoogleToRechat = async (google, credential) => {
  ...
  if ( calendarResult.status ) {
    if ( calendarResult.ex === 'rechat-primary-cal-is-deleted' ) {
      const message = 'Job Error - Google-Calendar Sync Stopped [rechat-to-google calendars]'
      return {
...
const syncCalendar = async (data) => {
  ...
  Context.log('SyncGoogleCalendar - last_updated_gt', new Date(last_updated_gt * 1000))
