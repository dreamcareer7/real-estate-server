...
const handleException = async function(data, msg, ex) {
...
const syncCalendar = async (data) => {
  ...
  if ( data.googleCredential.scope_summary.includes('calendar') ) {
    if (data.googleCredential.google_calendar) {
      ...
      if ( !calendarResult.status ) {
        ...
        await handleException(data, msg, calendarResult.ex)
        return
      }
      ...
      if ( !calendarEventsResult.status ) {
        ...
        await handleException(data, msg, calendarEventsResult.ex)
        return
      }
  ...
  // Update as Success
  if (!errorMsg) {
    const newestCalEventTS = calEvents[0] ? calEvents[0].last_updated_at : start_time
    ...
    Context.log(`SyncGoogleCalendar - Job Finished ${data.googleCredential.id} - ${data.googleCredential.email} - ${sync_duration}`)
    return
  }

  await UsersJobs.upsertByGoogleCredential(data.googleCredential, 'calendar', 'failed')
  Context.log(`SyncGoogleCalendar - Job Finished With Failure ${data.googleCredential.id} - ${data.googleCredential.email} - ${sync_duration} - Err: ${errorMsg}`)
  return
}
