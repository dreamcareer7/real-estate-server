...
const syncRechatToGoogle = async (credential, userJob, query) => {
  ...
  Context.log('SyncGoogleCalendar - RechatToGoogle', {user, calEvents, firstEvent})
  let error = null
  ...
  Context.log('SyncGoogleCalendar - RechatToGoogle', {isInitialSync})
  const { created, updated, deleted } = await refineEvents(credential, calEvents, user.timezone, isInitialSync)
  Context.log('SyncGoogleCalendar - RechatToGoogle',  { created, updated, deleted, error } )
