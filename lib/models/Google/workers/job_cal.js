...
const syncCalendar = async (data) => {
  ...
  const last_updated_gt  = cal_last_sync_at.getTime() / 1000 || null
  const query            = last_updated_gt ? { last_updated_gt } : {}

  const job = await UsersJobs.getByGoogleCredential(data.googleCredential.id, 'calendar')
  ...
  if ( duplicateCheck || currentCredential.revoked || currentCredential.deleted_at || ( job && job.status === 'pending') ) {
  ...
  const handleException = async function(msg, ex) {
    Context.log('handleException', msg, ex)
  ...
  Context.log('SyncGoogleCalendar - Start Job [Rechat To Google]', data.googleCredential.id, data.googleCredential.email, query)
  ...
  if ( !errorMsg && created.length !== 0 ) {
    const { error } = await handleCreatedEvents(data.googleCredential, created.slice(0, 15))
  ...
  // Update as Success
  if (!errorMsg) {
    const lastCalEventTS = calEvents[calEvents.length - 1].last_updated_gt // check last succesfull saved event

    await GoogleCredential.updateCalendarsLastSyncAt(data.googleCredential.id, new Date(lastCalEventTS))
