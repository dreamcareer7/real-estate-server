...
const syncGoogleToRechat = async (google, credential) => {
  ...
  Context.log('SyncGoogleCalendar - calendarResult', credential.id, credential.email)
  ...
  Context.log('SyncGoogleCalendar - calendarEventsResult', credential.id, credential.email)
...
const syncCalendar = async (data) => {
  ...
  if ( duplicateCheck || currentCredential.revoked || currentCredential.deleted_at ) {
    Slack.send({ channel: 'integration_logs', text: `Google-Calendar Sync Job Is Skipped - ${credential.id}`, emoji: ':bell:' })
    await UsersJobs.upsertByGoogleCredential(credential, 'calendar', 'failed')
    return
  }
  ...
  if (!google) {
    Slack.send({ channel: 'integration_logs', text: `Google-Calendar Sync Job Is Skipped, Client Is Failed - ${credential.id}`, emoji: ':skull:' })
    await UsersJobs.upsertByGoogleCredential(credential, 'calendar', 'failed')
    return
  }
  ...
  Context.log('SyncGoogleCalendar - [Google To Rechat] Done', credential.id, credential.email)
...
const googleCalWebhook = async (data) => {
  Context.log('SyncGoogle-debouncer googleCalWebhook-jobHandler', data)
  ...
  try {
    ...
    Context.log('SyncGoogle-debouncer gmailWebhook-jobHandler forceSyncCalendar', calendar.google_credential, calendar.summary)
  } catch (ex) {
    // do nothing
    Context.log('SyncGoogle-debouncer gmailWebhook-jobHandler forceSyncCalendar-ex', calendar.google_credential, calendar.summary, ex.message)
  }
