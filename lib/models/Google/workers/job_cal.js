...
const syncCalendar = async (data) => {
  ...
  if (!credential.scope_summary.includes('calendar') || !credential.google_calendar) {
    Slack.send({ channel: 'integration_logs', text: `Google Sync-Calendar Job Is Skipped - ${data.googleCredential.id} - ${data.googleCredential.email}`, emoji: ':bell:' })
  ...
  if ( duplicateCheck || currentCredential.revoked || currentCredential.deleted_at ) {
    Slack.send({ channel: 'integration_logs', text: `Google-Calendar Sync Job Is Skipped - ${credential.id} - ${credential.email}`, emoji: ':bell:' })
    await UsersJobs.upsertByGoogleCredential(credential, 'calendar', 'failed')
    return
  }
