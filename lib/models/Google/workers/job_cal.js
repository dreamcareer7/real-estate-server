...
const { retrieveTasks }   = require('./calendars/helpers/tasks')
...
const syncCalendar = async (data) => {
  ...
  if ( upsertedTaskIds.length || deletedTaskIds.length ) {
    const upserted = await retrieveTasks(credential, upsertedTaskIds)
    Socket.send('Calendar.Updated', credential.user, [{ upserted, deleted: deletedTaskIds }])
  }
