...
const syncCalendar = async (data) => {
  ...
  if ( !credential.google_calendar || credential.revoked || credential.deleted_at ) {
  ...
  if (!userJob) {
    Context.log('SyncGoogleCalendar - Job skipped due to a pending job', credential.id, credential.email)
    return
  }
...
const handleWebhooks = async (data) => {
  Context.log('handleWebhooks', data)
  ...
  Context.log('handleWebhooks calendar', calendar)
  ...
  Context.log('handleWebhooks credential', credential.id)
  ...
  if ( calendar.watcher_channel_id !== channelId ) {
    ...
    Context.log('handleWebhooks here')
  ...
  try {
    ...
    Context.log('handleWebhooks ex', ex)
    // do nothing
  }

  Context.log('handleWebhooks end')
