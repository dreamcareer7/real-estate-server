...
const syncRechatToGoogle = async (credential, userJob, query) => {
  Context.log(`SyncGoogleCalendar - RechatToGoogle credential:${credential}, userJob:${userJob}, query:${query}`)
  const { user, calEvents } = await getEvents(credential, query)
  
  const firstEvent = calEvents[calEvents.length - 1] || null
  Context.log(`SyncGoogleCalendar - RechatToGoogle user:${user}, calEvents:${calEvents}, firstEvent:${firstEvent}`)
  let error = null
  ...
  Context.log(`SyncGoogleCalendar - RechatToGoogle isInitialSync:${isInitialSync}`)
  const { created, updated, deleted } = await refineEvents(credential, calEvents, user.timezone, isInitialSync)
  Context.log(`SyncGoogleCalendar - RechatToGoogle  created:${created}, updated:${updated}, deleted:${deleted}, error:${error}`)
