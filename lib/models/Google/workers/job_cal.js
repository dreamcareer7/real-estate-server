...
const { getEvents }       = require('../../Calendar/feed')
...
const syncGoogleToRechat = async (google, credential) => {
  ...
  upsertedNum = calendarEventsResult.upsertedNum
  deletedNum  = calendarEventsResult.deletedNum
...
const syncCalendar = async (data) => {
  ...
  if ( result.upsertedNum || result.deletedNum ) {
    Socket.send('Calendar.Updated', credential.user, { upsertedNum: result.upsertedNum, deletedNum: result.deletedNum })
  }
