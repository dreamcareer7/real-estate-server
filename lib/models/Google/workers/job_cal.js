...
const syncGoogleToRechat = async (google, credential) => {
  ...
  if ( !calendarResult.status ) {
    const message = 'Job Error - Google-Calendar Sync Failed [rechat-to-google calendars]'
  ...
  if ( !calendarEventsResult.status ) {
    const message = 'Job Error - Google-Calendar Sync Failed [rechat-to-google events]'
...
const syncRechatToGoogle = async (credential, query) => {
  const associations = ['calendar_event.full_crm_task', 'crm_task.assignees', 'crm_task.reminders']
  ...
  Context.log('SyncGoogleCalendar calEvents[first]', calEvents[0].title, new Date(calEvents[0].timestamp * 1000), new Date(calEvents[0].last_updated_at))
  Context.log('SyncGoogleCalendar calEvents[last]', calEvents[calEvents.length - 1].title, new Date(calEvents[calEvents.length - 1].timestamp * 1000), new Date(calEvents[calEvents.length - 1].last_updated_at))
  ...
  if ( !error && updated.length !== 0 ) {
    const result = await handleUpdatedEvents(updated)
...
const syncCalendar = async (data) => {
  ...
  const last_updated_gt  = cal_last_sync_at.getTime()
  ...
  Context.log('SyncGoogleCalendar - [Google To Rechat]', credential.id, credential.email, ' - Result:', result)


  Context.log('SyncGoogleCalendar - Job Started [Rechat To Google]', credential.id, credential.email)
  ...
  const ts = result.firstEvent ? result.firstEvent.last_updated_at : start_time
...
const syncCalendarTest = async (data) => {
  const credential = data.googleCredential
  ...
  Context.log('SyncGoogleCalendar - start_time', start_time)
  Context.log('SyncGoogleCalendar - last_updated_gt', last_updated_gt)


  let result = {}

  // Context.log('SyncGoogleCalendar - Job Started [Google To Rechat]', credential.id, credential.email)
  // result = await syncGoogleToRechat(google, credential)
  // if (result.error) {
  //   await reportFailure(credential, result.error)
  //   return
  ...
  Context.log(`SyncGoogleCalendar - Job Finished sync_duration: ${sync_duration}`)
  Context.log(`SyncGoogleCalendar - Job Finished timestatmp: ${ts}`)
