...
const syncAvatars = async (google, credential) => {
  ...
  const helper = async (gcontact) => {
    ...
    toUpdateGoogleContacts.push({ google_credential, entry_id, photo, processed_photo: photo ? true : false })
  ...
  try {
    ...
    const googleContacts = await GoogleContact.filter({ google_credential: credential.id, source: 'contacts', processed_photo: false })
