    ...
    for (let i = 0; i < entry.emailes.length; i++) {
      ...
      if (emailObj.address) {
        attributes.push({
          ...
          label: label,
          is_primary: false //i === 0 ? true : false
        })
      }
    }
  }
...
const findNewAttributes = async (contact, oldAttributes) => {
  ...
  // console.log('\n\nrawData', rawData)
  // console.log('\n\noldRawData', oldRawData)
  // console.log('\n\nrefinedAtts', refinedAtts)
...
const syncContacts = async (google, data) => {
  ...
  try {
    ...
    if (entries.length) {
      ...
      for (const contact of contacts) {
        if ( oldGoogleContactEntryIds.includes(contact.entry_id) ) {
          ...
          if ( !oldGContact.deleted_at ) {
            ...
            const result = await Contact.filter(brand, [], { google_id: oldGContact.id })
      ...
      // Updated Contacts
      const updatedMicrosoftContacts = await GoogleContact.create(toUpdateRecords)
      const updatedContacts          = await Contact.update(toUpdateContacts, user, brand, 'google_integration')


      console.log('****** in-syncContacts-worker Here', new Date())
    ...
    console.log('****** in-syncContacts-worker Done', new Date())
    return
