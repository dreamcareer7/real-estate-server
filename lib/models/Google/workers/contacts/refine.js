...
const refineConnections = (connections) => {
  return connections
    ...
    .map(connection => {
      ...
      const names        = connection.names ? contacts.getNames(connection.names) : null
      ...
      const phones       = connection.phoneNumbers ? contacts.getPhones(connection.phoneNumbers) : null
      const emailes      = connection.emailAddresses ? contacts.getEmails(connection.emailAddresses) : null
      const addresses    = connection.addresses ? contacts.getAddresses(connection.addresses) : null
      const memberships  = connection.memberships ? contacts.getMemberships(connection.memberships) : null
...
const refineOtherContacts = (connections) => {
  return connections
    ...
    .map(connection => {
      ...
      const names        = connection.names ? otherContacts.getNames(connection.names) : null
      const phones       = connection.phoneNumbers ? otherContacts.getPhones(connection.phoneNumbers) : null
      const emailes      = connection.emailAddresses ? otherContacts.getEmails(connection.emailAddresses) : null
