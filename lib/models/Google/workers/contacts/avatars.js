...
const syncAvatars = async (google, credential) => {
  ...
  try {
    ...
    Context.log('SyncGoogleContactsAvatars - syncAvatars method toUpdateGoogleContacts', toUpdateGoogleContacts.length)
    Context.log('SyncGoogleContactsAvatars - syncAvatars method toUpdateContacts', toUpdateContacts.length)

    Context.log('SyncGoogleContactsAvatars - syncAvatars method before bulkUpdate')

    // Updated Contacts
    await GoogleContact.bulkUpdate(toUpdateGoogleContacts)
    Context.log('SyncGoogleContactsAvatars - syncAvatars method after bulkUpdate')

    await Contact.update(toUpdateContacts, user, brand, 'google_integration')

    Context.log('SyncGoogleContactsAvatars - syncAvatars method after update')
