...
const syncPeople = async (google, credential) => {
  try {
    ...
    if (credential?.scope_summary?.includes('contacts.other.read')) {
      ...
      Context.log('SyncGoogleContacts - confirmedOther.length', confirmedOther.length)
      Context.log('SyncGoogleContacts - deletedOther.length', deletedOther.length)
      ...
      */
      ...
      Context.log('SyncGoogleContacts - deletedOtherFiltered.length', deletedOtherFiltered.length)
