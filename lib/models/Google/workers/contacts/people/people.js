...
const rechatToGoogle = async (google, credential, last_updated_gt) => {
  try {
    ...
    console.log('created.ids', created.ids)
    console.log('updated.ids', updated.ids)
    console.log('deleted.ids', deleted.ids)
    ...
    const retrieveCreated = async () => {
      ...
      const alreadySyncedContactIds = intRecords.map(r => r.contact)
