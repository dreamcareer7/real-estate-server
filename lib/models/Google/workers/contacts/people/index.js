...
const googleToRechat = async (google, credential) => {
  try {
    Context.log('syncGoogleContacts', 'in googleToRechat method')
    let upsertedNum = 0
    let deletedNum  = 0
    ...
    Context.log('syncGoogleContacts', 'after refine')
    ...
    Context.log('syncGoogleContacts', 'after process', {upsertedNum, deletedNum, numOfUpserted, numOfDeleted} )
    ...
    if (credential?.scope_summary?.includes('contacts.other.read')) {
      ...
      */
      ...
      Context.log('syncGoogleContacts', 'after process other contacts', {upsertedNum, deletedNum, numOfUpserted, numOfDeleted} )  
      if (otherContactsResult.syncToken) {
        await updateOtherContactsSyncToken(credential.id, otherContactsResult.syncToken)
      }
    }

    Context.log('syncGoogleContacts', 'before return', {upsertedNum, deletedNum} )
    return {
      status: true,
      upsertedNum,
      deletedNum
