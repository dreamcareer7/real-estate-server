...
async function handleUpdated (google, credential, contactIds) {
  ...
  Context.log('handleUpdated contactIds', contactIds)
  ...
  Context.log('handleUpdated googleContactsById', googleContactsById)
  
  const records = generateGContacts(contacts)

  const refined = records.map(record => {
    Context.log('handleUpdated record', record)
    Context.log('handleUpdated googleContactsById[record.gcontactId]', googleContactsById[record.gcontactId])
  ...
  Context.log('handleUpdated refined', refined)
