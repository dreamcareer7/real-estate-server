...
const handleNewRecords = async (credential, contactIds, googleContacts) => {
  ...
  Context.log('SyncGoogleContacts - handleNewRecords - integrationRecords.length', integrationRecords.length)
...
const handleToUpdateRecords = async (credential, contactIds, googleContacts) => {
  ...
  Context.log('SyncGoogleContacts - handleToUpdateRecords - integrationRecords.length', integrationRecords.length)
  ...
  Context.log('SyncGoogleContacts - handleToUpdateRecords - updatedGContactIds.length', updatedGContactIds.length)
  Context.log('SyncGoogleContacts - handleToUpdateRecords - alreadyCreatedRecords.length', alreadyCreatedRecords.length)
  Context.log('SyncGoogleContacts - handleToUpdateRecords - contactsWithoutIntegrationRecords.length', contactsWithoutIntegrationRecords.length)
  Context.log('SyncGoogleContacts - handleToUpdateRecords - googleContactsWithoutIntegrationRecords.length', googleContactsWithoutIntegrationRecords.length)
...
const processConfirmed = async (credential, confirmed, parked) => {
  ...
  Context.log('SyncGoogleContacts - googleContacts.length', googleContacts.length)

  const oldDeletedGContactEntryIds = googleContacts.filter(c => c.deleted_at).map(c => c.entry_id)
  const oldGContacts               = googleContacts.filter(c => !c.deleted_at)
  const oldGContactEntryIds        = oldGContacts.map(c => c.entry_id)
  const oldGContactsByEntryId      = keyBy(oldGContacts, 'entry_id')

  Context.log('SyncGoogleContacts - oldDeletedGContactEntryIds.length', oldDeletedGContactEntryIds.length)
  Context.log('SyncGoogleContacts - oldGContacts.length', oldGContacts.length)

  const refined = confirmed.filter(c => !oldDeletedGContactEntryIds.includes(c.entry_id))

  Context.log('SyncGoogleContacts - refined.length', refined.length)


  for (const contact of refined) {
    const temp = {
  ...
  Context.log('SyncGoogleContacts - toUpdateGContacts.length', toUpdateGContacts.length)
  Context.log('SyncGoogleContacts - toExcludeGContactIds.length', toExcludeGContactIds.length)
  Context.log('SyncGoogleContacts - toUpdateContactIds.length', toUpdateContactIds.length)
  ...
  Context.log('SyncGoogleContacts - toUpdateContacts.length', toUpdateContacts.length)
  Context.log('SyncGoogleContacts - createdGContacts.length', createdGContacts.length)
  ...
  Context.log('SyncGoogleContacts - newContacts.length', newContacts.length)

  const createdContactIds = await Contact.create(newContacts, credential.user, credential.brand, _REASON, { activity: false, relax: true, get: false })
  const updatedContactIds = await Contact.update(toUpdateContacts, credential.user, credential.brand, _REASON)
  const updatedGContacts  = await GoogleContact.create(toUpdateGContacts)


  Context.log('SyncGoogleContacts - createdContactIds.length', createdContactIds.length)
  Context.log('SyncGoogleContacts - updatedContactIds.length', updatedContactIds.length)
  Context.log('SyncGoogleContacts - updatedGContacts.length', updatedGContacts.length)

  const updatedGContactsExcludingDeleted = updatedGContacts.filter(ugc => !toExcludeGContactIds.includes(ugc.id))

  Context.log('SyncGoogleContacts - updatedGContactsExcludingDeleted.length', updatedGContactsExcludingDeleted.length)
