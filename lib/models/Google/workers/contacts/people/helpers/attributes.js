const { isEqual, groupBy, difference } = require('lodash')
...
const findNewAttributes = (resource, oldResource, oldAttributes, contactGroups) => {
  ...
  /** @type {UUID[]} */
  const deletedAtt = [] // eslint-disable-line no-unused-vars
  ...
  if ( oldResource?.memberships && resource.memberships && !haveSameMembers(resource.memberships, oldResource.memberships) ) {
  ...
  for ( const phone of resource.phones ) {
    ...
    const label = phoneLabelMapping[phone?.type?.toLowerCase?.()] ?? 'Other'
  ...
  for ( const email of resource.emailes ) {
    ...
    const label = emailLabelMapping[email?.type?.toLowerCase?.()] ?? 'Other'
  ...
  if ( oldResource && resource.addresses.length ) {
    ...
    let maxIndex = Math.max(1, ...addressArr.map(e => Number(e.index)))
    ...
    for ( const address of addressesArr ) {
      ...
      const label = addressLabelMapping[address?.type?.toLowerCase?.()] ?? 'Other'
