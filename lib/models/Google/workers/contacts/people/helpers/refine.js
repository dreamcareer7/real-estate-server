...
const refineConnections = (connections) => {
  ...
  */
  ...
  const passed        = connections.filter(c => !c.error)

  const toRetry = notFound.length + failed.length

  Context.log('refineConnections - passed:', passed.length, ' - failed:', failed.length, ' - notFound:', notFound.length, ' - QuotaExceeded:', QuotaExceeded.length, ' - toRetry:', toRetry)
...
const refineOtherContacts = (connections) => {
  ...
  Context.log('refineOtherContacts - passed:', passed.length, ' - failed:', failed.length, ' - notFound:', notFound.length, ' - QuotaExceeded:', QuotaExceeded.length, ' - toRetry:', toRetry)

  const deleted = passed
    .filter(c => !c.error)
    .filter(con => ((con?.metadata?.objectType === 'PERSON') && con.metadata.deleted))
    .map(con => {
