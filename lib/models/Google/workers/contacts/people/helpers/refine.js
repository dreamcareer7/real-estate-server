...
const refineConnections = (connections) => {
  ...
  */
  ...
  const failed   = connections.filter(c => (c.error && c.error.code !== 404))
  const notFound = connections.filter(c => (c.error && c.error.code === 404))

  // We need a way to extract details of the failed and notFound records. we can exclude confirmed of connections ???


  if ( failed.length ) {
    Context.log('SyncGoogleContacts - refineConnections failed', JSON.stringify(failed))
    throw Error.Validation('Some of Google contact objects have invalid payload!')
  }
