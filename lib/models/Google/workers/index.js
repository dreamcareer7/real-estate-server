...
const syncGoogle = async (data) => {
  ...
  if ( data.googleCredential.scope.some(entry => config.google_scopes_map.contacts.includes(entry)) ) {
    ...
    Socket.send('Google.Contacts.Imported', data.googleCredential.user, [contactsLastRsult.contactsNum])
  }
  ...
  await GoogleCredential.updateLastSyncTime(data.googleCredential.id, sync_finish_time, sync_duration)
