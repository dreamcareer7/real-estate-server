...
const Gmail = {
  ...
  syncByQuery: async () => {
    ...
    const records = await db.select('users_job/google/gmail_sync_by_query', [config.emails_integration.sync_by_quey.time_gap])
...
const Contacts = {
  ...
  syncAvatars: async () => {
    ...
    const records = await db.select('users_job/google/gcontacts_avatarts_sync_due', [config.contacts_integration.time_gap])

    for (const record of records) {

      Context.log('Google-syncAvatars record', record)
      ...
      Context.log('Google-syncAvatars data', data)
