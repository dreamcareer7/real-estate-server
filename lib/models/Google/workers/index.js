...
const syncGoogle = async (data) => {
  ...
  const handleException = async function(msg, ex) {
    Context.log('------ handleException', msg, ex.message, ex)
  ...
  if ( data.googleCredential.scope.some(entry => config.google_scopes_map.profile.includes(entry)) ) {
    ...
    Context.log('profileResult', data.googleCredential.id, profileResult)
  ...
  if ( data.googleCredential.scope.some(entry => config.google_scopes_map.contacts.includes(entry)) ) {
    ...
    Context.log('contactGroupsResult', data.googleCredential.id, contactGroupsResult)
    ...
    Context.log('contactsLastRsult', data.googleCredential.id, contactsLastRsult)
  ...
  Context.log('--- here 1', data.googleCredential.id)
  await GoogleCredential.updateLastSync(data.googleCredential.id, sync_finish_time, sync_duration)
  Context.log('--- here 2', data.googleCredential.id)
  await addLastSyncRecord(true)
  Context.log('--- here 3', data.googleCredential.id)
