...
const Contacts = {
  syncDue: async () => {
    ...
    for (const record of records) {
      ...
      Context.log('SyncGoogleContacts-Debug - record', record)
      Context.log('SyncGoogleContacts-Debug - diff', diff)

      if ( diff < config.contacts_integration.miliSec ) {
        Context.log('SyncGoogleContacts-Debug - Job queuing skipped due to recently queued job', record.id)
        ...
        Context.log('SyncGoogleContacts-Debug - Job  queued', record.id)
