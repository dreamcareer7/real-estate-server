...
const postponeJob = async (userJob, ex) => {
  Context.log('Postpone-Google-Job', userJob.google_credential, userJob.job_name, ex.code, ex.statusCode, ex.message, ex.errors, ex)
...
const googleClient = async (credential, userJob) => {
  ...
  if (!google) {
    Slack.send({ channel: 'integration_logs', text: `Initiate Google Client Failed - ${credential.id}`, emoji: ':skull:' })
    await updateStatus(userJob.id, 'failed')
