...
const { postponeByGoogleCredential, deleteByGoogleCredential } = require('../../UsersJob/microsoft')
...
const reportFailure = async (cid, error) => {
  const emoji = ':skull:'
  const text  = `SyncGoogleCalendar - Job Finished With Failure ${cid} - ${error}`
...
const postpone = async (userJob, ex) => {
  ...
  await postponeByGoogleCredential(userJob.google_credential, userJob.job_name, interval)
}

const handleException = async (userJob, msg, ex) => {
  const text  = `${msg} - StatusCode: ${ex.statusCode || ex.code} - Message: ${ex.message} - CID: ${userJob.google_credential}`
  const emoji = ':skull:'
