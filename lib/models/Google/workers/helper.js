...
const lockJob = async (credential, userJob) => {
  /*
  ...
  */
  ...
  await UsersJob.lockByGoogleCredential(credential.id, userJob.job_name)
...
const postpone = async (credential, userJob, ex) => {
  Context.log('Postpone-Google-Job', credential.id, userJob.job_name, ex.code, ex.statusCode, ex.message, ex.errors)
  ...
  await UsersJob.postponeByGoogleCredential(credential.id, userJob.job_name, interval)
}

const handleException = async (credential, userJob, msg, ex) => {
  const obj = {
    id: credential.id,
  ...
  if (invalidGrant) {
    ...
    await postpone(credential, userJob, ex)
  }
}
