...
const syncContacts = async (data) => {
  ...
  */
  ...
  if ( (userJob !== 'waiting') && (diff < config.contacts_integration.miliSec) ) {
    // Context.log('SyncGoogleContacts - Job skipped due to recently finished job', credential.id, credential.email)
    return
  }
