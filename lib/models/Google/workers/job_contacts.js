...
const syncContacts = async (data) => {
  ...
  if ( credential?.scope_summary?.includes('contacts.read') ) {
    const contactGroupsResult = await syncContactGroups(google, credential)
  ...
  // People
  if ( credential?.scope_summary?.includes('contacts.read') ) {
    const peopleResult = await syncPeople(google, credential)

    if ( !peopleResult.status ) {
      const message = 'Job Error - syncPeople Failed [Google To Rechat]'
      await handleException(userJob, message, peopleResult.ex)
      return
    }

    synced_contacts_num += (peopleResult.createdNum || 0)
  }
