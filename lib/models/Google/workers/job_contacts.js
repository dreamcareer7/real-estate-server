...
const syncGoogleToRechat = async (google, credential, userJob) => {
  Context.log('syncGoogleContacts', 'in syncGoogleToRechat method')
  ...
  Context.log('syncGoogleContacts', 'after syncContactGroups', JSON.stringify(cGroupResult))
  if (!cGroupResult.status) {
    return cGroupResult
  }

  // People [Google To Rechat]
  Context.log('syncGoogleContacts', 'before googleToRechat')
...
const syncContacts = async (data) => {
  ...
  Context.log('SyncGoogleContacts - start', credential.id, credential.email)
