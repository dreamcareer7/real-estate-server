...
const removeConnection = async (brand, user, conn) => {
  ...
  if (record) {
    ...
    await ContactAttribute.delete(ids, user, 'google_integration')
    await Contact.delete([currentContact.id], user, 'google_integration')
  }
...
const checkForUpdate = async (brand, conn) => {
  ...
  const gap         = Math.abs(Math.floor(contactObj.updated_at) - Math.floor(gContactObj.updated_at.getTime() / 1000))
