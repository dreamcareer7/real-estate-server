...
const setupMapping = async (credential, calendar, confirmed, cancelled) => {
  ...
  Context.log('newEventSeries', JSON.stringify(newEventSeries))
  if(newEventSeries.length > 0) {
    const toRemove = oldEventsRemoteId.filter((oldEventId) => newEventSeries.includes(oldEventId))
    Context.log('toRemove', JSON.stringify(toRemove))
    deletedEventRemoteIds.push(...toRemove)
  }
  Context.log('deletedEventRemoteIds', JSON.stringify(deletedEventRemoteIds))
...
const syncCalendarEvents = async (google, credential) => {
  ...
  try {
    ...
    for ( const calendar of toSyncRemoteCalendars ) {
      ...
      Context.log('after fetch confirmed', JSON.stringify(confirmed))
      Context.log('after fetch canceled', JSON.stringify(cancelled))
