...
const syncCalendarEvents = async (google, data) => {
  ...
  try {
    ...
    for ( const calendar of toSyncRemoteCalendars ) {
      ...
      let canceledGoogleEventRemoteIds = []
      ...
      for ( const event of cancelled ) {
        canceledGoogleEventRemoteIds.push(event.id)
      }

      const confirmedEventRemoteIds = confirmed.map(e => e.id)
      const oldGoogleEvents         = await GoogleCalendarEvents.getByCalendarAndEventIds(googleCredentialId, calendar.id, confirmedEventRemoteIds)
      ...
      */
      ...
      for ( const event of confirmed ) {
        ...
        const e = generateCalendarEventRecord(googleCredentialId, calendar, e)
        if (e) newEvents.push(event)
      ...
      const canceledGoogleEventIds = await GoogleCalendarEvents.deleteManyByRemoteIds(googleCredentialId, calendar.id, canceledGoogleEventRemoteIds)
