...
const handleNewCrmTasks = async (updatedEventRemoteIds, associationsMap, credential, confirmed) => {
...
const syncCalendarEvents = async (google, data) => {
  ...
  try {
    ...
    for ( const calendar of toSyncRemoteCalendars ) {      
      ...
      /***  Handle new CRM_TASK  ***/
      await handleNewCrmTasks(updatedEventRemoteIds, associationsMap, data.googleCredential, confirmed)
