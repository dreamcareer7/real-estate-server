...
const setupMapping = async (credential, calendar, confirmed, cancelled) => {
  ...
  if(newEventSeries.length > 0) {
    ...
    Context.log('oldSingleGoogleEvents', JSON.stringify(oldSingleGoogleEvents))
    const toRemove = oldSingleGoogleEvents.map(x => x.event_id)
    deletedEventRemoteIds.push(...toRemove.filter(x => x !== null ))
  }
