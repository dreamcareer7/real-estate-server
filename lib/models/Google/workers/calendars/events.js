...
const syncCalendarEvents = async (google, data) => {
  ...
  try {
    ...
    for ( const calendar of toSyncRemoteCalendars ) {
      ...
      const oldGoogleEvents         = await GoogleCalendarEvent.getByCalendarAndEventIds(googleCredentialId, calendar.id, confirmedEventRemoteIds)
      ...
      */
      ...
      const canceledGoogleEventIds = await GoogleCalendarEvent.deleteManyByRemoteIds(googleCredentialId, calendar.id, canceledGoogleEventRemoteIds)
