...
const setupMapping = async (credential, calendar, confirmed, cancelled) => {
  Context.log('SyncGoogleCalendar', credential.id, calendar.id, 'confirmed.length', confirmed.length, confirmed)
  Context.log('SyncGoogleCalendar', credential.id, calendar.id, 'cancelled.length', cancelled.length, cancelled)
