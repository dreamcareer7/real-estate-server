...
const setupMapping = async (credential, calendar, confirmed, cancelled) => {
  ...
  if(newEventSeries.length > 0) {
    Context.log('oldEventsRemoteId', JSON.stringify(oldEventsRemoteId))
    const toRemove = oldEventsRemoteId.filter((oldEventRemoteId) => newEventSeries.includes(oldEventRemoteId))
