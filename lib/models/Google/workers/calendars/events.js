    ...
    for ( const calendar of toSyncRemoteCalendars ) {
      ...
      for ( const event of confirmed ) {
        ...
        if ( confirmedEvents.length === 50 ) {
          const events = await GoogleCalendarEvents.bulkUpsert(confirmedEvents)
      ...
      for ( const event of cancelled ) {
        ...
        const id = await GoogleCalendarEvents.updateLocalStatusByRemoteId(googleCredentialId, calendar.id, event.id, 'cancelled')
