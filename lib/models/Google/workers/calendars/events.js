...
const getToSyncCalendars = async function (gcid) {
  ...
  return calendars.filter(cal => (cal.watcher_status !== 'stopped' && !cal.deleted_at ))
}
...
const hadnleConfirmedEvents = async (calendar, confirmed, oldEventsRemoteId, updatedEventRemoteIds) => {
  const filterd = confirmed.filter(c => { if ( !oldEventsRemoteId.includes(c.id) || updatedEventRemoteIds.includes(c.id)) return true })
  const events  = filterd.map(event => generateCalendarEventRecord(calendar, event))
...
const updateCrmTasks = async (updatedEventRemoteIds, eventsByRemoteId, tasksByGoogleId, credential, associationsMap, confirmed) => {
  ...
  const toUpdateCalIntRecords = confirmed.filter(event => { if (updatedEventRemoteIds.includes(event.id)) return true }).map(event => {
    return {
  ...
  await CalendarIntegration.upsert(toUpdateCalIntRecords)

  await CrmTask.updateMany(toUpdateTasks, credential.user) // In case of restoring a deleted remote-event, create a new crm_task record !!!!
}

const handleNewCrmTasks = async (oldEventsRemoteId, eventsByRemoteId, associationsMap, credential, confirmed) => {
  if ( confirmed.length === 0 ) {
    return
  }
...
const syncCalendarEvents = async (google, credential) => {
  ...
  try {
    ...
    for ( const calendar of toSyncRemoteCalendars ) {
      ...
      const { oldEventsRemoteId, canceledEventRemoteIds, updatedEventRemoteIds, tasksByGoogleId, associationsMap } = await setupMapping(credential, calendar, confirmed, cancelled)

      confirmedNum += confirmed.length


      /***  Handle Confirmed(Created/Updated) Events ***/
      const eventsByRemoteId = await hadnleConfirmedEvents(calendar, confirmed, oldEventsRemoteId, updatedEventRemoteIds)
      ...
      /***  Handle new CRM_TASK  ***/
      await handleNewCrmTasks(oldEventsRemoteId, eventsByRemoteId, associationsMap, credential, confirmed)
