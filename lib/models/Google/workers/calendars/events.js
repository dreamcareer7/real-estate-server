...
const setupMapping = async (credential, calendar, confirmed, cancelled) => {
  ...
  const updatedEventRemoteIdsBySameEtag = confirmed.filter(event => { if (oldEventsRemoteId.includes(event.id) && (oldEventsByEventId[event.id].etag === event.etag)) return true }).map(event => event.id)

  Context.log('SyncGoogleCalendar updatedEventRemoteIds.length', updatedEventRemoteIds.length)
  Context.log('SyncGoogleCalendar updatedEventRemoteIdsBySameEtag.length', updatedEventRemoteIdsBySameEtag.length)
