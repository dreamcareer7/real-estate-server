...
const handleUpdatedEvents = async (credential, events) => {
  const remoteEvents = events.map(e => ({
    gid: e.integrations[0].googleCalEvent.id,
  ...
  if (googleCalEvents) {
    ...
    if ( failedEventIds && failedEventIds.length > 0 ) {
      ...
      const intRecords   = await CalendarIntegration.getByGoogleIds(failedEventIds)
      ...
      // await GoogleCalendarEvent.deleteMany(failedEventIds)
