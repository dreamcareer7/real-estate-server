...
const generateGoogleCalendarEvent = (event, timezone) => {
  ...
  // event.full_crm_task.reminders

  if (event.full_crm_task && event.full_crm_task.assignees) {
    attendees = event.full_crm_task.assignees.map(a => { if(a.email) return { 'email': a.email, 'displayName': a.display_name } })
  }
...
const refineEvents = async (credential, calEvents, timezone) => {
  ...
  for (const event of events_level_2) {
    event.resource = generateGoogleCalendarEvent(event, timezone)
    ...
    for (const record of event.integrations) {
      ...
      if ( record.googleCal.google_credential === credential.id ) {
        ...
        if ( record.etag === record.local_etag ) {
          Context.log('SyncGoogleCalendar skip event', event.id, record.id)
          break
        }

        // Context.log('SyncGoogleCalendar push event to updated', event.id, record.id)
        event.integrations = [record]

        // Handle all_day events
        const isGEventAllDay = record.googleCalEvent.event_start.date ? true : false
        const isREventAllDay = event.resource.start.date ? true : false

        if (isGEventAllDay && !isREventAllDay ) {
          event.resource.start.date = new Date(event.resource.start.dateTime).toISOString().slice(0,10)
          event.resource.end.date   = new Date(event.resource.end.dateTime).toISOString().slice(0,10)
...
const handleCreatedEvents = async (credential, events) => {
  ...
  if (googleCalEvents) {
    const records = googleCalEvents.map(record => {
      const event = eventsById[record.extended_properties.shared.rechat_cal_event_id]
      return {
        google_id: record.id,
        microsoft_id: null,
...
const handleUpdatedEvents = async (credential, events) => {
  ...
  const { googleCalEvents, failedEventIds, error } = await GoogleCalendarEvent.batchUpdate(credential, remoteEvents)
  ...
  if (googleCalEvents) {
    ...
    const result = await CalendarIntegration.upsert(toUpdateCalIntRecords)


    // delete calendar_integration and google_Calendar_events records 
    // Find relted_crm_tasks and set their updated_at as now()
    const intRecords   = await CalendarIntegration.getByCrmTasks(failedEventIds)
    const intRecordIds = intRecords.map(r => r.id)
    const geventIds    = intRecords.filter(r => r.google_id).map(r => r.google_id)
    const crmTaskIds   = intRecords.filter(r => r.crm_task).map(r => r.crm_task)
    const _REASON      = config.google_integration.crm_task_update_reason

    console.log('---- failedEventIds', failedEventIds)
    console.log('---- intRecords', intRecords)
    console.log('---- intRecordIds', intRecordIds)
    console.log('---- crmTaskIds', crmTaskIds)

    await CalendarIntegration.deleteMany(intRecordIds)
    await GoogleCalendarEvent.deleteMany(geventIds)
