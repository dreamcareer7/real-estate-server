  ...
  for (const event of events_level_1) {
    ...
    for (const record of event.integrations) {      
      ...
      if ( !googleCal.deleted_at ) {
        // console.log('***** refineEvents level#2 event\'s calendar is available', event.id, record.id, googleCalEvent.id, googleCal.id)
        ...
        // console.log('***** refineEvents level#2 event\'s calendar is deleted', event.id, record.id, googleCalEvent.id, googleCal.id)
        continue
      }
    }
  }
