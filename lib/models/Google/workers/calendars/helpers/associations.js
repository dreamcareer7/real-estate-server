...
const getAssociationsMap = async function (credential, confirmed) {
  Context.log('in getAssociationsMap', JSON.stringify({credential, confirmed}))
  ...
  Context.log('emails before clean up', JSON.stringify(emails))
  emails = [...new Set(emails)] // Remove duplicate emails
  Context.log('emails after clean up', JSON.stringify(emails))
  const index = emails.indexOf(credential.email)
  if (index > -1) {
    emails.splice(index, 1) // Remove the event owner email for email
  }
  Context.log('emails remove owner', JSON.stringify(emails))
