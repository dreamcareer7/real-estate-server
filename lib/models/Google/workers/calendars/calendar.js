...
const deleteCalendars = async (credential, deletedCalendarIds) => {
  // Deletion of google_calendar and google_calendar_events have been handled in persistRemoteCalendars method.
  // So here we only need to delete relevant calendar_integration and crm_task records

  const gids    = await GoogleCalendarEvent.getByCalendarIds(credential.id, deletedCalendarIds)
  const records = await CalendarIntegration.getByGoogleIds(gids)

  const crmTaskIds = records.filter(record => record.crm_task).map(record => record.crm_task)
  const recordIds  = records.filter(record => record.crm_task).map(record => record.id)

  await CrmTask.remove(crmTaskIds, credential.user)
