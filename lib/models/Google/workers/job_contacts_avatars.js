...
const syncContactsAvatars = async (data) => {
  ...
  try {
    ...
    Context.log('SyncGoogleContactsAvatars - syncAvatars done')
    ...
    Context.log('SyncGoogleContactsAvatars - syncAvatars done 2')

    // Report success
    await updateJobStatus(userJob.id, 'success')
    Context.log('SyncGoogleContactsAvatars - syncAvatars done 3')
