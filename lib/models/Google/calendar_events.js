...
const createRemote = async function (googleCredentialId, googleCalendarId, body) {
...
const updateRemote = async function (googleCredentialId, googleCalendarId, eventId, body) {
  ...
  const localEvent = await GoogleCalendarEvent.get(googleCredentialId, googleCalendarId, eventId)
  ...
  await GoogleCalendarEvent.updateLocal(localEvent.id, updatedEvent)
...
const deleteRemote = async function (googleCredentialId, googleCalendarId, googleCalendarEvent) {
  ...
  const event = await GoogleCalendarEvent.get(googleCredentialId, googleCalendarId, googleCalendarEvent)
...
GoogleCalendarEvent.get = async (id) => {
  ...
  if (calendars.length < 1)
    throw Error.ResourceNotFound(`Google calendar event by id ${id} not found.`)

  return calendars[0]
}

GoogleCalendarEvent.getByCalendar = async (googleCredentialId, calendarId) => {
  const ids = await db.selectIds('google/calendar_events/get_by_calendar', [googleCredentialId, calendarId])

  return await GoogleCalendarEvent.getAll(ids)
}

GoogleCalendarEvent.getByCalendarAndEventIds = async (googleCredentialId, calendar_id, event_ids) => {
  const ids = await db.selectIds('google/calendar_events/get_by_calendar_and_event_ids', [googleCredentialId, calendar_id, event_ids])

  return await GoogleCalendarEvent.getAll(ids)
}

GoogleCalendarEvent.getxxxx = async (googleCredentialId, calendarId, eventId) => {
  const ids = await db.selectIds('google/calendar_events/get_by_calendar_event', [googleCredentialId, calendarId, eventId])

  if (ids.length < 1)
    throw Error.ResourceNotFound(`Google Calendar event by channel ${eventId} not found.`)

  return await GoogleCalendarEvent.get(ids[0])
}

GoogleCalendarEvent.createLocal = async function (googleCredentialId, calendarId, event) {
...
GoogleCalendarEvent.deleteLocal = async function (eventId) {
  return await db.select('google/calendar_events/delete', [eventId])
}

GoogleCalendarEvent.deleteManyByRemoteIds = async (googleCredentialId, googleCalendarId, remoteEventIds) => {
  return await db.select('google/calendar_events/delete_by_remote_ids', [googleCredentialId, googleCalendarId, remoteEventIds])
}

GoogleCalendarEvent.restoreManyByRemoteIds = async (googleCredentialId, googleCalendarId, remoteEventIds) => {
  return await db.select('google/calendar_events/restore_by_remote_ids', [googleCredentialId, googleCalendarId, remoteEventIds])
}

GoogleCalendarEvent.deleteLocalByRemoteCalendarId = async (googleCredentialId, googleCalendarId) => {
...
GoogleCalendarEvent.create = async (googleCredentialId, googleCalendarId, body) => {
  /*
    {
      "status": string,
      "summary": string,
  ...
  */
  ...
  const { googleCalendar, event } = await createRemote(googleCredentialId, googleCalendarId, body)

  return await GoogleCalendarEvent.createLocal(googleCredentialId, googleCalendar.id, event)
}

GoogleCalendarEvent.update = async (googleCredentialId, googleCalendarId, eventId, body) => {
  const { status, event, updatedEvent } = await updateRemote(googleCredentialId, googleCalendarId, eventId, body)

  const id = await GoogleCalendarEvent.updateLocal(event.id, updatedEvent)
...
GoogleCalendarEvent.delete = async (googleCredentialId, googleCalendarId, googleCalendarEvent) => {
  const event = await deleteRemote(googleCredentialId, googleCalendarId, googleCalendarEvent)

  return await GoogleCalendarEvent.deleteLocal(event.id)
}
