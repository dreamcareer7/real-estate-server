...
GoogleCalendarEvent.bulkInsert = async (credential, calendar, events) => {
  ...
  const newEvents = confirmedArr.filter(c => !(c.error && c.error.code === 404)).map(c => generateCalendarEventRecord(calendar, c))
...
GoogleCalendarEvent.bulkUpdate = async (credential, events) => {
  ...
  try {
    ...
    // Context.log('SyncGoogleCalendar - GoogleCalendarEvent.bulkUpdate confirmedArr:', JSON.stringify(confirmedArr))
    ...
    Context.log('SyncGoogleCalendar - GoogleCalendarEvent.bulkUpdate ex:', ex.message, JSON.stringify(confirmedArr))
    throw ex
  }
}
