...
const createRemote = async function (googleCalendar, resource) {
  const google = await getClient(googleCalendar.google_credential, 'calendar')

  return await google.createEvent(googleCalendar.calendar_id, resource)
}

const updateRemote = async function (oldEvent, googleCalendar, resource) {
  const google = await getClient(googleCalendar.google_credential, 'calendar')
  ...
  const updatedEvent = await google.updateEvent(googleCalendar.calendar_id, oldEvent.event_id, resource)
...
GoogleCalendarEvent.createAll = async events => {
  return db.selectIds('google/calendar_events/bulk_insert', [JSON.stringify(events)])
}
...
GoogleCalendarEvent.bulkUpsert = async (records) => {
  ...
  return await db.chunked(records, Object.keys(records[0]).length, (chunk, i) => {
    const q = squel
      ...
      .returning('id, google_credential, google_calendar, event_id, etag')
...
GoogleCalendarEvent.insert = async (googleCalendar, resource) => {
  validateResource(resource)

  const remote = await createRemote(googleCalendar, resource)
  const id     = await GoogleCalendarEvent.createLocal(googleCalendar, remote)
...
GoogleCalendarEvent.bulkInsert = async (calendar, events) => {
  for (const event of events) {
    checkStartEnd(event.resource)
  }

  const google = await getClient(calendar.google_credential, 'calendar')
  const { confirmed, error } = await google.batchInsertEvent(events)
...
GoogleCalendarEvent.update = async (id, googleCalendar, resource) => {
  ...
  const { status, local, remote } = await updateRemote(oldEvent, googleCalendar, resource)
  ...
  const updatedLocal = await GoogleCalendarEvent.get(local.id)
...
GoogleCalendarEvent.bulkUpdate = async (events) => {
  for (const event of events) {
    checkStartEnd(event.resource)
  }
  ...
  console.log('byRemoteId', byRemoteId)

  const google_credential = events[0].calendar.google_credential

  const google = await getClient(google_credential, 'calendar')
  const { confirmed, error } = await google.batchUpdateEvent(events)

  const updatedEvents = confirmed.map(c => { return generateCalendarEventRecord(byRemoteId[c.id].calendar, c) })
