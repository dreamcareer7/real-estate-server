 ...
 * @param {UUID} id
 */
GoogleCalendarEvent.deleteMany = async function (ids) {
  return await db.select('google/calendar_events/delete', [ids])
}
...
GoogleCalendarEvent.bulkDelete = async (credential, events) => {
  const google = await getClient(credential.id, 'calendar')
  await google.batchDeleteEvents(events)

  const eventIds = events.map(e => e.id)
  return await GoogleCalendarEvent.deleteMany(eventIds)
}
