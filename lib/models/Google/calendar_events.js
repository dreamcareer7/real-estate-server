 ...
 * @param {string[]} events_remote_ids GoogleCalendarEvent remote-ids
 */
GoogleCalendarEvent.getByCredentialAndRemoteIds = async (credential_id, events_remote_ids) => {
  if ( events_remote_ids.length === 0 ) {
    return []
  }

  const ids = await db.selectIds('google/calendar_events/get_by_credential_and_remote_ids', [credential_id, events_remote_ids])
