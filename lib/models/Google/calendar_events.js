...
const validateResource = resource => {
  ...
  try {
    ...
    Context.log('SyncGoogleCalendar validateResource failed', JSON.stringify(resource))
    throw ex
  }
}
 ...
 * @param {string[]} events_remote_ids GoogleCalendarEvent remote-ids
 */
GoogleCalendarEvent.getMovedEvents = async (credential_id, calendar_id, events_remote_ids) => {
 ...
 */
GoogleCalendarEvent.updateCalendar = async (ids, calendar_id) => {
  if ( ids.length === 0 ) {
    return []
  }
...
GoogleCalendarEvent.batchInsert = async (credential, calendar, events) => {
  try {
    ...
    Context.log('SyncGoogleCalendar batchInsert validateResource failed', credential.id, calendar.id)
    throw ex
  }
...
GoogleCalendarEvent.batchUpdate = async (credential, events) => {
  try {
    ...
    Context.log('SyncGoogleCalendar batchUpdate validateResource failed', credential.id)
    throw ex
  }
