...
GoogleCalendarEvents.getAll = async (googleCredentialId, calendarId, eventIds) => {
  return await db.select('google/calendar_events/get', [googleCredentialId, calendarId, eventIds])
}

GoogleCalendarEvents.get = async (googleCredentialId, calendarId, eventId) => {
  const calendars = await GoogleCalendarEvents.getAll(googleCredentialId, calendarId, [eventId])
...
GoogleCalendarEvents.bulkInsert = async (records) => {
  return await db.chunked(records, 100, (chunk, i) => {
    const q = squel
      .insert()
...
GoogleCalendarEvents.createLocal = async (googleCredentialId, calendar, event) => {
...
GoogleCalendarEvents.create = async (googleCredentialId, googleCalendarId, body) => {
  const google   = await googleClient(googleCredentialId)
  const calendar = await GoogleCalendar.get(googleCredentialId, googleCalendarId)

  if (!calendar)
    throw Error.ResourceNotFound(`Google Calendar ${googleCalendarId} not found.`)

  // if ( calendar.origin !== 'rechat' )
  //   throw Error.Forbidden(`Access forbidden to calendar ${googleCalendarId}`)
  ...
  */
  ...
  return await GoogleCalendarEvents.createLocal(googleCredentialId, calendar, event)
}
...
GoogleCalendarEvents.update = async (googleCredentialId, googleCalendarId, googleCalendarEvent, body) => {
  const google   = await googleClient(googleCredentialId)
  const calendar = await GoogleCalendar.get(googleCredentialId, googleCalendarId)
...
GoogleCalendarEvents.deleteLocalByCalendarId = async (googleCredentialId, googleCalendarId) => {
  return await db.select('google/calendar_events/delete_by_cal_id', [googleCredentialId, googleCalendarId])
}

GoogleCalendarEvents.deleteLocalByRemoteId = async (googleCredentialId, googleCalendarId, eventId) => {
  return await db.select('google/calendar_events/delete_by_remote_id', [googleCredentialId, googleCalendarId, eventId])
}
