...
const helper = async function (googleCredentialId) {
...
const updateRemote = async function (googleCredentialId, eventId, body) {
  ...
  const event = await GoogleCalendarEvents.get(googleCredentialId, googleCalendar.id, eventId)

  if (!event)
    throw Error.ResourceNotFound(`Google Calendar Event ${eventId} not found.`)

  const updatedEvent = await google.updateEvent(googleCalendar.calendar_id, event.event_id, body)
