...
GoogleAuthLink.requestGmailAccess = async (user, brand, redirect) => {
  ...
  const state = `${user}::${brand}`
  const url   = await google.getAuthenticationLink(state)
  const scope = google.scope

  await GoogleAuthLink.deleteAndCreateNewAuthLink(user, brand, url, redirect, scope)
...
GoogleAuthLink.deleteAndCreateNewAuthLink = async (user, brand, url, redirect, scope) => {
  await GoogleAuthLink.hardDelete(user, brand)
  return await GoogleAuthLink.create([user, brand, url, redirect, scope])
}

GoogleAuthLink.grantAccess = async (code, state) => {
  const stateArr = state.split('::')
  const user     = stateArr[0]
  const brand    = stateArr[1]

  const gmailAuthLink = await GoogleAuthLink.getByUser(user, brand)

  try {
    const oldGoogleCredential = await GoogleCredential.getByUser(user, brand)
