...
GoogleAuthLink.requestGmailAccessCheck = async (user, brand) => {
  ...
  if (googleCredential) {
    ...
    // handle incremental autch (scopes)
    let hasDefaultScope    = false
    let hasGmailROScope    = false
    let hasContactsROScope = false

    if ( googleCredential.scope.includes(config.google_scopes_map.default[0]) && googleCredential.scope.includes(config.google_scopes_map.default[1]) )
      hasDefaultScope = true

    if ( googleCredential.scope.includes(config.google_scopes_map.gmailReadOnly) )
      hasGmailROScope = true

    if ( googleCredential.scope.includes(config.google_scopes_map.contactsReadOnly) )
      hasContactsROScope = true


    if( hasDefaultScope && hasGmailROScope && hasContactsROScope )
      return false
  }
...
//   const url = await GoogleAuthLink.requestGmailAccess('9fdcdf2a-8e0f-11e9-988e-0a95998482ac', '0237f9a8-8e0f-11e9-9a0e-0a95998482ac', 'https://chappar.rechat.com/dashboard/contacts?letter&s=0')
//   console.log(url)
// }
// test()
