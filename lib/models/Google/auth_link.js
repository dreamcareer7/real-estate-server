...
async function handleCalendarScope(credential) {
  ...
  */
  ...
  await UsersJob.upsertByGoogleCredential(credential, 'calendar')
}
...
GoogleAuthLink.grantAccess = async (data) => {
  ...
  try {
    ...
    await UsersJob.upsertByGoogleCredential(credential, 'gmail')
    await UsersJob.upsertByGoogleCredential(credential, 'contacts')
