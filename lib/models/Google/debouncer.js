...
module.exports = debounce(handleGmailWebhook, BOUNCE_DELAY)
*/
...
const handleGmailWebhook = async (key) => {
  console.log('SyncGoogle-debouncer gmail trigger', key)

  // @ts-ignore
  await GoogleWorker.Gmail.pushEvent.immediate({ email: key })
}

const handleCalendarWebhook = async (key, payload) => {
  console.log('SyncGoogle-debouncer calendar trigger', key, payload)
  GoogleWorker.Calendar.pushEvent({ email: key, payload })
