...
GoogleCredential.getByUser = async (user, brand) => {
  const ids = await db.selectIds('google/credential/get', [user, brand])

  if (ids.length < 1)
    throw Error.ResourceNotFound(`Google-Credential by-user ${user} not found`)
...
GoogleCredential.diableSync = async (id) => {
  return db.update('google/credential/diable_sync', [id, new Date()])
}
