...
GoogleCredential.lockForSync = async (id, lock) => {
  await db.update('google/credential/lock_for_sync', [id, lock])
}
...
GoogleCredential.syncJob = async () => {
  ...
  for (const id of ids) {
    ...
    if ( !googleCredential.sync_lock ) {
      const job = Job.queue.create('google_sync', data).removeOnComplete(true)
      await GoogleCredential.lockForSync(id, true)
