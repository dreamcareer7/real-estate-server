...
GoogleCredential.updateMessagesSyncToken = async (id, syncToken) => {
  return db.update('google/credential/update_messages_sync_token', [
    id,
    syncToken,
    new Date()
  ])
...
GoogleCredential.updateThreadsSyncToken = async (id, syncToken) => {
  return db.update('google/credential/update_threads_sync_token', [
    id,
    syncToken,
    new Date()
  ])
...
GoogleCredential.updateLastSyncTime = async (id, ts) => {
...
GoogleCredential.syncJob = async () => {
  const rows = await db.select('google/credential/sync_due', [config.google_credential.sync_gap_hour])
