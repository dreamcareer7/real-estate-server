...
GoogleCredential.updateLastSync = async (id, ts, duration) => {
  const res = await db.select('google/credential/update_last_sync', [
    new Date(ts),
    duration,
    id
  ])
...
GoogleCredential.updateSyncStatus = async (id, status) => {
  const res = await db.select('google/credential/update_last_sync_status', [
    id,
    status
  ])
}
...
GoogleCredential.disableEnableSync = async (id, action) => {
  ...
  if ( action === 'disable' ) {
    if (googleCredential.deleted_at)
      return true
  ...
  return db.update('google/credential/diable_sync', [googleCredential.id, deleted_at])
}
