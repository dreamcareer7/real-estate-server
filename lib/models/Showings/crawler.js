...
const crawler = async data => {
  try {
    ...
    if( data.meta.isFirstCrawl ) {
      ...
      startDate.setMonth(endtDate.getMonth() - 30)
    ...
    for(const bookedShowing of bookedShowings) {
      ...
      taskBody = {
        user: data.crm_task_meta.user,
        brand: data.crm_task_meta.brand,
      ...
      showingId = await Showings.create(body, taskBody)
...
const startCrawler = async data => {
  if (process.env.NODE_ENV === 'tests')
    return await offlineCrawler(data)
