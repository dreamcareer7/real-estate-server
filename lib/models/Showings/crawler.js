  ...
  try {
    ...
    if(successLogin) {
      ...
      for(const bookedShowing of bookedShowings) {
        ...
        if( resultArr.indexOf('declined by seller') >= 0 || resultArr.indexOf('cancelled by agent') >= 0 )
          taskStatus = 'DONE'

        if( resultArr.indexOf('declined by seller') >= 0 || resultArr.indexOf('cancelled by agent') >= 0 )
          taskStatus = 'DONE'

        if( bookedShowing.result === 'Showing / Cancelled by Agent' )
          taskStatus = 'DONE'

        if( bookedShowing.result === 'Preview / Declined By Seller' )
          taskStatus = 'DONE'

        if( bookedShowing.result === 'Showing / Declined By Seller' )
          taskStatus = 'DONE'

        if( TS > bookedShowing.start_date)
          taskStatus = 'DONE'
        ...
        const taskBody = {
          ...
          due_date: (new Date(bookedShowing.start_date).getTime() / 1000),
          status: taskStatus,
          task_type: 'Showing'
        }
