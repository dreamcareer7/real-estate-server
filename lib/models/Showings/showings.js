...
// if declined or canceled => dont create crm_task
Showings.create = async (showing, task) => {
  try {
    ...
    if( currentShowing ) {
      ...
      // const listing = await Listing.getByMLSNumber(showing.mls_number)
      // if listing is null hten dont add association

      // const deal = await Deal.getByListingId(listing.id)
      // id deal, the just add deal to association
      // {
      //   association_type: 'deal',
      //   deal: deal.id
      // }

      const createdTask = await CrmTask.create({
        brand: task.brand,
        assignees: [task.user],
    ...
    console.log()
    console.log('showingBody', showing)
