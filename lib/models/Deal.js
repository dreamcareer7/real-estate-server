...
Deal.create = function (deal, cb) {
  validate(deal, function (err) {
    ...
    db.query('deal/insert', [
      ...
      Deal.get(res.rows[0].id, (err, deal) => {
        if (err)
          return cb(err)

        Deal.scheduleScrape(deal)
  ...
  validate(deal, function (err) {
    ...
    Deal.get(deal.id, (err, old) => {
      ...
      db.query('deal/update', [
        ...
        if (Deal.getContext(old, 'full_address') !== Deal.getContext(deal, 'full_address'))
          Deal.scheduleScrape(deal)
...
Deal.scheduleScrape = deal => {
  const job = Job.queue.create('scrape_deal', {deal})
