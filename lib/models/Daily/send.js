...
const send = async daily_id => {
  ...
  const integrations = await CalendarIntegration.getByCrmTasks(_calendar.map(r => r.id))
  ...
  if (today.length < 1 && celebrations.length < 1 && active_deals.length < 1 && email_campaigns.length < 1 && holidays.length < 1)
    return
