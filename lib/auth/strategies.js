var oauth2orize = require('oauth2orize');
var passport = require('passport');

// create OAuth 2.0 server
var server = oauth2orize.createServer();

// Exchange username & password for access token.
server.exchange(oauth2orize.exchange.password(function (client, username, password, scope, done) {
  User.getByUsername(username, function (err, user) {
    if (err)
      return done(err);

    if (!user)
      return done(null, false);

    if (user.password !== password)
      return done(null, false);

    getNewTokens(user, client, function(err, tokens) {
      if(err)
        return done(err);

      done(null, tokens.access.token, tokens.refresh.token, { 'expires_in': 86400, code:'OK' });
    });
  });
}));

function getNewTokens(user, client, cb) {
  Token.removePrevious(user.id, client.id);

  var token = {
    client_id : client.id,
    user_id: user.id,
    expire_date: ((new Date()).getTime() / 1000) + 86400,
    type:'access'
  }

  Token.create(token, function(err, access) {
    if (err)
      return cb(err);

    token.type = 'refresh';
    delete token.expire_date;

    Token.create(token, function(err, refresh) {
      if(err)
        return cb(err);

      cb(null, {access:access, refresh:refresh});
    });
  });
}

// Exchange refreshToken for access token.
server.exchange(oauth2orize.exchange.refreshToken(function (client, refresh_token, scope, done) {
  Token.get(refresh_token, function (err, token) {
    if (err)
      return done(err);

    if (!token)
      return done(null, false);

    if(token.type !== 'refresh')
      return done(null, false);

    User.get(token.user_id, function (err, user) {
      if (err)
        return done(err);

      if (!user)
        return done(null, false);

      getNewTokens(user, client, function(err, tokens) {
        if(err)
          return done(err);

        done(null, tokens.access.token, tokens.refresh.token, { 'expires_in': 86400, code:'OK' });
      });
    });
  });
}));

module.exports = function (app, passport) {
  app.post('/oauth2/token', [
    passport.authenticate(['oauth2-client-password'], { session: false }),
    server.token(),
    server.errorHandler()
  ]);
}