WITH recs AS (
    SELECT recommendations.id,
           ...
           ARRAY_AGG(recommendations_eav."user") FILTER (WHERE recommendations_eav.action = 'TourRequested') AS tour_requested_by,
           MAX(recommendations_eav."created_at") FILTER (WHERE recommendations_eav.action = 'TourRequested') AS updated_at
...
SELECT id,
       ...
       LOWER($3)
FROM recs
WHERE ARRAY_LENGTH(COALESCE(tour_requested_by, '{}'), 1) > 0
AND CASE
