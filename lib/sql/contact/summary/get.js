...
module.exports = function(contact_ids) {
  const unnest_ids = squel.rstr('unnest(ARRAY[?]::uuid[]) AS cids(id)', contact_ids).toString()
  ...
  const query = `SELECT
    ...
    contacts_summaries.last_modified_on_source,
  ...
  FROM
    ${unnest_ids}
    LEFT JOIN crosstab($$${crosstab_query(contact_ids)}$$, $$
    VALUES
      ('title'),
      ('first_name'),
      ('middle_name'),
