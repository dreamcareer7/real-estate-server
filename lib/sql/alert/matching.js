...
orders.close_price = (q, a) => {
  q.order('close_price', a.order_dir || false)
}

orders.price = (q, a) => {
  q.order('price', a.order_dir || false)
}
...
orders.list_date = (q, a) => {
  q.orderNullsLast('list_date', a.order_dir || true)
}

orders.bedrooms = (q, a) => {
  q.orderNullsLast('bedroom_count', a.order_dir || false)
}

orders.bathrooms = (q, a) => {
  q.orderNullsLast('full_bathroom_count', a.order_dir || false)
}

orders.square_feet = (q, a) => {
  q.orderNullsLast('square_meters', a.order_dir || false)
}

orders.lot_size = (q, a) => {
  q.orderNullsLast('lot_size_area', a.order_dir || false)
}

orders.year_built = (q, a) => {
  q.orderNullsLast('year_built', a.order_dir || false)
}
...
module.exports = function (a) {
  ...
  if (a.minimum_bathrooms)
    q.where('full_bathroom_count >= ?', a.minimum_bathrooms)

  if (a.maximum_bathrooms)
    q.where('full_bathroom_count <= ?', a.maximum_bathrooms)
  ...
  if (a.agents && a.agents.length > 0) {
    const condition = `(
        (list_agent_mui       = agents.matrix_unique_id AND listings_filters.mls = agents.mls)
     OR (co_list_agent_mui    = agents.matrix_unique_id AND listings_filters.mls = agents.mls)
     OR (selling_agent_mui    = agents.matrix_unique_id AND listings_filters.mls = agents.mls)
     OR (co_selling_agent_mui = agents.matrix_unique_id AND listings_filters.mls = agents.mls)
    )`
