...
const HAS_OPEN_HOUSE = `SELECT
                          count(*) > 0 FROM open_houses
                        WHERE
                          listing_mui = listings_filters.matrix_unique_id
                          AND start_time > NOW()`
...
const addBrand = (q, a) => {
  const brand_agents = squel
    .select()
    .from(squel.rstr('get_brand_agents(?)', a.brand))

  q.with('brand_agents', brand_agents)

  q.where(`list_agent_mls_id   IN (SELECT mlsid FROM brand_agents)
    OR selling_agent_mls_id    IN (SELECT mlsid FROM brand_agents)
    OR co_list_agent_mls_id    IN (SELECT mlsid FROM brand_agents)
    OR co_selling_agent_mls_id IN (SELECT mlsid FROM brand_agents)`)
}
...
module.exports = function (a) {
  ...
  if (a.high_schools && a.high_schools.length > 0)
    q.where(`high_school_name IN ?
             OR senior_high_school_name IN ?
             OR junior_high_school_name IN ?`,
            a.high_schools, a.high_schools, a.high_schools)
