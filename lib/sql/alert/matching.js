...
const addBrand = (q, a) => {
  ...
  q.where(`list_agent_mui   IN (SELECT mui FROM brand_agents WHERE enabled IS TRUE)
    OR selling_agent_mui    IN (SELECT mui FROM brand_agents WHERE enabled IS TRUE)
    OR co_list_agent_mui    IN (SELECT mui FROM brand_agents WHERE enabled IS TRUE)
    OR co_selling_agent_mui IN (SELECT mui FROM brand_agents WHERE enabled IS TRUE)`)
}
