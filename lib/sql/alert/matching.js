...
var HAS_OPEN_HOUSE = 'SELECT count(*) > 0 FROM open_houses WHERE listing_mui = listings_filters.matrix_unique_id AND start_time > NOW()';
...
orders.office = (q, a) => {
  q.order(
    'CASE WHEN \''+a.sort_office+'\'::text = list_office_mls_id OR \''+a.sort_office+'\'::text = selling_office_mls_id THEN 0 ELSE 1 END'
  );
}

orders.close_price = (q, a) => {
  q.order('close_price', true);
}
...
function addAreas(q, a) {
  ...
  var err = () => {
    if (!Array.isArray(a.mls_areas))
      throw new Error.Validation('Invalid mls area object');
  }

  var expr = squel.expr();
  q.where(expr);

  a.mls_areas.forEach(item => {
    if(!item.area || !parseInt(item.area))
      err();

    var cur = squel.expr();
    expr.or(cur);

    cur.and('mls_area_major = ?', item.area);

    var subs = item.sub_areas;
    if(subs === undefined)
      return ;

    if(!Array.isArray(subs))
      err(); //It should be undefined or an array of sub_areas
    ...
    cur.and('mls_area_minor IN ?', subs);
  })
}
...
module.exports = function(a) {
  var points = a.points ? Address.getGeomTextFromLocationArray(a.points) : null;
  ...
  if(a.minimum_square_meters)
    q.where('square_meters >= ?', a.minimum_lot_square_meters);

  if(a.maximum_square_meters)
    q.where('square_meters <= ?', a.maximum_lot_square_meters);
  ...
  if(a.minimum_parking_spaces)
    q.where('number_of_parking_spaces >= ?', a.minimum_parking_spaces);
  ...
  if(a.list_agents && a.list_agents.length > 0)
    q.where('list_agent_mls_id IN ?', a.list_agents);
  ...
  if(a.selling_agents && a.selling_agents.length > 0)
    q.where('selling_agent_mls_id IN ?', a.selling_agents);
