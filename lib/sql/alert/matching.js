...
const addBrand = (q, a) => {
  const brand_agents = squel
    .select()
    .function('get_brand_agents(?) AS id', a.brand)

  const brand_agent_mls_ids = squel
    .select()
    .field('mlsid')
    .from('agents')
    .where('id IN (SELECT id FROM brand_agents)')

  q.with('brand_agents', brand_agents)
  q.with('brand_agent_mls_ids', brand_agent_mls_ids)

  q.where(`list_agent_mls_id   IN (SELECT mlsid FROM brand_agent_mls_ids)
    OR selling_agent_mls_id    IN (SELECT mlsid FROM brand_agent_mls_ids)
    OR co_list_agent_mls_id    IN (SELECT mlsid FROM brand_agent_mls_ids)
    OR co_selling_agent_mls_id IN (SELECT mlsid FROM brand_agent_mls_ids)`)
}
