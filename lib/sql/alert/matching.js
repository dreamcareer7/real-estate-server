...
/**
 * @param {import('@rechat/squel').Select} q 
 */
const addBrandMlsLimitation = async (q) => {
  const current_brand = Brand.getCurrent()
  if (current_brand) {
    // FIXME: This is a temporary measure to avoid disrupting people's work
    const enabled_mls = await BrandMls.getForBrand(current_brand.id)

    if (Array.isArray(enabled_mls) && enabled_mls.length > 0) {
      q.join('brands_mls', undefined, 'listings_filters.mls = brands_mls.mls')
      q.join(squel.rstr('brand_parents(?)', current_brand.id), 'bp(id)', 'brands_mls.brand = bp.id')
    }
  }
}
...
const base = function (a) {
  ...
  if (a.mls_filter) {
    if (!Array.isArray(a.mls_filter)) {
      a.mls_filter = [a.mls_filter]
    }

    q.where('listings_filters.mls = ANY(?::mls[])', a.mls_filter)
  }
  ...
  if (a.agents && a.agents.length > 0) {
    const condition = '(listings_filters.mls = agents.mls AND agents.matrix_unique_id IN(list_agent_mui, co_list_agent_mui, selling_agent_mui, co_selling_agent_mui, co_list_agent2_mui, co_list_agent3_mui, co_selling_agent2_mui, co_selling_agent3_mui))'
...
const list = a => {
  const q = base(a)
  q.field('listings_filters.id')
  q.group('listings_filters.id')
...
const count = a => {
  const q = base(a)
  q.field('COUNT(DISTINCT listings_filters.id)', 'total')
  // We distinct for the same reason we GROUP BY by listings_filters.id on the list function. To prevent duplicates.
