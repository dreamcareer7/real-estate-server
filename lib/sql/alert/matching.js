...
const sortMode = (a, defaultValue = false) => {
  if (!a.sort_dir) return defaultValue
  return  a.sort_dir === 'ASC'
}
...
orders.close_price = (q, a) => {
  q.order('close_price', sortMode(a, true))
}

orders.price = (q, a) => {
  q.order('price', sortMode(a, true))
}
...
orders.list_date = (q, a) => {
  q.orderNullsLast('list_date', sortMode(a, true))
}

orders.bedrooms = (q, a) => {
  q.orderNullsLast('bedroom_count', sortMode(a))
}

orders.bathrooms = (q, a) => {
  q.orderNullsLast('bathroom_count', sortMode(a))
}

orders.square_feet = (q, a) => {
  q.orderNullsLast('square_meters', sortMode(a))
}

orders.lot_size = (q, a) => {
  q.orderNullsLast('lot_size_area', sortMode(a))
}

orders.year_built = (q, a) => {
  q.orderNullsLast('year_built', sortMode(a))
}
