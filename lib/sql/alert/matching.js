...
const HAS_OPEN_HOUSE = `
  SELECT count(*) > 0
  FROM open_houses
  WHERE      open_houses.matrix_unique_id = listings_filters.matrix_unique_id
...
const MASTER_BEDROOM_IN_FIRST_FLOOR = `
  ...
  AND    property_rooms.matrix_unique_id = listings_filters.matrix_unique_id
...
const addBrand = (q, a) => {
  ...
  q.where(`list_agent_mls_id   IN (SELECT mlsid FROM brand_agents WHERE enabled IS TRUE)
    OR selling_agent_mls_id    IN (SELECT mlsid FROM brand_agents WHERE enabled IS TRUE)
    OR co_list_agent_mls_id    IN (SELECT mlsid FROM brand_agents WHERE enabled IS TRUE)
    OR co_selling_agent_mls_id IN (SELECT mlsid FROM brand_agents WHERE enabled IS TRUE)`)
}
