...
/*
 * Request type is just for decoration for now.
 * Also the JOIN_REQUEST's dont expect a reply.
 * Therefore it just calls the callback anyways.
 */

adapter.customHook = (data, cb) => {
  if (!data.request_type === JOIN_REQUEST)
    return cb()
  ...
  Object.keys(sockets.sockets).map(socket_id => {
    ...
    socket.user.rooms.push(data.room_id)
  })
}
