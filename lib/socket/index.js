...
queue.process('socket_join', 10000, (job, done) => {
  ...
  io.to(job.data.room_id).emit('User.Online', job.data.user_id);
...
queue.process('socket_has_members', 10000, function(job, done) {
  done(null, SocketServer.hasMembers(job.data.user_id))
});