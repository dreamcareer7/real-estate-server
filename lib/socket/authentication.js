...
function considerOffline (socket) {
  ...
  SocketServer.hasMembers(socket.user.id).nodeify((err, has) => {
    if (err)
      console.log('Error while calling hasMembers', err)
...
SocketServer.getUserStatus = (user_id, cb) => {
  const i = `Getting Status ${user_id}`

  console.time(i)

  SocketServer.hasMembers(user_id).nodeify((err, is_online) => {
    console.timeEnd(i)
    console.log(i, err, is_online)
