...
function joinRooms(socket, user, cb) {
  ...
  var signalOnline = room => io.to(room.id).emit('Room.UserOnline', user.id);

  var peersPresence = (room) => {
    var onlines = room.users.filter( user => SocketServer.hasMembers(user.id) )
                  .map( user => user.id )

    io.to(room.id).emit('Room.OnlineUsers', room.id, onlines)
  }
...
function sendSignals(socket) {
  ...
  socket.user.rooms.map( room_id => io.to(room_id).emit('Room.UserOffline', socket.user.id) )
}
