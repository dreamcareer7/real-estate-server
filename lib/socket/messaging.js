...
io.use((socket, next) => {
  ...
  function handleMessage (payload, cb) {
    post(payload.room_id, payload.message, payload.push, cb)
  }
  ...
  socket.on('Task.Message.Send', taskMessage)
  socket.on('Message.Send', normalMessage)
