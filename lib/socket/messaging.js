...
io.use((socket, next) => {
  const queueMessage = (socket, room_id, message, push, cb) => {
  ...
  function handleMessage (payload, cb) {
    postMessage(payload.socket, payload.room_id, payload.message, payload.push, cb)
  }
  ...
  const normalMessage = (socket, room_id, message, cb) => {
    queueMessage(socket, room_id, message, true, cb)
  }
