...
function create (req, res) {
  ...
  website.brand = req.body.brand || Brand.getCurrent()?.id
...
function deleteWebsite(req, res) {
  ...
  Website.get(req.params.id, (err, old) => {
    ...
    Website.delete(req.params.id, err => {
      if (err)
        return res.error(err)
...
function addHostname(req, res) {
  ...
  Website.addHost(options, (err, success) => {
    if (err)
      return res.error(err)

    if (!success)
      return res.error(new Error.Validation('Hostname is already taken'))
...
const router = function (app) {
  ...
  app.delete('/websites/:id', auth, deleteWebsite)
