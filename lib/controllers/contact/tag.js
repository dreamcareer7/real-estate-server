...
async function createTag(req, res) {
  await ContactTag.create(
    ...
    req.body.tag,
    req.body.touch_freq,
    req.body.auto_enroll_in_super_campaigns
  )
...
async function patchTag(req, res) {
  const user_id = req.user.id
  const brand_id = getCurrentBrand()
  let src = req.params.tag
  const { tag: dst, touch_freq, auto_enroll_in_super_campaigns } = req.body
  ...
  if (typeof auto_enroll_in_super_campaigns === 'boolean') {
    if (auto_enroll_in_super_campaigns) {
      await ContactTag.enableAutoEnrollInSuperCampaigns(brand_id, user_id, src)
    } else {
      await ContactTag.disableAutoEnrollInSuperCampaigns(brand_id, user_id, src)
    }
  }
