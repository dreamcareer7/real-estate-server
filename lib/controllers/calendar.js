...
async function calendarView(req, res) {
  ...
  /**
   * @param {'low' | 'high' | 'limit'} name 
   * @returns {boolean}
   */
  function isValid(name) {
    return req.query[name] && /^-?[0-9.]+$/.test(req.query[name])
  }

  // low and high validation
  if (!isValid('high') && !isValid('low') || (!isValid('limit') && (!isValid('high') || !isValid('low'))))
    throw Error.Validation('At least low or high and/or limit arguments are required and expected to be numeric.')

  const high = isValid('high') ? parseInt(req.query.high) : undefined
  const low = isValid('low') ? parseInt(req.query.low) : undefined
  const limit = isValid('limit') ? parseInt(req.query.limit) : undefined
  ...
  const result = await Calendar.filter([{
    brand: brand_id,
    users,
  }], { ...req.query, limit, low, high, event_types, object_types, accessible_to: req.user.id })

  return res.collection(result, {
    low: low !== undefined ? new Date(low * 1000).toUTCString() : undefined,
    high: high !== undefined ? new Date(high * 1000).toUTCString() : undefined,
    limit,
  })
}
