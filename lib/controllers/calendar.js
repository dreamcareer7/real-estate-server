...
const attachCalendarEventHandler = require('../models/Calendar/events')
...
async function calendarView(req, res) {
  ...
  const result = await Calendar.filter([{
    brand: brand_id,
    users,
  }], { ...req.query, low, high, event_types, object_types, user: req.user.id })
...
async function calendarFeed(req, res) {
  const low = moment('2018-01-01', 'YYYY-MM-DD').unix()
