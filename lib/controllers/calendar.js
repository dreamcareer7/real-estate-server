...
const exportAsFeed = require('../models/Calendar/calendar-feed').exportAsFeed
const FeedSetting = require('../models/Calendar/FeedSetting')
...
async function calendarFeed(req, res) {
  ...
  const events = await Calendar.filter(filter, {
    low: oneYearAgo,
    high: oneYearFromNow,
    event_types: types
  })
