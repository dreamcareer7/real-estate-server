...
async function detach(req, res) {
  ...
  /*
   * deal.files could be null
   * but we need to pass it to _.difference
   */
  const deal_files = deal.files || []

  const diff = lo.difference(ids, deal_files)

  if (diff.length > 0)
    throw new Error.Forbidden(`Permission denied to remove files ${diff.join(',')}`)
