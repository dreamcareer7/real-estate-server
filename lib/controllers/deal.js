...
const submit = async (req, res) => {
  const { values, state, pdf, form, metadata } = req.body
...
async function detach(req, res) {
  ...
  const file = await AttachedFile.get(id)

  await AttachedFile.unlink('Deal', req.query.id)
...
async function detachFromTask(req, res) {
  ...
  const task = await Task.get(req.query.task)
  const file = await AttachedFile.get(id)

  await AttachedFile.unlink('Room', task.room)
...
const router = function (app) {
  ...
  app.delete('/tasks/:task/:file', auth, am(join), am(detachFromTask))
