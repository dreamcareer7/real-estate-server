...
const patchRequired = async (req, res) => {
  ...
  expect(req.params.task).to.be.uuid
  expect(req.body.required).to.be.a('boolean')

  const task = await Task.get(req.params.task)
  task.required = req.body.required
...
const router = function (app) {
  ...
  app.get('/tasks/:task/submission/:revision', auth, am(getRevision))

  app.patch('/tasks/:task/needs_attention', auth, am(patchAttention))
  app.patch('/tasks/:task/required', auth, am(patchRequired))
}
