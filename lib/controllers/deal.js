...
const addTask = async (req, res) => {
  ...
  await promisify(Activity.add)(req.user.id, 'User', activity)
...
async function updateTask(task_id, task, attrs, user_id) {
...
const patchTask = async (req, res) => {
  ...
  const task = await Task.get(req.params.task)
  const updated = await updateTask(req.params.task, task, req.body, req.user.id)
  await Deal.notifyById(updated.deal)
...
const patchAllTasks = async (req, res) => {
  ...
  for (const task of tasks) {
    updated.push(await updateTask(task.id, task, requestTasksAttrs[task.id], req.user.id))
  }
...
async function detach(req, res) {
  ...
  //TODO SECURITY do access control before deleting files
  for(const id of ids) {
...
const router = function (app) {
  ...
  app.post('/deals/:id/files', auth, attach)
  app.delete('/deals/:id/files', auth, access, am(detach))
  app.get('/brands/:brand/deals.(xls|xlsx)', auth, getDealsAsExcel)
