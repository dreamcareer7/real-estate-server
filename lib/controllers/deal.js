...
const getPdf = async (req, res) => {
   ...
   */
  const rev = await Form.getRevision(submission.last_revision)
  const pdf = await Form.flatSubmissionRevision(rev)
  res.end(pdf)
}
...
const acceptEmail = (req, res) => {
  Deal.Email.enqueue(req.body)
  res.status(200)
  res.end()
}

const attachGalleryItem = async (req, res) => {
  const deal = await promisify(Deal.get)(req.params.id)
...
const addGalleryItems = async (req, res) => {
  ...
  for(const item of items)
    item.gallery = deal.gallery

  const saved = await GalleryItem.createAll(items)
  res.collection(saved)
}
...
const deleteGalleryItem = async (req, res) => {
  await GalleryItem.delete(req.params.iid)
  res.status(204)
  return res.end()
}
...
const router = function (app) {
  ...
  app.post('/deals/:id/gallery/attach', auth, access, am(attachGalleryItem))
  app.post('/deals/:id/gallery/items', auth, access, am(addGalleryItems))
  ...
  app.delete('/deals/:id/gallery/items/:iid', auth, access, am(deleteGalleryItem))
  app.get('/deals/:id/gallery.zip', auth, access, am(zipGallery))
