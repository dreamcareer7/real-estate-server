...
function getActivesUser(req, res) {
  getRecommendationsSet(req, res, 'actives_user');
}
        ...
        (cb, results) => {
          async.map(results.phones.non_existing, (r, cb) => {
            var data = {
              ...
              listing_title: Address.getLocalized(results.listing.property.address)
            };

            return Listing.sendAsSMSToNonUser(user_id, data, r, cb);
          }, (err, results) => {
            if(err)
              return cb(err);
