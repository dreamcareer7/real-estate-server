...
function unfavoriteRecommendationForUserOnShortlist(req, res) {
  ...
  Recommendation.unfavorite(shortlist_id, user_id, favorite_id, function(err, recommendation) {
    if(err)
      return res.error(err);
...
var router = function(app) {
  ...
  app.get('/shortlists/:sid/users/:id/recs/feed', b(getRecommendationsForUserOnShortlist));
  app.get('/users/:id/recs/feed', b(getRecommendationsForUser));
  app.post('/shortlists/:sid/users/:id/favorites', b(favoriteRecommendationForUserOnShortlist));
  app.delete('/shortlists/:sid/users/:id/favorites', b(unfavoriteRecommendationForUserOnShortlist));
}
