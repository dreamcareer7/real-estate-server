...
function recommendManually(req, res) {
  ...
  validator(bulk_recommend, req.body, function (err) {
    if (err)
      return res.error(err)
    Listing.getByMLSNumber(mls_number).nodeify(function (err, listings) {
      if (err)
        return res.error(err)

      Recommendation.recommendListing(room_id, listings[0].id, external_info, function (err, rec) {
        if (err)
          return res.error(err)
...
async function recommendManuallyByListingId(req, res) {
  expect(req.body).not.to.be.null
  const room_id = req.params.id
  ...
  expect(listing_id).not.to.be.undefined
  expect(room_id).to.be.uuid
  expect(listing_id).to.be.uuid

  await promisify(validator)(v2_bulk_recommend, req.body)
...
const router = function (app) {
  ...
  app.post('/v2/rooms/:id/recs', b(recommendManuallyByListingId))
