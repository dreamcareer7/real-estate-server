...
// Mark a recommendation as read for a user on a shortlist
...
function patchFavoriteForUserOnShortlist(req, res) {
  ...
  Recommendation.patch(recommendation_id, user_id, favorite, function(err, recommendation) {
    if(err)
      return res.error(err);
...
function patchTourForUserOnShortlist(req, res) {
  ...
  Recommendation.patchTour(recommendation_id, user_id, tour, function(err, recommendation) {
    if(err)
      return res.error(err);
...
var router = function(app) {
  ...
  // Endpoint to mark a recommendation as read for a user on a shortlist
