  ...
  validator(bulk_recommend, req.body, function(err) {
    ...
    async.auto({
      ...
      user_rooms: ['listing',
                   function(cb, results) {
                     var override = {
                       title: Address.getLocalized(results.listing.property.address)
                     };

                     Room.bulkCreateWithUsers(user_id, req.body.users, override, cb);
                   }],
      share: ['user_rooms',
              'listing',
              function(cb, results) {
                ...
                async.map(rooms, function(r, cb) {
                  Room.recommendListing(r, results.listing.id, external_info, function(err, recommendation) {
                    if(err)
                      return cb(err);
                    ...
                    Message.post(r, clone, true, function(err) {
                      if(err)
                        return cb(err);
