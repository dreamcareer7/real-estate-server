...
const realtorAuthorization = async (req, res, next) => {
  ...
  Context.log(`realtor endpoint is called with header ${req.headers}`)
...
const captureRealtorLead = async (req, res) => {
  Context.log(`realtor endpoint worker is called with header ${req.body}`)
  await LeadChannel.leadCapture({
    source: 'Realtor',
    ...req.body
  })
