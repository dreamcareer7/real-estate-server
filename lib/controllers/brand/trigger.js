const BrandTrigger = {
  ...
  ...require('../../models/Trigger/brand_trigger/exclusions'),
}
const AttributeDef = require('../../models/Contact/attribute_def/get')
const attachBrandTriggerEventHandler = require('../../models/Trigger/brand_trigger/events')
...
/** @type ApiHandler */
async function makeBrandTriggerException (req, res) {
  const { brand, event_type } = req.params
  const { contact } = req.body
  const exceptions = await BrandTrigger.getExclusions(brand, event_type)
  if (exceptions.map(({ contact }) => (contact)).includes(contact)) {
    res.status(409).send('the contact is already excepted from this brand trigger')
  }
  else {
    await BrandTrigger.makeExclusion(brand, event_type, [ contact ])
    res.model(await BrandTrigger.getExclusions(brand, event_type))
  }
}

/** @type ApiHandler */
async function deleteBrandTriggerException (req, res) {
  const { brand, event_type } = req.params
  const { contact } = req.body
  const exceptions = await BrandTrigger.getExclusions(brand, event_type)
  if (!exceptions.map(({ contact }) => (contact)).includes(contact)) {
    res.status(404).send('the contact is not listed as an exclusion')
  }
  else {
    await BrandTrigger.deleteExclusion(brand, event_type, [ contact ])
    res.model(await BrandTrigger.getExclusions(brand, event_type))
  }
}
...
module.exports = function brandTriggerRouter ({ app, b, access, am }) {
  ...
  app.post(
    '/brands/:id/triggers/:brand_trigger/exception',
    b, access,
    am(makeBrandTriggerException)
  )

  app.delete(
    '/brands/:id/triggers/:brand_trigger/exception',
    b, access,
    am(deleteBrandTriggerException)
  )
