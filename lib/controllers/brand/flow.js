...
/**
 * @param {import('../../../types/monkey/controller').IAuthenticatedRequest<{ flow: UUID; id: UUID }, {}, import('../../models/Brand/flow_step/types').IBrandFlowStepInput>} req 
 * @param {import('../../../types/monkey/controller').IResponse} res 
 */
async function addStepToFlow(req, res) {
  const id = await BrandFlowStep.create(req.user.id, req.params.id, {
    ...req.body,
    flow: req.params.flow
  })
  const added = await BrandFlowStep.get(id)
...
const router = function ({app, b, access, am}) {
  ...
  app.post('/brands/:id/flows/:flow/steps', b, access, flowAccess('write'), am(addStepToFlow))
