...
const addMember = async (req, res) => {
  ...
  /** @type {string | null} will be set in assignUserId */
  let userId = null
  ...
  const assignUserId = async (body, throwIfExists) => {
    userId = userId ?? await User.getOrCreate(body, throwIfExists)
  }

  const { fields, file } = await saveFileFromRequest({
    req,
    public: true,
    resolveWithoutFile: true,

    beforeUpload: body => assignUserId(body, true),
    // beforeUpload will set the userId before calling these:
    relations: () => [{ role: 'User', role_id: userId }],
    path: () => userId,
  })

  /* If userId is truthy here, it means the user *created* and was not existed
   * before. So we can patch the user user avatar... */
  if (userId && file?.url) {
    await User.patchAvatars(userId, 'Profile', file.url)
  }

  /* If the payload doesn't contain the avatar file, beforeUpload won't be
   * called. Let's call assignUserId one more time to ensure that userId is set,
   * but this time it's OK to return an existing user ID. */
