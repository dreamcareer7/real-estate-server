...
const { callMethodJson } = require('../models/SupportBot/slack_web_api')
...
async function handleGetLoginLink(payload) {
  const link = await getLoginLink(payload.actions[0].value)
    ...
    case 'dialog_submission':
      ...
      switch (callback_id) {
        case 'move-contacts':
          ...
          Context.log('CRM data moved!')

          stateVal = await getUserById(state)
          ...
          callMethodJson('chat.postMessage', {
            channel: '#6-support',
            text: `Moved CRM data for ${stateVal[0].display_name}`,
            username: user.name
          })
        ...
        case 'move-deals':
          ...
          callMethodJson('chat.postMessage', {
            channel: '#6-support',
            text: `Moved deals for ${stateVal[0].display_name}`,
            username: user.name
          })
    ...
    case 'interactive_message':
      switch (req.body.actions[0].name) {
        ...
        case 'disconnect-docusign':
          ...
          break
        case 'get-login-link':
          res.json(await handleGetLoginLink(req.body))
          break
