...
function ackNotificationBulk (req, res) {
  const user_id = req.user.id
  const notification_ids = ObjectUtil.queryStringArray(req.query.ids)
...
function form (req, res) {
  const m = mustache.compileAndRender(__dirname + '/../html/notification/create.html', {
    objects: Notification.object_enum.map(o => { return {name: o} }),
    actions: Notification.action_enum.map(a => { return {name: a} })
  })
