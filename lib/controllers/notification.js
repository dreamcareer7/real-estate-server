...
function registerForPush (req, res) {
  ...
  const app = req.body.app ?? 'rechat'

  expect(user_id).to.be.uuid
  expect(channel).to.be.a('string')
  expect(app).to.be.oneOf(['rechat', 'showingapp'])

  Notification.registerForPush(user_id, channel, app, (err, user) => {
    if (err)
      return res.error(err)
...
function unregisterForPushWithoutApp (req, res) {
  const user_id = req.user.id
  const channel = req.params.token
  const app = 'rechat'

  Notification.unregisterForPush(user_id, channel, app, (err, user) => {
    if (err)
      return res.error(err)
...
function unregisterForPush (req, res) {
  ...
  Notification.unregisterForPush(userId, channel, app, (err, user) => {
    return err ? res.error(err) : res.model(user)
  })
}
...
const router = function (app) {
  ...
  app.delete('/notifications/tokens/:token', b(unregisterForPushWithoutApp))
  app.delete('/notifications/tokens/:app/:token', b(unregisterForPush))
