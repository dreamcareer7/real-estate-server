...
function getShortlistNotifications(req, res) {
  ...
  Notification.getForUserOnShortlist(user, shortlist, function(err, notifications) {
    if(err) {
      res.status(401);
      res.error(err);
      return;
...
function getMessageRoomNotifications(req, res) {
  ...
  Notification.getForUserOnShortlistOnMessageRoom(user, shortlist, message_room, function(err, notifications) {
    if(err) {
      res.status(401);
      res.error(err);
      return;
...
function getAllMessageRoomNotifications(req, res) {
  ...
  Notification.getForUserOnShortlistOnAllMessageRooms(user, shortlist, function(err, notifications) {
    if(err) {
      res.status(401);
      res.error(err);
      return;
