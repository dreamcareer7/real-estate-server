...
function getNotificationMessage (req, res) {
  ...
  const transport = req.query.t || 'generic'
  ...
  Notification.get(notification_id, (err, notification) => {
    ...
    Orm.populate(notification, {}, (err, data) => {
      ...
      Notification.formatForDisplay(notification, data, transport, (err, message) => {
        if(err)
          return res.error(err)
...
function ackNotificationBulk (req, res) {
  ...
  if (!Array.isArray(notification_ids))
    return res.error(Error.Validation('You must supply an array of ids'))
