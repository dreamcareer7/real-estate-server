...
function getNotificationMessage (req, res) {
  ...
  Notification.get(notification_id, (err, notification) => {
    ...
    Orm.populate(notification, {}, (err, data) => {
      if(err)
        return res.error(err)

      Notification.formatForDisplay(notification, {
        // comment: 'foo',
        inviting_user: data.subjects[0],
        invited_user: data.auxiliary_object,
        // from: data.subjects[0].start_time,
        // to: data.subjects[0].end_time,
        // author: data.subject[0],
        // status: data.subjects[0].status,
        // listing: data.objects[0],
        // before: data.subjects[0].last_price,
        // now: data.subjects[0].price,
        // user: data.subjects[0],
        room: data.room,
        // owner: data.subjects[0]
      }, 'generic', (err, message) => {
        if(err)
          return res.error(err)
...
const router = function (app) {
  ...
  app.get('/notifications/:id/message', getNotificationMessage)
