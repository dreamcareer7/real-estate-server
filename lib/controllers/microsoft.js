...
const requestMicrosoftAccess = async function (req, res) {
  ...
  const scopes   = req.body.scopes || ['Contacts.Read', 'Mail.Read', 'Mail.Send', 'Calendars.ReadWrite'] // 'Mail.ReadWrite' is included into 'Mail.Send'
...
const webhook = async function (req, res) {
  for (let i = 0; i < req.body.value.length; i++) {
    ...
    // const { handleMessageNotif } = require('../models/Microsoft/workers/job.js')
    // Context.log('***** OutlookSub event ==>', event)
    // await handleMessageNotif(event)
...
const router = function (app) {
  ...
  app.post('/webhook/microsoft', am(webhookValidation), am(webhook))
}
