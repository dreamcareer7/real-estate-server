...
const requestMicrosoftAccess = async function (req, res) {
  ...
  //   if ( process.env.API_HOSTNAME === 'api.rechat.com' && acls.includes('MCal') ) {
  //     if ( acls.includes('MCal') ) {
  //       scopes.push('Calendars.ReadWrite')
  //     }
  //   }
  // }
  ...
  for (let i = 0; i < req.body.value.length; i++) {
    ...
    if ( event.clientState === SUBSCRIPTION_SECRET ) {
      if ( event.lifecycleEvent === 'subscriptionRemoved' || event.lifecycleEvent === 'missed' ) {
        Context.log(`Microsoft-Hook ${event.lifecycleEvent} ${event.subscriptionId}`)
        MicrosoftWorker.Outlook.pushEvent({ payload: event })
      } else {
        const key = `${event.subscriptionId}-${event.changeType}`
        Context.log(`Microsoft-Hook debouncing ${key}`)
        debouncer.outlook(key, event)
      }
    }
    ...
    if ( event.clientState === SUBSCRIPTION_SECRET_CAL ) {
      if ( event.lifecycleEvent === 'subscriptionRemoved' || event.lifecycleEvent === 'missed' ) {
        ...
        debouncer.calendar(event.subscriptionId, event)
      }
    }
  }
