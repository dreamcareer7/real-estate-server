...
const downloadAttachment = async function (req, res) {
  const att = JSON.parse(Crypto.decrypt(req.params.hash))
  const uts = new Date().getTime()

  if ( att.expires_at < uts )
    throw Error.BadRequest('Link is expired!')

  const mcid = att.credential_id
  ...
  try {
    ...
    const text = `Microsoft-Download-Attachment-Failed - credential: ${mcid} - Ex: ${ex.message}`
    const msg  = `Microsoft-Download-Attachment-Failed Ex: ${JSON.stringify(ex)}`
    sendSlackMessage(text, msg)

    throw Error.BadRequest('Download outlook message\'s attachment failed!')
  }
}
...
const requestMicrosoftAccessTest = async function (req, res) {
  const brand    = '3bb28636-1284-11ea-8ade-0242ac150003' // getCurrentBrand()
  const user     = '80a227b2-29a0-11e7-b636-e4a7a08e15d4' // req.user.id
...
const outlookWebhookValidation = async function (req, res, next) {
  ...
  const clientStateSecret = 'secretClientValue'
  let clientState = true

  for (let i = 0; i < req.body.value.length; i++) {
    if (req.body.value[i].clientState !== clientStateSecret) {
      clientState = false
      break
    }
  }
...
const outlookWebhookTest = async function (req, res) {
  for (let i = 0; i < req.body.value.length; i++) {
    const resource       = req.body.value[i].resource
    const changeType     = req.body.value[i].changeType
    const resourceData   = req.body.value[i].resourceData
    const subscriptionId = req.body.value[i].subscriptionId

    // processNotification(resource, subscriptionId)
    
    console.log('\nresource.id', changeType)
...
const router = function (app) {
  ...
  app.get('/emails/microsoft/attachments/:hash', am(downloadAttachment))
  ...
  app.post('/users/self/auth/microsoft/test', am(requestMicrosoftAccessTest))
  app.post('/webhook/microsoft', am(outlookWebhookValidation), am(outlookWebhookTest))
