...
const downloadAttachment = async function (req, res) {
  ...
  try {
    ...
    const msg  = `Microsoft-Download-Attachment-Failed Ex: ${ex}`
  ...
  */
  ...
  try {
    ...
    try {
      ...
      // const sentMessage = await MicrosoftMessage.getRemoteMessageByInternetMessageId(credential, result.internetMessageId)
      ...
      const msg  = `Outlook-Send-Email Fetch-Remote-Message-Failed Ex: ${ex}`
      sendSlackMessage(text, msg)
    ...
    const text = `Outlook-Send-Email-Failed - credential: ${credential.id} - Ex: ${ex.message} - internetMessageId: ${result.internetMessageId}`
    const msg  = `Outlook-Send-Email-Failed Ex: ${ex}`
    sendSlackMessage(text, msg)
...
const updateReadStatus = async function (req, res) {
  ...
  try {
    ...
    const msg  = `Outlook-Update-Read-Status-Failed Ex: ${ex}`
    sendSlackMessage(text, msg)
  }
