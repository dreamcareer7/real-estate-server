...
const sendEmail = async function (req, res) {
  ...
  */
  ...
  if ( req.body.messageId ) {
    ...
    await MicrosoftMessage.getByMessageId(req.body.messageId, credential.id)
  }
  ...
  try {
    ...
    if (isReply) {
      ...
      // await MicrosoftCredential.forceSync(credential.id)
      // await MicrosoftMessage.deleteAttachments(params.attachments)
    ...
    if ( ex.message === 'File size could not be greater than 4MB!' )
      throw Error.BadRequest('Send email failed! Attachments file size could not be greater than 4MB!.')
    else
      throw Error.BadRequest('Send email failed! Please try again.')
  }
}
...
const outlookWebhookTest = async function (req, res) {
  ...
  /*
    if POST https://{notificationUrl}?validationToken={opaqueTokenCreatedByMicrosoftGraph}
      ...
      Tip: The client sends the validation token back to Microsoft Graph.
  */

  /*
    if it is notification webhook
      A 202 (OK) status code / Validate the clientState property. 
...
const router = function (app) {
  ...
  app.post('/webhook/microsoft', am(outlookWebhookTest))
