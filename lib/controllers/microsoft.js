...
const requestMicrosoftAccess = async function (req, res) {
  const brand    = getCurrentBrand()
  const user     = req.user.id
  const scopes   = req.body.scopes || ['xxxx']
...
const uploadAttachment = async function (req, res) {
  ...
  const credential = await MicrosoftCredential.get(req.params.gcid)
...
const router = function (app) {
  ...
  app.post('/users/self/microsoft/:gcid/send/test', auth, brandAccess, am(sendEmailTest))
  ...
  curl -XPOST 'http://localhost:3078/users/self/microsoft/89bf4a31-5b7a-4a60-9275-dc6a43cca358/send/test' \
  -H 'Authorization: Bearer OWIwMjk0NjgtYzRkOC0xMWU5LThjMTgtMDI3ZDMxYTFmN2Ew' \
