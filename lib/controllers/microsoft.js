...
const updateReadStatus = async function (req, res) {
  ...
  const credential = await MicrosoftCredential.get(req.params.mcid)
  const message    = await MicrosoftMessage.get(messageId, credential.id)

  if ( !message )
    throw Error.ResourceNotFound(`Microsoft-Message ${messageId} Not Found!`)
  ...
  try {
    ...
    Context.log('Outlook-Update-Read-Status-Failed', ex)

    Slack.send({ channel: '7-server-errors',  text: `Outlook-Update-Read-Status-Failed - credential: ${credential.id} - Ex: ${ex.message}`, emoji: ':skull:' })
    Slack.send({ channel: 'integration_logs', text: `Outlook-Update-Read-Status-Failed - credential: ${credential.id} - Ex: ${ex.message}`, emoji: ':skull:' })
  }
...
const router = function (app) {
  ...
  app.put('/emails/microsoft/:mcid/:mid', auth, brandAccess, am(updateReadStatus))
