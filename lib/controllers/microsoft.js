...
const uploadAttachment = async function (req, res) {
  ...
  if (credential.revoked)
    throw Error.ResourceNotFound('Microsoft-Credential Is Revoked!')

  if (credential.deleted_at)
    throw Error.ResourceNotFound('Microsoft-Credential Is Deleted!')

  if ( !credential.scope.includes('Mail.Send') )
...
const sendEmail = async function (req, res) {
  ...
  try {
    ...
    Slack.send({ channel: '7-server-errors', text: `Outlook-Send-Message-Failed - credential: ${credential.id} - Ex: ${ex.message}`, emoji: ':skull:' })
