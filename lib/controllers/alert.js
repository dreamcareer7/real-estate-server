...
async function listingsAgents(req, res) {
  ...
  const listings = await promisify(Alert.check)(alert)
  const agents_stats = {}
  
  listings.filter(l => l.list_agent || l.selling_agent).map(l => {
    
    if (l.list_agent && !agents_stats[l.list_agent]) 
      agents_stats[l.list_agent] = { selling: 0, listing: 0, volume_in: 0,  avg_price: 0}
    
    if (l.selling_agent && !agents_stats[l.selling_agent]) 
      agents_stats[l.selling_agent] = { selling: 0, listing: 0, volume_in: 0,  avg_price: 0}
