...
async function listingsAgents(req, res) {
  ...
  if (alert.mls && alert.postal_codes && alert.postal_codes.length) {
    ...
    if (mls && mls.length && !mls[0].enable_agent_network) {
      ...
      const data = await DistributionList.getByPostalCode(alert.postal_codes)
      const offices = {}
      const agents = data.map((r) => {
        ...
        return {
          ...
          office_id: r.id, // fake id, it is just used for references
          stats: {
            selling: 0,
            listing: '0',
            volume_in: 0,
    ...
    // we get some errors if these options goes down
    delete alert.mls
    delete alert.postal_codes
  }
