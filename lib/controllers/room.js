...
function search(req, res) {
  var user_id = req.user.id;
  var room_type = req.query.room_type ? ObjectUtil.queryStringArray(req.query.room_type) : undefined;
  ...
  validator(room_type_schema, room_type, err => {
    ...
    if (req.query.users) {
      var users = ObjectUtil.queryStringArray(req.query.users);
      ...
      Room.stringSearch(user_id, terms, room_type, function(err, rooms) {
        if(err)
          return res.error(err);
