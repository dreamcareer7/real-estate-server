...
function attach(req, res) {
  ...
  AttachedFile.saveFromRequest({
    ...
    Activity.postToRoom({room_id, user_id, activity}, (err, activity) => {
      if(err)
        res.error(err)
    })
    res.model(file)
...
function detach(req, res) {
  ...
  AttachedFile.delete(req.params.aid, err => {
    if (err)
      return res.error(err)
...
const router = (app) => {
  ...
  app.delete('/rooms/:id/attachments/:aid', b(detach))
