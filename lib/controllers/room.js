...
function search(req, res) {
  ...
  if (req.query.users) {
    var users = req.query.users.split(',').map(function(r) { return r.trim(); });
    ...
    var terms = (req.query.q) ? req.query.q.split(',').map(function(r) { return r.trim(); }) : [];

    Room.stringSearch(user_id, terms, function(err, rooms) {
      if(err)
        return res.error(err);
