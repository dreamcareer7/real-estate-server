...
const AttributeDef = require('../models/Contact)
...
async function create(req, res) {
  ...
  Slack.send({
    channel: '6-support',
    text: `<mailto:${req.user.email}|${req.user.display_name}> created a ${added.data_type} custom attribute "${added.label}"`,
...
async function update(req, res) {
  ...
  await AttributeDef.update(def_id, attribute_def, req.user.id)
...
async function remove(req, res) {
  ...
  await AttributeDef.delete(def_id, req.user.id) // TODO: Get affected contacts to send live events
