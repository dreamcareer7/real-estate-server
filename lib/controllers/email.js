...
const downloadAttachment = async(req, res) => {
  ...
  try {
    ...
    const cid  = att.google_credential || att.microsoft_credential
    const text = `Download-Attachment-Failed - credential: ${cid} - Ex: ${err.message}`
...
const GoogleCredential = {
  ...require('../models/Google/credential/get'),
}

const MicrosoftCredential = {
  ...require('../models/Microsoft/credential/get'),
}


const Gmail   = require('../models/Google/gmail')
const Outlook = require('../models/Microsoft/outlook')
...
const insertGmailQueryJob = async (gCredential, metadata) => {
  const userJob = await UsersJob.find({ gcid: gCredential.id, mcid: null, jobName: 'gmail_query', metadata })

  if (!userJob) {
    await UsersJob.upsertByGoogleCredential(gCredential, 'gmail_query', null, metadata)
  }
}

const insertOutlookQueryJob = async (mCredential, metadata) => {
  const userJob = await UsersJob.find({ gcid: null, mcid: mCredential.id, jobName: 'outlook_query', metadata })

  if (!userJob) {
    await UsersJob.upsertByMicrosoftCredential(mCredential, 'outlook_query', null, metadata)
  }
}

const test = async (req, res) => {
  const contact_address = req.body.contact_address || 'saeed.uni68@gmail.com' 

  const user  = '95b9c3ac-49a7-11e9-8abe-0a95998482ac' // getCurrentBrand()
  const brand = 'a0454440-be05-11e9-9b6e-1663bb1b90f0' // req.user.id
  ...
  for ( const cred of gcredentials ) {
    promises.push(insertGmailQueryJob(cred, { contact_address }))
  }

  for ( const cred of mcredentials ) {
    promises.push(insertOutlookQueryJob(cred, { contact_address }))
  }
  ...
  // const g = await Gmail.searchByContact(gcid, 'saeed.uni68@gmail.com')
  // const m = await Outlook.searchByContact(mcid, 'saeed.uni68@gmail.com')

  // return res.json({ g: Array.from(g), m: Array.from(m) })
}

const router = function (app) {
  const auth = app.auth.bearer.middleware

  app.get('/emails/test', am(test))
