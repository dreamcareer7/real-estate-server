...
const attach = async (req, res) => {
  ...
  switch (req.query.origin) {
    case 'gmail': 
      fileSize = config.google_integration.attachment_size_limit
      break

    case 'outlook':
      fileSize = config.microsoft_integration.attachment_size_limit
      break

    case 'mailgun':
      fileSize = config.mailgun_integration.attachment_size_limit
      break
...
const downloadAttachment = async(req, res) => {
  ...
  try {
    ...
    const msg  = `Download-Attachment-Failed Ex: ${JSON.stringify(err)}`
...
const pixelTracking = async function (req, res) {
  const email_campaign_email_id = req.params.eceid

  const record = await EmailCampaignEmail.get(email_campaign_email_id)

  if (!record) {
    throw Error.Validation({ slack: false, message: 'Cannot save event with bad email_campaign_email_id' })
  }

  const event = {
    email: record.email,
    event: config.email_events.opened,
    created_at: new Date().getTime(),
    recipient: record.email_address,
    object: { email_campaign_email_id },
...
const linkProxy = async function (req, res) {
  const email_campaign_email_id = req.params.eceid
  const redirect_to = req.query.redirect
  ...
  const event = {
    email: record.email,
    event: config.email_events.clicked,
    created_at: new Date().getTime(),
    recipient: record.email_address,
    object: { email_campaign_email_id, redirect_to },
    origin: 'gmail-outlook'
  }
...
const handleFailureEvents = async function (req, res) {
  Context.log('==== handleFailureEvents query', req.query)
  Context.log('==== handleFailureEvents body', req.body)
  Context.log('==== handleFailureEvents headers', req.headers)
...
const router = function (app) {
  ...
  app.post('/emails/events/failure', am(handleFailureEvents))

  app.get('/emails/events/:eceid', am(pixelTracking))
  app.get('/emails/events/redirect/:eceid', am(linkProxy))
