...
const attach = async (req, res) => {
  try {
    const { err, file } = await promisify(AttachedFile.saveFromRequest)({
      path: req.user.id,
      req,
      relations: [{ role: 'User', role_id: req.user.id }],
      public: false,
      busboyOptions: { limits: { fileSize: 4 } }
    })

    Context.log('email-upload-attachment-failed err:', err, 'file:', file)

    res.model(file)

  } catch (err) {

    Context.log('email upload attachment failed.', err)
