...
const attach = async (req, res) => {
  try {
    const { file } = await promisify(AttachedFile.saveFromRequest)({
      ...
      public: false,
      busboyOptions: { limits: { fileSize: 3 * 1024 * 1024 } }
    })
    ...
    Context.log('attach-ex', ex)

    if ( ex.message === 'File size exceeds' )
      throw Error.Validation('File size exceeds! Attachments could not be greater than 3MB.')
