 ...
 * @returns {number=}
 */
function parseUint (val, name) {
  const intVal = parseInt(val)
  if (Number.isSafeInteger(intVal) && intVal >= 0) { return intVal }

  throw Error.BadRequest(`${name} must be a non-negative safe integer`)
}
...
/**
 * @typedef {object} GetCampaignsQuery
 * @property {string=} start
 * @property {string=} limit
 * @property {string=} order
 * @property {('draft' | 'scheduled' | 'executed' | 'any')=} status
 */
/**
 * @param {IAuthenticatedRequest<{ brand: IBrand['id'] }, GetCampaignsQuery, never>} req
 * @param {IResponse} res
 */
const getCampaigns = async (req, res) => {
  ...
  const { status, limit, start, order } = req.query

  /** @type {Parameters<typeof EmailCampaign.getByBrand>[1]} */
  const opts = {}
  ...
  if (!isNil(limit)) {
    opts.limit = parseUint(limit, 'limit')
  }
  if (!isNil(start)) {
    opts.start = parseUint(start, 'start')
  }
  if (!isNil(order) && order?.trim?.()) {
    opts.order = order
  }

  const campaigns = await EmailCampaign.getByBrand(req.params.brand, opts)
