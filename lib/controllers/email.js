...
const attach = async (req, res) => {
  try {
    const { file } = await promisify(AttachedFile.saveFromRequest)({
      ...
      public: false,
      busboyOptions: { limits: { fileSize: 4 * 1024 * 1024 } }
    })
    ...
    Context.log('attach-file', err.message, err, JSON.stringify(err))

    if ( err.message === 'File size exceeds.' ) {
      Context.log('attach-file send err response')
      throw Error.Validation('File size exceeds! Attachments could not be greater than 4 MB.')
    }

    Context.log('attach-file throw err')
    throw err
  }
}
...
const downloadAttachment = async(req, res) => {
  ...
  if ( att.expires_at < uts )
    throw Error.BadRequest('URL is expired!')
