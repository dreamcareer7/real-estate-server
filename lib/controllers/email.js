...
const pixelTracking = async function (req, res) {
  ...
  if (!hash) {
    throw Error.Validation({ slack: false, message: 'Invalid hash!' })
  }

  const data = Crypto.decrypt(hash)
  const email_campaign_email_id = data.eceid
...
const router = function (app) {
  ...
  app.get('/emails/events/:hash', am(pixelTracking))
  app.get('/emails/events/redirect/:hash', am(linkProxy))
