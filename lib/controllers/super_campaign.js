...
async function enroll(req, res) {
  ...
  if (!isString(user)) { throw Error.Validation('`user` must be a string') }

  if (!isString(brand)) { throw Error.Validation('`brand` must be a string') }
  ...
  // XXX: ensure that `user` belongs to the `brand`
  
  const currBrand = Brand.getCurrent()
  const brandParents = await Brand.getParents(brand)

  if (!brandParents.includes(currBrand)) {
    throw Error.Validation(
      `Current Brand (${currBrand.id}) is not parent of brand ${brand}`
    )
  }
  ...
  res.status(201).model(enrollment)
}
 ...
 * @param {IResponse} res 
 */
async function mySuperCampaigns(req, res) {
  res.end()
}
...
const router = function (app) {
  ...
  app.get('/email/super-campaigns/me', brandAccess, am(mySuperCampaigns))
}
