...
async function enroll(req, res) {
  ...
  res.status(201) // XXX: always 201? even in case update?
    .model(enrollment)
}
...
async function mySuperCampaigns(req, res) {
  ...
  if (!enrollments.length) { return res.collection([]) }

  const superCampaignIds = [...new Set(enrollments.map(e => e.super_campaign))]
  const superCampaigns = await SuperCampaign.getAll(superCampaignIds)

  // TODO: probably the response should be changed
  res.collection(superCampaigns)
}
