 ...
 */
async function ensureChildrenOfCurrentBrand (brandIds, message) {  
  ...
  for (const brandId of [...new Set(brandIds)]) {
    ...
    if (!parentIds.includes(currentBrandId)) {
      throw Error.Forbidden(
        message ?? `Current brand is not parent of brand ${brandId}`
      )
    }
  }
}
...
async function get(req, res) {
  ...
  ensureChildrenOfCurrentBrand([superCampaign.brand], 'Access Denied')
...
async function getEligibleAgents (req, res) {
  ...
  ensureChildrenOfCurrentBrand([brandId])
