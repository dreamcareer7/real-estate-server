...
async function create(req, res) {
  ...
  const formattedCriteria = await ContactList.formatCriteria(req.body.filters)

  Slack.send({
    channel: '6-support',
    text: `${req.user.display_name} created a contact list: ${formattedCriteria}`,
    emoji: ':busts_in_silhouette:'
  })

  await promisify(Activity.add)(req.user.id, 'User', {
    action: 'UserCreatedContactList',
    object_class: 'ContactList',
    object: result
  })
