...
const models = {
  ...
  // contact_mailer_export: ContactMailerExportQueryBuilder,
  contact_joint_export: ContactMailerExportQueryBuilder,
  contact_export: ContactExportQueryBuilder,
}
...
async function facts(req, res) {
  /** @type {'deals'} */
  const cube_name = req.params.cube
  const {drilldown, filter, project, ids, excludes} = req.body
  ...
  try {
    ...
    if (Array.isArray(ids)) options.ids = ids
    if (Array.isArray(excludes)) options.excludes = excludes
