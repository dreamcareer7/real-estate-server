...
async function calendarView(req, res) {
  ...
  const result = await Calendar.getForUser(user_id, undefined, low, high)
...
async function calendarFeed(req, res) {
  ...
  const data = await FeedSetting.get(userId)
  if (!data.length) {
    return res.model({})
  }
  const brandId = data[0].selected_brand
  let types = data[0].selected_types

  if (Array.isArray(types) && types.length === 0) {
    types = null
  }

  const events = await Calendar.getForUser(userId, brandId, oneYearAgo, oneYearFromNow, types)
  const result = await Orm.populate({ models: events })

  exportAsFeed(userId, res, result)
}
...
async function getCalendarFeedSetting(req, res) {
  ...
  const url = encryptDataAndMakeUrl({
    userId,
    // brandId: result[0].selected_brand,
    // types: result[0].selected_types
