...
async function calendarView(req, res) {
  ...
  const result = await Calendar.filter(users, brand_id, {low, high})
...
async function calendarFeed(req, res) {
  ...
  const user_ids = data.selected_users
  const brand_id = data.selected_brand
  ...
  const events = await Calendar.filter(user_ids, brand_id, {
    low: oneYearAgo,
    high: oneYearFromNow,
    event_types: types
  })
