...
async function deleteGmailThreads (credentials, threadKeys) {
  try {
    ...
    const text = `Gmail-Thread-Delete-Failed - Ex: ${ex.message}`
    const msg  = `Gmail-Thread-Delete-Failed Ex: ${ex}`
...
async function deleteOutlookThreads (credentials, threadKeys) {
  try {
    ...
    const text = `Outlook-Thread-Delete-Failed - Ex: ${ex.message}`
    const msg  = `Outlook-Thread-Delete-Failed Ex: ${ex}`
    microsoftCommon.sendSlackMessage(text, msg)
  }
}

async function deleteGmailMessages (credentials, messageIds) {
  try {
    for await ( const credential of credentials ) {
    ...
    const text = `Gmail-Message-Delete-Failed - Ex: ${ex.message}`
    const msg  = `Gmail-Message-Delete-Failed Ex: ${ex}`
    googleCommon.sendSlackMessage(text, msg)
  }
}

async function deleteOutlookMessages (credentials, messageIds) {
  try {
    for await ( const credential of credentials ) {
    ...
    const text = `Outlook-Message-Delete-Failed - Ex: ${ex.message}`
    const msg  = `Outlook-Message-Delete-Failed Ex: ${ex}`
    microsoftCommon.sendSlackMessage(text, msg)
  }
}
...
const deleteThreads = async function (req, res) {
  ...
  const gCredentials = await GoogleCredential.getAll(gCredentialIds)
  const mCredentials = await MicrosoftCredential.getAll(mCredentialIds)
...
const deleteMessages = async function (req, res) {
  const brand = getCurrentBrand()
  const user  = req.user.id
