...
const insertGmailQueryJob = async (gCredential, jobName, metadata) => {
  const userJob = await UsersJob.find({ gcid: gCredential.id, mcid: null, jobName, metadata })

  if (!userJob) {
    await UsersJob.upsertByGoogleCredential(gCredential, jobName, null, metadata)
  }
}

const insertOutlookQueryJob = async (mCredential, jobName, metadata) => {
  const userJob = await UsersJob.find({ gcid: null, mcid: mCredential.id, jobName, metadata })

  if (!userJob) {
    await UsersJob.upsertByMicrosoftCredential(mCredential, jobName, null, metadata)
  }
}
...
const syncThreads = async (req, res) => {
  ...
  if (!contact_address) {
    throw Error.BadRequest('Contact Address is not specified!')
  }

  const user  = getCurrentBrand()
