...
async function deleteFlow(req, res) {
  await BrandFlow.delete(req.user.id, req.params.id)
...
/**
 * @param {import('../../types/monkey/controller').IAuthenticatedRequest<{ id: UUID }, {}, { steps: Array<IBrandFlowStepInput & { id: UUID }> }>} req 
 * @param {import('../../types/monkey/controller').IResponse} res 
 */
...
/**
 * @param {import('../../types/monkey/controller').IAuthenticatedRequest<{ brand: UUID, id: UUID; step: UUID }, {}, {}>} req 
 * @param {import('../../types/monkey/controller').IResponse} res 
 */
async function deleteStep(req, res) {
  await BrandFlowStep.delete(req.user.id, req.params.id, req.params.step)
...
const router = function (app) {
  ...
  app.delete('/brands/:brand/flows/:id', b, brandAccess, flowAccess('write'), am(deleteFlow))

  app.post('/brands/:brand/flows/:id/steps', b, brandAccess, flowAccess('write'), am(addStepsToFlow))
  app.patch('/brands/:brand/flows/:id/steps', b, brandAccess, flowAccess('write'), am(bulkEditStep))
