...
const setFormTemplate = async (req, res) => {
  const { values } = req.body

  expect(values).to.be.an('object')
  expect(req.params.form).to.be.uuid

  const submission = await Brand.setFormTemplate({
    user: req.user.id,
...
const getFormTemplates = async (req, res) => {
  ...
  const submissions = await Brand.getFormTemplates({
    form: req.params.form,
    brand: req.params.id,
  })
...
const router = function (app) {
  ...
  app.post('/brands/:id/form-templates/:form', b, access, am(setFormTemplate))
  app.get('/brands/:id/form-templates/:form', b, access, am(getFormTemplates))
