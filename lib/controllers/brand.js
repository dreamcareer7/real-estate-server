...
const getAgents = async (req, res) => {
   ...
   * The only difference is, if you are an agent on this brand,
   * Which includes any of the agents of *sub brands*,
   * you still should have access to this endpoint.
   */

  const users = await promisify(User.getAll)(user_ids)

  const hasAccess = users.includes(req.user.id)
...
const setTemplate = async (req, res) => {
  ...
  const submission = await Brand.setTemplate({
    user: req.user.id,
...
const getTemplates = async (req, res) => {
  ...
  const submissions = await Brand.getTemplates({
    form: req.params.form,
    brand: req.params.id,
  })

  res.model(submissions)
}
...
const router = function (app) {
  ...
  app.post('/brands/:id/templates/:form', b, access, am(setTemplate))
  app.get('/brands/:id/templates/:form', b, access, am(getTemplates))
