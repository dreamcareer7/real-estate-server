...
function createVerification(req, res) {
  var verificationObject = {user_id:req.user.id,
...
function get(req, res) {
  Verification.get([req.params.id], function (err, verification) {
    if (err)
      return res.error(err);
...
var router = function (app) {
  ...
  app.get('/verifications/:code/users/:id', verify);
  app.get('/verifications/:id', get);
