...
function createPhoneVerification (req, res) {
  ...
  PhoneVerification.create(user_id, err => {
    if (err)
...
function createEmailVerification (req, res) {
  ...
  EmailVerification.create(user_id, err => {
    if (err)
      return res.error(err)
  ...
  validator(email_verification_schema, req.body, err => {
    ...
    async.auto({
      user: cb => {
        ...
        return User.getByEmail(email).nodeify(cb)
      },
      upgrade: [
        'user',
        (cb, results) => {
...
function verifyPhoneSA (req, res) {
  ...
  PhoneVerification.audit(phone_number, code, err => {
    if (err)
      return res.error(err)
    ...
    User.getByPhoneNumber(phone_number).nodeify((err, user) => {
      ...
      if (!user)
        return res.error(Error.ResourceNotFound())
