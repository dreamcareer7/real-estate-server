...
function verifyEmailSA(req, res) {
  if (!req.body.email)
    return res.error(Error.Validation('You must provide an email to verify'));

  if (!req.body.code)
    return res.error(Error.Validation('You must provide a code to verify your email'));
  ...
  var upgrade = req.body.token && req.body.agent;
  ...
  async.auto({
    ...
    upgrade: [
      'user',
      (cb, results) => {
        User.upgradeToAgentWithToken(results.user, token, agent, cb);
      }
    ],
    audit: cb => {
      EmailVerification.audit(email, code, cb);
