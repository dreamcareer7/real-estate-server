...
/**
 * @param {IAuthenticatedRequest<{ id: UUID }, {}, import('../models/Showing/appointment/types').ShowilngAppointmentRequestPayload>} req
 * @param {IResponse} res
 */
async function requestAppointment(req, res) {
  const id = await Appointment.request(req.params.id, req.body)
...
async function cancelAppointmentByBuyer(req, res) {
  const appointment = await Appointment.decodeToken(req.params.token)
  await Appointment.cancel(appointment)
  res.status(204)
  res.end()
}
...
const router = function (app) {
  ...
  app.post('/showings/:id/appointments', am(requestAppointment))
  app.delete('/showings/:id/appointments/:appointment', auth, showingAccess, am(cancelAppointmentByRole))
  ...
  app.post('/showings/appointments/:token/cancel', am(cancelAppointmentByBuyer))
}
