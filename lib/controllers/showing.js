 ...
 * @param {IResponse} res
 */
async function getAppointmentByBuyer(req, res) {
  expect(req.params.id).to.be.uuid
  const rows = await Appointment.getAllPublic([req.params.id])
...
/**
 * @param {IAuthenticatedRequest<{ id: UUID }, {}, { message: string; time: string; }>} req
 * @param {IResponse} res
 */
async function rescheduleAppointment(req, res) {
  expect(req.params.id).to.be.uuid
  await Appointment.reschedule(req.params.id, req.body.time, req.body.message)
  res.status(204)
  res.end()
}

/**
 * @param {IAuthenticatedRequest<{ id: UUID }, {}, { message: string }>} req
 * @param {IResponse} res
 */
async function cancelAppointmentByBuyer(req, res) {
  expect(req.params.id).to.be.uuid
  await Appointment.cancel(req.params.id, req.body.message)
  res.status(204)
  res.end()
}
...
const router = function (app) {
  ...
  app.get('/showings/public/appointments/:id', am(getAppointmentByBuyer))
  app.post('/showings/public/appointments/:id/cancel', am(cancelAppointmentByBuyer))
  app.post('/showings/public/appointments/:id/reschedule', am(rescheduleAppointment))
