...
async function changeAppointmentApprovalViaToken (req, res) {
  const { id, user, approved } = Crypto.decryptJSON(req.params.token)
  ...
  /* If this endpoint used directly in email (GET), we've no comment to store in
   * DB. on the other hand, if front-end reaches this endpoint (PUT),
   * req.body.comment exists. */
  const approval = { approved, comment: req.body?.comment ?? '' }
...
const router = function (app) {
  ...
  app.get('/showings/public/appointments/:token/approval', am(changeAppointmentApprovalViaToken))
  app.put('/showings/public/appointments/:token/approval', am(changeAppointmentApprovalViaToken))
