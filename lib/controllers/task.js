...
async function create(req, res) {
  ...
  Slack.send({
    channel: '6-support',
    text: `${req.user.display_name} created ${type_msg} event`,
...
async function addAssociation(req, res) {
  ...
  const added = await Task.Associations.create(req.body, task_id, user_id, brand_id)
...
async function removeAssociation(req, res) {
  ...
  const rowCount = await Task.Associations.remove(association_id, task_id, user_id)
...
async function bulkAddAssociation(req, res) {
  ...
  const added = await Task.Associations.createMany(req.body, task_id, user_id, brand_id)
...
async function bulkRemoveAssociation(req, res) {
  ...
  await Task.Associations.removeMany(association_ids, task_id, user_id)
