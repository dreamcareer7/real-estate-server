...
function limitAccess(action) {
  return (req, res, next) => {
    ...
    Brand.limitAccess({ user, brand })
      .then(() => {
        return Task.hasAccess(req.user.id, getCurrentBrand(), action, [task_id])
      }, err => res.error(err))
      .nodeify((err, accessIndex) => {
        if (err) {
...
async function create(req, res) {
  ...
  const task = await Task.create(data, user_id, brand)
