...
const attachTaskEventHandler   = require('../models/CRM/Task/events')
const attachTouchEventHandler  = require('../models/CRM/Touch/events')
const attachCalIntEventHandler = require('../models/CalendarIntegration/events')
const attachContactIntEventHandler = require('../models/ContactIntegration/events')
...
async function patchStatus(req, res) {
  ...
  if (req.body.status === 'DONE' && end_date) {
    new_end_date = Date.now() / 1000 + 3600
  }

  const task = await Task.update(task_id, {
    title,
    description,
    due_date: (req.body.status === 'DONE') ? Date.now() / 1000 : due_date,
