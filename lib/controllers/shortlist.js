...
function recommendManually(req, res) {
  ...
  Shortlist.recommendListing(shortlist_id, listing_id, alert_id, function(err, recs) {
    if(err)
      return res.error(err);
...
function recommendManuallyByMLSNumber(req, res) {
  ...
  Listing.getByMUI(mls_number, function(err, listing) {
    if(err)
      return res.error(err);

    Shortlist.recommendListing(shortlist_id, listing.id, alert_id, function(err, recs) {
      if(err)
        return res.error(err);
...
function inviteList(req, res) {
  var shortlist_id = req.params.id;

  Shortlist.inviteList(shortlist_id, function(err, invitees) {
    if(err)
      return res.error(err);
...
var router = function(app) {
  ...
  app.post('/shortlists/:id/manual', b(recommendManually));

  app.post('/shortlists/:id/manual/mls', b(recommendManuallyByMLSNumber));

  // Getting Shortlist invited users
  app.get('/shortlists/:id/invitees', b(inviteList));
}
