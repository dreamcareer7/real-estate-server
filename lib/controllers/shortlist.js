...
function archiveShortlist(req, res) {
  Shortlist.get(req.params.id, function(err, shortlist) {
    if(err)
      return res.error(err);
    ...
    Shortlist.archive(shortlist.id, function(err, ok) {
      if(err)
        return res.error(err);
...
function getCorrespondingRecommendationForListingOnShortlist(req, res) {
  var shortlist_id = req.params.sid;
  var user_id = req.params.id;
  var listing_id = req.params.lid;

  Recommendation.mapListingToRecommendationForUser(user_id, shortlist_id, listing_id, function(err, rec_id) {
    if(err)
      return res.error(err);
    ...
    if(!rec_id) {
      ...
      Recommendation.getOnShortlist(rec_id, shortlist_id, function(err, rec) {
        if(err)
          return res.error(err);
...
var router = function(app) {
  ...
  app.delete('/shortlists/:id', b(archiveShortlist));
  ...
  app.get('/shortlists/:sid/users/:id/listings/:lid', b(getCorrespondingRecommendationForListingOnShortlist));
}
