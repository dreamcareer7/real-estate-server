...
const updateReadStatus = async function (req, res) {
  ...
  if ( !req.body.status )
    throw Error.BadRequest('Status is not specified.')
  ...
  try {
    ...
    Context.log('Gmail-Update-Read-Status-Failed', ex)

    Slack.send({ channel: '7-server-errors',  text: `Gmail-Update-Read-Status-Failed - credential: ${credential.id} - Ex: ${ex.message}`, emoji: ':skull:' })
    Slack.send({ channel: 'integration_logs', text: `Gmail-Update-Read-Status-Failed - credential: ${credential.id} - Ex: ${ex.message}`, emoji: ':skull:' })
  }
...
const router = function (app) {
  ...
  app.post('/users/self/google/:gcid/attachments', auth, brandAccess, am(uploadAttachment)) // old route
  ...
  app.post('/users/self/google/:gcid/send', auth, brandAccess, am(sendEmail)) // old route
  app.post('/emails/google/:gcid/send', auth, brandAccess, am(sendEmail))

  app.put('/emails/google/:gcid/:mid', auth, brandAccess, am(updateReadStatus))
