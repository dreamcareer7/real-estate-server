...
const configureCaledars = async function (req, res) {
  ...
  const length = req.body.toSync.length + req.body.toSync.toStopSync

  if ( length === 0 )
    return res.status(204).end()

  await GoogleCalendar.configureCaledars(credential, req.body)

  return res.status(204).end()
}

const calendarHooks = async function (req, res) {
  Context.log('GoogleCalendar - Hooks - headers', req.headers)
...
const router = function (app) {
  ...
  // curl -XPOST 'http://localhost:3078/users/google/calendars/b215445e-ee05-4761-a4ec-085a164426ba/watch/test'
  // curl -XDELETE 'http://localhost:3078/users/google/calendars/b215445e-ee05-4761-a4ec-085a164426ba/watch/test'
}
