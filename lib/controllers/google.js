...
const GoogleWorker      = require('../models/Google/workers')
...
const calendarHooks = async function (req, res) {
  ...
  // debouce_key = calendarId + channelId + resourceId
...
const handleGmailWebhook = async function (key, promise) {
  Context.log('SyncGoogle - Gmail Notif - forceSync', key)
  await promise()

  return 
}

const debouncer = debounce(handleGmailWebhook, 2000, 5)
...
const gmailHooks = async function (req, res) {
  ...
  const googleCredentials = await GoogleCredential.getByEmail(req.query.email)
  ...
  for (const credential of googleCredentials) {
    if ( credential.scope_summary && credential.scope_summary.includes('mail.read') && !credential.revoked && !credential.deleted_at ) {
      try {
        const promise = GoogleCredential.forceSync(credential.id)
        debouncer(promise)
      } catch (ex) {
        // do nothing
      }
...
const gmailHooks_old = async function (req, res) {
  if ( req.body.subscription !== SUBSCRIPTION_ID ) {
    return res.status(200).end()
  }
