...
const requestGmailAccess = async function (req, res) {
  ...
  const scopes   = req.body.scopes || ['contacts.readonly', 'gmail.readonly', 'gmail.send', 'calendar'] // 'calendar'
...
const calendarHooks = async function (req, res) {
  ...
  if (!calendar) {
    Context.log('SyncGoogle - Skip Calendar Hook - Calendar not found', calendarId)
    return res.status(200).end()
  }
  ...
  if (!credential) {
    Context.log('SyncGoogle - Skip Calendar Hook - Credential not found', calendar.google_credential)
    return res.status(200).end()
  }

  if (credential.revoked || credential.deleted_at) {
    Context.log('SyncGoogle - Skip Calendar Hook - Credential is not active', calendar.google_credential)
    return res.status(200).end()
  }
  ...
  if ( calendar.watcher_channel_id !== channelId ) {
    ...
    Context.log('SyncGoogle - Calendar Hooks - Stop old channel', channelId, resourceId)
