...
const uploadAttachment = async function (req, res) {
  ...
  if (credential.revoked)
    throw Error.ResourceNotFound('Google-Credential Is Revoked!')

  if (credential.deleted_at)
    throw Error.ResourceNotFound('Google-Credential Is Deleted!')
...
const sendEmail = async function (req, res) {
  ...
  try {
    ...
    Slack.send({ channel: '7-server-errors', text: `Gmail-Send-Message-Failed - Params: ${JSON.stringify(params)} - Ex: ${JSON.stringify(ex)}`, emoji: ':skull:' })
