    ...
    async.auto({
      ...
      reset_password: [
        ...
        (cb, results) => {
          ...
          return User.resetPasswordByShadowToken({
            email: email,
            token: shadow_token,
            password: password
          }, cb)
...
function patchShadow(req, res) {
  const shadow = req.body.shadow
  const user_id = req.user.id

  expect(shadow).to.be.a('boolean')

  User.patchShadow(user_id, shadow, (err, user) => {
    if(err)
      return res.error(err)
...
const router = function (app) {
  ...
  app.patch('/users/self/is_shadow', patchShadow)
