...
function createUser(req, res) {
  ...
  User.getByEmail(user.email, (err, current) => {
    ...
    if(!current) {
      ...
      User.sendActivation(current.id, err => {
        if(err)
          return res.error(err);
    ...
    async.map({
      ...
      upgrade: [
        ...
        (cb, results) => {
          ...
          return User.upgradeToAgentWithToken(results.user.id, agent, shadow_token, cb);
        },
      ],
      activate: [
        'user',
...
function upgrade(req, res) {
  ...
  if(!agent_id || !secret)
    return res.error(Error.Validation('Require both target agent and answer to their secret question'));

  Agent.auditSecret(agent_id, secret, (err, success) => {
    if(err)
      return res.error(err);

    if(!success)
      return res.error(Error.Unauthorized('Invalid response to secret question'));

    User.upgradeToAgent(user_id, agent_id, (err) => {
      if(err)
        return res.error(err);
