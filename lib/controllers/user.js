...
function getUserByParam(req, res) {
  if (req.query.email) {
    ...
    User.getByCode(code, function(err, user) {
      ...
      res.status(200);
      // var compact_user = {};
      // compact_user.type = 'compact_user';
      // compact_user.id = user.id;
      // compact_user.first_name = user.first_name;
      // compact_user.last_name = user.last_name;
      // compact_user.user_type = user.user_type;
      // compact_user.profile_image_url = user.profile_image_url;

      // FIXME: Security bug related to exposing user information
      res.model(user);
    });
  } else {
    res.status(405);
...
var router = function(app) {
  ...
  // Search for a User on Rechat by email
  app.get('/users/search', getUserByParam);
