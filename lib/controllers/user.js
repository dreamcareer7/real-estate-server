...
const lookup = async (req, res) => {
  ...
  /*
    * We don't have a user with this email address.
    * However, it is possible that we know the SSO Provider who is supposed
    * to handle her authentication.
    * If that's the case, we can propose an SP-Initiated SSO Process.
  */
  if (!user) {
    ...
    res.collection([provider])
    return
  }

  const providers = await SsoProvider.getByUser(user)
