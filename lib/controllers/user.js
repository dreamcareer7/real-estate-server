...
function patchFacebookAccessToken (req, res) {
  const user_id = req.user.id
  const token = req.body.token

  User.patchFacebookAccessToken(user_id, token, function (err, ok) {
    if (err)
      return res.error(err)
...
function recordActivity(req, res) {
  const user_id = req.user.id

  const activity = req.body
  expect(user_id).to.be.uuid

  UserActivity.add(user_id, activity, (err, activity) => {
    if(err)
      return res.error(err)
...
const router = function (app) {
  ...
  app.patch('/users/self/facebook/token', b(patchFacebookAccessToken))
  ...
  app.post('/users/self/timeline', b(recordActivity))
