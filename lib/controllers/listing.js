...
function searchListings (req, res) {
  ...
  if (req.query.q) {
    expect(req.query.q).to.be.a('string').and.to.match(/[A-z]{3,}|[0-9]{5,}/)
...
function getSimilars (req, res) {
  expect(req.params.mls_number).to.be.mlsid
...
const searchCounties = (req, res) => {
  ...
  expect(term).to.be.a('string').and.to.have.length.above(1)
