...
const Listing = {
  ...require('../models/Listing').Listing,
  ...require('../models/Listing/link'),
}
...
const findListingById = promisify(Listing.get)
...
/**
 * @param {IAuthenticatedRequest<{ id: UUID }, never, never>} req
 * @param {IResponse} res
 */
async function getBranchLink (req, res) {
  const listing = await findListingById(req.params.id)
  const url = await Listing.getBranchLink(listing)
...
const router = function (app) {
  ...
  app.get('/listings/:id/deep-link', am(getBranchLink))
