...
function searchListings (req, res) {
  if (req.query.mui) {
    ...
    return Listing.getByMUI(mui).nodeify((err, listing) => {
      if (err)
        return res.error(err)
  ...
  if (req.query.mls_number) {
    ...
    return Listing.getByMLSNumber(mls_number).nodeify((err, listing) => {
      if (err)
        return res.error(err)
  ...
  if (req.query.q) {
    ...
    return Listing.getStatuses((err, allStatuses) => {
      ...
      Listing.stringSearch({query, status, limit}).nodeify(function (err, listings) {
        if (err)
          return res.error(err)
