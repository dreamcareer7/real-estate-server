...
const render = async (req, res) => {
  ...
  for(const key in values)
    _.set(variables, key, values[key])
  ...
  const parent_ids = await Brand.getParents(current_brand.id)
  ...
  const output = await renderTemplate({html, template, brand, variables})
...
const sms = async (req, res) => {
  ...
  const asset = listing ? listing.address.street_address : 'your photo'
