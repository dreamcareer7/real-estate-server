...
const getCategoriesForBrand = async (req, res) => {
  const { types, mediums, filter } = req.query
  ...
  if (filter)
    expect(filter).to.contain.oneOf(['template', 'asset'])
  ...
  const categories = await BrandTemplate.getCategoriesForBrand({
    brands: [brand],
    types,
    mediums,
    filter
...
const render = async (req, res) => {
  ...
  const brand = await Brand.getParentByType(current_brand.id, BrandConstant.BROKERAGE)
...
const router = function (app) {
  ...
  app.get('/brands/:brand/templates/categories', b, am(getCategoriesForBrand))
