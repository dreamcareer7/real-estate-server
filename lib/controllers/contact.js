...
const Worker = {
  ...require('../models/Contact/worker/contact'),
  ...require('../models/Contact/worker/duplicate'),
  ...require('../models/Contact/worker/import'),
}
...
async function deleteContacts(req, res) {
  const user = req.user.id
  const brand = getCurrentBrand()
  const { ids, total } = await getMatchingIds(req)

  // Delete tends to time out if the number of contacts to be deleted is too
  // high. We offload the work to a background worker in that case
  if (total > 1000) {
    const { query, filter } = await _getFilterFromRequest(brand, req)
    await Worker.delete_contacts(user, brand, filter, query)
  } else {
...
async function leadsEmail(req, res) {
  ...
  Context.log(`Received a webhook from mailgun for recipients are : ${req.body.recipient}`)
  Context.log(`Object.keys(req.body): ${Object.keys(req.body)}`)
  Context.log(`webhook mailgun_id : ${req.body['message-id']} or ${req.body['message_id']}`)
  ...
  if (!user.last_seen_at) {
    await LtsLead.saveContact('FormData', result, 'Website', user.id, brands[0])
  } else {
    await LtsLead.saveAndNotify({brand: brands[0], user: user.id, protocol: 'FormData'}, result)
  }
