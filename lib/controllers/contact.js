...
async function fastFilter(req, res) {
  /** @type {IContactFilterOptions & PaginationOptions & { root_object: 'contact' | 'contact_summary' }} */
  const query = req.query || {}
  ...
  const rows = await (query.root_object === 'contact_summary'
...
async function getMatchingIds(req) {
  ...
  const { filters: request_filters, ...body_args } = req.body

  let filters = []

  if (request_filters) {
    expect(request_filters).to.be.an('array')
    filters.push(request_filters)
  }

  /** @type {IContactFilterOptions} */
  const query = Object.assign({}, req.query, body_args)
  ...
  if (query.lists) {
    ...
    const lists = await ContactList.getAll(query.lists)
