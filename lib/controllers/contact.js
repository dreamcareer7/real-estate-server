...
//   // TODO: We should implement a bulk add to get advantage of bulk insert
//   //  operations in PG. But we'll need to prepare data and do all validation
//   //  work before hand.

//   // if (contacts.length > 10) {
//   //   return bulkAddContacts(user_id, contacts, options, res)
//   // }

//   const result = []
//   for (const contact of contacts) {
//     const added = await Contact.add(user_id, contact, options)
//     result.push(added)
//   }

//   return res.collection(result.filter(Boolean))
// }
...
function addContacts(req, res) {
  ...
  const contacts = req.body.contacts || []
  const options = req.body.hasOwnProperty('options') ? req.body.options : {}
  const CONTACT_SAVE_RATE = 10

  expect(contacts).to.be.a('array')
  expect(options).to.be.a('object')
  
  async.mapLimit(contacts, CONTACT_SAVE_RATE, (r, cb) => {
    Contact.add(user_id, r, options).nodeify((err, contact) => {
      if (err) {
        return cb(err)
      }
...
async function bulkAddContacts(user_id, contacts, options, res) {
