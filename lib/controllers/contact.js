...
async function deleteTag(req, res) {
  ...
  await ContactTag.delete(brand_id, user_id, [tag])
...
async function deleteTags(req, res) {
  ...
  await ContactTag.delete(brand_id, user_id, tags)
...
async function postEmails(req, res) {
  ...
  /*
   * Right now this endpoint allows you to send on behalf on any other user.
   * Which is obviously a problem.
   *
   * This should only allow sending on behalf of your teammates.
   */

  /** @type {any[]} */
  const { emails, from } = req.body

  expect(from).to.be.uuid
  ...
  for (let i = 0; i < emails.length; i++) {
    ...
    if (email.contact) {
      ...
      if (!Array.isArray(email.list)) email.list = [email.list]

      const access = await ContactList.hasAccess(getCurrentBrand(), 'read', email.list)
      ...
      const membership_records = await ContactListMember.findByListIds(email.list)
      const contact_ids = _.uniq(membership_records.map(mr => mr.contact))
      ...
      if (!Array.isArray(email.tag)) email.tag = [email.tag]
