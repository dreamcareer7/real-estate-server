...
async function getMatchingIds(req) {
  ...
  if (req.body.filters) {
    ...
    filters = filters.concat(req.body.filters)
  }
  ...
  if (req.body.lists) {
    ...
    filters = filters.concat(lists.map(list => list.filters).filter(x => x))
  }

  if (filters.length > 0) {
    const query = req.queyr
    _prepareQueryForFilters(query)
...
async function updateContacts(req, res) {
  ...
  for (const attr of req.body.attributes) {
    for (let i = 0; i < ids.length; i++) {
      attributes[attributes.length] = {
        contact: ids[i],
        created_by: user_id,
        attribute_def: attr.attribute_def,
        label: attr.label,
        index: attr.index,
        text: attr.text,
        date: attr.date,
  ...
  await ContactAttribute.create(attributes, true)
...
async function importContactsCsv(req, res) {
  ...
  const defs = await AttributeDef.getForUser(req.user.id)
  const mapped_defs = _.map(mappings, m => m.attribute_def).filter(x => x)
