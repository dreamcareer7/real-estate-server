...
async function fastFilter(req, res) {
  ...
  const rows = await (req.query.root_object === 'contact_summary'
    ? ContactSummary.getAll(Array.from(filter_res.ids))
    : Contact.getAll(Array.from(filter_res.ids))
  )
 ...
 */
async function getMatchingIds(req, default_args = {}) {
  const brand_id = getCurrentBrand()
  ...
  const filter = Array.isArray(req.body.filter) ? req.body.filter : (Array.isArray(req.body.filters) ? req.body.filters : [])
...
async function updateContacts(req, res) {
  ...
  const affected_contacts_ids = await Contact.update(req.body.contacts, user_id, brand_id)
...
async function bulkAddAttributes(req, res) {
  ...
  const affected_contacts_ids = await Contact.update(contacts, user_id, brand_id)
...
async function addAttributes(req, res) {
  ...
  const added_ids = await Contact.addAttributes(contact_id, attributes, user_id, getCurrentBrand())
