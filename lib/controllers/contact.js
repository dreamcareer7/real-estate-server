...
async function returnContactsCollection(models, req, res) {
  ...
  const references = defs.length > 0 ? Object.assign(populated.references, {
    contact_attribute_def: _.keyBy(defs, 'id')
...
// async function exportAsCSVWithIDs(req, res, ,next, contacts) {
//   const ids = req.body.ids
//   await limitAccess('read', req.user.id, ids)
//   const allContacts = await Contact.getAll(ids)
//   const fileName = `Rechat_${moment().format('MM_DD_YY')}.csv"`
//   res.attachment(fileName)
//   await contactExport.convertToOutlookCSV(allContacts, res)
//   res.end()  
// }
...
const router = function (app) {
  ...
  app.get('/contacts/outlook.csv', auth, am(exportAsCSV))
