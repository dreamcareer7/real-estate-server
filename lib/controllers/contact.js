...
async function getDuplicatesForUser(req, res) {
  const user_id = req.user.id

  const clusters = await ContactDuplicate.findForUser(user_id)

  res.collection(clusters)
}

async function getDuplicatesForContact(req, res) {
  const user_id = req.user.id
  const contact_id = req.params.id

  const clusters = await ContactDuplicate.findForContact(user_id, contact_id)
...
const router = function (app) {
  ...
  app.get('/contacts/duplicates', auth, am(getDuplicatesForUser))
  ...
  app.get('/contacts/:id/duplicates', auth, access('read'), am(getDuplicatesForContact))
