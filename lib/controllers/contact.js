...
async function getMatchingIds(req) {
  ...
  await limitAccess('update', brand_id, ids)
  ...
  if (filters.length > 0) {
    const filter_result = await Promise.all(filters.map(filter => Contact.filter(brand_id, filter, query)))
    ids = ids.concat(_.flatten(filter_result.map(res => Array.from(res.ids))))
  }
  else if (!ids || ids.length < 1) {
    const filter_result = await Contact.filter(brand_id, [], query)
...
async function updateContacts(req, res) {
  ...
  await limitAccess('write', brand_id, contact_ids)
...
async function bulkMerge(req, res) {
  ...
  await limitAccess('update', brand_id, contact_ids)
...
async function exportAsCSV(req, res) {
  ...
  if (req.body.filters || req.body.ids || req.body.lists) {
    ...
    contacts = await Contact.getForBrand(getCurrentBrand(), [])  
  }
