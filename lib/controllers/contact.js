...
async function postEmailsNew(req, res) {
  const brand_id = getCurrentBrand()
  /** @type {IEmailCampaign} */
  ...
  const access = await ContactList.hasAccess(brand_id, 'read', campaign.to.filter(x => x.list).map(x => x.list))
  ...
  const contact_ids = await EmailCampaign.create(campaign, brand_id)
  const associations = contact_ids.map(id => ({
...
const router = function (app) {
  ...
  app.post('/contacts/emails', auth, am(postEmailsNew))
