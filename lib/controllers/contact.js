...
function addContact(req, res) {
  ...
  var fatal = req.body.hasOwnProperty('fail_on_error') ? req.body.fail_on_error : true;
...
function search(req, res) {
  ...
  if (req.query.q) {
    ...
    var terms = ObjectUtils.queryStringArray(req.query.q);
    var limit = req.query.limit || 5;
    var similarity = req.query.similarity || 0.05;
    var all = (req.query.all ? req.query.all == 'true' : true);

    Contact.stringSearch(user_id, terms, limit, similarity, all, (err, rooms) => {
      if(err)
        return res.error(err);
