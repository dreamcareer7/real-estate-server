...
async function fastFilter(req, res) {
  ...
  const filter_res = await Contact.fastFilter(user_id, filters, query)
...
async function updateContacts(req, res) {
  ...
  const affected_contacts_ids = await Contact.update(user_id, req.body.contacts)
...
async function bulkAddAttributes(req, res) {
  ...
  const affected_contacts_ids = await Contact.update(user_id, contacts)
...
async function addContacts(req, res) {
  ...
  try {
    ...
    if (source_type && source_type.text === 'ExplicitlyCreated') {
      ...
      Slack.send({
        channel: '6-support',
        text: `${req.user.display_name} created ${n_contacts} manually`,
...
async function merge(req, res) {
  ...
  Slack.send({
    channel: '6-support',
    text: `${req.user.display_name} manually merged ${sub_contacts.length + 1} contacts`,
    emoji: ':busts_in_silhouette:'
  })
...
async function bulkMerge(req, res) {
  ...
  for (const id in counts) {
    if (counts[id] > 1) {
      throw Error.Validation(`Contact ${id} appears in the clusters more than once.`)
    }
  }

  await limitAccess('update', user_id, contact_ids)
  ...
  const job = Job.queue
    ...
    .on('complete', () => {
      Slack.send({
        channel: '6-support',
        text: `${req.user.display_name} merged ${clusters.length} duplicate clusters`,
        emoji: ':busts_in_silhouette:'
      })
    })
...
async function getDuplicateClusters(req, res) {
  ...
  const clusters = await ContactDuplicate.findForUser(user_id, query)
...
async function getContactDuplicateCluster(req, res) {
  ...
  const cluster = await ContactDuplicate.findForContact(user_id, contact_id)
...
const router = function (app) {
  ...
  app.get('/contacts/duplicates', auth, am(getDuplicateClusters))

  app.post('/contacts/filter', auth, am(fastFilter))
  app.post('/contacts/fast-filter', auth, am(filter))
  ...
  app.get('/contacts/jobs/:job_id', auth, am(jobStatus))
