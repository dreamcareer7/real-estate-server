...
const nodeify = promise => {
   ...
   * is not actually bound to a context. Therefore, our efforts in
   * bubbling it up will actually fail.
   *
   * By bounding it to next tick, node will consider this rejection
   * as a handled one. Also the error will be bound to the context.
  */
  ...
  return cb => {
    if (typeof cb !== 'function') {
      Context.trace('Nodify: callback is not a function')
    }
    callback = cb
  }
}
