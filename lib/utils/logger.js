...
function saveQueryArgs(req) {
  ...
  redisClient.hset('query_args', req.rechat_id, JSON.stringify(req.query))  
}
...
module.exports = function (app) {
  ...
  setImmediate(() => {
    app.get('/logs/bodies/:id', function(req, res) {
      const req_id = req.params.id
      redisClient.hget('bodies', req_id, (err, body) => {
        if (err) {
        ...
        db.conn((err, client, done) => {
          ...
          db.query('requests/get', [req_id], client, (err, result) => {
            ...
            redisClient.hget('query_args', req_id, (err, query_args) => {
              if (err) {
                return res.json({
                  code: 'error',
                  error: 'Redis connection failed.'
