...
class PeanarJob {
   ...
   */
  constructor(name, args, queue, handler) {
    this.id = uuid.v4()
  ...
  async perform() {
    Context.log(`PeanarJob: perform('${this.queue}', ${JSON.stringify(this, null, 2)}))`)
    ...
    try {
      const result = await this.handler.apply(null, this.args)
