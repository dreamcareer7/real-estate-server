...
class Peanar {
   ...
   * @param {import('./peanar').IPeanarRequest} job 
   */
  async _enqueueJob(routing_key, job) {
    this.log(`Peanar: _enqueueJob('${routing_key}', ${JSON.stringify(job)})`)

    const channel = await this._ensureConnected()
    await this.broker.declareQueue(routing_key)
