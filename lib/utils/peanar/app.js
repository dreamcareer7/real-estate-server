...
class RechatPeanar extends Peanar {
  ...
  async enqueueContextJobs() {
    ...
    const job_groups = _.groupBy(jobs, ({ def }) => `${def.queue}:${def.name}`)

    for (const group of Object.values(job_groups)) {
      const def = group[0].def
      const transactor = this._createTransactor(def)
      const t = transactor()

      await t.begin()
      for (const { req } of group) {
        await t.call(req.name, req.args)
      }
      ...
      try {
        ...
        Slack.send({
          ...
          text: `Peanar error: Error while committing transaction ${t.queueName}!\n\`${ex}\``,
          emoji: ':skull:'
        })
      }
    }
