...
function transaction (req, res, next) {
  ...
  db.conn(function (err, conn, done) {
    ...
    res.end = function (data, encoding, callback) {
      debug('Committing transaction for', req.rechat_id)
    ...
    req.on('close', function () {
      console.log('× Connection closed by peer', req.rechat_id.green, req.url, req.user ? req.user.email : 'Guest', domain)
      rollback()
    })
  })
}
