...
const templateInstanceCreated = ({instance, template, created_by}) => {
  const text = `Template ${template.name} (${template.variant}, ${template.medium}) created by <mailto:${created_by.email}|${created_by.display_name}>`

  reportActivity({
    text,
    attachments: [{
      thumb_url: instance.file.url
    }]
  })
}
