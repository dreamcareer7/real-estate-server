...
const userCreated = (user_id) => {
  User.get(user_id, (err, user) => {
    ...
    const text = format('New User: %s %s (%s) as %s', user.first_name, user.last_name, user.email, user.user_type)
...
const getUserName = () => {
  if (process.domain && process.domain.user)
    return process.domain.user.displayName

  return 'Unknown User'
}

const dealCreated = deal => {
  const text = format('Deal created by %s', getUserName())
  reportActivity(text)
}

const submissionCreated = deal => {
  const text = format('Form created by %s', getUserName())
  reportActivity(text)
}

const envelopeCreated = deal => {
