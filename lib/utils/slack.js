...
var userCreated = (user_id) => {
  User.get(user_id, (err, user) => {
    ...
    var text = format('New User: %s %s (%s)', user.first_name, user.last_name, user.email);
    reportActivity(text);
  })
}
...
var phoneVerificationSent = (verification, sms) => {
...
var emailVerified = (email) => {
  var text = format('Email %s verified', email);
  reportActivity(text)
}

var phoneVerified = (phone_number) => {
  var text = format('Phone %s verified', phone_number);
  reportActivity(text)
}
