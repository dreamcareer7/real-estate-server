...
function reportHttpError (req, e) {
  ...
  const user = req.user ? (`${req.user.first_name} ${req.user.last_name} (${req.user.email})`) : 'Guest'
  let brand_line = ''
...
function reportSocketError (e, context) {
  ...
  const username = user ? user.display_name : 'Guest'
...
const envelopeCreated = envelope => {
  const text = format('Envelope "%s" created by %s', envelope.title, getUserName())
  reportActivity(text)
}
...
const templateInstanceCreated = ({instance, template, created_by}) => {
  const text = `Template ${template.name} (${template.variation}, ${template.medium}) created by ${created_by.display_name})`
