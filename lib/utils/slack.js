...
const { PhoneVerification, EmailVerification } = require('../models/Verification')
const TemplateInstance = require('../models/Template/instance/index')
const Url = require('../models/Url')
const { get: getUser } = require('../models/User/get')
...
function reportSocketError (e, context) {
  ...
  let text = ':x: Socket:%s (Error %s)\n :memo: %s\n:person_with_blond_hair::skin-tone-5: %s\n---\n'
  text = format(text, fn, e.http, e.message, username)
  Slack.send({
    channel: '7-server-errors',
    text: text,
    emoji: ':skull_and_crossbones:'
...
module.exports = (app) => {
  ...
  Emitter.on('user created', userCreated)
