...
const enableMetrics = () => {
  setInterval(() => {
    ...
    Metric.set('pg.total', totalCount)
    Metric.set('pg.idle', idleCount)
    Metric.set('pg.waiting', waitingCount)
    Context.log('Pool stats', totalCount, idleCount, waitingCount)
  }, 500)
}
...
function getConnection (cb) {
  ...
  pool.connect((err, conn, _done) => {
    ...
    conn.query('SET application_name TO \'$1\'', Context.getId(), (err) => {
      if (err) {
        done()
        return cb(err)
      }
 ...
 * @param {(err: any, res: import('pg').QueryResult) => void} cb
 */
